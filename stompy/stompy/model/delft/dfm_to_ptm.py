...
class DFlowToPTMHydro(object):
    ...
    def open_dflow_output(self):
        ...
        if len(map_fns) > 1:
            ...
            if len(map_fns) == 1:
                log.info(
                    "Found multiple map files but only 1 merged map file.  Will use that"
                )
        ...
        if "mesh2d" in subset_ds:
            ...
            max_side_dim = subset_ds.mesh2d.attrs.get(
        ...
        self.mod_map_ds = subset_ds.rename(
            {
        ...
        self.g.edges["mark"][:] = translator[
            self.map_ds.mesh2d_edge_type.values.astype(np.int32)
        ]

        self.g.cells["mark"][:] = 0
        # punt, and call any cell adjacent to a marked edge BC a stage-bc cell
        bc_edges = np.nonzero(self.g.edges["mark"] > 1)[0]
        bc_cells = self.g.edge_to_cells(bc_edges).max(
            axis=1
        )  # drop the negative neighbors
        ...
        edge_depths = np.maximum(
            self.g.cells["cell_depth"][n1], self.g.cells["cell_depth"][n2]
        )
    ...
    def write_grd(self, grd_fn):
        self.g.write_ptm_gridfile(grd_fn, overwrite=self.overwrite, subgrid=True)
    ...
    def template_ds(self):
        ...
        out_ds = out_ds.rename(
            {"maxnode_per_face": "nMaxMesh2_face_nodes", "node_per_edge": "Two"}
        )

        # Additional grid information:
        # xarray wants the dimension made explicit here -- don't know why.
        out_ds["Mesh2_face_x"] = ("nMesh2_face",), self.mod_map_ds[
            "Mesh2_face_x"
        ].values
        out_ds["Mesh2_face_y"] = ("nMesh2_face",), self.mod_map_ds[
            "Mesh2_face_y"
        ].values

        out_ds["Mesh2_edge_x"] = ("nMesh2_edge",), self.mod_map_ds[
            "Mesh2_edge_x"
        ].values
        out_ds["Mesh2_edge_y"] = ("nMesh2_edge",), self.mod_map_ds[
            "Mesh2_edge_y"
        ].values
        ...
        face_edges = np.array(
            [self.g.cell_to_edges(c, pad=True) for c in range(self.g.Ncells())]
        )

        out_ds["Mesh2_face_edges"] = ("nMesh2_face", "nMaxMesh2_face_nodes"), face_edges
        out_ds["Mesh2_face_depth"] = ("nMesh2_face",), -self.mod_map_ds[
            "mesh2d_flowelem_bl"
        ].values
        out_ds["Mesh2_face_depth"].attrs.update(
            {
                "positive": "down",
                "unit": "m",
        ...
        if bedlevtype == 3:
            edge_z = self.map_ds.mesh2d_node_z.values[self.g.edges["nodes"]].mean(
                axis=1
            )
        elif bedlevtype == 4:
            ...
            raise Exception(
                "Only know how to deal with bed level type 3,4 not %d" % bedlevtype
            )
        # mindful of positive-down sign convention needed by PTM
        out_ds["Mesh2_edge_depth"] = ("nMesh2_edge",), -edge_z
        out_ds["Mesh2_edge_depth"].attrs.update(
            {
                "positive": "down",
                "unit": "m",
                "standard_name": "sea_floor_depth_below_geoid",
        ...
        if 1:
            ...
            out_ds["Mesh2_sea_surface_elevation"] = (
                "nMesh2_face",
                "nMesh2_data_time",
            ), s1.T.values
    ...
    def initialize_output_variables(self):
        ...
        self.salt_var = self.out_nc.createVariable(
            "Mesh2_salinity_3d", np.float64, self.cell_3d_data_dims
        )

        self.nut_var = self.out_nc.createVariable(
            "Mesh2_vertical_diffusivity_3d", np.float64, self.cell_3d_data_dims
        )

        # Layer index variables
        self.edge_k_bot_var = self.out_nc.createVariable(
            "Mesh2_edge_bottom_layer", np.int32, self.edge_2d_data_dims
        )
        self.edge_k_top_var = self.out_nc.createVariable(
            "Mesh2_edge_top_layer", np.int32, self.edge_2d_data_dims
        )
        self.cell_k_bot_var = self.out_nc.createVariable(
            "Mesh2_face_bottom_layer", np.int32, self.cell_2d_data_dims
        )
        self.cell_k_top_var = self.out_nc.createVariable(
            "Mesh2_face_top_layer", np.int32, self.cell_2d_data_dims
        )

        # hydro variables
        self.h_flow_var = self.out_nc.createVariable(
            "h_flow_avg", np.float64, self.edge_3d_data_dims
        )
        # what are the expectations for surface/bed vertical velocity?
        self.v_flow_var = self.out_nc.createVariable(
            "v_flow_avg", np.float64, self.cell_3d_data_dims
        )

        self.vol_var = self.out_nc.createVariable(
            "Mesh2_face_water_volume", np.float64, self.cell_3d_data_dims
        )
        self.A_edge_var = self.out_nc.createVariable(
            "Mesh2_edge_wet_area", np.float64, self.edge_3d_data_dims
        )
        self.A_face_var = self.out_nc.createVariable(
            "Mesh2_face_wet_area", np.float64, self.cell_3d_data_dims
        )

    def write_time_strings(self):
        # special handling for character array which xarray botches
        time_strings = [
            utils.to_datetime(t).strftime("%Y-%m-%d %H:%M:%S")
        ...
        time_string_array = np.array(
            [np.frombuffer(t.encode(), dtype="S1") for t in time_strings]
        )
        self.out_nc.createDimension("date_string_length", time_string_array.shape[1])
        time_string_var = self.out_nc.createVariable(
            "Mesh2_data_time_string", "c", ("date_string_length", "nMesh2_data_time")
        )
    ...
    def open_waq_output(self):
        ...
        self.hyd_fn = os.path.join(
            self.model.mdu.base_path,
    ...
    def init_waq_mappings(self):
        ...
        for link_idx, (l_from, l_to) in enumerate(self.hyd.links):
            ...
            if l_from >= 0 and l_to >= 0:
                ...
                assert (
                    l_to >= 0
                ), "Was only expecting 'from' for the link to be negative"
                ...
                if len(potential_edges) == 1:
                    ...
                    print(
                        "No boundary edge for link %d->%d. Likely src/sink"
                        % (l_from, l_to)
                    )
                    link_to_edge_sign[link_idx, :] = [
                        9999999,
                        0,
                    ]  # may be able to relax this
                    continue
                else:
                    print(
                        "Link %d->%d could map to %d edges - will choose first unclaimed"
                    ...
                    for j in potential_edges:
                        ...
                        raise Exception(
                            "Couldn't find an edge for link %d->%d" % (l_from, l_to)
                        )
                mapped_edges[j] = link_idx
                j_cells = self.g.edge_to_cells(j)
                if j_cells[0] == i_to:
    ...
    def write_time_steps(self):
        ...
        for ti, t in enumerate(times):
            ...
            cell_water_depth = self.mod_map_ds.mesh2d_waterdepth.isel(
                nMesh2_data_time=ti
            )
            if 0:
                # try setting edges to be dry by k_top=0.  this is specific to nk=1
                # all or nothing for sigma layers
                self.cell_k_top_var[:, ti] = np.where(
                    cell_water_depth > 0, self.nkmax, 0
                )
            else:
            ...
            if ti + 1 < len(times):
                ...
                if np.any(~valid):
                    ...
                    print(
                        "Ignoring fluxes from %d faces, sum(abs(Q))=%.3f m3/s"
                        % ((~valid).sum(), absQlost)
                    )
                h_flow_avg[js[valid], ptm_ks[valid]] = (Qs * sgns)[valid]

                h_area_avg[js[valid], ptm_ks[valid]] = np.where(
                    js[valid] >= 0, areas[exchs[valid]], 0.0
                )
                ...
                if self.nkmax > 1:
                    ...
                    # tested.
                    for exch in range(
                        self.hyd.n_exch_x + self.hyd.n_exch_y, self.hyd.n_exch
                    ):
                        ...
                        assert (
                            self.poi0[exch][0] >= 0
                        ), "Wasn't expecting BC exchanges in the vertical"
                        seg_up, seg_down = self.poi0[exch][
                            :2
                        ]  # used to have an extra 0, -- not sure why
                        ...
                        assert (
                            elt == self.hyd.seg_to_2d_element[seg_down]
                        ), "Maybe this wasn't a vertical exchange"
            ...
            vols = vols.reshape((self.nkmax, self.hyd.n_2d_elements))[
                ::-1, :
            ]  # [k,element]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--subgrid",
        "-s",
        help="Write fake subgrid output_sub.grd too",
        action="store_true",
    )
    parser.add_argument(
        "--skip-nc",
        "-n",
        help="Do not write netcdf, usu. in conjunction with --subgrid",
        action="store_true",
    )
    ...
    if args.times is not None:
        ...
        parts = [int(p) if p else None for p in args.times.split(":")]
