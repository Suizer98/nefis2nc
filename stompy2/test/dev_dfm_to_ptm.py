...
converter = dfm_to_ptm.DFlowToPTMHydro(
    model.mdu.filename, hydro_out_fn, grd_fn=grd_fn, overwrite=True
)
...
def check_continuity(hydro_nc, c, k):
    ...
    k = 0
    cell_vol = (
        hydro_nc["Mesh2_face_water_volume"]
        .isel(nMesh2_face=c, nMesh2_layer_3d=k)
        .values
    )
...
c = grd.select_cells_nearest([208.0, 55.0])
