...
def read_bathymetry_offset():
    ...
    comp_path = os.path.join(
        os.environ["HOME"],
        "classes/research/spatialdata/us/ca/suntans/bathymetry/compiled",
    )
...
def apply_vdatum(src_vdatum, dest_vdatum, lonlat_locs):
    ...
    command = (
        "cd %s ; java VDatum -lonlat -heights -hin WGS84 -units meters -vin %s -vout %s %s %s"
        % (vdatum_dir, src_vdatum, dest_vdatum, tmp_in, tmp_out)
    )
...
class MPIrunner(object):
    ...
    def __init__(self, cmdlist, np=4, no_mpi_for_1=True, wait=1):
        ...
        if mpi_version is None:
            ...
            try:
                pipe = subprocess.Popen(
                    ["mpirun", "-V"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
        ...
        if mpi_version == 0:
            ...
            must_wait()
            subprocess.call(
                ["mpiexec", "-np", "%d" % np, "-host", "localhost"] + cmdlist
            )
            ...
            raise Exception(
                "Failed to find a way to run this! mpi version is %s" % mpi_version
            )

    def run_bobstar(self, np, cmdlist, wait=True):
        self.run_qsub(np, cmdlist, ppn=8, walltime=None, wait=wait)
...
class SuntansCrash(object):
    ...
    def depth_info(self):
        ...
        for c in cells:
            print("Cell %i" % c)
            print(
                "Depth[cell=%i]: %g" % (c, self.sun.depth_for_cell(c, self.processor))
            )
            if self.z_id == 0:
...
def spin_mapper(spin_sun, target_sun, target_absdays, scalar="salinity", nonnegative=1):
    ...
    print(
        "Difference between target time and spinup time: %gs"
        % (24 * 3600 * (target_absdays - spin_times[step]))
    )
    ...
    def mapper(p, c, k):
        ...
        if (p, c) not in proc_cell_to_spin_cell:
            # The real way:
            proc_cell_to_spin_cell[(p, c)] = spin_sun.closest_cell(
                grids[p].vcenters()[c]
            )
        ...
        try:
            ...
            print(
                "Trouble with production run (%d,%d,%d) mapping to spinup (%d,%d,??) shaped %s"
                % (p, c, k, spin_proc, spin_c, spin_ctops[spin_proc].shape)
            )
            raise
...
class SediStoreFile(object):
    ...
    def seek_bed(self, layer):
        self.fp.seek(
            self.hdr_bytes
            + self.nclasses * self.stride_Nc_Nk * REALSIZE
...
class StoreFile(object):
    ...
    def time(self):
        ...
        return self.sun.time_zero() + datetime.timedelta(
            self.timestep() * self.sun.conf_float("dt") / (24.0 * 3600)
        )
    ...
    def copy_salinity(self, spin_sun):
        ...
        mapper = spin_mapper(spin_sun, self.sun, my_absdays, scalar="salinity")
...
class SunConfig(GenericConfig):
    ...
    def set_simulation_duration(self, days=None, delta=None, seconds=None):
        ...
        def to_quanta(td):
            ...
            us_per_quanta = 1000000 // datenum_precision_per_s
            return (td.days * 86400 + td.seconds) * datenum_precision_per_s + int(
                round(td.microseconds / us_per_quanta)
            )
        ...
        print(
            "Simulation duration requires %i steps (rounding error=%s)"
            % (self.conf_int("nsteps"), err)
        )
...
class SunReader(object):
    ...
    def save_to_folder(self, folder):
        ...
        if folder != orig_folder:
            ...
            shutil.copyfile(
                os.path.join(orig_folder, "suntans.dat"),
    ...
    def modify_dt(self, dt):
        ...
        self.conf.set_simulation_period(eff_t0, eff_t0 + duration / (24 * 3600.0))
    ...
    def grid(self, processor=None, readonly=True):
        ...
        if processor not in self._grids:
            self._grids[processor] = orthomaker.OrthoMaker(
                suntans_reader=self, processor=processor, readonly=readonly
            )
    ...
    def steps_available(self, output="grid"):
        ...
        if output == "grid":
            ...
            # profile data:
            from_step_dat = old_div(
                (self.step_data()["step"] - self.starting_time_step()),
                self.conf_int("ntoutProfs"),
            )
            ...
            if n_steps_in_file != from_step_dat and not self.warned_profile_filesize:
                print(
                    "Filesize suggests %i profile timesteps, but step.dat suggests %i"
                    % (n_steps_in_file, from_step_dat)
                )
    ...
    def step_data(self, reload=False):
        ...
        if self._step_data is None or reload:
            ...
            m = re.match(
                "On (\d+) of (\d+), t=([0-9\.]+) \(([0-9\.]+)% Complete, (\d+) output\)",
                raw,
            )
            if m is None:
                print("------step.dat------")
                ...
                raise Exception(
                    "Failed to parse step.dat.  Probably transient - try again"
                )
            self._step_data = {
                "step": int(m.group(1)),
    ...
    def domain_decomposition(self, np=None):
        ...
        cmd = [
            ...
            "-g",
            "-vvv",
            "--datadir=%s" % os.path.abspath(self.datadir),
        ]
        ...
        if not os.path.exists(self.file_path("topology", 0)):
            raise Exception(
                "Appears that partitioning failed - probably couldn't find sun (%s) or MPI"
                % self.sun_exec
            )
    ...
    def cell_scalar(self, filename_name, processor, time_step=None):
        ...
        if processor not in self._cell_scalars[filename_name]:
            ...
            try:
                full_scal = memmap(s_name, dtype=REALTYPE, mode="r", shape=data_shape)
    ...
    def freesurface(self, processor, time_step=None):
        ...
        if processor not in self._freesurface:
            ...
            try:
                self._freesurface[processor] = memmap(
                    fs_name, dtype=REALTYPE, mode="r", shape=data_shape
                )
            except mmap.error:
                fp = open(fs_name, "rb")
                self._freesurface[processor] = fromstring(
                    fp.read(), dtype=REALTYPE
                ).reshape(data_shape)
                fp.close()
    ...
    def profdata(self):
        ...
        if self._profdata is None:
            ...
            pdata["allIndices"] = fromstring(
                fp.read(4 * pdata["numTotalDataPoints"]), int32
            )
            # Wait a second - this file doesn't even have proc/cell info...
            dataxy = fromstring(
                fp.read(REALSIZE * 2 * pdata["numTotalDataPoints"]), REALTYPE
            )
            ...
            xvyv = fromstring(
                fp.read(
                    2
                    * REALSIZE
                    * pdata["numTotalDataPoints"]
                    * pdata["numInterpPoints"]
                ),
                REALTYPE,
            )
            pdata["xvyv"] = xvyv
    ...
    def profile_data(self, scalar, timestep=None):
        ...
        if timestep is not None:
            if timestep >= data.shape[0]:
                print(
                    "Bad timestep %d, last valid step is %d"
    ...
    def profile_to_transect(self, xy, absdays_utc, scalar):
        ...
        if self.nkmax_profs() != self.conf_int("Nkmax"):
            ...
            raise Exception(
                "Not quite smart enough to handle profiles that are different Nkmax than grid"
            )
        ...
        if 0:
            ...
            z_level_centers = (
                self.bathymetry_offset() - self.z_levels() + 0.5 * self.dz()
            )
    ...
    def map_global_cells_to_local_cells(
        self, cells=None, allow_cache=True, check_chain=True, honor_ghosts=False
    ):
        ...
        global_to_local = zeros(
            len(cells), [("global", int32), ("proc", int32), ("local", int32)]
        )
        ...
        if cache_fn is not None:
            fp = open(cache_fn, "wb")
            pickle.dump(global_to_local, fp)
            fp.close()
    ...
    def full_to_transect(self, xy, absdays, scalar_file, min_dx=10.0):
        ...
        utm_points, sources = upsample_linearring(
            xy, density=200.0, closed_ring=0, return_sources=1
        )
        ...
        ## choose a timestep:
        timeline = date2num(self.time_zero()) + self.timeline(
            output="grid", units="days"
        )

        steps = searchsorted(
            timeline, absdays_expanded
        )  # the output right after the requested date

        # adjust to whichever step closer:
        for i in range(len(steps)):
            if (
                steps[i] > 0
                and timeline[steps[i]] - absdays_expanded[i]
                > absdays_expanded[i] - timeline[steps[i] - 1]
            ):
                steps[i] -= 1
        ...
        interface_elevs = bathy_offset + concatenate(
            ([0], -self.z_levels())
        )  # Nk + 1 entries!
        elev_fs = zeros(
            len(global_cells), float64
        )  # these will be corrected for bathy_offset
        ...
        for proc in range(self.num_processors()):
            ...
            local_g, full_scal = self.cell_scalar(
                scalar_file, proc
            )  # [Ntimesteps,cells,z-level]
    ...
    def timeline(self, units="seconds", output="grid"):
        ...
        if output == "grid":
            ...
            steps = (
                arange(0, steps_output) * output_interval
                + self.starting_time_step()
                - offset
            )
            steps[0] += offset
            ...
            tseries_seconds = arange(
                0, steps_output
            ) * dt + first_prof_output_step * self.conf_float("dt")
    ...
    def simulation_period(self, end_is_last_output=True):
        ...
        if os.path.lexists(start_fn):
            if os.path.exists(store_fn):
                ...
                run_duration = (
                    self.conf.timestep() * int(self.conf["ntout"]) * (grid_outputs - 1)
                )
                ...
                if self.starting_time_step() == 0:
                    raise Exception(
                        "%s looks like a restart, but can't find Start or Store file"
                        % self.datadir
                    )
                ...
                raise Exception(
                    "Looks like a restart, but store and start files are missing"
                )
        else:
    ...
    def effective_time_zero(self):
        ...
        if len(suns) > 0:
            ...
            t = suns[0].time_zero() + datetime.timedelta(
                total_past_seconds / (24.0 * 3600.0)
            )
            t0 = t - datetime.timedelta(
                self.starting_time_step() * self.conf_float("dt") / (24.0 * 3600)
            )
        else:
            t0 = self.time_zero()
    ...
    def plot_edge_vector(self, u, proc, offset=0.0, **kwargs):
        ...
        if u.ndim != 1:
            raise Exception(
                "velocity vector has shape %s - should be 1-D" % str(u.shape)
            )

        # g = self.grid(proc)
        edata = self.edgedata(proc)
        vec_u = u[:, newaxis] * edata[:, 2:4]
        vec_origin = edata[:, 4:6] + offset * (edata[:, 0])[:, newaxis] * (
            edata[:, 3:1:-1] * [-1, 1]
        )
        ...
        # g.plot()
        quiver(vec_origin[:, 0], vec_origin[:, 1], vec_u[:, 0], vec_u[:, 1], **kwargs)

    def plot_scalar(
        self, scalar_for_proc, procs=None, clip=None, cmap=None, vmin=None, vmax=None
    ):
    ...
    def write_celldata(self, processor):
        ...
        for i in range(Nc):
            fp.write(
                "%.6f %.6f  %.6f  %.6f  %d  %d %d %d  %d %d %d  %d %d %d %.6f %.6f %.6f\n"
                % tuple(cdata[i, :])
            )
        fp.close()
    ...
    def show_topology(self, procs_per_node=4, topo_edges=None):
        ...
        proc_locs[:, :1] = proc_locs[:, :1] + 0.4 * (
            random(proc_locs[:, :1].shape) - 0.5
        )
            ...
            if l.find("Run is blowing up!") >= 0:
                ...
                if not m:
                    ...
                    for i in range(100):  # search for CFL details up to 100 lines away
                        ...
                        m = re.match(
                            r"Courant number problems at \((\d+),(\d+)\), Wmax=([-0-9\.]+), dz=([0-9\.]+) CmaxW=([0-9\.]+) > ([0-9\.]+)",
                            l,
                        )
                        if m:
                        ...
                        m = re.match(
                            r"Courant number problems at \((\d+),(\d+)\), Umax=([-0-9\.]+), dx=([0-9\.]+) CmaxU=([0-9\.]+) > ([0-9\.]+)",
                            l,
                        )
                        if m:
    ...
    def write_bov(self, label, proc, dims, data):
        ...
        for i in range(len(dims)):
            if dims[i] == "time_step":
                ...
                print(
                    "WARNING: cell dimension - data shape is %i but grid reports %i"
                    % (data.shape[i], g.Ncells())
                )
            elif dims[i] == "z_level" and data.shape[i] != self.conf_int("Nkmax"):
                print(
                    "WARNING: z_level dimension - data shape is %i but Nkmax is %i"
        ...
        formatted_name = os.path.join(
            self.datadir, label + "-" + "-".join(dims) + ".raw.%i" % proc
        )
    ...
    def harm_decomposition(
        self, consts=["constant", "M2"], ref_data=None, phase_units="minutes", skip=0.5
    ):
        ...
        if ref_data == "forcing":
            ...
            for proc in range(self.num_processors()):
                ...
                if forcing.n_bcells > 0:
                    print(
                        "Getting forcing data for boundary cell 0, processor %s" % proc
                    )
        ...
        for proc in range(self.num_processors()):
            ...
            for i in range(len(consts)):
                ...
                self.write_bov(
                    "%s_amp" % consts[i], proc=proc, dims=["cell"], data=amps[:, i]
                )
                ...
                self.write_bov(
                    "%s_phase" % consts[i], proc=proc, dims=["cell"], data=phase_data
                )
    ...
    def write_random_dataxy(self, fraction=0.05):
        ...
        if os.path.exists(dataxy_path):
            raise Exception(
                "Please remove existing profile locations file %s first" % dataxy_path
            )
    ...
    def write_initial_salinity(self, func, dimensions=3, func_by_index=0):
        ...
        """
        self.write_initial_cond(
            func, "InitSalinityFile", dimensions, func_by_index=func_by_index
        )

    def write_initial_temperature(self, func, dimensions=3, func_by_index=0):
        self.write_initial_cond(
            func, "InitTemperatureFile", dimensions, func_by_index=func_by_index
        )

    def copy_scalar_to_initial_cond(
        self,
        sun_in,
        ...
        if self.num_processors() != sun_in.num_processors():
            raise Exception(
                "Mismatch in number of processors: %d vs %d"
                % (self.num_processors(), sun_in.num_processors())
            )
    ...
    def write_initial_cond(
        self, func, scalar_file="InitSalinityFile", dimensions=3, func_by_index=0
    ):
                ...
                for i in range(Nc):
                    for k in range(Nk[i]):
                        if func_by_index:
                            ...
                            scalar[column_starts[i] + k] = func(
                                centers[i, 0], centers[i, 1], mid_elevations[k]
                            )
                fp = open(fname, "wb")
                fp.write(scalar.tostring())
                fp.close()
    ...
    def read_edge_z_level_scalar(self, label, proc=None, time_step=None):
        ...
        edgescalar = [
            raw_scalar[column_starts[i] : column_starts[i] + Nkc[i]] for i in range(Ne)
        ]
        return edgescalar

    def read_cell_z_level_scalar(
        self, label=None, fname=None, proc=None, time_step=None
    ):
        ...
        cellscalar = [
            raw_scalar[column_starts[i] : column_starts[i] + Nk[i]] for i in range(Nc)
        ]
        return cellscalar
        ...
        for p in range(self.num_processors()):
            ...
            try:
                ...
                if not full:
                    ...
                    print(
                        "This must be on a subdomain.  The best point wasn't in one of our cells"
                    )
    ...
    def grid_stats(self):
        ...
        print(
            ...
            % (mean(dg_total), median(dg_total), dg_total.min(), dg_total.max())
        )
    ...
    def calc_Cvol_distribution(self):
        ...
        dzmin_surface = (
            2 * self.dzmin
        )  # the dzmin cutoff for merging a surface cell with the one below it.
        ...
        for proc in range(self.num_processors()):
            ...
            valid_edge = (edata[:, 12] == 0) | (
                (edata[:, 12] == 5) & (edata[:, 8] >= 0)
            )
            ...
            for j in range(g.Nedges()):
                ...
                this_dzf[Nke[j] - 1] -= (
                    bed_edge[j] - z[Nke[j] - 1]
                )  # trim bed layer (which could be same as surface)
    ...
    def averaging_weights(self, proc, time_step, ztop=None, zbottom=None, dz=None):
        ...
        if 0:  # non vectorized - takes 0.8s per processor on the test run
            for i in range(len(Nk)):
                ...
                all_dz[
                    i, cbeds[i] :
                ] = 0.0  # empty cells below bed - might be an error...
                all_dz[i, ctops[i]] = h[i] - z[ctops[i]]  # recalc surface cell
                all_dz[i, cbeds[i] - 1] -= (
                    bed[i] - z[cbeds[i] - 1]
                )  # trim bed layer (which could be same as surface)
