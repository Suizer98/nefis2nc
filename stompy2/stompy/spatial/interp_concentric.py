...
class ConcentricInterpolator(object):
    ...
    def plot_rings(self):
        ...
        scat = ax.scatter(
            self.samples["x"], self.samples["y"], 20, self.samples["value"], cmap="jet"
        )
    ...
    def create_triangulation(self):
        ...
        for ring in self.rings:
            ...
            nodes = [g.add_or_find_node(x=x) for x in ring]
            for a, b in utils.circular_pairs(nodes):
                if a == b:
                    continue  # probably repeated start/end node
                try:
    ...
    def create_solution_grid(self):
        ...
        for c in g2.valid_cell_iter():
            if self.region.contains(geometry.Point(centroids[c])):
                continue
            g2.delete_cell(c)
    ...
    def solve(self):
        ...
        res = interp_4d.weighted_grid_extrapolation(
            self.grid,
            self.samples,
            alpha=self.alpha,
            x_col="x",
    ...
    def plot_result(self, **kw):
        ...
        ccoll = self.grid.plot_cells(values=self.result, ax=ax, cmap="jet", **kw)
        scat = ax.scatter(
            self.samples["x"],
            self.samples["y"],
            40,
    ...
    def rasterize(self, dx, dy=None):
        if dy is None:
            dy = dx
        return field.rasterize_grid_cells(
            self.grid, values=self.result, dx=dx, dy=dy, stretch=True
        )
