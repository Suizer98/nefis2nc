...
def test_read_otps():
    times = np.arange(
        np.datetime64("2010-01-01 00:00"),
    ...
    pred_h, pred_u, pred_v = read_otps.tide_pred(
        modfile, lon=[235.25], lat=[44.5], time=times
    )
...
def test_compare():
    ...
    for quant in ["z", "u", "U"]:
        ...
        if quant in ["z", "u", "v"]:
            pred_h, pred_u, pred_v = read_otps.tide_pred(
                modfile, lon=lonlats[:, 0], lat=lonlats[:, 1], time=times
            )
            if quant == "z":
                pred = pred_h[:, 0]
            elif quant == "u":
            ...
            _h, pred_U, pred_V = read_otps.tide_pred(
                modfile, lon=lonlats[:, 0], lat=lonlats[:, 1], time=times, z=1.0
            )
            if quant == "U":
                pred = pred_U[:, 0]
            elif quant == "V":
