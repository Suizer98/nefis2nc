...
class Diffuser(object):
    ...
    def set_dirichlet(self, value, cell=None, xy=None, on_duplicate="error"):
        ...
        if cell is None:
            raise self.PointOutsideDomain(
                "no cell found for dirichlet BC (cell=%s  xy=%s)" % (cell, xy)
            )
    ...
    def init_grid_geometry(self):
        ...
        # to janet
        self.d_j[bdry] = 2 * utils.mag(self.vc[self.c1[bdry]] - self.ec[bdry]).clip(
            self.d_j_min, np.inf
        )
    ...
    def vector_edge_to_cell(self, F_edge_normal):
        ...
        for c in range(self.grid.Ncells()):
            ...
            F_cell[c] = (
                self.normal_j[js] * F_edge_normal[js, None] * dist_edge_face[:, None]
            ).sum(axis=0) / self.area_c[c]
        return F_cell
    ...
    def solve_linear_system(self, animate=False):
        ...
        if 1:
            ...
            C_solved, code = linalg.gmres(
                self.A, self.b, x0=x0, tol=self.solve_tol, callback=print_progress
            )
    ...
    def calc_fluxes(self):
        ...
        self.Fperp = (
            -self.K_j * (self.C_solved[self.c2] - self.C_solved[self.c1]) / self.d_j
        )
    ...
    def calc_flux_vectors_and_grad(self):
        ...
        self.grad_flux_j = (
            self.flux_mag_c[self.c2] * (self.c2 >= 0) - self.flux_mag_c[self.c1]
        ) / self.d_j
    ...
    def vector_edge_to_node(self, c, v_normal_j):
        ...
        for ns in range(Nsides):
            ...
            mat = np.array([self.normal_j[j_prev], self.normal_j[j_next]])
