...
def trim_to_ij_convex_walk(grid, sel):
    ...
    while True:
        ...
        for c in candidates:  #  np.nonzero(sel)[0]:
            if not sel[c]:
                continue  # over-zealous queueing below
        ...
        while problems:
            ...
            if unmark:
                ...
                if 0:
                    ...
                    plt.axis((647100.0, 647278.0, 4185764.0, 4185913.0))
                    plt.pause(0.2)
        if not dirty:
            break
    # one option:
...
def splice_with_doubling(grid, refined):
    ...
    node_map, edge_map, cell_map = grid.add_grid(
        refined, merge_nodes=zip(merge_nodes, merge_nodes)
    )
    ...
    def refine(self, grid, direction):
        ...
        def ij2_to_xy(ij2):
            ...
            if (i2 % 2) and (j2 % 2):
                ...
                ij2s = [(i2 - 1, j2), (i2 + 1, j2)]
            elif j2 % 2:
                ij2s = [(i2, j2 - 1), (i2, j2 + 1)]
            else:
                assert False
        ...
        for c in grid.valid_cell_iter():
            ...
            a, b, c, d = np.concatenate([c_nodes[start_idx:], c_nodes[:start_idx]])
            # d----c
            # |    |
            # |    |
            # a----b
            ...
            if i2_step == 1 and j2_step == 1:
                ...
                new_cells = [[a, b, (b, c), (a, d)], [(a, d), (b, c), c, d]]
            elif j2_step == 2:
                new_cells = [[a, (a, b), (c, d), d], [(a, b), b, c, (c, d)]]
            else:
                assert False
...
class Coarsener:
    ...
    def coarsen(self, g_coarse, direction):
        ...
        for i in range(ij_min[0] + i_start, ij_max[0] + 1, i_step):
            for j in range(ij_min[1] + j_start, ij_max[1] + 1, j_step):
                ...
                nodes = np.array(
                    [ij_to_n.get(ij_corner, -1) for ij_corner in ij_corners]
                )
                if np.any(nodes < 0):
                    continue
    ...
    def coarsen_new(self, g_coarse, direction):
        ...
        def coarsen_one(iA, iB, jA, jB):
            # so i,j gives the nodes of one corner.
            ij_corners = [(iA, jA), (iB, jA), (iB, jB), (iA, jB)]
            nodes = np.array([ij_to_n.get(ij_corner, -1) for ij_corner in ij_corners])
            if np.any(nodes < 0):
                return
