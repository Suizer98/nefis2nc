    ...
    def from_dict(d):
        ...
        for ci in range(9):
            ...
            if d[cN] is not None:
                coefs.append(d[cN])
        cal = Calibration(
            txt="equation=%s" % d["equation"], coefs=coefs, units=d["units"]
        )
...
class Rbr(object):
    ...
    def synthesize_fields(self):
        ...
        if "Cond" in self.columns and "Temp" in self.columns:
            ...
            salt = seawater.salt(self.Cond / 42.90, self.Temp, Pres)
...
class RbrRsk(Rbr):
    ...
    def read_headers(self):
        self.txt_model = self.curs.execute("select model from instruments").fetchone()[
            0
        ]
        ...
        self.dt_s = self.curs.execute(
            "select samplingPeriod/1000.0 from schedules"
        ).fetchone()[0]

        # Number of channels =  4, number of samples =   5110, mode: Logging Complete
        self.Nchannels = self.curs.execute("select count(*) from channels").fetchone()[
            0
        ]
    ...
    def read_calibrations(self):
        ...
        self.curs.execute(
            """select channelID,equation,c0,c1,c2,c3,units 
    ...
    def read_data_raw(self):
        ...
        all_data = self.curs.execute(
            "select * from data order by tstamp asc"
        ).fetchall()
    ...
    def read_data(self):
        ...
        dt_a = datetime.datetime.fromtimestamp(
            self.data[0, 0] / 1000.0, pytz.utc
        ).replace(tzinfo=None)
        ...
        self.curs.execute(
            """select channelID,longName from channels
                              where not isDerived order by channelID"""
        )
        ch_map = dict(
            [("channel%02d" % cid, str(shortname)[:4]) for cid, shortname in self.curs]
        )
...
class RbrHex(Rbr):
    ...
    def read_headers(self):
        ...
        # Number of channels =  4, number of samples =   5110, mode: Logging Complete
        m = re.match(
            r"Number of channels\s*=\s*(\d+),\s*number of samples\s*=\s*(\d+)",
            self.txt_Nchannels,
        )
    ...
    def read_extras(self):
        ...
        while 1:
            ...
            m = re.match(
                "(Timestamp|Reset stamp) (\S+) (\S+) at sample (\d+) of type: (.*) STAMP",
                l,
            )
            if m is not None:
    ...
    def parse_datetime(self, d, t):
        try:
            ...
            dt = datetime.datetime.strptime(d + " " + t, "%Y/%m/%d %H:%M:%S")
        return (
            self.instrument_tz.localize(dt)
            .astimezone(self.target_tz)
            .replace(tzinfo=None)
        )
    ...
    def read_data(self):
        ...
        if nbytes_line.find("Number of bytes of data") == 0:
            ...
            if self.dt_s is not None and self.dt_s >= 1:
                ...
                self.data[:, 0] = (
                    self.data[0, 0] + self.dt_s * np.arange(len(self.data)) / 86400.0
                )
        else:
            # simple text format
    ...
    def parse_raw(self):
        ...
        for stamp_type, dtime, idx in self.timebases:
            ...
            if stamp_type == "STOP":
                ...
                dnum = base_times[-1] + nsteps * self.dt_s / 86400.0
        ...
        if self.dt_s is not None:
            ...
            base_times.append(
                base_times[-1]
                + self.dt_s * (base_samples[-1] - base_samples[-2]) / 86400.0
            )

        sample_i = np.arange(len(self.data))  # indices where we want times.
        self.data[:, 0] = np.interp(
            sample_i, base_samples, base_times, left=np.nan, right=np.nan
        )
