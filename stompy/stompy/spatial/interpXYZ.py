...
class interpXYZ(object):
    ...
    def _invdistweight(self):
        ...
        self.Finterp = idw(
            self.XY, self.XYout, maxdist=self.maxdist, NNear=self.NNear, p=self.p
        )

    def _krig(self):
        """Kriging interpolation"""
        self.Finterp = kriging(
            self.XY, self.XYout, maxdist=self.maxdist, NNear=self.NNear
        )

    def _linear(self):
        self.Finterp = LinearNDInterpolator(
            self.XY,
            np.zeros(
                (self.XY.shape[0]),
            ),
    ...
    def save(self, outfile="DEM.nc"):
        ...
        intparamstr = (
            "Interpolation Type: %s, Number of neighbours: %d, Maximum search distance: %3.1f m"
            % (self.method, self.NNear, self.maxdist)
        )
        if self.method == "idw":
            intparamstr += ", IDW power: %2.1f" % self.p
        elif self.method == "kriging":
            intparamstr += (
                ", Variogram model: %s, sill: %3.1f, nugget: %3.1f, range: %3.1f"
                % (self.varmodel, self.sill, self.nugget, self.vrange)
            )
        ...
        # Write the topo data
        tmpvarz = nc.createVariable(
            "topo", "f8", (dimnamey, dimnamex), zlib=True, least_significant_digit=1
        )
        tmpvarz[:] = self.Z
...
class Inputs(object):
    ...
    def _returnXY(self, x, y):
        ...
        return np.hstack(
            (np.reshape(np.ravel(X), (nx, 1)), np.reshape(np.ravel(Y), (nx, 1)))
        )
...
class idw(object):
    ...
    def __init__(self, XYin, XYout, **kwargs):
        ...
        XYout = XYout - ctr
        XYout[:, 0] = (XYout[:, 0] + 180.0) % 360 - 180

        # Perform query on all of the points in the grid
        dist, self.ind = kd.query(
            XYout, distance_upper_bound=self.maxdist, k=self.NNear
        )
