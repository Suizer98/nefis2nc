...
def cdec_dataset(
    ...
    params = dict(Stations=station, SensorNums=str(sensor), dur_code=duration)
    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        times_local = (
            pd.Series([interval_start, interval_end])
            .dt.tz_localize("UTC")
            .dt.tz_convert("America/Los_Angeles")
        )

        utc_strings = [
            utils.to_datetime(t).strftime("%Y-%m-%d")
        ...
        if (
            (cache_mode != "refresh")
            and (cache_fn is not None)
            and os.path.exists(cache_fn)
        ):
            ...
            df = pd.read_csv(
                StringIO(req.text), na_values=["---"], parse_dates=["DATE TIME"]
            )
            if len(df) == 0:
                continue
            ...
            if not (
                np.issubdtype(df["VALUE"].dtype, np.integer)
                or np.issubdtype(df["VALUE"].dtype, np.floating)
            ):
