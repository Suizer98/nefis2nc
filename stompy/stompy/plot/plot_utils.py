...
def ax_picker(ax):
    ...
    def on_release(event):
        ...
        if event.xdata and ax.pick_start is not None:
            new_pnt = np.array(
                [ax.pick_start[0], event.xdata, ax.pick_start[1], event.ydata]
            )
    ...
    **kwargs is any list of keyword arguments accepted by fun.
    ...
    def recolor(obj, col):
        try:
            ...
            pass
        try:
            obj.set_facecolor(col)
        except:
            pass
        try:
            obj.set_edgecolor(col)
        except:
            pass
        try:
            ch = obj.get_children()
...
def scalebar(
    ...
    if xy_transform is None:
        ...
        xy_transform = ScaleXOnly(xy_transform, ax.transData, xoffset=xy[0])
    ...
    for div in divisions:
        ...
        txts.append(
            ax.text(xmin + div, baseline, div_txt, ha="center", transform=txt_trans)
        )
...
def show_slopes(ax=None, slopes=[-5.0 / 3, -1], xfac=5, yfac=3):
    ax = ax or plt.gca()
...
class LogLogSlopeGrid(object):
    ...
    def __init__(
        self, ax=None, slopes=[-5 / 3.0], intervals=[10], xmin=None, xmax=None
    ):
...
def pad_pcolormesh(
    X, Y, Z, ax=None, dx_single=None, dy_single=None, fallback=True, **kwargs
):
...
class Sloper(object):
    def __init__(
        ...
        self.text = self.ax.text(
            xs[0], 1.5 * ys[0], "%g" % self.slope, transform=self.ax.transData
        )
...
def function_contours(f=lambda x, y: x - y, ax=None, Nx=20, Ny=20, V=10, fmt=None):
...
def bode(G, f=np.arange(0.01, 100, 0.01), fig=None):
...
def annotate_line(
    l, s, norm_position=0.5, offset_points=10, ax=None, buff=None, **kwargs
):
    """
...
def klabel(k, txt, color="0.5", ax=None, y=None, **kwargs):
    ...
    args = dict(rotation=90, va="bottom", ha="right", color=color)
    args.update(kwargs)
    if y is None:
        y = 0.02 + 0.6 * np.random.random()
    return ax.text(k, y, txt, transform=ax.get_xaxis_transform(), **args)
    ...
    def __eq__(self, other):
        if isinstance(other, ScaleXOnly):
            return (
                ...
                and (self._xscale == other._xscale)
                and (self._xoffset == other._xoffset)
            )
        else:
...
def cbar_interactive(cbar, extras=[], symmetric=False):
    ...
    def cb_u_cbar(event=None):
        if event is None or (
            cbar.solids is not None and cbar.solids not in cbar.ax.get_children()
        ):
...
def rgb_key(vel_scale, ax):
    ...
    syn_cMag, syn_cDir = np.meshgrid(
        0.5 * (syn_mag[1:] + syn_mag[:-1]), 0.5 * (syn_dir[1:] + syn_dir[:-1])
    )
    ...
    coll = ax.pcolormesh(syn_X, syn_Y, syn_rgb[:, :, 0], facecolors=rgba_ravel)
    coll.set_array(None)
...
# Transect methods:
def transect_tricontourf(*args, **kwargs):
    """
    ...
    positive_down: how to handle a ycoord with a positive:down attribute.
       ...
       "none"   -- ignore
    """
    return transect_tricontour_gen(*args, style="fill", **kwargs)


def transect_tricontour(*args, **kwargs):
    """
    xcoord, ycoord: name of the respective coordinate variables.
    ...
    positive_down: how to handle a ycoord with a positive:down attribute.
       ...
       "none"   -- ignore
    """
    return transect_tricontour_gen(*args, style="line", **kwargs)


def transect_tricontour_gen(
    data,
    xcoord,
    ycoord,
    V=None,
    ...
    **kwargs
    ...
    tri, mapper = transect_to_triangles(
        data,
        xcoord,
        ycoord,
...
def transect_to_triangles(
    data, xcoord, ycoord, elide_missing_columns=True, sortx=True, y_scale=1.0
):
    """
        ...
        for a in range(len(texts)):
            for b in range(a + 1, len(texts)):
                ...
                if np.all(int_min < int_max):
                    ...
                    opt = utils.to_unit(
                        np.array([int_max[1] - int_min[1], int_max[0] - int_min[0]])
                    )
