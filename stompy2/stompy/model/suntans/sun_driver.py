...
def dt_round(dt):
    ...
    if isinstance(dt, datetime.timedelta):
        ...
        return datetime.timedelta(
            days=td.days, seconds=new_seconds, microseconds=new_microseconds
        )
    else:
    ...
    def is_equal(self, other, limit_to_keys=None):
        ...
        for k in self.entries.keys():
            ...
            if k not in other.entries:
                ...
                return False
            elif self.val_to_str(other.entries[k][0]) != self.val_to_str(
                self.entries[k][0]
            ):
                log.debug(
                    "Different values key %s => %s, %s"
                    % (k, self.entries[k][0], other.entries[k][0])
                )
                return False
            ...
            if key not in self.entries:
                for other in self.entries:
                    if key.lower() == other.lower():
                        raise Exception(
                            "Probably a case-sensitive error: %s vs %s" % (key, other)
                        )
...
class SunConfig(GenericConfig):
    def time_zero(self):
        ...
        if start_year is not None:
            ...
            start_datetime = datetime.datetime(start_year, 1, 1, tzinfo=utc) + dt_round(
                datetime.timedelta(start_day)
            )
...
class SuntansModel(hm.HydroModel):
    ...
    def __init__(self, *a, **kw):
        self.load_template(
            os.path.join(os.path.dirname(__file__), "data", "suntans.dat")
        )
    ...
    def time0(self):
        ...
        dt = datetime.datetime.strptime(self.config["starttime"], "%Y%m%d.%H%M%S")
    ...
    def set_grid(self, grid):
        ...
        if "z_bed" not in grid.cells.dtype.names:
            if "depth" in grid.cells.dtype.names:
                ...
                self.log.warning(
                    "No depth information in grid nodes or cells.  Creating zero-depth"
                )
        ...
        e2c = (
            grid.edge_to_cells()
        )  # this is assumed in other parts of the code that do not recalculate it.
        nc1 = e2c[:, 0].copy()
        nc2 = e2c[:, 1].copy()
        nc1[nc1 < 0] = nc2[nc1 < 0]
        nc2[nc2 < 0] = nc1[nc2 < 0]
        ...
        if "edge_z_bed" in grid.edges.dtype.names:
            ...
            if np.any(deep_edges):
                self.log.info(
                    "%d edges had a specified depth deeper than neighboring cells.  Replaced them"
                    % deep_edges.sum()
                )
    ...
    def load_grid(self):
        ...
        if ("z_bed" not in g.cells.dtype.names) and (os.path.exists(cell_depth_fn)):
            self.log.debug("Will read cell depths, too")
            cell_xyz = np.loadtxt(cell_depth_fn)
            assert (
                cell_xyz.shape[0] == g.Ncells()
            ), "%s didn't have the right number of cells (%d vs %d)" % (
                cell_depth_fn,
                cell_xyz.shape[0],
                g.Ncells(),
            )
        ...
        if ("edge_z_bed" not in g.edges.dtype.names) and (
            os.path.exists(edge_depth_fn)
        ):
            self.log.debug("Will read edge depths, too")
            edge_xyz = np.loadtxt(edge_depth_fn)
            assert (
                edge_xyz.shape[0] == g.Nedges()
            ), "%s didn't have the right number of edges (%d vs %d)" % (
                edge_depth_fn,
                edge_xyz.shape[0],
                g.Nedges(),
            )
            assert np.allclose(edge_xyz[:, :2], g.edges_center()), (
                "%s edge locations don't match" % edge_depth_fn
            )
            # on disk these are positive down, but model driver convention is positive up
    ...
    def chain_restarts(self, count=None, load_grid=False):
        ...
        if isinstance(count, tuple):
            ...
            if len(runs) == 0:
                log.warning(
                    "chain_restarts wound up with zero runs for count=%s" % str(count)
                )
        return runs
    ...
    def write_monitor(self):
        ...
        np.savetxt(
            os.path.join(self.run_dir, self.config["DataLocations"]),
            np.array(valid_xys),
        )
    ...
    def write_startfiles(self):
        src_base = os.path.join(
            os.path.dirname(self.restart), self.restart_model.config["StoreFile"]
        )
    ...
    def copy_ic_to_bc(self, ic_var, bc_var):
        ...
        for ci, c in enumerate(
            utils.progress(self.bc_ds.cellp.values, msg="IC=>Open BCs")
        ):
        ...
        for ei, e in enumerate(
            utils.progress(self.bc_ds.edgep.values, msg="IC=>Flow BCs")
        ):
            c = self.grid.edges["cells"][e, 0]
            assert c >= 0, "Is this edge flipped"
            ic_values = self.ic_ds[ic_var].values[0, :, c]
            self.bc_ds["boundary_" + bc_var].isel(Ntype2=ei).values[:, :] = ic_values[
                None, :
            ]
    ...
    def set_initial_h_from_bc(self):
        ...
        if self.bc_ds is not None:
            if len(self.bc_ds.Ntype3) == 0:
                log.warning(
                    "Cannot set initial h from self.bc_ds because there are no type 3 edges"
                )
                return
            ...
            for bc in self.bcs:
                ...
                log.warning(
                    "Cannot set initial h from self.bcs because there are no Stage BCs"
                )
                return
        ...
        log.info(
            "Setting initial eta from BCs, value=max(z_bed,%.4f) (including z_offset of %.2f)"
            % (h, self.z_offset)
        )
    ...
    def write_forcing(self, overwrite=True):
        ...
        # map edge to BC data
        self.bc_type2 = defaultdict(
            lambda: defaultdict(list)
        )  # [<edge index>][<variable>]=>[DataArray,...]
        # map cell to BC data
        self.bc_type3 = defaultdict(
            lambda: defaultdict(list)
        )  # [<cell index>][<variable>]=>[DataArray,...]
        ...
        self.bc_type2_segments = defaultdict(
            lambda: defaultdict(list)
        )  # [<segment name>][<variable>]=>[DataArray,...]
        ...
        if common_time[0] >= self.run_start:
            common_time = np.concatenate(([self.run_start - pad], common_time))
    ...
    def ds_time_units(self):
        ...
        time_units = "seconds since %s-%s-%s %s:%s:%s" % (
            basetime[0:4],
            basetime[4:6],
            basetime[6:8],
            basetime[9:11],
    ...
    def write_bc_ds(self):
        self.bc_ds.to_netcdf(
            os.path.join(self.run_dir, self.config["netcdfBdyFile"]),
            encoding=dict(time={"units": self.ds_time_units()}),
        )

    def load_bc_ds(self):
        fn = os.path.join(self.run_dir, self.config["netcdfBdyFile"])
        if not os.path.exists(fn):
            return False
    ...
    def write_met_ds(self):
        fn = os.path.join(self.run_dir, self.config["metfile"])
        ...
        self.met_ds.to_netcdf(
            fn,
            encoding=dict(
                nt={"units": self.ds_time_units()}, Time={"units": self.ds_time_units()}
            ),
        )
    ...
    def layer_data(
        self, with_offset=False, edge_index=None, cell_index=None, z_bed=None
    ):
        """
        ...
        if r > 1.0:
            ...
            dzs[:] = depth / float(Nk)
        z_interface = np.concatenate(([z_max], z_max - np.cumsum(dzs)))
    ...
    def compile_bcs(self):
        ...
        for edge in list(self.bc_type2.keys()):
            if not (
                ("u" in self.bc_type2[edge])
                or ("v" in self.bc_type2[edge])
                or ("Q" in self.bc_type2[edge])
            ):
        ...
        ds["uc"] = (
            ...
            "Nk",
            "Ntype3",
        ), np.zeros((Nt, Nk, Ntype3), np.float64)
        ds["vc"] = (
            "Nt",
            "Nk",
            "Ntype3",
        ...
        ds["T"] = (
            ...
            "Nk",
            "Ntype3",
        ), 20 * np.ones((Nt, Nk, Ntype3), np.float64)
        ds["S"] = (
            "Nt",
            "Nk",
            "Ntype3",
        ...
        def interp_time(da):
            ...
            return np.interp(
                utils.to_dnum(ds.time.values), utils.to_dnum(da.time.values), da.values
            )
        ...
        for type3_i, type3_cell in enumerate(self.bc_type3):  # each edge/cell
            ...
            for v in bc.keys():  # each variable on that edge/cell
                ...
                combine_items(ds[v].isel(Ntype3=type3_i).values, bc[v], offset=offset)
        ...
        ds["boundary_h"] = ("Nt", "Ntype2"), np.zeros(
            (Nt, Ntype2), np.float64
        ) + self.z_offset
        ds["boundary_u"] = ("Nt", "Nk", "Ntype2"), np.zeros(
            (Nt, Nk, Ntype2), np.float64
        )
        ds["boundary_v"] = ("Nt", "Nk", "Ntype2"), np.zeros(
            (Nt, Nk, Ntype2), np.float64
        )
        ds["boundary_w"] = ("Nt", "Nk", "Ntype2"), np.zeros(
            (Nt, Nk, Ntype2), np.float64
        )
        ds["boundary_T"] = ("Nt", "Nk", "Ntype2"), np.zeros(
            (Nt, Nk, Ntype2), np.float64
        )
        ds["boundary_S"] = ("Nt", "Nk", "Ntype2"), np.zeros(
            (Nt, Nk, Ntype2), np.float64
        )
        ...
        # index.
        segment_names = list(
            self.bc_type2_segments.keys()
        )  # this establishes the order of the segments
        # make this distinct from 0 or 1 to aid debugging
        segment_ids = 100 + np.arange(len(segment_names))
        ds["seg_name"] = (
            "Nseg",
        ), segment_names  # not read by suntans, but maybe helps debugging
        ...
        for seg_i, seg_name in enumerate(segment_names):
            ...
            for v in bc.keys():  # only Q, but stick to the same pattern
                combine_items(ds["boundary_" + v].isel(Nseg=seg_i).values, bc[v])
        ...
        for type2_i, type2_edge in enumerate(self.bc_type2):  # each edge
            ...
            for v in bc.keys():  # each variable on that edge/cell
                ...
                if v != "Q":
                    combine_items(
                        ds["boundary_" + v].isel(Ntype2=type2_i).values, bc[v], offset
                    )
                else:
                    seg_name = bc[v]
                    ...
                    if len(seg_name) != 1:
                        log.warning(
                            "Only tested with a single value, but got %s"
                            % str(seg_name)
                        )
                    seg_name = seg_name[0]
        ...
        ds["point_Q"] = ("Nt", "Npoint"), np.zeros(
            (Nt, Npoint), np.float64
        )  # np.stack(point_Q,axis=-1)
        ds["point_S"] = ("Nt", "Npoint"), np.zeros(
            (Nt, Npoint), np.float64
        )  # np.stack(point_S,axis=-1)
        ds["point_T"] = ("Nt", "Npoint"), np.zeros(
            (Nt, Npoint), np.float64
        )  # np.stack(point_T,axis=-1)
        ...
        for pnt_idx, key in enumerate(self.bc_point_sources.keys()):
            ...
            combine_items(
                ds["point_Q"].isel(Npoint=pnt_idx).values,
            ...
            if "T" in self.bc_point_sources[key]:
                combine_items(
                    ds["point_T"].isel(Npoint=pnt_idx).values,
                    self.bc_point_sources[key]["T"],
                )
            if "S" in self.bc_point_sources[key]:
                combine_items(
                    ds["point_S"].isel(Npoint=pnt_idx).values,
    ...
    def write_stage_bc(self, bc):
        water_level = bc.data()
        assert (
            len(water_level.dims) <= 1
        ), "Water level must have dims either time, or none"
    ...
    def write_scalar_bc(self, bc):
        ...
        if bc.geom.type == "Point":
            self.log.info(
                "Assuming that Point geometry on a scalar bc implies point source"
            )
    ...
    def dredge_boundary(self, linestring, dredge_depth):
        ...
        return super(SuntansModel, self).dredge_boundary(
            linestring, dredge_depth, edge_field="edge_z_bed", cell_field="z_bed"
        )

    def dredge_discharge(self, point, dredge_depth):
        print("Call to dredge discharge, restart is", self.restart)
        return super(SuntansModel, self).dredge_discharge(
            point, dredge_depth, edge_field="edge_z_bed", cell_field="z_bed"
        )
    ...
    def bc_geom_to_interior_cell(self, geom):
        ...
        assert (
            c is not None
        ), "%s did not match any cells. LineString may be reversed?" % str(coords)
        return c
    ...
    def set_times_from_config(self):
        ...
        if self.restart:
            ...
            if start_files:
                start = store_file.StoreFile(
                    model=self, proc=0, filename=start_files[0]
                )
            ...
            start_dt = datetime.datetime.strptime(
                self.config["starttime"], "%Y%m%d.%H%M%S"
            )
    ...
    def update_config(self):
        ...
        if not self.restart:
            ...
            log.info(
                "starttime pulled from previous run: %s" % self.config["starttime"]
            )
            restart_time = self.restart_model.restartable_time()
            assert (
                self.run_start == restart_time
            ), "Configured sim start and restart timestamp don't match"
        ...
        if self.coriolis_f == "auto":
            if self.projection is None:
                ...
                f = 2 * Omega * np.sin(lat * np.pi / 180.0)
                self.config["Coriolis_f"] = "%.5e" % f
                log.debug(
                    "Using %.2f as latitude for Coriolis => f=%s"
                    % (lat, self.config["Coriolis_f"])
                )
    ...
    def restart_copier(self, src, dst, on_exists="replace_link"):
        ...
        if os.path.lexists(dst):
            ...
            if on_exists == "replace_link" and os.path.islink(dst):
                ...
                raise Exception(
                    "Restart copier %s=>%s found destination already exists. on_exists='fail'"
                    % (src, dst)
                )
            else:
    ...
    def write_grid(self):
        if not self.restart:
            ...
            self.grid.write_suntans_hybrid(
                self.run_dir, overwrite=True, z_offset=self.z_offset
            )
            self.write_grid_bathy()
        else:
            parent_base = os.path.dirname(self.restart)
            for fn in ["cells.dat", "edges.dat", "points.dat", "depths.dat-voro"]:
                self.restart_copier(
                    os.path.join(parent_base, fn), os.path.join(self.run_dir, fn)
                )
    ...
    def write_grid_bathy(self):
        ...
        if np.any(shallow):
            log.warning(
                "%d of %d cell depths extend above z=0 even with offset of %.2f"
                % (np.sum(shallow), len(shallow), self.z_offset)
            )
    ...
    def grid_as_dataset(self):
        ...
        ds["nfaces"] = ("Nc",), [
            self.grid.cell_Nsides(c) for c in range(self.grid.Ncells())
        ]
        ds["edges"] = ("Ne", "two"), self.grid.edges["nodes"]
        ds["neigh"] = ("Nc", "numsides"), [
            self.grid.cell_to_cells(c, pad=True) for c in range(self.grid.Ncells())
        ]
    ...
    def zero_met(self, times=None):
        ...
        ds_met["Pair"] = const(("nt", "NPair"), 1000.0)  # units?
        ds_met["RH"] = const(("nt", "NRH"), 80.0)
        ds_met["rain"] = const(("nt", "Nrain"), 0.0)
    ...
    def avg_outputs(self):
        ...
        if int(self.config["calcaverage"]):
            fns = glob.glob(
                os.path.join(self.run_dir, self.config["averageNetcdfFile"] + "*")
            )
            fns.sort()
            return fns
        else:
    ...
    def map_outputs(self):
        ...
        if int(self.config["outputNetcdf"]):
            if self.config["mergeArrays"] is None or int(self.config["mergeArrays"]):
                ...
                fns = glob.glob(
                    os.path.join(
                        self.run_dir, self.config["outputNetcdfFile"] + "_*.nc"
                    )
                )
                fns.sort()
                return fns
                ...
                fns = glob.glob(
                    os.path.join(self.run_dir, self.config["outputNetcdfFile"] + "*")
                )
    ...
    def parse_profdata(cls, fn):
        ...
        with open(fn, "rb") as fp:
            ...
            pdata["dzz"] = ("layer",), np.fromfile(
                fp, np.float64, pdata["nkmax_profs"].item()
            )
            pdata["all_indices"] = np.fromfile(
                fp, np.int32, pdata["num_total_data_points"].item()
            )
            dataxy = np.fromfile(
                fp, np.float64, 2 * pdata["num_total_data_points"].item()
            )
            pdata["request_xy"] = ("request", "xy"), dataxy.reshape((-1, 2))
            pdata["request_xy"].attrs[
                "description"
            ] = "Coordinates of the requested profiles"

            xvyv = np.fromfile(
                fp,
                np.float64,
                2
                * (pdata["num_total_data_points"] * pdata["num_interp_points"]).item(),
            )
    ...
    def read_profile_data_raw(self, scalar, pdata=None, memmap=True):
        ...
        if not os.path.exists(prof_fname):
            log.debug(
                "Request for profile for %s, but %s does not exist"
                % (scalar, prof_fname)
            )
            return None
    ...
    def monitor_output(self, nan_nodata=False, dv_from_map=False):
        ...
        """
        if "DataLocations" not in self.config:
            return None
        ...
        for scalar in list(file_to_var.keys()):
            ...
            if raw_data is not None:
                ...
                if nan_nodata and np.any(raw_data == self.monitor_nodata):
                    ...
                    raw_data = np.where(
                        raw_data == self.monitor_nodata, np.nan, raw_data
                    )
        ...
        pdata["time"] = ("time",), (
            self.run_start + dt_prof * np.arange(pdata.dims["time"])
        )
        ...
        if dv_from_map:
            if self.monitor_dv is None:
                if 0:  # read from map file, but that may not be valid until end of run
                    ...
                    self.monitor_dv = self.extract_station_map(
                        xy=pdata.prof_xy.values[:, :], data_vars="dv"
                    )
                    ...
                    assert np.all(
                        np.isfinite(mon_dv)
                    ), "Failed to get depths for all profile locatins"
    ...
    def subdomain_grid(self, p):
        ...
        if p not in self._subdomain_grids:
            sub_g = unstructured_grid.UnstructuredGrid.read_suntans_hybrid(
                path=self.run_dir,
                points="points.dat",
                edges="edges.dat.%d" % p,
                cells="cells.dat.%d" % p,
            ...
            if ("dv" in sub_g.cells.dtype.names) and (
                "z_bed" not in sub_g.cells.dtype.names
            ):
    ...
    def extract_transect_monitor(
        ...
        if time_mode == "outer":
            assert (
                time.ndim == 0
            ), "Not ready for array-valued time with time_mode='outer'"
        ...
        for loc, t in xyt():
            ...
            stn = self.extract_station_monitor(
                xy=loc, chain_count=t_slice, dv_from_map=dv_from_map
            )
            if stn is None:
                log.warning(
                    "Found no monitor data for %s. Skip transect" % str(t_slice)
                )
                return None
            if np.isscalar(t):
                if (t < stn.time.values[0]) or (t > stn.time.values[-1]):
                    log.info(
                        "Requested time %s is outside the range of the model output" % t
                    )
                    return None
        ...
        for v in tran.data_vars:
            if not np.issubdtype(tran[v].dtype, np.floating):
                continue
    ...
    def adjust_transect_for_dzmin_surface(
        self, tran, update_vars=["salt", "temp"], dzmin_surf=0.25
    ):
        """
    ...
    def extract_transect(
        ...
        if np.issubdtype(type(time), np.integer):
            ...
            if dt < self.run_start:
                ...
                if run is not self:  # avoid inf. recursion
                    return run.extract_transect(
                        xy=xy, ll=ll, time=time, dx=dx, vars=vars
                    )
                else:
        ...
        def time_to_isel(ds, times, mode="nearest"):
            ...
            if isinstance(times, slice):
                return slice(
                    time_to_isel(ds, times.start, mode="before"),
            ...
            for pnti, pnt in enumerate(xy):
                ...
                if c is not None:
                    ...
                    if ds is None:
                        ...
                        del ds[
                            "Nk"
                        ]  # delete old variable, leaving Nk as just a dimension
        ...
        renames = dict(Nk="layer", Nkw="interface", uc="Ve", vc="Vn", w="Vu_int")
        renames = {
            x: renames[x] for x in renames if (x in transect) or (x in transect.dims)
        }
        transect = transect.rename(**renames)
        transect["U"] = ("sample", "layer", "xy"), np.concatenate(
            [transect.Ve.values[..., None], transect.Vn.values[..., None]], axis=-1
        )
        ...
        if "dzz" not in transect:
            # fabricate a dzz
            eta_2d, dv_2d, z_w_2d = xr.broadcast(
                transect["eta"], transect["dv"], -ds["z_w"]
            )
        ...
        z_int = np.concatenate([z_top[:, :1], z_int, z_bot[:, -1:]], axis=1)
    ...
    def extract_station_monitor(
        self, xy=None, ll=None, chain_count=1, dv_from_map=False, data_vars=None
    ):
        """
        ...
        if chain_count != 1:
            ...
            dss = [
                mod.extract_station_monitor(
                    xy=xy, ll=ll, chain_count=1, data_vars=data_vars, dv_from_map=False
                )
                for mod in restarts
            ]
            if len(dss) == 0:
            ...
            if dv_from_map:
                # just to get dv...
                me = self.extract_station_monitor(
                    xy=xy, ll=ll, chain_count=1, dv_from_map=True
                )
                chained["dv"] = me.dv
            return chained
        ...
        if orig_ndim == 1:
            ...
            raise Exception(
                "Can only handle single coordinates or an list of coordinates"
            )
    ...
    def extract_station(
        ...
        if source in ["auto", "monitor"]:
            ds = self.extract_station_monitor(
                xy=xy,
                ll=ll,
        ...
        if source in ["auto", "map"]:
            return self.extract_station_map(
    ...
    def extract_station_map(self, xy=None, ll=None, chain_count=1, data_vars=None):
        ...
        for run in runs:
            ...
            for proc, map_fn in enumerate(run.map_outputs()):
                if proc not in hot_procs:
                    continue  # doesn't have any hits
                ...
                if model_out is None:
                    ...
                    for d in map_ds.data_vars:
                        ...
                        if "Nc" in map_ds[d].dims:
                            ...
                            model_out[d] = tuple(new_dims), np.zeros(
                                new_shape, map_ds[d].dtype
                            )
                ...
                # proc
                Nc_indexer = xr.DataArray(
                    [m[1] for m in matches if m[0] == proc], dims=["proc_station"]
                )
                ...
                proc_stations = np.array(
                    [i for i, m in enumerate(matches) if m[0] == proc]
                )

                for d in map_ds.data_vars:
                    if d not in model_out:
                        continue
                    ...
                    dims = ["proc_station"] + [
                        d for d in extracted.dims if d != "proc_station"
                    ]
        ...
        if "numsides" in model_out.dims:
            if max_numsides != min_numsides:
                log.warning(
                    "numsides varies %d to %d over restarts.  Kludge around"
        ...
        # copy from matches
        combined_ds["distance_from_target"] = ("station",), np.zeros(
            num_stations, np.float64
        )
        ...
        combined_ds["station_cell"].attrs[
            "description"
        ] = "Cell index in subdomain grid"
        combined_ds["station_x"] = ("station",), xy[..., 0]
        combined_ds["station_y"] = ("station",), xy[..., 1]
