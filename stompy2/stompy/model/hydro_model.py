...
class BC(object):
    ...
    def write_t3d(self, da, z_bed, fn=None):
        ...
        assert np.all(
            np.diff(sigma) > 0
        ), "Need more sophisticated treatment of sigma in t3d file"
    ...
    def as_data_array(self, data, quantity="value"):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if "time" not in data.dims:
                print(
                    "warning: Converting pd.DataFrame to xr.Dataset, but no time dimension."
                )
        elif isinstance(data, pd.Series):
            data = xr.DataArray.from_series(data)
            if "time" not in data.dims:
                print(
                    "warning: Converting pd.Series to xr.DataArray, but no time dimension."
                )
        ...
        if isinstance(data, xr.Dataset):
            if len(data.data_vars) == 1:
                ...
                raise Exception(
                    "Dataset has multiple data variables -- not sure which to use: %s"
                    % (str(data.data_vars))
                )
    ...
    def data(self):
        ...
        if "time" in da.dims:
            ...
            if (data_start > self.model.run_start) or (data_stop < self.model.run_stop):
                msg = "Run: %s -- %s, but BC data for %s is %s -- %s" % (
                    self.model.run_start,
                    self.model.run_stop,
                    self.name,
                    data_start,
                    data_stop,
                ...
                if self.on_insufficient_data == "exception":
                    ...
                    raise Exception(
                        "Bad setting for on_insufficient_data='%s'"
    ...
    def write_bokeh(self, filename=None, path=".", title=None, mode="cdn"):
        ...
        bio.output_file(output_fn, title=title, mode=mode)
...
class LowpassGodin(BCFilter):
    ...
    def transform_output(self, da):
        ...
        da.values[:] = filters.lowpass_godin(da.values, utils.to_dnum(da.time))
        return da
...
class Lowpass(BCFilter):
    ...
    def transform_output(self, da):
        ...
        assert (
            self.cutoff_hours is not None
        ), "Must specify lowpass threshold cutoff_hors"
        ...
        if np.any(~np.isfinite(data_in)):
            if self.fill_nan:
                log.info(
                    "Lowpass: %d of %d data values will be filled"
                    % (np.sum(~np.isfinite(data_in)), len(data_in))
                )
                data_in = utils.fill_invalid(data_in, ends="constant")
            else:
                log.error(
                    "Lowpass: %d of %d data values are not finite"
                    % (np.sum(~np.isfinite(data_in)), len(data_in))
                )
        ...
        ), "Lowpass: %d of %d output data values are not finite" % (
...
class PadTime(BCFilter):
    ...
    def transform_output(self, da):
        ...
        if self.bc.model.run_stop + self.pad_time > da.time[-1]:
            self.bc.model.log.warning(
                "Will extend flow for %s with %s! (data end %s)"
                % (self.bc.name, self.pad_value, da.time.values[-1])
            )
        ...
        if self.bc.model.run_start - self.pad_time < da.time[0]:
            self.bc.model.log.warning(
                "Will prepend flow for %s with %s! (data starts %s)"
...
class RoughnessBC(BC):
    ...
    def write_bokeh(self, filename=None, path=".", title=None, mode="cdn"):
        ...
        p = bplt.figure(
            plot_width=750, plot_height=750, title=title, active_scroll="wheel_zoom"
        )
    ...
    def plot_bokeh(self, da, plot, label=None):
        ...
        color_mapper = LinearColorMapper(
            palette="Viridis256", low=rough.min(), high=rough.max()
        )
        ...
        colors = [
            "#%02x%02x%02x" % (int(m[0] * 255), int(m[1] * 255), int(m[2] * 255))
            for m in mapped
        ]
    ...
    def evaluate(self, t):
        ...
        if "time" in water_level.dims:
            water_level = np.interp(
                utils.to_dnum(t), utils.to_dnum(water_level.time), water_level.values
            )
        return water_level
...
class HarmonicStageBC(StageBC):
    ...
    def __init__(self, **kw):
        ...
        consts = [k for k in kw if k in tide_consts.const_names]
        for k in consts:
...
class ScalarBC(BC):
    ...
    def __init__(self, **kw):
        ...
        if "parent" in kw:
            ...
            new_kw = dict(name=self.parent.name, geom=self.parent.geom)
            new_kw.update(kw)
            kw = new_kw
...
class MpiModel(object):
    ...
    def mpirun_mpiexec(self, cmd, num_procs, working_dir, wait):
        ...
        """
        real_cmd = [self.mpiexec, "-n", "%d" % num_procs] + list(self.mpi_args) + cmd
        if not wait:
            raise Exception(
                (
                    "Request to start MPI process "
                    "(flavor=%s) without waiting not supported"
                )
                % self.mpi_flavor
            )
    ...
    def slurm_check_mpi_ntasks(self, n):
        ...
        if n > n_global:
            print(
                "In SLURM task, but ntasks(%d) != local_config num_procs(%d)"
                % (n_global, n),
                flush=True,
            )
    ...
    def slurm_srun_options(self, n):
        ...
        if n_tasks == n:
            ...
            options = ["-n", str(n)]
            print(f"Homogeneous oversized job.  Add {' '.join(options)}", flush=True)
            return options
...
class HydroModel(object):
    ...
    restart = None
    restart_model = (
        None  # reference to HydroModel instance(subclass) that we are continuing
    )
    ...
    def create_with_mode(self, path, mode="create"):
        ...
        if mode == "create":
            ...
            assert not os.path.exists(path), (
                "Directory %s exists, but mode is noclobber" % path
            )
            os.makedirs(path)
    ...
    def dredge_boundary(
        ...
        nodes_to_dredge = np.concatenate([g.cell_to_nodes(c) for c in cells_to_dredge])
        nodes_to_dredge = np.unique(nodes_to_dredge)

        if edge_field:
            if edge_field in g.edges.dtype.names:
                assert np.all(feat_edges >= 0)
                g.edges[edge_field][feat_edges] = np.minimum(
                    g.edges[edge_field][feat_edges], dredge_depth
                )
            else:
        ...
        if node_field:
            ...
            g.nodes[node_field][nodes_to_dredge] = np.minimum(
                g.nodes[node_field][nodes_to_dredge], dredge_depth
            )
        if cell_field:
            assert np.all(cells_to_dredge >= 0)
            g.cells[cell_field][cells_to_dredge] = np.minimum(
                g.cells[cell_field][cells_to_dredge], dredge_depth
            )

    def dredge_discharge(
        self, point, dredge_depth, node_field=None, edge_field=None, cell_field=None
    ):
        if not (node_field or edge_field or cell_field):
            raise Exception("dredge_boundary: must specify at least one depth field")
        ...
        if edge_field:
            ...
            g.edges[edge_field][edges] = np.minimum(
                g.edges[edge_field][edges], dredge_depth
            )
        if node_field:
            nodes = g.cell_to_nodes(cell)
            g.nodes[node_field][nodes] = np.minimum(
                g.nodes[node_field][nodes], dredge_depth
            )
    ...
    def write_bc(self, bc):
        if isinstance(bc, MultiBC):
            ...
            raise Exception(
                "BC type %s not handled by class %s" % (bc.__class__, self.__class__)
            )
    ...
    def add_bcs(self, bcs):
        ...
        if isinstance(bcs, BC):
            bcs = [bcs]
        for bc in bcs:
            assert (bc.model is None) or (
                bc.model == self
            ), "Not expecting to share BC objects"
            bc.model = self
        self.bcs.extend(bcs)
...
def extract_transect(
    ...
    bundle_components=[["U", ("ucx", "ucy")], ["Uavg", ("ucxz", "ucyz")]],
):
    """
    ...
    cell_map = [
        grid.select_cells_nearest(line_sampled[samp_i], inside=True)
    ...
    new_ds["d_sample_bnd"] = (cell_dim, "two"), np.array(
        [distance - dx_sample / 2, distance + dx_sample / 2]
    ).T
...
class OTPSHelper(object):
    ...
    def dataset(self):
        ...
        ds = xr.Dataset()
        times = np.arange(self.data_start, self.data_stop, 15 * np.timedelta64(60, "s"))
        ...
        pred_h, pred_U, pred_V = read_otps.tide_pred(
            modfile, lon=ll[:, 0], lat=ll[:, 1], time=times, z=1.0
        )
...
class HycomMultiBC(MultiBC):
    ...
    def populate_files(self):
        self.data_files = hycom.fetch_range(
            self.ll_box[:2],
            self.ll_box[2:],
    ...
    def init_bathy(self):
        ...
        hy_bathy = self.hy_bathy = hycom.hycom_bathymetry(
            self.model.run_start, self.cache_dir
        )
        # xr.open_dataset( os.path.join(self.cache_dir,'depth_GLBa0.08_09.nc') )
        lon_min, lon_max, lat_min, lat_max = self.ll_box

        sel = (
            (hy_bathy.Latitude.values >= lat_min)
...
class HycomMultiScalarBC(HycomMultiBC):
    ...
    def populate_values(self):
        ...
        for i, sub_bc in enumerate(self.sub_bcs):
            ...
            sub_ds[sun_var] = ("time", "layer"), np.zeros(
                (sub_ds.dims["time"], layers.dims["Nk"]), np.float64
            )
            sub_bc.edge_depth = edge_depth = self.model.grid.edge_depths()[
                sub_bc.grid_edge
            ]  # positive up
            ...
            sun_z_interface = (-self.model.z_offset) + layers.z_interface.values.clip(
                edge_depth, np.inf
            )
            sub_bc.sun_z_interfaces = sun_z_interface
            # And the pointwise data from hycom:
            hy_layers = hy_ds0.depth.values.copy()
            sub_bc.hy_valid = valid = np.isfinite(
                hy_ds0[hy_scalar].isel(lat=row, lon=col).values
            )
            ...
            if len(hycom_depths) != 0:
                sub_bc.hy_bed_depth = max(
                    hycom_depths[-1] + 1.0, self.bathy(hy_xy[sub_bc.hy_row_col])
                )
                sub_bc.hycom_depths = np.concatenate(
                    [hycom_depths, [sub_bc.hy_bed_depth]]
                )
            else:
                ...
                log.warning(
                    "Hmm - got a dry hycom edge, even though should be skipping those now"
                )
                sub_bc._dataset[sun_var].values[:] = 100000000.0
        ...
        for ti, fn in enumerate(self.data_files):
            ...
            for i, sub_bc in enumerate(self.sub_bcs):
                ...
                sub_scalar_val = np.concatenate(
                    [scalar_val[z_sel, row, col], scalar_val_bottom[None, row, col]]
                )
                ...
                if 0:
                    ...
                    valdz = np.concatenate(
                        ([0], np.cumsum(np.diff(hy_depths) * interval_mean_val))
                    )
                    sun_valdz = np.interp(-sub_bc.sun_z_interfaces, hy_depths, valdz)
                    sun_d_veldz = np.diff(sun_valdz)

                    sub_bc._dataset[sun_var].values[ti, sun_valid] = (
                        sun_d_veldz[sun_valid] / sun_dz[sun_valid]
                    )
                else:
                    ...
                    sub_bc._dataset[sun_var].values[ti, :] = np.interp(
                        depth_middle, hy_depths, sub_scalar_val
                    )
...
class HycomMultiVelocityBC(HycomMultiBC):
    ...
    class VelocityProfileBC(VelocityBC):
        ...
        def update_Q_in(self):
            ...
            Q_in = self.edge_length * (
                self.inward_normal[0] * Uint + self.inward_normal[1] * Vint
            )
            ds["Q_in"].values[:] = Q_in
            ds["Uint"].values[:] = Uint
            ds["Vint"].values[:] = Vint
    ...
    def populate_values(self):
        ...
        for i, sub_bc in enumerate(self.sub_bcs):
            ...
            sub_ds["u"] = ("time", "layer"), np.zeros(
                (sub_ds.dims["time"], layers.dims["Nk"]), np.float64
            )
            sub_ds["v"] = ("time", "layer"), np.zeros(
                (sub_ds.dims["time"], layers.dims["Nk"]), np.float64
            )
            # depth-integrated transport on suntans layers, in m2/s
            sub_ds["Uint"] = ("time",), np.nan * np.ones(
                sub_ds.dims["time"], np.float64
            )
            sub_ds["Vint"] = ("time",), np.nan * np.ones(
                sub_ds.dims["time"], np.float64
            )
            # project transport to edge normal * edge_length to get m3/s
            sub_ds["Q_in"] = ("time",), np.nan * np.ones(
                sub_ds.dims["time"], np.float64
            )
            ...
            hy_layers = hy_ds0.depth.values.copy()
            sub_bc.hy_valid = valid = np.isfinite(
                hy_ds0.water_u.isel(lat=row, lon=col).values
            )
            hycom_depths = hy_ds0.depth.values[valid]
            # possible that hy_bed_depth is not quite correct, and hycom has data
            # even deeper.  in that case just pad out the depth a bit so there
        ...
        for ti, fn in enumerate(self.data_files):
            ...
            for i, sub_bc in enumerate(self.sub_bcs):
                ...
                for water_vel, water_vel_bottom, sun_var, trans_var in [
                    (water_u, water_u_bottom, "u", "Uint"),
                    (water_v, water_v_bottom, "v", "Vint"),
                ]:
                    sub_water_vel = np.concatenate(
                        [water_vel[z_sel, row, col], water_vel_bottom[None, row, col]]
                    )
                    ...
                    veldz = np.concatenate(
                        ([0], np.cumsum(np.diff(hy_depths) * interval_mean_vel))
                    )
                    sun_veldz = np.interp(-sub_bc.sun_z_interfaces, hy_depths, veldz)
                    sun_d_veldz = np.diff(sun_veldz)

                    sub_bc._dataset[sun_var].values[
                        ti, ~sun_valid
                    ] = 0.0  # just to be sure...
                    sub_bc._dataset[sun_var].values[ti, sun_valid] = (
                        sun_d_veldz[sun_valid] / sun_dz[sun_valid]
                    ...
                    int_A = sun_veldz[-1]
                    int_B = (
                        np.diff(-sub_bc.sun_z_interfaces)
                        * sub_bc._dataset[sun_var].values[ti, :]
                    ).sum()
        ...
        for i, sub_bc in enumerate(self.sub_bcs):
            Q_in = sub_bc.edge_length * (
                sub_bc.inward_normal[0] * sub_bc._dataset["Uint"].values
            ...
            total_flux_A += sub_bc.edge_length * (
                sub_bc.sun_z_interfaces[0] - sub_bc.sun_z_interfaces[-1]
            )

        Q_error = total_Q - target_Q
        vel_error = Q_error / total_flux_A
        log.info(
            "Velocity error: %.6f -- %.6f m/s" % (vel_error.min(), vel_error.max())
        )
        ...
        for i, sub_bc in enumerate(self.sub_bcs):
            ...
            sub_bc._dataset["u"].values[:, :] -= (
                vel_error[:, None] * sub_bc.inward_normal[0]
            )
            sub_bc._dataset["v"].values[:, :] -= (
                vel_error[:, None] * sub_bc.inward_normal[1]
            )
            sub_bc.update_Q_in()
        ...
        log.info(
            "Post-adjustment velocity error: %.6f -- %.6f m/s"
            % (adj_vel_error.min(), adj_vel_error.max())
        )
...
class CdecFlowBC(CdecBC, FlowBC):
    ...
    def src_data(self):
        ...
        if ds is not None:
            ...
            log.warning(
                "CDEC station %s, sensor %d found no data" % (self.station, self.sensor)
            )
            return self.default
    ...
    def fetch_for_period(self, period_start, period_stop):
        ...
        ds = cdec.cdec_dataset(
            station=self.station,
            start_date=period_start - self.pad,
            end_date=period_stop + self.pad,
            sensor=self.sensor,
            cache_dir=self.cache_dir,
        )
        if ds is not None:
...
class CdecStageBC(CdecBC, StageBC):
    ...
    def fetch_for_period(self, period_start, period_stop):
        ...
        ds = cdec.cdec_dataset(
            station=self.station,
            start_date=period_start - pad,
            end_date=period_stop + pad,
            sensor=self.sensor,
            cache_dir=self.cache_dir,
        )
        if ds is not None:
...
class NwisScalarBC(NwisBC, ScalarBC):
    def src_data(self):
        ...
        scalar_name = [
            n for n in ds.data_vars if n not in ["tz_cd", "datenum", "time"]
        ][0]
        return ds[scalar_name]

    def write_bokeh(self, **kw):
        defaults = dict(
            title="Scalar: %s (%s, product %s)"
            % (self.name, self.station, self.product_id)
        )
        defaults.update(kw)
