...
def get_z_dz(tran):
    ...
    if "z_int" in tran:
        ...
        # indices.
        dz_middle = 0.5 * (dctr[b(slice(None, -1))] + dctr[b(slice(1, None))])

        dz = np.concatenate(
            [dz_middle[b(slice(None, 1))], dz_middle, dz_middle[b(slice(-1, None))]],
            axis=axis,
        )
        ...
        dz[b(slice(None, -1))] = np.where(
            np.isnan(dz[b(slice(None, -1))]),
            dz[b(slice(1, None))],
            dz[b(slice(None, -1))],
        )
        dz[b(slice(1, None))] = np.where(
            np.isnan(dz[b(slice(1, None))]),
            dz[b(slice(None, -1))],
            dz[b(slice(1, None))],
        )
...
def get_dx_sample(tran):
    if "dx_sample" not in tran:
        tran["dx_sample"] = ("sample",), utils.center_to_interval(
            get_d_sample(tran).values
        )
...
def add_rozovski_angles(tran, src, name="roz_angle", force_left=False):
    ...
    # left of east)
    roz_angle = np.arctan2(quv.values[..., 1], quv.values[..., 0])
    if force_left:
        ln = left_normal(tran)
        qnorm = np.sum(quv.values * ln, axis=1)
...
def add_normal_tangential(
    tran, src="U", dst="Utrn", frame="trn", comp_names=["downstream", "left"]
):
    """
    ...
    tran[angle_fld] = ("sample",), np.arctan2(
        dxy[:, 0], -dxy[:, 1]
    )  # includes rotation to get normal
...
def add_rotated(
    ...
    vec_norm = xr.concat([np.cos(angle), np.sin(angle)], dim=frame).transpose(
        "sample", frame
    )
    ...
    with open(filename, "rt") as fp:
        ...
        while 1:  # Loop over transects
            ...
            while 1:  # Loop over locations within one transect:
                ...
                dist_m, x, y, idx, k_bed, k_surf, z_bed, z_surf = [
...
def section_hydro_parsed_to_transect(section, filename):
    ...
    ds["z_surf"] = ("sample",), np.array(
        [p["z_surf"] for p in wet_profiles]
    )  # z_surf is positive up
    ds["z_bed"] = ("sample",), np.array(
        [-p["z_bed"] for p in wet_profiles]
    ...
    for sample_i, p in enumerate(wet_profiles):
        ...
        assert (
            np.abs(interface_elevations[-1] - p["z_surf"]) < 0.01
        ), "Layers did not add up"
    ...
    ds["U"] = ("sample", "layer", "xy"), np.concatenate(
        (Ve[:, :, None], Vn[:, :, None]), axis=2
    )
...
def resample_z(tran, new_z, save_original=None, new_z_positive="same"):
    ...
    for v in tran.data_vars:
        ...
        for index in np.ndindex(*iter_shape):
            ...
            my_src_ints = np.concatenate(
                (my_src_bottom[src_valid], my_src_top[src_valid][-1:])
            )
...
def extrapolate_vertical(tran, var_methods, eta=0, z_bed="z_bed", save_original=False):
    ...
    z_bed = (
        z_sgn * tran["z_bed"]
    )  # assumed same coordinates as z_ctr, and a value per sample
    ...
    for data_var, isel_kw, bed_mode, surface_mode in var_methods:
        ...
        u, z = xr.broadcast(data, ds["z_ctr"])  # ds['Uroz'].sel(roz='downstream'),
        ...
        for s in range(ds.dims["sample"]):
            ...
            def powfit(mode):
                ...
                alpha = np.sum((zab**beta * u_col.values)[u_valid]) / np.sum(
                    (zab ** (2 * beta))[u_valid]
                )
            ...
            if N > 0:
                if bed_mode == "linear":
                    u_col[bed_idx:bottom_valid_idx] = np.linspace(
                        0, u_col[bottom_valid_idx], N + 1
                    )[:-1]
...
def interp_to_grid(tran, v, expand_x=True, expand_y=True):
    ...
    if expand_y:
        ...
        Yexpand[:, -1] = np.nan
        Yexpand[:, 1:] = np.where(
            np.isfinite(Yexpand[:, 1:]), Yexpand[:, 1:], Y + 0.5 * Dz
        )
    ...
    if expand_x:
        ...
        Xexpand2 = np.concatenate(
            (Xexpand - 0.5 * dx[:, None], Xexpand[-1:, :] + 0.5 * dx[-1:, None]), axis=0
        )
        Yexpand2 = np.concatenate(
            (Yexpand[:1, :], safe_midpnt(Yexpand[:-1], Yexpand[1:]), Yexpand[-1:, :]),
            axis=0,
        )
    else:
        Xexpand2 = Xexpand
...
def plot_scalar_polys(tran, v, ax=None, xform=None, **kw):
    ...
    Yexpand = np.concatenate((Ybot, Ybot[:, -1:]), axis=1)
    Yexpand[:, -1] = np.nan
    Yexpand[:, 1:] = np.where(np.isfinite(Yexpand[:, 1:]), Yexpand[:, 1:], Y + 0.5 * Dz)
    # Expands the horizontal coordinate in the vertical
    Xexpand = np.concatenate((X, X[:, -1:]), axis=1)
    ...
    for samp, zi in np.ndindex(v.shape):
        ...
        poly = np.array(
            [
                [Xexpand2[samp, zi], Yexpand[samp, zi]],
...
def surface_bed_linestrings(tran, v, xform=None, dzmin=0.005):
    ...
    for samp in np.arange(V.shape[0]):
        ...
        if np.abs(Yexpand[samp, wet_i[0]] - Yexpand[samp, wet_i[-1] + 1]) < dzmin:
            ...
            coords.append(
                [
                    Xexpand2[samp, wet_i[0]],
                    Yexpand[samp, wet_i[0]],
                    Yexpand[samp, wet_i[-1] + 1],
                ]
            )
            coords.append(
                [
                    Xexpand2[samp + 1, wet_i[0]],
                    Yexpand[samp, wet_i[0]],
                    Yexpand[samp, wet_i[-1] + 1],
                ]
            )
...
def transects_to_segment(trans, unweight=True, ax=None, n_nodes=2):
    ...
    all_dx = [get_dx_sample(tran).values for tran in trans]
    ...
    for tran_i, tran in enumerate(trans):
        ...
        if unweight:
            ...
            xy = linestring_utils.downsample_linearring(
                xy, density=3 * median_dx, closed_ring=False
            )
        all_xy.append(xy)
...
def resample_to_common_z(trans, dz=None, save_original=None):
    ...
    all_dz = [np.abs(get_z_dz(tran).values.ravel()) for tran in trans]
    ...
    ds_resamp = [resample_z(tran, new_z, save_original=save_original) for tran in trans]
    return ds_resamp
...
def resample_to_common(
    ...
    if resample_x:
        ...
        trans = [
            resample_d(tran, new_xy, save_original=save_original) for tran in trans
        ]
    return trans


def average_transects(
    trans, dz=None, dx=None, resample_x=True, resample_z=True, seg=None
):
    """
    ...
    trans = resample_to_common(
        trans, dz=dz, dx=dx, resample_x=resample_x, resample_z=resample_z, seg=seg
    )
...
def calc_secondary_strength(tran, name="secondary"):
    ...
    if name is not None:
        ...
        tran[name].attrs[
            "description"
        ] = "Average left-ward velocity in upper water column"
...
def mask_bed(tran, v, depth_fraction=0.9, z_top=0.0):
    ...
    if pos == "up":
        ...
        z_sgn = -1
    depth = (z_top - z_sgn * z_bed).clip(0.0)
