...
def request_square(ax, max_bounds=None):
    ...
    if max_bounds is not None:
        bounds = ax.axis()
        if (
            (bounds[0] >= max_bounds[0])
            and (bounds[1] <= max_bounds[1])
            and (bounds[2] >= max_bounds[2])
            and (bounds[3] <= max_bounds[3])
        ):
            pass  #
        else:
            ax.axis(max_bounds)
...
class UnstructuredGrid(Listenable, undoer.OpHistory):
    ...
    max_sides = (
        4  # N.B. - must be set before or during __init__, or taken from cells.shape
    )
    ...
    node_defaults = None
    cell_dtype = (
        [  # edges/nodes are set dynamically in __init__ since max_sides can change
    ...
    def __init__(
        ...
        self.cell_dtype = (
            [
                ("edges", (np.int32, self.max_sides)),
                ("nodes", (np.int32, self.max_sides)),
            ]
            + self.cell_dtype
            + extra_cell_fields
        )
            ...
            for name in new_def.dtype.names:
                if name in default.dtype.names:
                    if new_def[name].shape == default[name].shape:
                        ...
                        assert new_def[name].ndim == default[name].ndim, (
                            "ndim changed: %s" % name
                        )
    ...
    def copy_from_grid(self, grid):
        ...
        if self.max_sides < grid.max_sides:
            assert np.all(
                cell_nodes[:, self.max_sides :] < 0
            ), "Trying to copy from grid with greater max_sides"
            cell_nodes = cell_nodes[:, : self.max_sides]

        self.from_simple_data(
            points=grid.nodes["x"], edges=grid.edges["nodes"], cells=cell_nodes
        )
        ...
        with open(fn, "rt") as fp:
            ...
            while 1:
                ...
                if blk_start == "$Entities":
                    ...
                    for blk_i in range(nblocks):
                        ent_dim, ent_tag, ent_typ, blk_nelts = [
                            int(s) for s in fp.readline().strip().split()
                        ]
                        elt_nnodes = {15: 1, 1: 2, 2: 3}  # nodes  # edges  # triangles
                        tags = []
    ...
    def from_ugrid(
        ...
        skip_edges=False,
        fields="auto",
        auto_max_bytes_per_element=256,
        dialect=None,
    ):
        ...
        if mesh_name is None:
            ...
            for vname in nc.variables.keys():
                if nc[vname].attrs.get("cf_role", None) == "mesh_topology":
                    meshes.append(vname)
            if len(meshes) != 1:
                raise AmbiguousMeshException(
                    "Could not uniquely determine mesh variable", meshes=meshes
                )
            mesh_name = meshes[0]
        ...
        def process_as_index(varname):
            ...
            if start_index not in [0, 1]:
                ...
                if max_idx == len(node_xy):
                    start_index = 1
                    logging.warning(
                        "Variable %s has bad start_index, assume %d"
                        % (varname, start_index)
                    )
                    ...
                    if start_index is not None:
                        ...
                        logging.warning(
                            "Variable %s has bad start_index, assume 0" % (varname)
                        )
                    start_index = 0
                else:
                    assumed = 0
                    if start_index is not None:
                        logging.warning(
                            "Variable %s has bad start_index, punting with %d"
                            % (varname, assumed)
                        )
                    start_index = assumed
        ...
        if fields is not None:  # fields=='auto':
            ...
            for dim_attr, struct, adder in [
                ("node_dimension", ug.nodes, ug.add_node_field),
                ...
                if dim_name:
                    ...
                    for vname in nc.data_vars:
                        ...
                        if fields == "all":
                            ...
                            # Check size
                            if (
                                nc[vname].size / len(struct)
                                > auto_max_bytes_per_element
                            ):
                                continue
    ...
    def read_tin_xml(fn):
        ...
        gtin = UnstructuredGrid(points=P[:, :2], cells=F)
        ...
        try:
            if twod_area_name is None:
                ...
                if len(twod_names) == 1:
                    ...
                    raise Exception(
                        "Must specify twod_area_name from %s" % (", ".join(twod_names))
                    )
                else:
                    raise Exception("No viable twod_area_name values")

            cell_center_xy = h[
                "Geometry/2D Flow Areas/" + twod_area_name + "/Cells Center Coordinate"
            ]
            ...
            points_xy = h[
                "Geometry/2D Flow Areas/" + twod_area_name + "/FacePoints Coordinate"
            ]
            ...
            edge_nodes = h[
                "Geometry/2D Flow Areas/" + twod_area_name + "/Faces FacePoint Indexes"
            ]
            ...
            cell_nodes = h[
                "Geometry/2D Flow Areas/" + twod_area_name + "/Cells FacePoint Indexes"
            ]
            ...
            for i in range(len(cell_nodes)):
                if (
                    cell_nodes[i, 2] < 0
                ):  # first ghost cell (which are sorted to end of list)
            ...
            grd = UnstructuredGrid(
                edges=edges, points=points, cells=cells, max_sides=max_cell_faces
            )
    ...
    @staticmethod
    def read_dfm(nc=None, fn=None, cells_from_edges="auto", max_sides=6, cleanup=False):
        """
        ...
        meshes = [
            ...
            if getattr(nc[v], "cf_role", "none") == "mesh_topology"
        ]
        if meshes:
            mesh = nc[meshes[0]]
        ...
        kwargs = dict(
            points=np.array([nc[var_points_x].values, nc[var_points_y].values]).T,
        ...
        if "partitions_node_start" in nc.variables:
            nodes_are_contiguous = np.all(
                np.diff(nc.partitions_node_start.values)
                == nc.partitions_node_count.values[:-1]
            )
            assert (
                nodes_are_contiguous
            ), "Merged grids can only be handled when node indices are contiguous"
        else:
            nodes_are_contiguous = True

        if "partitions_edge_start" in nc.variables:
            edges_are_contiguous = np.all(
                np.diff(nc.partitions_edge_start.values)
                == nc.partitions_edge_count.values[:-1]
            )
            assert (
                edges_are_contiguous
            ), "Merged grids can only be handled when edge indices are contiguous"
        else:
            edges_are_contiguous = True

        if "partitions_face_start" in nc.variables:
            faces_are_contiguous = np.all(
                np.diff(nc.partitions_face_start.values)
                == nc.partitions_face_count.values[:-1]
            )
            assert (
                faces_are_contiguous
            ), "Merged grids can only be handled when face indices are contiguous"
            if cleanup:
                log.warning(
                    "Some MPI grids have duplicate cells, which cannot be cleaned, but cleanup=True"
                )
        else:
    ...
    @staticmethod
    def read_suntans_hybrid(
        path=".", points="points.dat", edges="edges.dat", cells="cells.dat"
    ):
        """
        For backwards compatibility.  Better to use read_suntans which auto-detects
        format.
        """
        return UnstructuredGrid.read_suntans(
            path=path, points=points, edges=edges, cells=cells, dialect="hybrid"
        )

    @staticmethod
    def read_suntans_classic(
        path=".", points="points.dat", edges="edges.dat", cells="cells.dat"
    ):
        """
        For backwards compatibility.  Better to use read_suntans which auto-detects
        ...
        """
        return UnstructuredGrid.read_suntans(
            path=path, points=points, edges=edges, cells=cells, dialect="classic"
        )
    ...
    def read_suntans(
            ...
            with open(cells_fn, "rt") as fp:
                for line in fp:
                    ...
                    try:
                        ...
                        logging.error(
                            "Error parsing nsides in suntans hybrid -- maybe a classic suntans grid?",
                            exc_info=True,
                        )
                        raise
        ...
        g.from_simple_data(points=xyz[:, :2], edges=edge_nnmcc, cells=all_cells)
    ...
    def read_sms(fname, open_marker=2, land_marker=1):
        ...
        for i in range(Ncells):
            ...
            if nvertices > real_max_sides:
                real_max_sides = nvertices
    ...
    def write_gr3(self, fn, z_flip="node_z_bed", z="depth", bc_marks="auto"):
        ...
        fp.write(
            node_df.to_csv(
                None, index=False, sep=" ", header=False, float_format="%.6f"
            )
        )
        ...
        fp.write(
            cell_df.to_csv(None, index=False, sep=" ", header=False, float_format="%d")
        )
        ...
        if bc_marks:
            for bc_type, bc_label in [(self.OPEN, "open"), (self.LAND, "land")]:
                ...
                fp.write(
                    "%d = Total number of %s boundary nodes\n" % (nodes_total, bc_label)
                )
                self.edge_to_cells()
                ...
                for bc_id in range(n_bc):
                    ...
                    he = self.halfedge(
                        j0, 1
                    )  # have it face out so we traverse the outside
                    # sanity checks
                    ...
                    fp.write(
                        "%d %s= number of nodes for %s boundary %d\n"
            ...
            for bc_id in range(Nbc):
                if mark == cls.OPEN:
                    (
                        N_this_bc,
                        _,
                    ) = value_key()  # 13 = Number of nodes for open boundary 1
                elif mark == cls.LAND:
                    ...
                    (
                        ...
                        interior,
                        _,
                    ) = value_value_key()  # 13 0 = Number of nodes for land boundary
    ...
    def write_cells_geopandas(
        self, crs="+proj=utm +zone=10 +datum=WGS84 +units=m +no_defs"
    ):
        """
    ...
    def write_xarray(
        ...
        ds[face_node_connectivity] = (
            (face_dimension, "maxnode_per_face"),
            self.cells["nodes"],
        )
        ds[edge_node_connectivity] = (
            (edge_dimension, "node_per_edge"),
            self.edges["nodes"],
        )
            ...
            for src_data, dim_name in [
                ...
                for field in src_data.dtype.names:
                    ...
                    if np.issubdtype(src_data[field].dtype, np.object_):
                        ...
                        continue
                    if dialect == "mdal" and not np.issubdtype(
                        src_data[field].dtype, np.floating
                    ):
                        logging.warning(
                            "write_ugrid: mdal dialect will drop non-floating %s"
                            % field
                        )
                        continue
                    if field in ds:  # avoid duplicate names
                        ...
                        if field.startswith(prefix):
                            logging.warning(
                                "Duplicate field already has prefix: %s" % field
                            )
                            continue
    ...
    def write_dfm(self, nc_fn, overwrite=False, node_elevation=None, check_depth=True):
        ...
        if 1:
            ...
            proj = nc.createVariable("projected_coordinate_system", "i4", ())
        ...
        if 1:
            ...
            wgs.longitude_of_prime_meridian = 0.0
            wgs.semi_major_axis = 6378137.0
        ...
        nc.source = (
            "Deltares, D-Flow FM Version 1.1.135.38878MS, Feb 26 2015, 17:00:33, model"
        )
        ...
        if 1:
            ...
            cell_var = nc.createVariable(
                "NetElemNode", "i4", ("nNetElem", "nNetElemMaxNode"), fill_value=missing
            )
    ...
    def from_shp(shp_fn, **kw):
        ...
        if "max_sides" not in kw:
            ...
            nsides = [len(ring) for ring in all_rings()]
            if nsides:
        ...
        for geo in features["geom"]:
            if geo.geom_type == "Polygon":
                ...
                for coords in all_rings(geo):
                    ...
                    nodes = [self.add_or_find_node(x=x) for x in coords]
    ...
    def add_edge_field(self, name, data, on_exists="fail"):
        ...
        if name in np.dtype(self.edge_dtype).names:
            ...
            self.edges = recarray_add_fields(self.edges, [(name, data)])
    ...
    def add_node_field(self, name, data, on_exists="fail"):
        ...
        if name in np.dtype(self.node_dtype).names:
            ...
            self.nodes = recarray_add_fields(self.nodes, [(name, data)])
    ...
    def add_cell_field(self, name, data, on_exists="fail"):
        ...
        if name in np.dtype(self.cell_dtype).names:
            ...
            self.cells = recarray_add_fields(self.cells, [(name, data)])
    ...
    def renumber_cells(self, order=None):
        ...
        cell_map = np.zeros(
            self.Ncells() + Nneg, np.int32
        )  # do this before truncating cells
        ...
        cell_map[
            ...
            -999
        )  # these should only remain for deleted cells, and never show up in the output
    ...
    def renumber_edges(self, order=None):
        ...
        edge_map[
            ...
            -999
        )  # these should only remain for deleted edges, which won't show up in the output
    ...
    def add_grid(self, ugB, merge_nodes=None, log=None, tol=0.0):
        ...
        if self.max_sides < ugB.max_sides:
            ...
            self.log.warning(
                "Increasing max_sides from %d to %d" % (self.max_sides, ugB.max_sides)
            )
        ...
        def bad_fields(Adata, Bdata):  # field froms B which get dropped
            ...
            B_bad = [f for f in B_fields if (f == "deleted") or (f not in A_fields)]
            return B_bad
        ...
        for n in ugB.valid_cell_iter():
            ...
            if merge_nodes is not None:
                ...
                if c is not None:
                    ...
                    if log:
                        log.info(
                            "Skipping existing cell: %d: %s => %d: %s"
                            % (n, str(orig_nodes), c, str(kwargs["nodes"]))
                        )
                    continue
    ...
    def boundary_cycle(self):
        ...
        potential_cells = self.find_cycles(
            max_cycle_len=max_nodes, starting_edges=edges_near, check_area=False
        )
        ...
        return np.array(
            best[1][::-1]
        )  # reverse, to return a CCW, positive area string.
    ...
    def find_cycles(self, max_cycle_len="auto", starting_edges=None, check_area=True):
        ...
        for ji, j in enumerate(starting_edges):
            ...
            if isinstance(j, HalfEdge):
                ...
                halfedges = [self.halfedge(j, 0), self.halfedge(j, 1)]
            # for A,B in (self.edges['nodes'][j], self.edges['nodes'][j,::-1]):
            # Skip half edges already visited:
            halfedges = [
                he for he in halfedges if (he.node_rev(), he.node_fwd()) not in visited
            ]
            for he in halfedges:
                cycle = [he.node_rev()]
                trav = he
    ...
    def interp_cell_to_raster_function(self, *a, **kw):
        ...
        def to_field(cell_values, fld=fld, M=M, invalid=invalid):
            ...
            f_result = field.SimpleGrid(
                extents=fld.extents, F=pix_values.reshape(fld.F.shape)
            )
    ...
    def interp_cell_to_raster_matrix(self, fld=None, dx=None, dy=None):
        ...
        valid = cell_counts > 0
        cell_counts[valid] = 1.0 / cell_counts[valid]
    ...
    def edge_to_cells(self, e=slice(None), recalc=False, on_missing="error"):
        """
        ...
        if recalc:
            ...
            if isinstance(e, slice):
                js = np.arange(e.start or 0, e.stop or self.Nedges(), e.step or 1)
            else:
                try:
    ...
    def make_edges_from_cells(self):
        edge_map = (
            {}
        )  # keys are tuples of node indices, mapping to index into new_edges
        new_edges = (
            []
        )  # each entry is [n1,n2,c1,c2].  assume for now that c1 is on the left of the edge n1->n2
        self._node_to_edges = None
    ...
    def make_edges_from_cells_fast(self):
        ...
        for a, b, cell in pair_iter():
            ...
            new_edges = np.zeros(
                len(a),
                [("a", np.int32), ("b", np.int32), ("c1", np.int32), ("c2", np.int32)],
            )
        ...
        breaks = np.r_[
            False, (np.diff(new_edges["a"]) != 0) | (np.diff(new_edges["b"]) != 0)
        ]
    ...
    def edges_center(self, edges=None):
        ...
        if edges is None:
            ...
            centers[valid, :] = self.nodes["x"][self.edges["nodes"][valid, :]].mean(
                axis=1
            )
            ...
            centers[valid, :] = self.nodes["x"][
                self.edges["nodes"][edges[valid], :]
            ].mean(axis=1)
            centers[~valid, :] = np.nan
    ...
    def cells_centroid_py(self, ids=None):
        ...
        cnodes = np.where(nodes >= 0, nodes, nodes[:, 0][:, None])
    ...
    def cells_center(self, refresh=False, mode=None):
        ...
        if do_update:  # np.sum(to_update) > 0:
            if mode == "first3":
                p1, p2, p3 = [
                    self.nodes["x"][self.cells["nodes"][to_update, i]]
                    for i in [0, 1, 2]
                ]
    ...
    def edges_normals(
        ...
        normals = np.diff(self.nodes["x"][self.edges["nodes"][edges]], axis=-2)[
            ..., 0, ::-1
        ]
        normals[..., 1] *= -1
    ...
    def nodes_to_cell(self, ns, fail_hard=True):
        ...
        for n in ns[1:]:
            ...
            cells = [c for c in cells if c in cells2]
        if len(cells) == 0:
            if fail_hard:
    ...
    def cell_to_adjacent_boundary_cells(self, c):
        ...
        """
        j_boundary = [j for j in self.cell_to_edges(c) if self.is_boundary_edge(j)]

        adj_edges = []
        for j in j_boundary:
            adj_edges += list(self.edge_to_edges(j))
        adj_edges = filter(lambda jj: self.is_boundary_edge(jj) and jj != j, adj_edges)
        cells = [c for j in adj_edges for c in self.edge_to_cells(j) if c >= 0]
        return cells
    ...
    def merge_edges(self, edges=None, node=None):
        ...
        for c in self.edges["cells"][A]:
            if c >= 0:  # it's a real cell
                c_nodes = [n for n in self.cell_to_nodes(c) if n != B]
                self.modify_cell(c, nodes=c_nodes)

        # Edge A will be the one to keep
        # modify_edge knows about changes to nodes
        new_nodes = [self.edges["nodes"][A, 1 - Ab], self.edges["nodes"][C, 1 - Cb]]
    ...
    def split_edge(
        self, j=None, x=None, split_cells=True, merge_thresh=-1, **node_args
    ):
        """
        ...
        for c_nbr in c_nbrs:
            ...
            if not split_cells:
                if len(c_nbr["nodes"]) + 1 > self.max_sides:
                    print(
                        "Will not insert node into cell because max_sides is too small"
                    )
                    split_cells = True
                else:
    ...
    def automerge_cells(self, n, thresh=0.1):
        ...
        for i, n_nbr in enumerate(n_nbrs):
            ...
            c = he.cell()
            if c < 0 or c in dirty_cells:
                continue
            if len(self.cell_to_nodes(c)) != 3:
                continue
            ...
            for he_check in [he, he.fwd()]:
                ...
                if c_nbr < 0 or c_nbr in dirty_cells:
                    continue
                if len(self.cell_to_nodes(c_nbr)) != 3:
                    continue
        ...
        for c in n1_cells:
            ...
            for ji, j in enumerate(cedges):
                if j in edge_map:
                    ...
                    if list(self.edges["nodes"][j]).index(n1) == list(
                        self.edges["nodes"][jo]
                    ).index(n0):
                        # same orientation
                        jo_cells_side = j_cells_side
                    elif list(self.edges["nodes"][j]).index(n1) == 1 - list(
                        self.edges["nodes"][jo]
                    ).index(n0):
                        ...
                        plt.axis("tight")
                        plt.axis("equal")
                        plt.axis((552453.0, 552569, 4123965.0, 4124066.0))
                    ...
                    if jo_cells[jo_cells_side] >= 0:
                        ...
                        self.plot_edges(mask=[j, jo], color="r", lw=1, labeler="id")
                        self.plot_cells(
                            mask=[c], color="r", alpha=0.3, labeler="id", centroid=True
                        )
                        plt.axis("tight")
                        plt.axis("equal")
                        # plt.axis((552453., 552569, 4123965., 4124066.) )
                        raise Exception(
                            "jo_cells[%d]=%s, expected <0. Verify CELL AREA!"
                            % (jo_cells_side, jo_cells[jo_cells_side])
                        )
    ...
    def enclosing_nodestring(self, x, max_nodes=None):
        ...
        potential_cells = self.find_cycles(
            max_cycle_len=max_nodes, starting_edges=edges_near
        )
    ...
    def add_cell(self, **kwargs):
        ...
        for side in range(len(edges)):
            ...
            if (n1 == self.edges["nodes"][j, 0]) and (n2 == self.edges["nodes"][j, 1]):
                ...
                print(
                    "side: %d j=%d n1=%d  n2=%d  edges[nodes][j]=%s"
    ...
    def elide_node(self, n):
        ...
        for c in cs:
            self.delete_cell(c)
        new_edge_nodes = [nn for nn in self.edges["nodes"][js].ravel() if nn != n]
    ...
    def plot_nodes(
        ...
        **kwargs
        ...
        coll = ax.scatter(
            self.nodes["x"][mask][:, 0], self.nodes["x"][mask][:, 1], sizes, **kwargs
        )
        bounds = [
            self.nodes["x"][mask][:, 0].min(),
        ...
        **kwargs
        ...
        if labeler is not None:
            ...
            for n in np.arange(self.Nedges())[mask]:
                ax.text(ec[n, 0], ec[n, 1], labeler(n, self.edges[n]))
    ...
    def plot_halfedges(
        ...
        **kwargs
        ...
        if values is not None:
            ...
            coll = ax.plot(
                offset_points[:, :, 0].ravel(), offset_points[:, :, 1].ravel(), "."
            )
    ...
    def trace_node_contour(
        self, cval, node_field, pos_side, n0=None, loc0=None, return_full=False
    ):
        """
        ...
        if return_full:
            ...
            points = [pnt for typ, idx, pnt in path if pnt is not None]
            trace = np.array(points)
            return trace

    def scalar_contour(
        self, scalar, V=10, smooth=True, boundary="reflect", return_segs=False
    ):
    ...
    def make_triangular(self, record_original=True):
        ...
        if record_original:
            self.add_cell_field(
                "orig_cell", np.arange(self.Ncells()), on_exists="overwrite"
            )
            self.add_edge_field(
                "orig_edge", np.arange(self.Nedges()), on_exists="overwrite"
            )
        if self.max_sides == 3:
    ...
    def mpl_triangulation(
        self, cell_mask=None, offset=[0, 0], return_sources=False, refresh=True
    ):
        """
        ...
        if cacheable and (not refresh) and (self._mpl_tri is not None):
            if return_sources:
    ...
    def smooth_matrix(self, f=0.5, K="scaled", dx="grid", V="grid", A="grid", dt=None):
        ...
        if isinstance(K, np.ndarray):
            ...
            K[internal] = (
                0.5
                * dx[internal]
                * (V[e2c[internal, 0]] + V[e2c[internal, 1]])
                / A[internal]
            )
        elif K is None:
            K = np.ones(Nj, np.float64)
    ...
    def edge_clip_mask(self, xxyy, ends=False):
        ...
        return (xmax > xxyy[0]) & (xmin < xxyy[1]) & (ymax > xxyy[2]) & (ymin < xxyy[3])
        ...
        **kwargs
        ...
        if not centers:
            ...
            if ragged_edges:
                ...
                plot_polys = [self.nodes["x"][cn[cn >= 0]] for cn in cell_nodes]
            else:
                plot_polys = polys
    ...
    def cells_area(self, sel=None):
        ...
        if sel is None:
            recalc = np.nonzero(
                np.isnan(self.cells["_area"]) & (~self.cells["deleted"])
            )[0]
        else:
    ...
    def select_edges_by_polyline(
        self, geom, rrtol=3.0, update_e2c=True, boundary=True, return_nodes=False
    ):
        """
        ...
        if boundary:
            ...
            outward = -self.edges_normals(
                edges=boundary_edges, force_inward=True, update_e2c=update_e2c
            )

            dis = np.maximum(
                0.5 * np.sqrt(self.cells_area()[adj_cells]), cell_to_edge_dist
            )
    ...
    def select_edges_intersecting(
        self, geom, invert=False, mask=slice(None), by_center=False, as_type="mask"
    ):
        """
        ...
        for j in np.nonzero(boundary_edges)[0]:
            ...
            while 1:
                if marked[trav.j]:
                    ...
                    print(
                        "edge centered at %s traversed twice"
                        % (self.edges_center()[trav.j])
                    )
                    # import pdb
                    # pdb.set_trace()
                    raise Exception(
                        "Hit a dead end -- boundary maybe not closed."
                        + "edge centered at %s traversed twice"
                        % (self.edges_center()[trav.j])
                    )
    ...
    def boundary_polygon_by_edges(self, allow_multiple=False):
        ...
        """
        lines = self.boundary_linestrings()
        polys, extras = join_features.lines_to_polygons(
            lines, close_arc=False, single_feature=False
        )
    ...
    def boundary_polygon(self):
        ...
        try:
            ...
            self.log.info(
                "Warning, boundary_polygon() failed using edges!  Trying polygon union method"
            )
    ...
    def extract_linear_strings(self, edge_select=None, end_func=None):
        ...
        for j0 in self.valid_edge_iter():
            ...
            for trav in [(node_fwd, node_rev), (node_rev, node_fwd)]:
                while 1:
    ...
    def select_quad_subset(
        self, ctr, max_cells=None, max_radius=None, node_set=None, return_full=False
    ):
        """
        ...
        visited_cells = (
            {}
        )  # cell index => the index, 0..3, of its node that has the min i and min j.
        ...
        if node_set is None:
            ...
            he = None
            for j in self.node_to_edges(n):
                if (self.edges["nodes"][j, 0] in node_set) and (
        ...
        for n, n_ij in zip(node_idxs, ij):
            IJ[2 * n_ij[0], 2 * n_ij[1]] = n
    ...
    def select_nodes_intersecting(
        self, geom=None, xxyy=None, invert=False, as_type="mask"
    ):
    ...
    def select_cells_intersecting(
        ...
        if geom.geom_type == "LineString" and order:
            ...
            dist_along = np.array(
                [geom.project(geometry.Point(center)) for center in centers]
            )
    ...
    def points_to_cells(self, points, method="kdtree"):
        ...
        if method == "cell_index":
            ...
            cell_index = gen_spatial_index.RectIndex(
                zip(range(self.Ncells()), boxes, [None] * self.Ncells()),
                interleaved=False,
            )
            ...
            idx = gen_spatial_index.PointIndex(
                zip(range(len(points)), xxyy, [None] * len(points)), interleaved=False
            )
            ...
            for c, candidates in enumerate(pnts):
                p = self.cell_path(c)
                hits = [cand for cand in candidates if p.contains_point(points[cand])]
                cells[hits] = c
        elif method == "mpl":
    ...
    def node_index(self):
        if self._node_index is None:
            tuples = [
                (i, self.nodes["x"][i, self.xxyy], None)
    ...
    def select_nodes_nearest(self, xy, count=None, max_dist=None):
        ...
        if max_dist is not None:
            hits = [hit for hit in hits if mag(xy - self.nodes["x"][hit]) <= max_dist]
            ...
            if return_type in ["nodes", "cells"]:
                ...
                if traverse == "nodes":
                    return_value = np.array(
                        [
                            self.nodes_to_edge(path[i], path[i + 1])
                    ...
                    return_value = np.array(
                        [
                            self.cells_to_edge(path[i], path[i + 1])
                            for i in range(len(path) - 1)
                        ]
                    )
                else:
    ...
    def select_cells_along_ray(self, x0, vec):
        ...
        p_dtype = [("x", np.float64, 2), ("j_last", np.int32), ("c", np.int32)]
        p = np.zeros((), p_dtype)
        p["x"] = x0
    ...
    def create_dual(
        ...
        if remove_disconnected and not create_cells:
            ...
            raise Exception(
                "Creating the dual without cells is not compatible with removing disconnected"
            )
        gd = UnstructuredGrid()
        ...
        for j in self.valid_edge_iter():
            if e2c[j].min() < 0:
                if extend_to_boundary:
                    ...
                    boundary_edge_to_dual_node[j] = dnj = gd.add_node(
                        x=edge_center[j], dual_edge=j
                    )
    ...
    def cells_connected_components(self, edge_mask, cell_mask=None, randomize=True):
        ...
        graph = sparse.csr_matrix(
            (np.ones(len(cell_pairs)), (cell_pairs[:, 0], cell_pairs[:, 1])),
        ...
        mapper = (
            np.zeros(1 + unique_labels.max()) - 1
        )  # map original labels to compressed labels
    ...
    def cell_center_index(self):
        ...
        if self._cell_center_index is None:
            if self.cell_center_index_point == "circumcenter":
                ...
                cc = self.cells_centroid()
            tuples = [
                (i, cc[i, self.xxyy], None)
                for i in range(self.Ncells())
                if not self.cells["deleted"][i]
            ]
            self._cell_center_index = gen_spatial_index.PointIndex(
                tuples, interleaved=False
            )
    ...
    def select_edges_nearest(self, xy, count=None, fast=True):
        ...
        if fast:
            ...
            geoms = [
                geometry.LineString(self.nodes["x"][self.edges["nodes"][j]])
                for j in j_near
            ]
            P = geometry.Point(xy)
    ...
    def select_cells_nearest(self, xy, count=None, inside=False, method="auto"):
        ...
        # deletion.  bandaid.
        Nc = self.Ncells()
        hits = [hit for hit in hits if (hit < Nc) and not self.cells["deleted"][hit]]
    ...
    def circum_errors(self):
        ...
        for nsi in range(3, self.max_sides + 1):
            ...
            if nsi == 3:
                ...
                offsets = (
                    self.nodes["x"][self.cells["nodes"][sel, :nsi]]
                    - centers[sel, None, :]
                )
                dists = mag(offsets)
    ...
    def circumcenter_errors(self, radius_normalized=False, cells=None, cc=None):
        """
        ...
        for nsi in range(3, self.max_sides + 1):
            sel = (self.cells["nodes"][cells, nsi - 1] >= 0) & (
                ~self.cells["deleted"][cells]
            )
            if nsi < self.max_sides:
            ...
            if nsi == 3:
                ...
                offsets = (
                    self.nodes["x"][self.cells["nodes"][cells][sel, :nsi]]
                    - centers[sel, None, :]
                )
                dists = mag(offsets)
    ...
    def angle_errors(self):
        ...
        edge_vecs = (
            self.nodes["x"][self.edges["nodes"][:, 1]]
            - self.nodes["x"][self.edges["nodes"][:, 0]]
        )
        link_vecs = centers[e2c[:, 1]] - centers[e2c[:, 0]]
        boundary = e2c.min(axis=1) < 0
        link_vecs[boundary, :] = np.nan
        dots = (edge_vecs * link_vecs).sum(axis=1) / mag(link_vecs) / mag(edge_vecs)
        angle_err = np.arccos(dots) - np.pi / 2
        return angle_err

    def edge_clearance(
        self, edges=None, mode="min", cc=None, Ac=None, eps_Ac=1e-5, recalc_e2c=False
    ):
        """
    ...
    def write_suntans(self, path, overwrite=False):
        ...
        with open(fn_pnts, "wt") as fp:
            ...
            for n in range(self.Nnodes()):
                fp.write(
                    "%.5f %.5f 0\n" % (self.nodes["x"][n, 0], self.nodes["x"][n, 1])
                )
    ...
    def global_refine(self, l_thresh=1.0):
        ...
        def copy_edge_attributes_to_refined(g_orig, g_new):
            ...
            for j in range(g_new.Nedges()):
                ...
                if (
                    a < g_orig.Nnodes()
                    and b >= g_orig.Nnodes()
                    and b < g_orig.Nnodes() + g_orig.Nedges()
                ):
        ...
        new_points = [self.nodes["x"], self.edges_center(), self.cells_center()]
        ...
        gr.from_simple_data(points=new_points, cells=new_cells)
    ...
    def write_cells_shp(self, shpname, extra_fields=[], overwrite=False):
        ...
        for poly_id in range(self.Ncells()):
            if poly_id % 500 == 0:
                print("%0.2g%%" % (100.0 * poly_id / self.Ncells()))

            # older code put this together manually.
            cell_geoms[poly_id] = self.cell_polygon(poly_id)

        print(cell_data.dtype)
        result = wkb2shp.wkb2shp(
            shpname, input_wkbs=cell_geoms, fields=cell_data, overwrite=overwrite
        )
        return result
    ...
    def write_edges_shp(self, shpname, extra_fields=[], overwrite=False):
        ...
        for edge_id in range(edges.shape[0]):
            if edge_id % 5000 == 0:
                print("%0.3g%%" % (100.0 * edge_id / edges.shape[0]))
        ...
        wkb2shp.wkb2shp(
            shpname, input_wkbs=edge_geoms, fields=edge_data, overwrite=overwrite
        )
    ...
    def write_nodes_shp(self, shpname, extra_fields=[], overwrite=False):
        ...
        wkb2shp.wkb2shp(
            shpname, input_wkbs=node_geoms, fields=node_data, overwrite=overwrite
        )
    ...
    def write_ptm_gridfile(self, fn, overwrite=False, subgrid=True):
        ...
        with open(fn, "wt") as fp:
            ...
            for v in range(self.Nnodes()):
                fp.write(
                    " %10d %16.7f %16.7f\n"
                    % (v + 1, self.nodes["x"][v, 0], self.nodes["x"][v, 1])
                )
            ...
            for c in range(self.Ncells()):
                ...
                fp.write(
                    cell_write_str1
                    % (c + 1, nsides, cc[c, 0], cc[c, 1], cell_depths[c])
                )
                fp.write(edge_str)
    ...
    def add_rectilinear(self, p0, p1, nx, ny, reverse_cells=False):
        ...
        return {"cells": cell_ids, "nodes": node_ids}
    ...
    def add_rectilinear_on_line(self, centerline, profile, add_streamwise=True):
        ...
        center_deltas = np.concatenate(
            [center_deltas[:1], center_deltas, center_deltas[-1:]]
        )
    ...
    def add_quad_ring(
        ...
        def ring(n, r, stagger=0):
            theta = (
                theta0 + stagger * 2 * np.pi / n + np.linspace(0, 2 * np.pi, n + 1)[:-1]
            )
        ...
        for ra, rb in zip(rings[:-1], rings[1:]):
            if len(ra) == len(rb):
                if sides == 4:
                    ...
                    segss = [zip(ra, rb), zip(np.roll(ra, -1, axis=0), rb)]
            else:  # stitch
                segss = [
                    zip(ra, rb[::2]),
                    zip(ra, rb[1::2]),
    ...
    def cell_to_halfedges(self, c):
        return [
            HalfEdge(self, j, 1 - int(self.edges["cells"][j, 0] == c))
            for j in self.cell_to_edges(c, ordered=True)
            if j >= 0
        ]
...
class UnTRIM08Grid(UnstructuredGrid):
    ...
    def __init__(
        ...
        super(UnTRIM08Grid, self).__init__(
    ...
    def renumber_cells_ordering(self):  # untrim version
        ...
        Nactive = sum(~self.cells["deleted"])
        return np.argsort(
            -self.cells["mark"] * 2 - self.cells["red"] + 10 * self.cells["deleted"],
            kind="mergesort",
        )[:Nactive]
    ...
    def renumber_edges_ordering(self):  # untrim version
        ...
        Nactive = sum(~self.edges["deleted"])
        return np.argsort(
            mark_order[self.edges["mark"]] + 10 * self.edges["deleted"],
            kind="mergesort",
        )[:Nactive]
    ...
    def infer_depths_edges_from_cells(self):
        ...
        edge_from_cell = self.cells["depth_mean"][self.edges["cells"][sel_edges]].min(
            axis=1
        )
    ...
    def copy_mean_depths_to_subgrid_outside_polygon(
        self, polygon_shp, overwrite=True, cells=True, edges=True
    ):
        """
        copy all depth_mean values (all edges, all cells), to single-entry
        subgrid.
        ...
        if cells:
            ...
            for c in range(self.Ncells()):
                if overwrite or self.cells["subgrid"][c] == 0:
                    ...
                    cell_poly = geometry.Polygon(
                        np.asarray(self.nodes["x"][cell_nodes])
                    )
    ...
    def overwrite_field(
        self, cells=None, edges=None, source="depth_max", target="depth_mean"
    ):
        """
    ...
    def copy_depths_to_subgrid(
        self, cells="missing", edges="missing", depth_stat="depth_mean"
    ):
        """
        ...
        def depth_stat_to_subgrid(selector, elements, extent, depth):
            ...
            for s in selector:
                ...
                if (
                    overwrite
                    or elements["subgrid"][s] == 0
                    or elements["subgrid"][s] == ([], [])
                ):
                    # funny indexing to add unit dimension,
                    # and zip to make these into tuples like ([area[0]],[depth[0]])
        ...
        if cells is not False:
            depth_stat_to_subgrid(
                cells, self.cells, self.cells_area(), self.cells[depth_stat]
            )
        if edges is not False:
            depth_stat_to_subgrid(
                edges, self.edges, self.edges_length(), self.edges[depth_stat]
            )
    ...
    def read_from_file(self, grd_fn):
        ...
        if hdr == self.hdr_08:
            ...
            for c in range(Npolys):
                ...
                if check_c != c + 1:
                    print(
                        "ERROR: while reading cell subgrid, cell index mismatch: %s vs. %d"
                        % (c + 1, check_c)
                    )
            ...
            for e in range(Nflow_sides):
                ...
                if check_e != e + 1:
                    print(
                        "ERROR: While reading edge subgrid, edge index mismatch: %s vs. %s"
                        % (e + 1, check_e)
                    )
                ...
                if sum(lengths) <= 0:
                    ...
                    self.edges["depth_mean"][e] = np.sum(lengths * depths) / sum(
                        lengths
                    )
    ...
    def write_untrim08(self, fn, overwrite=False):
        ...
        with open(fn, "wt") as fp:
            ...
            for c in range(self.Ncells()):
                ...
                fp.write(
                    "%10d %14d %13.4f %17.4f "
                    % (c + 1, nsides, centers[c, 0], centers[c, 1])
                )
            ...
            for e in range(self.Nedges()):
                fp.write(
                    ...
                    % (
                        e + 1,
                        self.edges["nodes"][e, 0] + 1,
                        self.edges["nodes"][e, 1] + 1,
                        e2c1[e, 0],
                        e2c1[e, 1],
                    )
                )
...
class SuntansGrid(UnstructuredGrid):
    ...
    def __init__(self, suntans_path, elev2depth=False):
        logging.warning(
            "SuntansGrid will be deprecated.  Use UnstructuredGrid.read_suntans"
        )
    ...
    def read_from_file(self, suntans_path):
        ...
        edges = np.loadtxt(os.path.join(suntans_path, "edges.dat"), dtype=np.int32)
        cell_dtype = [
            ("center", np.float64, 2),
            ("nodes", np.int32, 3),
            ("nbrs", np.int32, 3),
        ]
        cells = np.loadtxt(os.path.join(suntans_path, "cells.dat"), dtype=cell_dtype)
...
class Sms2DM(UnstructuredGrid):
    ...
    def __init__(self, grd_fn, elev2depth=False):
        super(Sms2DM, self).__init__(
            extra_cell_fields=[("depth", np.float64)],
    ...
    def read_from_file(self, grd_fn, elev2depth=False):
        ...
        elementlines = [
            line.strip().split()[1:5] for line in all_lines if line.startswith("E3T")
        ]
        nelement = len(elementlines)

        # ND <id> <x> <y> <z>
        nodelines = [
            line.strip().split()[1:5]
...
class PtmGrid(UnstructuredGrid):
    ...
    def read_polygons(self, polygon_data_offset):
        ...
        for i in range(self.Ncells()):
            ...
            self.cells["edges"][i, :nsides_this_poly] = [
                int(s) - 1 for s in line[5 : 5 + nsides_this_poly]
            ]
...
class RgfGrid(UnstructuredGrid):
    ...
    def __init__(self, grd_fn, dep_fn="infer", enc_fn="infer"):
        ...
        for row in range(n_count):
            for col in range(m_count):
                if np.isfinite(xy[row, col, 0]):
                    node_idxs[row, col] = self.add_node(
                        x=xy[row, col], row=row, col=col
                    )
        ...
        for row in range(n_count - 1):
            for col in range(m_count - 1):
                ...
                if np.any(np.array(nodes) < 0):
                    continue
                cell_idxs[row, col] = self.add_cell_and_edges(
                    nodes=nodes, row=row, col=col
                )
    ...
    def read_depth(self, dep_fn):
        ...
        dep2d = dep_data.reshape(
            (self.rowcol_to_node.shape[0] + 1, self.rowcol_to_node.shape[1] + 1)
        )
