...
def cimis_json_to_xr(data):
    ...
    if records[0]["Scope"] == "daily":
        df["Date"] = (("time",), [rec["Date"] for rec in records])
        datetimes = [datetime.datetime.strptime(s, "%Y-%m-%d") for s in df.Date.values]
    else:  # hourly
        df["Date"] = (
            ("time",),
            ["%s %s" % (rec["Date"], rec["Hour"]) for rec in records],
        )
        # annoying convention where midnight is 2400.
        datetimes = [
            (
                datetime.datetime.strptime(s.replace("2400", "0000"), "%Y-%m-%d %H%M")
    ...
    for field in [
        ...
        qc, value = zip(
            *[(rec[field]["Qc"], cnv(rec[field]["Value"])) for rec in records]
        )
...
def cimis_fetch_station_metadata(station, df=None, cimis_key=None, cache_dir=None):
    ...
    if (cache_fn is not None) and os.path.exists(cache_fn):
        ...
        req = requests.get(
            "http://et.water.ca.gov/api/station/%s" % station,
...
def cimis_fetch_to_xr(
    ...
    if cimis_key is None:
        try:
            ...
            raise Exception(
                "cimis_key was not supplied and CIMIS_KEY was not in the environment"
            )
    ...
    start_date, end_date = [
        utils.to_datetime(d).strftime("%Y-%m-%d") for d in (start_date, end_date)
    ]
    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        if cache_dir is not None:
            ...
            cache_fn = os.path.join(
                cache_dir,
                "%s_%s_%s_%s.nc"
                % ("_".join(stations), fields_label, begin_str, end_str),
            )
        else:
            cache_fn = None
