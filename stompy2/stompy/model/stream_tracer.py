...
def steady_streamline_oneway(
    g, Uc, x0, max_t=3600, max_steps=1000, max_dist=None, u_min=1e-3, bidir=False
):
    """
    ...
    while (t < max_t) and (c >= 0):
        ...
        for j in g.cell_to_edges(c):
            ...
            out_normal = (
                csgn * edge_norm[j]
            )  # normal of edge j pointing away from cell c
        ...
        if (
            dt_max_edge == 0.0
            and j_cross is not None
            and is_convergent(j_cross, c, c_U, c_cross, c_cross_U)
        ):
            # print("Moving along edge")
            edge_tan = np.array([-edge_norm[j_cross, 1], edge_norm[j_cross, 0]])
...
def steady_streamline_twoways(g, Uc, x0, **kw):
    ...
    ds = xr.concat([ds_rev.isel(time=slice(None, None, -1)), ds_fwd], dim="time")
    del ds["stop_condition"]
    ds["stop_condition"] = ("leg",), [
        ds_rev.stop_condition.values,
...
class StreamDistance(object):
    ...
    def stream_distance(
        self, x_target, sample, x_along=None, x_across=None, plot=False
    ):
        if x_along is None:
        ...
        if plot:  # plot the pieces
            ...
            ax.plot(
                self.source_ds.x.values[sample, 0],
                self.source_ds.x.values[sample, 1],
                "go",
            )

            ax.plot(
                x_along.x.values[:, 0], x_along.x.values[:, 1], "b-", label="x along"
            )
            ax.plot(
                x_across.x.values[:, 0], x_across.x.values[:, 1], "k-", label="x across"
            )
            ax.plot(
                s_along.x.values[:, 0], s_along.x.values[:, 1], "r-", label="s along"
            )
            ax.plot(
                s_across.x.values[:, 0],
                s_across.x.values[:, 1],
        ...
        if xs1.type != "Point":
            ...
            print(
                "x_target=%s  sample=%s  expected point but got %s"
                % (str(x_target), sample, xs1.wkt)
            )
            return np.array([np.nan, np.nan])
        if xs2.type != "Point":
        ...
        return np.array(
            [0.5 * (dist_along1 + dist_along2), 0.5 * (dist_across1 + dist_across2)]
        )
