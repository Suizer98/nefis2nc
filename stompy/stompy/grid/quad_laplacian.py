...
class NodeDiscretization(object):
    ...
    def construct_matrix(
        ...
        for n in range(g.Nnodes()):
            if skip_dirichlet and (n in dirichlet_nodes):
                continue
            if n in gradient_nodes:
                continue
            if n in tangential_nodes:
                continue
        ...
        log.info(
            f"row breakdown:  Lap: {nlaplace_rows}  "
        ...
        for n in gradient_nodes:
            ...
            if self.gradient_by_normal:
                ...
                alphas = (
                    np.array(dx_alphas) * normal[0] + np.array(dy_alphas) * normal[1]
                )
                B[row] = 0
            else:
                # So if vec = [1,0]
    ...
    def node_discretization(self, n0, op="laplacian"):
        ...
        if is_boundary:
            ...
            nbr_boundary = [self.g.is_boundary_node(n) for n in N]
        ...
        for e in range(M):
            ...
            if op == "laplacian":
                alpha0 += (
                    -beta(e) / (4 * A[e]) * ((y[e] - y[ep]) ** 2 + (x[ep] - x[e]) ** 2)
                )
            elif op == "dx":
        ...
        if op == "laplacian" and P > M:
            ...
            gamma = (
                3
                / AT
                * (beta(0) * norm_grad * L01 / 2 + beta(P - 1) * norm_grad * L0P / 2)
            )
        else:
            gamma = 0
        assert np.isfinite(alpha0)
        return ([n0] + list(N), [alpha0] + list(alphas), -gamma)
...
def snap_angles(gen):
    ...
    """
    gen.add_edge_field(
        "turn_fwd", np.zeros(gen.Nedges(), np.float64), on_exists="overwrite"
    )
    gen.add_edge_field(
        "turn_rev", np.zeros(gen.Nedges(), np.float64), on_exists="overwrite"
    )
...
def prepare_angles_halfedge(gen):
    ...
    j_turns = np.c_[gen.edges["turn_fwd"], gen.edges["turn_rev"]]
        ...
        while stack:
            ...
            if np.isfinite(existing_angle):
                if existing_angle != angle:
                    plt.figure(2).clf()
                    gen.plot_edges(
                        labeler=lambda j, r: ["", edge_angles[j]][
                            int(np.isfinite(edge_angles[j]))
                        ]
                    )
                    gen.plot_nodes(labeler="id")
                    edge_tans = utils.to_unit(
                        np.diff(gen.nodes["x"][gen.edges["nodes"]], axis=1)[:, 0, :]
                    )
...
def add_bezier(gen):
    ...
    for n in gen.valid_node_iter():
        ...
        def he_ij_angle(he):
            return (gen.edges["angle"][he.j] + (he.orient * 180.0)) % 360.0
        ...
        for grp in np.arange(len(hes)):
            ...
            rot = (
                180
                / np.pi
                * np.arctan2(np.sin(xy_to_ij[sel]).mean(), np.cos(xy_to_ij[sel]).mean())
            )
        ...
        for he, xy_err in zip(hes, xy_errs):
            ...
            cp = gen.nodes["x"][n] + utils.rot(-xy_err * np.pi / 180.0, 1.0 / 3 * vec)
...
class QuadGen(object):
    ...
    def set_scales_diffusion(self):
        ...
        nd = self.nd
        M, B = nd.construct_matrix(
            op="laplacian", dirichlet_nodes=i_scale_dir, skip_dirichlet=True
        )
        i_scale = sparse.linalg.spsolve(M.tocsr(), B)
        M, B = nd.construct_matrix(
            op="laplacian", dirichlet_nodes=j_scale_dir, skip_dirichlet=True
        )
    ...
    def create_intermediate_grid_tri_boundary(self, scale=None):
        ...
        for j in gen.valid_edge_iter():
            ...
            points = self.gen_bezier_linestring(
                j=j, samples_per_edge=10, span_fixed=False
            )
            ...
            points = self.gen_bezier_linestring(
                j=j, samples_per_edge=N, span_fixed=False
            )
            nodes = [g.add_or_find_node(x=p, tolerance=0.1) for p in points]
    ...
    def create_intermediate_grid_tri(self):
        ...
        if self.triangle_method == "gmsh":
            ...
            try:
                ...
                log.error(
                    "Intermediate grid generation failed. Check for self-intersection in bezier grid"
                )
                raise
            ...
            gnew = triangulate_hole.triangulate_hole(
                g, nodes=nodes, hole_rigidity="all", method=self.triangle_method
            )
        gnew.add_node_field(
            "rigid",
    ...
    def plot_gen_bezier(self, num=10, ax=None):
        ...
        for n12 in self.internal_edges:
            ax.plot(
                self.gen.nodes["x"][n12[:2], 0], self.gen.nodes["x"][n12[:2], 1], "g-"
            )
    ...
    def gen_bezier_curve(self, j=None, samples_per_edge=10, span_fixed=True):
        ...
        """
        points = self.gen_bezier_linestring(
            j=j, samples_per_edge=samples_per_edge, span_fixed=span_fixed
        )
        if j is None:
    ...
    def gen_bezier_linestring(
        self, j=None, nodestring=None, samples_per_edge=10, span_fixed=True, gen=None
    ):
        """
        ...
        if nodestring is None:
            if j is None:
                bound_nodes = (
                    self.gen.boundary_cycle()
                )  # probably eating a lot of time.
        ...
        for a, b in node_pairs:
            ...
            edge_points = discretize_bezier_halfedge(
                gen, he, samples_per_edge=samples_per_edge
            )
    ...
    def calc_bc_gradients(self, gtri):
        ...
        j_angles = self.gen.edges["angle"][gtri.edges["gen_j"]] * np.pi / 180.0
        ...
        for ji, (n1, n2) in enumerate(zip(bcycle[:-1], bcycle[1:])):
            ...
            Rpsi = np.array([[tang_ij[0], tang_ij[1]], [-tang_ij[1], tang_ij[0]]])
        ...
        N = len(bcycle)
        for ni in range(N):
            bc_grad_psi[ni, :] = 0.5 * (j_grad_psi[ni, :] + j_grad_psi[(ni - 1) % N, :])
    ...
    def process_internal_edges(self, gen):
            ...
            for nbr in gen.edges["nodes"][j]:
                for j_nbr in gen.node_to_edges(nbr):
                    ...
                    if (gen.edges["angle"][j_nbr] - angle) % 180 == 0:
                        parallel_count += 1
                        break  # parallel. good
            self.add_internal_edge(
                gen.edges["nodes"][j],
                gen.edges["angle"][j],
                colinear=parallel_count >= 2,
            )  # probably just ==2
            # DBG
        ...
        gen.add_node_field(
            "turn", np.zeros(gen.Nnodes(), np.float64), on_exists="overwrite"
        )
    ...
    def psi_phi_solve_separate(self):
        ...
        for coord in [0, 1]:
            ...
            log.info(
                f"About to process joins, starting with {len(noflux_tris)} nf tris"
            )
            for join in joins:
                found = False
                slim_noflux_tris = []
                ...
                if not found:
                    log.warning(
                        f"  Uh-oh! couldn't find a no-flux tri to drop for this internal edge"
                    )
                noflux_tris = slim_noflux_tris

            nf_block = sparse.dok_matrix(
                (len(noflux_tris), self.g_int.Nnodes()), np.float64
            )
            ...
            for idx, (n, tri) in enumerate(noflux_tris):
                ...
                nf_block[target_dof, tri[0]] = (
                    -d01[0] ** 2 + d01[0] * d02[0] - d01[1] ** 2 + d01[1] * d02[1]
                )
                nf_block[target_dof, tri[1]] = -d01[0] * d02[0] - d01[1] * d02[1]
                nf_block[target_dof, tri[2]] = d01[0] ** 2 + d01[1] ** 2
                nf_rhs[target_dof] = 0

            M_Lap, B_Lap = self.nd.construct_matrix(
                op="laplacian",
    ...
    def psi_phi_setup(self, n_j_dirichlet=1):
        ...
        for gen_edge in self.internal_edges:
            ...
            edge = [
                gtri.select_nodes_nearest(x) for x in self.gen.nodes["x"][gen_edge[:2]]
            ]
            ...
            if internal_angle % 180 == 0:  # join on i
                ...
                # pdb.set_trace()
                raise Exception(
                    "Internal edge doesn't appear to join same-valued contours"
                )
        ...
        def longest_straight(target_angle):
            ...
            j_cycle = [
                gtri.nodes_to_edge(na, nb) for na, nb in zip(cycle, np.roll(cycle, -1))
            ]
    ...
    def psi_phi_solve(self):
        ...
        Bblocks = []
        if 1:  # PSI
            M_psi_Lap, B_psi_Lap = nd.construct_matrix(
                op="laplacian",
        ...
        if 1:  # PHI
            ...
            M_phi_Lap, B_phi_Lap = nd.construct_matrix(
                op="laplacian",
    ...
    def plot_psi_phi_setup(self, num=11):
        ...
        for i_d in self.i_dirichlet_nodes:
            ax.annotate(
                f"$\psi$={self.i_dirichlet_nodes[i_d]}",
                self.g_int.nodes["x"][i_d],
                va="top",
                arrowprops=dict(arrowstyle="simple", alpha=0.4),
            )
        for j_d in self.j_dirichlet_nodes:
            ax.annotate(
                f"$\phi$={self.j_dirichlet_nodes[j_d]}",
                self.g_int.nodes["x"][j_d],
                va="bottom",
                arrowprops=dict(arrowstyle="simple", alpha=0.4),
            )
        ...
        for idx, i_grp in enumerate(self.i_tan_groups):
            ax.plot(
                self.g_int.nodes["x"][i_grp, 0],
                self.g_int.nodes["x"][i_grp, 1],
                ".",
        ...
        for idx, j_grp in enumerate(self.j_tan_groups):
            ax.plot(
                self.g_int.nodes["x"][j_grp, 0],
                self.g_int.nodes["x"][j_grp, 1],
                "+",
                color=next(group_colors),
                label=f"j grp {idx}",
            )
        ...
        i_quivs = np.array(
            [
                [self.g_int.nodes["x"][n], self.i_grad_nodes[n]]
                for n in self.i_grad_nodes
            ]
        )
        j_quivs = np.array(
            [
                [self.g_int.nodes["x"][n], self.j_grad_nodes[n]]
    ...
    def plot_psi_phi(self, num=4, thinning=2, ax=None):
        ...
        cset_psi = self.g_int.contour_node_values(
            self.psi,
            int(di / thinning),
            linewidths=1.5,
            linestyles="solid",
        ...
        cset_phi = self.g_int.contour_node_values(
            self.phi,
            int(dj / thinning),
            linewidths=1.5,
            linestyles="solid",
        ...
        ax.clabel(
            cset_psi, fmt="$\psi$=%g", fontsize=10, inline=False, use_clabeltext=True
        )
        ax.clabel(
            cset_phi, fmt="$\phi$=%g", fontsize=10, inline=False, use_clabeltext=True
        )
    ...
    def psiphi_to_ij(self, gen, g_int, src="ij", inverse=False):
        ...
        for coord in [0, 1]:  # i,j
            ...
            gen_to_int_nodes = [
                g_int.select_nodes_nearest(x) for x in gen.nodes["x"][gen_valid]
            ]
            ...
            coord_to_field = np.array(
                [
                    [k, np.mean(all_field[elts])]
    ...
    def remap_ij(self, g, src="ij"):
        ...
        IJ_in = g.nodes[
            "ij"
        ]  # g may be generated in IJ space, but the field is still 'ij'

        # Make a hash to ease navigation
        IJ_to_n = {tuple(IJ_in[n]): n for n in g.valid_node_iter()}
        ...
        for coord in [0, 1]:  # psi/i,  phi/j
            ...
            for gen_n in fixed:
                ...
                assert np.allclose(
                    g.nodes["x"][n], self.gen.nodes["x"][gen_n]
                ), "did not find a good match g~gen, based on x"
                ij_out[n, coord] = val
            ...
            interp_IJ_to_ij = utils.LinearNDExtrapolator(
                IJ_in[valid, :], ij_out[valid, coord]
            )
    ...
    def create_final_by_patches(self):
        ...
        g_final = exact_delaunay.Triangulation(
            extra_edge_fields=[("angle", np.float64), ("psiphi", np.float64, 2)]
        )
        ...
        def trace_contour(b, angle):
            ...
            cval = node_field[b]
            return g_int.trace_node_contour(
                n0=b,
                cval=cval,
        ...
        def insert_contour(trace_items, angle=None, psiphi0=[np.nan, np.nan]):
            assert np.isfinite(psiphi0[0]) or np.isfinite(psiphi0[1])

            trace_points = np.array(
                [pnt for typ, idx, pnt in trace_items if pnt is not None]
            )
            ...
            for i in [0, -1]:
                if trace_items[i][0] == "edge":
                    ...
                    best = [None, np.inf]
                    for j in np.nonzero(
                        g_final.edges["constrained"] & (~g_final.edges["deleted"])
                    )[0]:
                        d = utils.point_segment_distance(
                            pnt, g_final.nodes["x"][g_final.edges["nodes"][j]]
                        )
                        if d < best[1]:
                            best = [j, d]
                    j, d = best
            ...
            trace_nodes, trace_edges = g_final.add_constrained_linestring(
                trace_points, on_intersection="insert", on_exists="stop"
            )
        ...
        def trace_and_insert_contour(b, angle):
            ...
            gn = fixed_int_to_gen[
                b
            ]  # below we already check to see that b is in there.
            ...
            return insert_contour(trace_items, angle=angle, psiphi0=psiphi0)
        ...
        def trace_and_insert_boundaries(cycle):
            for a, b in utils.progress(zip(cycle, np.roll(cycle, -1))):
                ...
                trace_nodes, trace_edges = g_final.add_constrained_linestring(
                    trace_points, on_intersection="insert"
                )
                g_final.edges["angle"][trace_edges] = angle
        ...
        for a, b, c in zip(cycle, np.roll(cycle, -1), np.roll(cycle, -2)):
            ...
            for angle in [0, 90, 180, 270]:
                ...
                if ((angle_bc == angle_ba) and (angle != angle_bc)) or (
                    angle - angle_bc
                ) % 360 < ((angle_ba - angle_bc) % 360):
                    if b_final is None:
                        b_final = g_final.select_nodes_nearest(
                            g_int.nodes["x"][b], max_dist=0.0
                        )
                    dupe = False
                    assert (
                        b_final is not None
                    )  # should be in there from trace_boundaries
                    for exit_angle, exit_type in node_exits[b_final]:
                        if exit_angle == angle:
                            dupe = True
                            log.info(
                                "Duplicate exit for internal trace from %d. Skip" % b
                            )
                            break
                    if not dupe:
        ...
        def tri_to_grid(g_final):
            ...
            for j in np.nonzero(
                (~g_final2.edges["deleted"]) & (~g_final2.edges["constrained"])
            )[0]:
        ...
        if 1:
            ...
            ragged = np.isfinite(g_int.edges["angle"]) & (
                g_int.edges["angle"] % 90 != 0.0
            )
            j_ints = np.nonzero(ragged)[0]
            for j_int in j_ints:
                nodes = [
                    g_final2.add_or_find_node(g_int.nodes["x"][n])
                    for n in g_int.edges["nodes"][j_int]
                ]
                j_fin2 = g_final2.nodes_to_edge(nodes)
                angle = g_int.edges["angle"][j_int]
                if j_fin2 is None:
                    j_fin2 = g_final2.add_edge(
                        nodes=nodes,
                        constrained=True,
                        angle=angle,
        ...
        for j in g_final2.valid_edge_iter():
            ...
            if g_final2.edges["angle"][j] % 180 == 0:  # guess failed.
                ...
                log.warning(
                    "What? Ragged edge okay, but it shouldn't have both cell neighbors"
                )

        n_comp_i, labels_i = sparse.csgraph.connected_components(
            i_adj.astype(np.int32), directed=False
        )
        ...
        def add_swath_contours_new(comp_cells, node_field, coord, scale):
            ...
            for comp_cell in comp_cells:
                comp_pp = np.array(
                    g_final2.edges["psiphi"][g_final2.cell_to_edges(comp_cell)]
                )
            ...
            swath_poly = ops.cascaded_union(
                [g_final2.cell_polygon(c) for c in comp_cells]
            )
            ...
            for comp_cell in comp_cells:
                swath_boundaries.append(
                    np.nonzero((e2c.min(axis=1) < 0) & (e2c.max(axis=1) == comp_cell))[
                        0
                    ]
                )
            ...
            for angle in [0, 90, 180, 270]:
                angle_to_edges = swath_boundaries[
                    g_final2.edges["angle"][swath_boundaries] == angle
                ]
                angle_to_nodes[angle] = set(
                    g_final2.edges["nodes"][angle_to_edges].ravel()
                )

            # hoping these can be mapped back to gen...
            for angle in [0 + 90 * coord, 180 + 90 * coord]:
                cornerA = (
                    angle_to_nodes[angle] & angle_to_nodes[(angle - 90) % 360]
                ).pop()
                cornerB = (
                    angle_to_nodes[angle] & angle_to_nodes[(angle + 90) % 360]
                ).pop()
                ...
                gen_edges = self.gen.shortest_path(
                    genA,
                    genB,
                ...
                if np.all(scales < 0):
                    ...
                    if n_swath_cells is None:
                        ...
                        raise Exception(
                            "Both ends of this swath have set counts, but the counts don't match"
                        )
            ...
            adj_contours = np.interp(s_contours, s, o_vals)
        ...
        if 1:  # Swath processing
            for coord in [0, 1]:  # i/j
                ...
                for comp in range(n_comp):
                    ...
                    add_swath_contours_new(
                        comp_cells, node_field, coord, self.scales[coord]
                    )
            ...
            for c in c_ragged:
                ...
                for j in g_final2.cell_to_edges(c):
                    ...
                    if min(c1, c2) < 0:
                        continue  # only internal edges matter
                    if c1 == c:
                        c_nbr = c2
                    elif c2 == c:
                        c_nbr = c1
        ...
        for c in utils.progress(g_final2.valid_cell_iter()):
            ...
            g_patch.add_rectilinear(
                [0, 0],
                [len(psi_cvals) - 1, len(phi_cvals) - 1],
                len(psi_cvals),
                len(phi_cvals),
            )
            g_patch.add_node_field("ij", g_patch.nodes["x"].astype(np.int32))
            pp = np.c_[
                psi_cvals[g_patch.nodes["ij"][:, 0]],
            ...
            for n in g_patch.valid_node_iter():
                x = g_int.fields_to_xy(
                    g_patch.nodes["pp"][n], node_fields=[self.psi, self.phi], x0=x0
                )
                if np.isnan(x[0]):
    ...
    def smooth_to_scale(self, g=None, smooth_iters=3, nudge_iters=2):
        ...
        tweaker.smooth_to_scale(
            n_free, target_scales, smooth_iters=smooth_iters, nudge_iters=nudge_iters
        )
    ...
    def do_nudging(self, nudge_lines):
        ...
        for rec in nudge_lines:
            ...
            for n in g_final.valid_node_iter():
                if (
                    g_final.nodes["ij"][n, orient] == rec["contour"]
    ...
    def find_nudge_lines(self):
        ...
        nudge_lines = (
            []
        )  # [ ( {0 for i, 1 for j}, contour value, nodestring from gen_orig, linestring), ]
        internal = e2c.min(axis=1) >= 0
        ...
        for orient in [0, 90]:
            ...
            orient_ls = gen_orig.extract_linear_strings(
                edge_select=edge_sel, end_func=lambda n, js: len(js) < 2
            )
            internal_linestrings.extend(orient_ls)

        node_to_ij = (
            {}
        )  # map node index of gen_orig to ij value in g_final, based on exact matches.
        ...
        for internal_ls in internal_linestrings:
            ...
            for end_node in end_nodes:
                node_paths = gen_orig.shortest_path(
                    n1=end_node, n2=list(node_to_ij.keys()), return_type="nodes"
                )
                ...
                for n_target, node_path in node_paths:
                    ...
                    for ii in [0, 1]:
                        ...
                        if np.isnan(ij_recon[ii]):
                            ...
                            print(
                                f"reconstructured coord: {ij_recon[ii]} vs {one_ij_recon[ii]}"
                            )
                for ii in [0, 1]:
                    if np.isfinite(ij_recon[ii]):
                        continue
                    # This isn't great -- ideally we'd get a fractional value here.
                    nodes_final = g_final.select_nodes_nearest(
                        gen_orig.nodes["x"][end_node], count=4
                    )
            ...
            parallel_angle = gen_orig.edges["angle"][
                gen_orig.nodes_to_edge(internal_ls[:2])
            ]
            ...
            contour_value = int(
                np.round(0.5 * (end_ijs[0][orient] + end_ijs[1][orient]))
            )
            ...
            if contour_value == min_value:
                contour_value += 1
            if contour_value == max_value:
                contour_value -= 1
            ...
            if 0:
                ...
                linestring = self.gen_bezier_linestring(
                    nodestring=internal_ls, gen=gen_orig
                )
...
class SimpleQuadGen(object):
    ...
    def __init__(self, gen, cells=None, execute=True, adjust_scale=True, **kw):
        ...
        if adjust_scale:
            ...
            self.gen.edges["scale"] = adjusted_scale(
                self.gen, self.gen.edges["scale"], self.nom_res
            )
        if execute:
            self.execute()
...
def patch_contours(
    g_int, node_field, scale, count=None, Mdx=None, Mdy=None, lowpass_ds_dval=True
):
    """
    ...
    o_ds_dval = 1.0 / (o_dval_ds * local_scale)
...
class SimpleSingleQuadGen(QuadGen):
    ...
    def __init__(self, gen, cell, **kw):
        super(SimpleSingleQuadGen, self).__init__(
            gen, execute=False, cells=[cell], **kw
        )
    ...
    def discretize_perimeter(self):
        ...
        node_to_idx = {}
        angle_to_segments = {0: [], 90: [], 180: [], 270: []}
        ...
        while 1:
            ...
            scales0 = np.linspace(
                self.scales[0](pnts[0]), self.scales[0](pnts[-1]), len(pnts)
            )
            scales1 = np.linspace(
                self.scales[1](pnts[0]), self.scales[1](pnts[-1]), len(pnts)
            )
        ...
        for a, b, count in string:
            if count == 0:
                ...
                if idx_a < idx_b:
                    ...
                    pnts = np.concatenate(
                        [self.perimeter[idx_a:], self.perimeter[: idx_b + 1]]
                    )
                    dens_pc = np.concatenate([density[idx_a:], density[: idx_b + 1]])
                assert len(pnts) > 0
                ...
                if j is None:
                    ...
                    seg = np.c_[
                        np.linspace(pnt_a[0], pnt_b[0], count + 1),
                    ...
                    seg = self.gen_bezier_linestring(
                        j=j, samples_per_edge=count, span_fixed=False
                    )
    ...
    def select_node_counts(self):
        ...
        (left_i, left_i_rigid), (right_i, right_i_rigid) = self.calculate_coord_count(
            self.angle_to_segments[0],
            self.angle_to_segments[180],
            self.perimeter_scales[:, 0],
        )
        (left_j, left_j_rigid), (right_j, right_j_rigid) = self.calculate_coord_count(
            self.angle_to_segments[90],
    ...
    def create_patch(self):
        ...
        patch = unstructured_grid.UnstructuredGrid(
            max_sides=4,
            extra_node_fields=[("rigid", np.bool8), ("pp", np.float64, 2)],
            extra_edge_fields=[("orient", np.float32)],
        )
        elts = patch.add_rectilinear(
            [0, 0],
        ...
        patch.edges["orient"][
            deltas[:, 0] == 0
        ] = 90  # should be consistent with gen.edges['angle']
    ...
    def execute(self):
        ...
        self.i_contours = patch_contours(
            self.g_int,
            self.phi,
            self.scales[0],
            len(self.left_i),
            Mdx=Mdx,
            Mdy=Mdy,
            lowpass_ds_dval=self.lowpass_ds_dval,
        )
        self.j_contours = patch_contours(
            self.g_int,
            self.psi,
            self.scales[1],
            len(self.left_j),
            ...
            for j in range(len(self.left_j)):
                ...
                if i == 0 and self.left_j_rigid[j]:
                    ...
                    pp = [
                        self.j_contours[j],
                        self.i_contours[-i - 1],
                    ]  # I think I have to reverse i

                    x = self.g_int.fields_to_xy(pp, [self.psi, self.phi], x)

                if rigid:
                    pp = [
                        min(1, max(-1, self.psi_field(x))),
    ...
    def smooth_patch_psiphi_implicit(self, aniso=0.02, monotonify=True):
        ...
        for coord in [0, 1]:
            ...
            while 1:  # monotonify loop
                ...
                if monotonify:
                    ...
                    if adj_aniso >= 1:
                        log.warning(
                            "Could not adjust anisotropy enough to regain monotonicity"
                        )
                        break
                    else:
                        log.warning(
                            "Will smooth with decreased anisotropy (%g)" % adj_aniso
                        )
                else:
                    break
        ...
        for n in np.nonzero(~patch.nodes["rigid"])[0]:
            ...
            patch.nodes["x"][n] = self.g_int.fields_to_xy(
                patch.nodes["pp"][n], [self.psi, self.phi], x_orig
            )
    ...
    def nudge_boundaries_monotonic(self):
        ...
        for nlist, coord, sign in [
            ...
            bad = (~rigid) & (
                (sign * vals[i_prev] >= sign * vals)
                | (sign * vals[i_next] <= sign * vals)
            )
            print(bad.sum())
            vals[bad] = np.interp(i[bad], i[~bad], vals[~bad])
            self.patch.nodes["pp"][nlist, coord] = vals
            for n in nlist[bad]:
                x = self.g_int.fields_to_xy(
                    self.patch.nodes["pp"][n],
                    [self.psi, self.phi],
    ...
    def smooth_patch_psiphi(self, n_iter=10):
        ...
        for it in range(n_iter):
            ...
            smooth[..., 0] = signal.fftconvolve(
                dpp_r[..., 0], win[:, None], mode="same"
            )
            smooth[..., 1] = signal.fftconvolve(
                dpp_r[..., 1], win[None, :], mode="same"
            )
            if 1:  # well, in some cases might need this...
                smooth[..., 0] = signal.fftconvolve(
                    smooth[..., 0], win[None, :], mode="same"
                )
                smooth[..., 1] = signal.fftconvolve(
                    smooth[..., 1], win[:, None], mode="same"
                )
