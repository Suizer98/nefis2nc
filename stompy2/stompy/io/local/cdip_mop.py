...
def hindcast_dataset(
    ...
    """
    url = (
        "http://thredds.cdip.ucsd.edu/thredds/dodsC/cdip/model/MOP_alongshore/%s_hindcast.nc"
        % station
    )
    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        ...
        if (cache_fn is not None) and os.path.exists(cache_fn):
            ...
            cmip_start, cmip_stop = np.searchsorted(
                t, [np.datetime64(interval_start), np.datetime64(interval_end)]
            )
