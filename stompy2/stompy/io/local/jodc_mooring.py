...
def jodc_mooring_to_ds(filename):
    ...
    while line:
        ...
        headers = [line, nextline(), nextline()]
        ...
        lat = signs[lat_s[6]] * (
            float(lat_s[:2]) + float(lat_s[2:4]) / 60.0 + float(lat_s[4:6]) / 3600.0
        )
        lon = signs[lon_s[7]] * (
            float(lon_s[:3]) + float(lon_s[3:5]) / 60.0 + float(lon_s[5:7]) / 3600.0
        )
        ds["lat"] = (), lat
        ds["lon"] = (), lon
