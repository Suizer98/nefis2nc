...
def rot(angle, pnts):
    ...
    return np.tensordot(
        R, pnts, axes=(1, -1)
    ).transpose()  # may have to tweak this for multiple points
...
class TriGrid(object):
    ...
    def read_triangle(self, tri_basename):
        ...
        Npoints, point_dimension, npoint_attrs, npoint_markers = [
            int(s) for s in points_fp.readline().split()
        ]
        ...
        Ncells, node_per_tri, ncell_attrs = [
            int(s) for s in elements_fp.readline().split()
        ]
    ...
    def renumber(self):
        ...
        if np.any(self.cells) < 0:
            raise Exception(
                "renumber: after remapping indices, have negative node index in cells"
            )
    ...
    def write_obj(self, fname):
        ...
        for i in six.move.range(self.Ncells()):
            fp.write(
                "f %d %d %d\n"
                % (self.cells[i, 0] + 1, self.cells[i, 1] + 1, self.cells[i, 2] + 1)
            )
    ...
    def write_sms(self, fname):
        ...
        for i in range(self.Npoints()):
            fp.write(
                "%10i %.11f %.11f %.11f\n"
                % (i + 1, self.points[i, 0], self.points[i, 1], 0.0)
            )
    ...
    def plot_edge_marks(self, edge_mask=None, clip=None):
        ...
        for e in np.nonzero(edge_mask)[0]:
            ...
            plt.annotate(
                "c%d-j%d,%d-c%d"
                % (self.edges[e, 3], e, self.edges[e, 2], self.edges[e, 4]),
                ec[e],
    ...
    def plot_scalar(self, scalar, pdata=None, clip=None, ax=None, norm=None, cmap=None):
        ...
        if not pdata:
            ...
            patch_coll = collections.PolyCollection(
                segments, edgecolors="None", antialiaseds=0, norm=norm, cmap=cmap
            )
    ...
    def bounds(self):
        ...
        return (
            self.points[valid, 0].min(),
            self.points[valid, 0].max(),
            self.points[valid, 1].min(),
            self.points[valid, 1].max(),
        )
    ...
    def write_shp(self, shpname, only_boundaries=1, edge_mask=None, overwrite=0):
        ...
        olayer = ods.CreateLayer(shpname, srs=srs, geom_type=ogr.wkbLineString)
    ...
    def write_contours_shp(self, shpname, cell_depths, V, overwrite=False):
        ...
        for v in V:
            ...
            on_contour = (cell_depths[self.edges[:, 3]] <= v) != (
                cell_depths[self.edges[:, 4]] <= v
            )
    ...
    def write_mat(self, fn, order="ccw"):
        ...
        d["readme"] = "\n".join(
            [
                "points: [Npoints,2] node locations",
    ...
    def write_suntans(self, pathname):
        ...
        if n_missing > 0:
            print(
                "WARNING: %d edges are on the boundary but have marker==0" % n_missing
            )
        ...
        for i in range(self.Ncells()):
            ...
            for j in 0, 1, 2:
                ...
                if n == -1 or i < n:
                    ...
                    edges.append((pnt_a, pnt_b, default_marker, i, n))
    ...
    def verify_bc(self, do_plot=True):
        ...
        if do_plot:
            for e in bad_edges:
                ...
                plt.plot(self.points[bad_points, 0], self.points[bad_points, 1], "r-o")

        if len(bad_edges) > 0:
            print(
                "BAD: there are %d edges without BC that have only 1 cell"
                % len(bad_edges)
            )
            return 0
        else:
    ...
    def cell2edges(self, cell_i):
        ...
        edges = [
            self.find_edge((pnts[(i + 1) % 3], pnts[(i + 2) % 3])) for i in range(3)
        ]
        return edges
    ...
    def cell_divergence_of_edge_flux(self, edge_flux):
        ...
        flux_divergence = np.sum(
            np.sum(nxy * (edge_len[:, None] * edge_flux)[cell_to_edges], axis=1), axis=1
        )  # maybe...
    ...
    def build_index(self):
        if self.index is None:
            ...
            tuples = [
                (i, self.points[i, xxyy], None)
                for i in range(self.Npoints())
                if np.isfinite(self.points[i, 0])
            ]
    ...
    def cells_on_line(self, xxyy):
        ...
        extent
        """
        m = np.array([[xxyy[0], xxyy[2], 1], [xxyy[1], xxyy[3], 1], [1, 1, 1]])
    ...
    def delete_node_and_merge(self, n):
        ...
        if len(edges) == 2:
            if self.verbose > 1:
                print(
                    "  deleting node %d, will merge edges %d and %d"
                    % (n, edges[0], edges[1])
                )
        ...
        if len(edges) != 0:
            print(
                "Should have removed all edges to node %d, but there are still some" % n
            )
    ...
    def merge_edges(self, e1, e2):
        ...
        self.push_op(
            self.unmerge_edges, e1, e2, self.edges[e1].copy(), self.edges[e2].copy()
        )
    ...
    def delete_cell(self, c, replace_with=-2, rollback=1):
        ...
        for e in [ab, bc, ca]:
            if self.edges[e, 3] == c:
                ...
                print(
                    "Cell: %d  check on edge %d  with nbrs: %d %d"
    ...
    def split_edge(self, nodeA, nodeB, nodeC):
        ...
        if any(self.edges[e1, 3:5] >= 0):
            print(
                "While trying to split the edge %d (%d-%d) with node %s"
                % (e1, nodeA, nodeC, nodeB)
            )
    ...
    def add_edge(
        self, nodeA, nodeB, marker=0, cleft=-2, cright=-2, coerce_boundary=None
    ):
        ...
        if cleft == -2 or cright == -2:
            ...
            neighbors_from_a = np.setdiff1d(
                self.edges[edges_from_a, :2].ravel(), [nodeA, nodeB]
            )
            neighbors_from_b = np.setdiff1d(
                self.edges[edges_from_b, :2].ravel(), [nodeA, nodeB]
            )
            ...
            if len(candidates) > 0:
                ...
                for c in candidates:
                    ...
                    adj_ids = [
                        self.edges[e, check] for e, check in zip(ccw_edges, check_cells)
                    ]
                    ...
                    if any(adj_ids >= 0) and any(adj_ids != adj_ids[0]):
                        ...
                        print(
                            "During call to add_edge(nodeA=%d nodeB=%d marker=%d cleft=%d cright=%d coerce=%s"
                            % (nodeA, nodeB, marker, cleft, cright, coerce_boundary)
                        )
                        raise Exception(
                            "cell neighbor values for new cell using point %d are inconsistent: %s"
                            % (c, adj_ids)
                        )
                    elif all(adj_ids == -1):
                        ...
                        self.push_op(
                            self.unmodify_edge, edge_ac, self.edges[edge_ac].copy()
                        )
                        self.push_op(
                            self.unmodify_edge, edge_bc, self.edges[edge_bc].copy()
                        )
            ...
            for flip in [0, 1]:
                ...
                if bool(ccw) == bool(is_ccw(points)):
                    ...
                    if flip:
                        ...
                        edges_used[
                            np.intersect1d(failed_edges_used1, failed_edges_used2)
                        ] = 2
    ...
    def edges_to_rings_and_holes(self, edgemask):
        ...
        for oi in outer_rings:
                ...
                try:
                    if i != oi and outer_poly.contains(cw_polys[i]):
                        if not cw_polys[i].contains(outer_poly):
                            ...
                            print(
                                "Whoa - narrowly escaped disaster with a congruent CW poly"
                            )
            ...
            children = [
                possible_children_i[j] for j in outermost_rings(possible_children_poly)
            ]

            outer_polys.append([ccw_rings[oi], [cw_rings[i] for i in children]])
    ...
    def trim_to_left(self, path):
        ...
        if np.dot(self.points[other_point1], bad) > np.dot(
            self.points[other_point2], bad
        ):
            bad_cell = cell1
        else:
            bad_cell = cell2
    ...
    def revert(self, cp):
        ...
        if serial != self.op_stack_serial:
            raise ValueError(
                "The current op stack has serial %d, but your checkpoint is %s"
                % (self.op_stack_serial, serial)
            )
