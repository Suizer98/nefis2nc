...
class UgridParticles(object):
    ...
    def scan_ncs(self):
        self.nc_catalog = np.zeros(
            len(self.ncs), dtype=[("start_t", "f8"), ("end_t", "f8")]
        )
    ...
    def update_velocity(self):
        changed = False
        nc_i = (
            np.searchsorted(self.nc_catalog["start_t"], self.t_unix, side="right") - 1
        )
        ...
        if self.current_nc_idx + 1 < len(self.ncs):
            next_step = min(
                next_step, self.nc_catalog["start_t"][self.current_nc_idx + 1]
            )

        assert np.isfinite(
            next_step
        )  # did we integrate off the end of the avail. data?

        self.velocity_valid_time = [self.nc_t_unix[self.nc_time_i], next_step]
        ...
        for i, p in enumerate(self.P):
            ...
            while part_t < stop_t:
                ...
                for j in g.cell_to_edges(p["c"]):
                    ...
                    closing = (
                        self.P["u"][i, 0] * normal[0] + self.P["u"][i, 1] * normal[1]
                    )
                ...
                # see if we're stuck
                if (
                    utils.mag(delta) / (utils.mag(delta) + utils.mag(self.P["x"][i]))
                    < 1e-14
                ):
                ...
                if j_cross is not None:
                    ...
                    if new_c < 0:
                        ...
                        recross = (
                            self.U[new_c, 0] * j_cross_normal[0]
