...
class DSM2Grid(unstructured_grid.UnstructuredGrid):
    def __init__(self, grid_fn, node_shp, channel_shp=None):
        ...
        super(DSM2Grid, self).__init__(
            extra_node_fields=extra_node_fields, extra_edge_fields=extra_edge_fields
        )
    ...
    def read_nodes(self):
        ...
        nodes = np.zeros(len(node_data), dtype=[("id", np.int32), ("x", np.float64, 2)])
