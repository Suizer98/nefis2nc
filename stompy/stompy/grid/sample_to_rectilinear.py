...
class Structurer(object):
    ...
    def process(self, tindex, var_name):
        ...
        for proc in range(self.Nsubdomains()):
            ...
            if var_name == "U_top1m":
                weights = self.averaging_weights(
                    proc=proc, time_step=tindex, ztop=0, dz=1.0
                )
                ...
                print(
                    "data_for_proc %d finite %d nan"
                    % (np.isfinite(data_for_proc).sum(), np.isnan(data_for_proc).sum())
                )
            elif var_name == "eta":
                data_for_proc = self.read_cell_scalar(
                    label="eta", processor=proc, time_step=tindex
                )
                ...
                if var_name == "u_wind":
                    ...
                    label = "wind_v"
                data_for_proc = self.read_cell_scalar(
                    label=label, processor=proc, time_step=tindex
                )
...
class UgridAverager(Structurer):
    ...
    def prepare(self):
        ...
        for nc_fn in self.nc_fns:
            ug = ugrid.UgridXr(
                nc_fn, face_eta_vname="eta", face_u_vname="uc", face_v_vname="vc"
            )
        ...
        self.global_to_local = np.zeros(
            gg.Ncells(),
            dtype=[("global", np.int32), ("proc", np.int32), ("local", np.int32)],
        )
        ...
        for p, g in enumerate(self.grids):
            ...
            for c, xy in enumerate(gcc):
                ...
                if global_c is None:
                    continue
                if self.global_to_local["proc"][global_c] >= 0:
                    continue
    ...
    def averaging_weights(self, proc, time_step, ztop=None, dz=None, zbottom=None):
        return self.ugrids[proc].vertical_averaging_weights(
            time_slice=time_step, ztop=ztop, dz=dz, zbottom=zbottom
        )
