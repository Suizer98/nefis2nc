...
class WaterLevel(BC):
    ...
    def write_data(self, mdu, feature, var_name, base_fn):
        tides = noaa_coops.coops_dataset_product(
            self.station,
            "water_level",
            mdu.time_range()[1],
            mdu.time_range()[2],
        ...
        water_level[:] = filters.lowpass(
            water_level[:].values, utils.to_dnum(water_level.time), cutoff=3.0 / 24
        )
...
class Flow(BC):
    ...
    def write_data(self, mdu, feature, var_name, base_fn):
        ...
        flows = np.array([0.0, 0.0, self.storm_flow, self.storm_flow, 0.0, 0.0])
