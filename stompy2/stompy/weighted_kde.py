...
class Covariator(object):
    ...
    def scotts_factor(self):
        return power(
            np.ceil(self.n * (1 - (self.weights**2).sum())), -1.0 / (self.d + 4)
        )

    def silverman_factor(self):
        return power(
            np.ceil(self.n * (1 - (self.weights**2).sum())) * (self.d + 2.0) / 4.0,
            -1.0 / (self.d + 4),
        )
    ...
    def _compute_covariance(self):
        ...
        self._norm_factor = (2 * pi) ** (self.d / 2.0) * sqrt(
            linalg.det(self.covariance)
        )
...
def kde_hist(
    ...
    **line_args
    ...
    for v, w in zip(x, weights):
        ...
        inv_cov, norm_factor = w_cov(
            float(range[1] - range[0]) / (1.25 * bins * w_xstd)
        )

        gkde = gaussian_kde(
            v[None, :], weights=w, inv_cov=inv_cov, norm_factor=norm_factor
        )
