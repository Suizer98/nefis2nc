...
def fill_tidal_data(da, fill_time=True):
    ...
    if fill_time:
        ...
        da = xr.DataArray(dense_values, dims=["time"], coords=[dense_times])
    else:
        pass
...
sel = utils.within(
    utils.to_dnum(da_orig.time), [734791.50788189541, 734793.34447256383]
)
...
plt.plot(da.time, da)
plt.axis(
    (734791.50788189541, 734793.34447256383, -287214.44701689074, 191235.44801606232)
)
...
def select_omegas(T, omegas=None, factor=0.25):
    ...
    while 1:
        ...
        if omega_beat.min() < min_omega_beat:
            ...
            print(
                (
                    "Periods %.2fh and %.2fh have beat period %.2fh,"
                    " too long to be resolved by %.2fh time"
                    " series"
                )
                % (T_a, T_b, 2 * np.pi / (3600 * omega_beat.min()), T / 3600)
            )
            ...
            sel_omegas = np.concatenate((sel_omegas[:drop], sel_omegas[drop + 1 :]))
        else:
            break
