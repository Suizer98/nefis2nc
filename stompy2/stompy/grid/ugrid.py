...
def cf_to_datenums(nc_t_var):
    ...
    div = dict(seconds=86400.0, minutes=60 * 24, hours=24, days=1)[units]
...
class Ugrid(object):
    surface_dzmin = (
        2 * 0.001
    )  # common value, but no guarantee that this matches suntans code.
    ...
    def Ncells(self, mesh_name=None):
        ...
        return len(
            self.nc.dimensions[
                self.nc.variables[mesh.face_node_connectivity].dimensions[0]
            ]
        )
    ...
    def get_cell_velocity(self, time_slice, mesh_name=None, face_slice=slice(None)):
        ...
        slices = {"time": time_slice, "n" + mesh_name + "_face": face_slice}
        u_comp = ncslice(self.nc.variables[mesh_name + "_cell_east_velocity"], **slices)
        v_comp = ncslice(
            self.nc.variables[mesh_name + "_cell_north_velocity"], **slices
        )

        U = np.concatenate((u_comp[..., None], v_comp[..., None]), axis=-1)
    ...
    def to_trigrid(self, mesh_name=None, skip_edges=False):
        ...
        node_xy = np.array(
            [nc.variables[node_x_name][...], nc.variables[node_y_name][...]]
        ).T
    ...
    def grid(self, mesh_name=None, skip_edges=False):
        ...
        ug = unstructured_grid.UnstructuredGrid(
            points=node_xy, cells=faces, edges=edges
        )
    ...
    def vertical_averaging_weights(
        self,
        ...
        surface = self.find_var(
            standard_name="sea_surface_height_above_geoid", mesh=mesh_name
        )

        face_select = {face_dim: face_slice}
        h = ncslice(self.nc.variables[surface], time=time_slice, **face_select)

        depth = self.find_var(
            standard_name=["sea_floor_depth_below_geoid", "sea_floor_depth"],
            mesh=mesh_name,
            location="face",
        )  # ala 'Mesh_depth'
...
class UgridXr(object):
    ...
    def get_cell_velocity(self, time_slice, face_slice=None):
        ...
        new_dims = [
            d for d in [self.time_dim, self.face_dim, self.layer_dim] if d in u_slc.dims
        ]
        if 0:
    ...
    def grid(self):
        ...
        if self._grid is None:
            self._grid = unstructured_grid.UnstructuredGrid.from_ugrid(
                self.nc, mesh_name=self.mesh_name
            )
        return self._grid
    ...
    def layer_var_name(self):
        ...
        for name in self.nc.variables:  # but also try looking for it.
            if self.nc[name].attrs.get("standard_name") in [
                "ocean_zlevel_coordinate",
                "ocean_z_coordinate",
    ...
    def vertical_averaging_weights(
        ...
        if self.face_depth_vname is None:
            ...
            self.face_depth_vname = self.find_var(
                stanford_name=["sea_floor_depth_below_geoid", "sea_floor_depth"],
                location="face",
            )  # ala 'Mesh_depth'
        if self.face_depth_vname is None:
            self.face_depth_vname = self.find_var(
                standard_name=["altitude"], location="face"
            )
        ...
        if "bounds" in layers.attrs:
            ...
            layer_bounds = np.concatenate(
                (layer_interfaces[:-1, None], layer_interfaces[1:, None]), axis=1
            )
