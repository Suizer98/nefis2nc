...
d3d_libdir = os.path.join(os.environ["HOME"], "code/delft/d3d/master/src/lib/")
...
if 0:
    data_dir = (
        "/home/rusty/models/delft/tutorial/waq/friesian_tidal_inlet/all_flow_output"
    )
...
def inquire_elt(fd, elt_name):
    ...
    elmnam = (c_char * 17)()
    elmnam.value = elt_name  #  create_string_buffer("U1")
    elmtyp = create_string_buffer(" " * 9)
    nbytsg = c_int(0)

    elmqty = (c_char * 17)()
    elmqty.value = " " * 16  # create_string_buffer(" "*17)
    elmunt = (c_char * 17)()
    elmunt.value = " " * 16  # create_string_buffer(" "*17)
    desc = (c_char * 65)()
    desc.value = " " * 64  # create_string_buffer(" "*65)
    elmndm = c_int(5)
    elmdms = (c_int * 5)()
    ...
    print(
        "     dimensions: {}".format(
            ",".join([str(elmdms[i]) for i in range(elmndm.value)])
        )
    )
...
def inqfcl(fd, describe_elements=False):
    ...
    err = nef.Inqfcl(
        byref(fd), byref(celnam), byref(nelems), byref(bytes), byref(elmnms)
    )
...
itemsize = 4
buflen = c_int(
    itemsize * np.prod(cell_dims) * ncells
)  # the two is in case ending index is inclusive
if 1:
...
for t in range(11):
    plt.imshow(
        u1[t, 9], interpolation="none", aspect="auto", vmin=-0.5, vmax=0.5, cmap="RdBu"
    )
    plt.pause(0.2)
...
plt.figure(1).clf()
mesh = plt.pcolormesh(
    xcor_m[:-1, :-1], ycor_m[:-1, :-1], dp0_m[:-1, :-1], edgecolor="k", lw=0.2
)
plt.axis("equal")

plt.axis(
    (165934.04627395154, 233318.9728923004, 586053.18933475052, 639279.13222990348)
)
...
plt.scatter(xcor[:-1, :-1], ycor[:-1, :-1], 40, dp0[:-1, :-1], vmin=0, vmax=5, lw=0)

plt.scatter(xz, yz, 100, active, lw=0)

plt.scatter(xz, yz, 40, dps0, vmin=0, vmax=5, lw=0)
...
class DflowGrid2D(object):
    def __init__(self, xcor, ycor, xz, yz, active):
        ...
        self.node_active[:-1, :-1] = (
            comp[1:, 1:] | comp[:-1, 1:] | comp[1:, :-1] | comp[:-1, :-1]
        )
    ...
    def to_triangulation(self):
        ...
        for row, col in zip(*np.nonzero(comp)):
            # may not be CCW
            tris.append([idx1d[row, col], idx1d[row, col + 1], idx1d[row + 1, col + 1]])
            tris.append([idx1d[row + 1, col + 1], idx1d[row + 1, col], idx1d[row, col]])
