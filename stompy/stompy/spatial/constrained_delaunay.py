...
class ConstrainedXYZField(field.XYZField):
    ...
    def __init__(self, X, F, projection=None, from_file=None, edges=None):
        ...
        """
        field.XYZField.__init__(
            self, X=X, F=F, projection=projection, from_file=from_file
        )
        if edges is None:
    ...
    def to_grid(self, *args, **kwargs):
        if "interp" in kwargs:
            if kwargs["interp"] != "linear" and len(self.edges) > 0:
                print(
                    "Warning - constrained triangulation doesn't like natural neighbors"
                )
        else:
    ...
    def linesearch(self, a, b):
        ...
        while 1:
            ...
            nbrs = [n for n in self.node_neighbors(nodes[-1]) if n != nodes[-2]]
            if len(nbrs) == 1:
                nodes.append(nbrs[0])
                continue
