...
class Transect(object):
    ...
    def __init__(self, xy, times, elevations, scalar, desc="", dists=None):
        ...
        if elevations.ndim == 1:
            ...
            elevations = np.repeat(
                elevations[:, None],  # 0 index is z, 1 index is xy
                [scalar.shape[1]],
                axis=1,
            )
        ...
        if elevations.shape[0] == scalar.shape[0]:
            ...
            self.interface_elevations = concatenate(
                ([elevations[0]], breaks, [elevations[-1]])
            )
            ...
            for i in range(scalar.shape[1]):
                ...
                if len(valid) > 1:
                    ...
                    self.interface_elevations[valid[0], i] = self.center_elevations[
                        valid[0], i
                    ]
                    # bottom of the last valid cell
                    self.interface_elevations[
                        valid[-1] + 1, i
                    ] = self.center_elevations[valid[-1], i]
    ...
    def bed_elevations(self):
        ...
        for i in range(len(elevs)):
            k_bot = np.nonzero(np.isfinite(self.scalar[:, i]))[0][
                -1
            ]  # index inclusive!
    ...
    def d_dz(self):
        ...
        for i in range(len(grads)):
            ...
            [m, b] = np.polyfit(
                self.center_elevations[valid, i], self.scalar[valid, i], 1
            )
            grads[i] = m
        return grads
    ...
    def vertical_average(self, z_top=None, z_bot=None, dz=None):
        ...
        for i in range(len(vals)):
            ...
            if self.scalar_location == "centers":
                ...
                eval_scal = np.interp(
                    eval_z, self.center_elevations[valid, i], self.scalar[valid, i]
                )
...
def cast_timeseries_to_transect(
    xy, times, z, scalar, z_surface="auto", min_cast_samples=10
):
    """
