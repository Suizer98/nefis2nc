...
def parse_his_file(fn):
    ...
    fdtype = np.dtype([("sub", "S10"), ("proc", "S10")])

    fields = np.fromfile(fp, fdtype, n_fields)

    regions = np.fromfile(fp, [("num", "i4"), ("name", "S20")], n_regions)

    # assume that data is 'f4'
    # following other Delft output, probably each frame is prepended by
    # 'i4' time index
    frame_dtype = np.dtype([("tsec", "i4"), ("data", "f4", (n_regions, n_fields))])
...
def bal_his_file_dataframe(fn):
    ...
    for ri, region in enumerate(regions):
        for fi, field in enumerate(fields):
            tuples.append(
                (region["name"].strip(), field["sub"].strip(), field["proc"].strip())
            )

    col_index = pd.MultiIndex.from_tuples(tuples, names=("region", "sub", "proc"))
    df = pd.DataFrame(
        data=frames["data"].reshape((-1, n_regions * n_fields)),
        index=frames["tsec"],
        columns=col_index,
    )
    return df
...
def his_file_xarray(fn, region_exclude=None, region_include=None):
    ...
    if region_include:
        region_mask = np.array(
            [bool(re.match(region_include, region)) for region in region_names]
        )
    else:
        region_mask = np.ones(len(region_names), np.bool8)

    if region_exclude:
        skip = [bool(re.match(region_exclude, region)) for region in region_names]
    ...
    ds["bal"] = (("time", "region", "field"), frames["data"][:, region_mask, :])
    return ds
...
def inp_tok_include(fp, fn, **kw):
    ...
    while 1:
        ...
        if tok.upper() != "INCLUDE":
            ...
            inc_path = os.path.join(os.path.dirname(fn), inc_fn)
        ...
        while 1:  # iterate over BC blocks
            ...
            while 1:  # iterate over the 3 subparts of a block
                if tok.upper() == "ITEM":
                    ...
                    matrix = np.zeros((len(defs[0][1]), len(defs[1][1])), "f8")
...
def pli_to_shp(pli_fn, shp_fn, overwrite=False):
    ...
    geoms = [geometry.LineString(clean_pnts(feat[1])) for feat in feats]
    names = [feat[0] for feat in feats]
    wkb2shp.wkb2shp(shp_fn, geoms, fields=dict(name=names), overwrite=overwrite)
    ...
    try:
        for feature in pli_data:
            ...
            if len(data) != len(node_labels):
                raise Exception(
                    "%d nodes, but there are %d node labels"
                    % (len(data), len(node_labels))
                )
            # .strip to trim leading white space
            block = "\n".join(
                [
                    "  ".join(["%15s" % d for d in row]).strip() + "   " + node_label
...
def pli_to_grid_edges(g, levees):
    ...
    for levee in utils.progress(levees, msg="Levees: %s"):
        ...
        xxyy = [xyz[:, 0].min(), xyz[:, 0].max(), xyz[:, 1].min(), xyz[:, 1].max()]
...
def read_map(
    ...
    assert not isinstance(
        hyd, six.string_types
    ), "Must pass in Hydro() object, not path"
    ...
    if extra != 0:
        log.warning(
            "Reading map file %s: %d or %d frames? bad length %d extra bytes (or %d missing)"
    ...
    mapped = np.memmap(
        fn,
        [("tsecs", "i4"), ("data", "f4", (n_layers, hyd.n_2d_elements, n_subs))],
        mode="r",
        shape=(nframes,),
        offset=data_start,
    )
    ...
    for idx, name in enumerate(ds.sub.values):
        ds[name] = (("time", "layer", "face"), mapped["data"][..., idx])
...
def dfm_wind_to_nc(wind_u_fn, wind_v_fn, nc_fn):
    ...
    u_var = nc.createVariable(
        "wind_u", np.float32, [tdim, ydim, xdim], fill_value=u_header["NODATA_value"]
    )
    v_var = nc.createVariable(
        "wind_v", np.float32, [tdim, ydim, xdim], fill_value=v_header["NODATA_value"]
    )
...
"""
    ...
    for fp, quant in [(fp_u, "x_wind"), (fp_v, "y_wind"), (fp_p, "pres")]:
        if fp is None:
            continue
        # Write the headers:
    ...
    for time_idx in range(time_idx_start, time_idx_stop):
        ...
        if (time_idx - time_idx_start) % 96 == 0:
            log.info(
                "Written %d/%d time steps"
                % (time_idx - time_idx_start, time_idx_stop - time_idx_start)
            )
        ...
        time_line = "TIME=%f seconds since 1970-01-01 00:00:00 +00:00" % utils.to_unix(
            t.values
        )

        for fp, data in [(fp_u, u), (fp_v, v), (fp_p, p)]:
            if fp is None:
                continue
            # double check order.
            fp.write(time_line)
...
class SectionedConfig(object):
    ...
    def set_value(self, sec_key, value):
        ...
        last_row_of_section = (
            {}
        )  # map [lower_section] to the index of the last entry in that section
        ...
        for row_idx, row_sec, row_key, row_value, row_comment in self.entries():
            ...
            if (row_key.lower() == key.lower()) and (
                section.lower() == row_sec.lower()
            ):
    ...
    def __contains__(self, sec_key):
        ...
        for row_idx, row_sec, row_key, row_value, row_comment in self.entries():
            if ((key is None) or (row_key.lower() == key)) and (
                section.lower() == row_sec.lower()
            ):
                return True
        else:
...
class MDUFile(SectionedConfig):
    ...
    def time_range(self):
        ...
        """
        t_ref = utils.to_dt64(
            datetime.datetime.strptime(self["time", "RefDate"], "%Y%m%d")
        )
        dt = self.t_unit_td64()
    ...
    def partition(self, nprocs, dfm_bin_dir=None, mpi_bin_dir=None):
        ...
        cmd = "%s -n %d %s --partition:ndomains=%d %s" % (
            mpiexec,
            nprocs,
            dflowfm,
            nprocs,
...
def read_bnd(fn):
    ...
    with open(fn, "rt") as fp:
        ...
        for group_idx in range(N_groups):
            group_name = token()
            N_link = int(token())
            links = np.zeros(N_link, dtype=[("link", "i4"), ("x", "f8", (2, 2))])
    ...
    with open(fn, "wt") as fp:
        ...
        for name, segs in bnd:
            ...
            for seg in segs:
                ...
                fp.write(
                    "%10d  %.7f  %.7f   %.7f  %.7f\n"
                    % (seg["link"], x[0, 0], x[0, 1], x[1, 0], x[1, 1])
                )
...
def read_dfm_bc(fn):
    ...
    if 1:
        ...
        while 1:  # looping over datasets
            ...
            assert (
                line.strip().lower() == "[forcing]"
            ), "Expected [forcing], got %s in %s" % (line, fn)
            line = fp.readline()
