...
def nwis_dataset(
    ...
    params = dict(site_no=station, format="rdb")
    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        ...
        if (cache_fn is not None) and os.path.exists(cache_fn):
            ...
            if os.path.getsize(cache_fn) == 0:
                ...
                log.warning(
                    " cache for %s -- %s says no-data" % (interval_start, interval_end)
                )
                continue
            ds = xr.open_dataset(cache_fn)
        elif cache_only:
            log.info(
                "Cache only - no data for %s -- %s" % (interval_start, interval_end)
            )
            continue
        else:
            ...
            if (
                ds is None
            ):  # There was no data there HERE - would like to have an option to record no data
    ...
    datasets = [
        ds.isel(time=np.r_[True, ds.time.values[1:] > ds.time.values[:-1]])
        for ds in datasets
    ]
    ...
    if clip:
        time_sel = (dataset.time.values >= start_date) & (
            dataset.time.values < end_date
        )
    ...
    for v in ds.data_vars:
        if v.startswith("specific_conductance"):
            ...
            if salt_name not in ds:
                ...
                salt = seawater.eos80.salt(
                    ds[v].values / 1000.0 / seawater.constants.c3515,
...
def station_metadata(station, cache_dir=None, force=False):
    ...
    url = (
        "https://waterdata.usgs.gov/nwis/inventory?agency_code=USGS&site_no=%s"
        % station
    )
...
def stations_period_of_record(ll_xxyy, parameter, cache_dir=None):
    ...
    if cache_dir is not None:
        cache_fn = os.path.join(
            cache_dir,
            f"stations_periods-{ll_xxyy[0]}_{ll_xxyy[1]}_{ll_xxyy[2]}_{ll_xxyy[3]}_{parameter}.rdb",
        )
...
def station_period_of_record(station, parameter):
    ...
    # a single parameter may have multiple time series and return multiple rows.
    resp = requests.get(
        f"http://waterservices.usgs.gov/nwis/site/?format=rdb&sites={station}&seriesCatalogOutput=true"
    )
