...
class Rdb(object):
    ...
    def __init__(self, text=None, source_file=None, fp=None, **kw):
...
def rdb_to_dataset(filename=None, text=None, to_utc=True):
    ...
    for key in usgs_data.keys():
        ...
        if m:
            ...
            if parameter is not None:
                ...
                varname = (
                    varname.lower().replace(" ", "_").replace(",", "").replace(".", "")
                )
    ...
    if "tz_cd" in usgs_data.keys() and to_utc:
        ...
        if isinstance(usgs_data["tz_cd"], np.ndarray):
            offset_hours = np.array(
                [tz_to_offset(tz_src) for tz_src in usgs_data["tz_cd"]]
            )
        ...
        ds = ds.assign(datenum=lambda x: x.datenum - offset_hours / 24.0)
