...
def base_model(run_dir="run-dfm-test00"):
    ...
    inflow = hm.FlowBC(name="inflow", geom=np.array([[0, 0], [0, 500]]), flow=Q)

    stage = hm.HarmonicStageBC(
        name="stage", msl=1.0, S2=(0.5, 0), geom=np.array([[500, 0], [500, 500]])
    )

    source = hm.SourceSinkBC(name="source", flow=20.0, geom=np.array([100, 100]))
