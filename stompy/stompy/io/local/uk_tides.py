    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        if cache_dir is not None:
            ...
            cache_fn = os.path.join(
                cache_dir, "uk_tides", "%s_%s_%sa.nc" % (station, begin_str, end_str)
            )
        else:
            cache_fn = None
        ...
        if (not cache_only) and (ds is None):
            ...
            params = dict(
                startdate=fmt_date(interval_start),
            ...
            try:
                ...
                log.warning(
                    "Likely server error retrieving JSON data from environment.data.gov.uk"
                )
...
def json_to_ds(data, params):
    ...
    for d in data["items"]:
        ...
        rec = dict(time=t, measure=d["measure"], value=d["value"])
        rows.append(rec)
