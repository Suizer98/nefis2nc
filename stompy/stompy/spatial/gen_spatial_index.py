...
class _PointIndexKDTree(object):
    ...
    def insert(self, idx, point_tuple):
        raise NotImplementedError(
            "KDTree in use by gen_spatial_index; insert not allowed; install rtree..."
        )

    def delete(self, idx, point_tuple):
        raise NotImplementedError(
            "KDTree in use by gen_spatial_index; delete not allowed; install rtree..."
        )
...
def rect_index_class_factory(implementation="rtree"):
    ...
    raise Exception(
        "Failed to load any spatial index based on implementation='%s'" % implementation
    )
