...
class DemToGraph(object):
    def __init__(
        ...
        **kwargs
        ...
        self.weight_function = lambda d_j, K_j, edge_depths: d_j / (
            K_j * edge_depths**2
        )
    ...
    def init_stencils(self):
        ...
        if self.degree >= 16:
            ...
            kerns.append(array([[1, 0], [1, 1], [0, 1]]))
            offsets.append(array([[-1, 0], [1, 1]]))  # should be [0,0],[2,1]

            kerns.append(array([[0, 1], [1, 1], [1, 0]]))
            offsets.append(array([[-1, 1], [1, 0]]))

            kerns.append(array([[1, 1, 0], [0, 1, 1]]))
    ...
    def process(self):
        ...
        self.forced_lin_idx = forced_lin_idx = idxs[
            self.forced_dem_idx[:, 0], self.forced_dem_idx[:, 1]
        ]
    ...
    def build_matrix(self):
        ...
        self.sg = sparse.coo_matrix(
            (weights_undirected, (ij_undirected[:, 0], ij_undirected[:, 1])),
            (Ncells, Ncells),
        )
        return self.sg
