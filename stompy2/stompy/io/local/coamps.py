...
def coamps_files(start, stop, cache_dir, fields=["wnd_utru", "wnd_vtru", "pres_msl"]):
    ...
    for hour in target_hours:
        ...
        for step_back in [0, 12, 24, 36]:
            if step_back != 0:
                log.warning(
                    "Trying to find data for %s, step back is %d"
                    % (str(day_dt), step_back)
                )
            ...
            # is not valid.
            base_url = (
                "https://www.usgodae.org/pub/outgoing/fnmoc/models/"
            ...
            for field_name in fields:
                if field_name in ["wnd_utru", "wnd_vtru"]:
                    cat = "MET"
                    elev_code = (
                        105  # 0001: surface?  0105: above surface  0100: pressure?
                    )
                    elev = 100
                ...
                url_file = (
                    "US058G%s-GR1dyn.COAMPS-CENCOOS_CENCOOS-n3-c1_"
                    "%03d"
                    "00F0NL"
                    "%s_%04d_%06d-000000%s"
                ) % (cat, hour_of_run, run_tag, elev_code, elev, field_name)

                output_fn = os.path.join(cache_dir, dataset_name, url_file)
                recs[field_name] = dict(
                    url=base_url + url_file, local=output_fn, timestamp=hour
                )
            ...
            raise Exception(
                "Couldn't find a run for date %s" % day_dt.strftime("%Y-%m-%d %H:%M")
            )
    ...
    for recs in coamps_files(start, stop, cache_dir, **kw):
        for field_name in recs:
            ...
            try:
                ...
                utils.download_url(
                    rec["url"], output_fn, on_abort="remove", verify=False
                )
                files.append(output_fn)
                ...
                log.error(
                    "Failed to download %s, will move on (%s)" % (rec["url"], exc)
                )
...
def coamps_press_windxy_dataset(bounds, start, stop, cache_dir):
    return coamps_dataset(
        bounds, start, stop, cache_dir, fields=["wnd_utru", "wnd_vtru", "pres_msl"]
    )
...
def coamps_dataset(
    ...
    for recs in coamps_files(start, stop, cache_dir, fields=fields):
        ...
        cache_fn = os.path.join(
            os.path.dirname(recs[fields[0]]["local"]),
            "%s-%s.nc" % (timestamp_dt.strftime("%Y%m%d%H%M"), "-".join(fields)),
        )
