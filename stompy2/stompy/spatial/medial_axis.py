...
class Boundary(object):
    ...
    def write_poly(self, poly_filename):
        ...
        if np.all(point_list[0] == point_list[-1]):
            raise Exception(
                "Boundary should have already stripped any repeated endpoints"
            )
...
class VoronoiDiagram(Graph):
    ...
    def merge_points(self, tol):
        ...
        while len(self.nodes) > 1:
            ...
            edge_lengths = np.sqrt(
                ((edge_ends[:, 1, :] - edge_ends[:, 0, :]) ** 2).sum(axis=1)
            )
    ...
    def merge_edge(self, e):
        ...
        if self.radii is not None:
            ...
            self.dual_nodes[a] = np.unique(
                np.concatenate((self.dual_nodes[a], self.dual_nodes[b]))
            )
    ...
    def plot_vor_points(self):
        try:
            ...
            plt.scatter(
                self.nodes[:, 0], self.nodes[:, 1], 50, colors, lw=0, vmin=200, vmax=250
            )
        except:
