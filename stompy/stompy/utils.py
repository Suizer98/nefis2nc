...
def center_to_edge(c, dx_single=None, axis=0):
    ...
    if len(crot) > 1:
        ...
        raise Exception(
            "only a single data point given to center to edge with no dx_single"
        )
...
class BruteKDE(object):
    ...
    def __call__(self, x):
        ...
        for idx in np.ndindex(res.shape):
            res[idx] = np.sum(
                self.weights * np.exp(-(((self.values - x[idx]) / self.bw) ** 2))
            )
...
def within_2d(vecs, xxyy):
    ...
    return within(vecs[..., 0], xxyy[:2], fmt="mask") & within(
        vecs[..., 1], xxyy[2:], fmt="mask"
    )
...
def dice_interval(subinterval, overlap_fraction, start, end=None):
    ...
    if subinterval is None:
        ...
        # With overlap:
        Nsubs = 1 + int(
            (end - start - subinterval) / ((1 - overlap_fraction) * subinterval)
        )
    ...
    for idx in np.ndindex(Atrans.shape[1:]):
        ...
        if ends == "linear":
            if np.isnan(Aslice[0]) or np.isnan(Aslice[-1]):
                mb = np.polyfit(i[valid][[0, -1]], Aslice[valid][[0, -1]], 1)
...
def fill_tidal_data(da, fill_time=True):
    ...
    if fill_time:
        ...
        for gap_i in gaps:
            ...
            pieces.append(
                np.arange(da.time.values[gap_i], da.time.values[gap_i + 1], dt)
            )
            last = gap_i + 1
        ...
        da = xr.DataArray(
            dense_values, dims=["time"], coords=[dense_times], name=da.name
        )
    else:
        pass
...
def LinearNDExtrapolator(points, values, eps=None):
    ...
    for i in range(nseg):
        ...
        for sig in [1, -1]:
            ...
            query = [midpoint - sig * eps * ij_norm, midpoint - sig * 2 * eps * ij_norm]
    ...
    int_ij_extra = LinearNDInterpolator(
        np.concatenate((points, extra_xy)), np.concatenate((values, extra_val))
    )
...
def interp_near(x, sx, sy, max_dx=None):
    ...
    dx = np.minimum(np.abs(sx[right_idx] - x), np.abs(x - sx[left_idx]))
...
def nearest(A, x, max_dx=None):
    ...
    A = np.asarray(A)
    N = len(A)
    xi_right = np.searchsorted(A, x).clip(
        0, N - 1
    )  # the index of the value to the right of x
...
def haversine(a, b):
    ...
    """
    # to radians:
    a = np.asanyarray(a) * np.pi / 180.0
    b = np.asanyarray(b) * np.pi / 180.0
    ...
    A = (
        np.sin(dlat / 2.0) ** 2
        + np.cos(a[..., 1]) * np.cos(b[..., 1]) * np.sin(dlon / 2.0) ** 2
    )
...
def segment_segment_alphas(segA, segB):
    ...
    mat = np.array(
        [
            [segA[1, 0] - segA[0, 0], -segB[1, 0] + segB[0, 0]],
            [segA[1, 1] - segA[0, 1], -segB[1, 1] + segB[0, 1]],
        ],
        np.float64,
    )
    b = np.array([-segA[0, 0] + segB[0, 0], -segA[0, 1] + segB[0, 1]], np.float64)
    try:
...
def segment_segment_intersection(segA, segB):
    ...
    if np.isnan(alphas[0]):
        ...
        return None, alphas
    if (alphas[0] >= 0) and (alphas[1] >= 0) and (alphas[0] <= 1) and (alphas[1] <= 1):
        a = alphas[0]
...
def find_slack(jd, u, leave_mean=False, which="both"):
    ...
    dt = jd[1] - jd[0]
    # ~1 hour lowpass
    u = filters.lowpass_fir(u, winsize=1 + np.round(2.0 / (dt * 24)))
    if not leave_mean:
        u -= filters.lowpass_fir(u, winsize=1 + np.round(33.0 / (dt * 24)))
...
def quadmesh_interp_one(X, Z, V, x, z):
    ...
    mesh_x = X[0, :]
    col = np.interp(x, mesh_x, np.arange(len(mesh_x)), left=np.nan, right=np.nan)
    if np.isnan(col):
        return np.nan
...
def resample_to_common(
    A, Z, z=None, dz=None, n_samples=None, max_z=None, min_z=None, left=None, right=None
):
    ...
    for i in range(A.shape[0]):
        if A.shape[1] == 2:
            old_z = np.linspace(
                data2["range_m"][i, 0], data2["range_m"][i, 1], data2["echo"].shape[1]
            )
        else:
            old_z = Z[i, :]
        new_A[i, :] = np.interp(z, old_z, data2["echo"][i, :], left=left, right=right)
    return z, new_A


def principal_theta(
    vec, eta=None, positive="flood", detrend=False, ambiguous="warn", ignore_nan=True
):
    """
...
def rotate_to_principal(
    vec, eta=None, positive="flood", detrend=False, ambiguous="warn"
):
...
def model_skill(xmodel, xobs, ignore_nan=True):
    ...
    den = np.sum(
        (np.abs(xmodel[sel] - xobs[sel].mean()) + np.abs(xobs[sel] - xobs[sel].mean()))
        ** 2
    )
...
def find_lag_xr(data, ref):
    ...
    return find_lag(t_data, data.values, t_ref, ref.values)
...
def break_track(xy, waypoints, radius_min=400, radius_max=800, min_samples=10):
    ...
    for waypt in waypoints:
        ...
        in_max = np.concatenate(([False], dists < radius_max, [False]))
    ...
    sections = np.array([breaks[:-1], breaks[1:]]).T
...
def to_dnum(x):
    ...
    if pd is not None:
        if isinstance(x, pd.DataFrame):
            log.warning(
                "to_dnum should be given either a series or an index, not a dataframe"
            )
    ...
    if np.isscalar(x):
        ...
        if np.issubdtype(
            x.dtype, np.floating
        ):  # used to be np.float, but that is deprecated
            return x
...
def to_dt64(x):
    ...
    if not isinstance(x, np.ndarray):
        ...
        x = np.asarray(x)
        if isinstance(x.flat[0], datetime.datetime) or isinstance(
            x.flat[0], datetime.date
        ):
            x = np.array(x, "M8[ns]")
...
def cf_string_to_dt64(x):
    ...
    mult = dict(seconds=1, minutes=60, hours=3600, days=86400)[units]
...
def to_datetime(x):
    ...
    if np.isscalar(x):
        ...
        if np.issubdtype(x.dtype, np.datetime64):
            ...
            return [datetime.datetime.utcfromtimestamp(x) for x in ts]
...
def dt64_to_dnum(dt64):
    ...
    for reftype, ref_to_days in [("M8[us]", 86400000000), ("M8[D]", 1)]:
...
def invert_permutation(p):
    ...
    s = np.zeros(
        p.size, p.dtype
    )  # np.zeros is better than np.empty here, at least on Linux
...
def orient_intersection(lsA, lsB, delta=1.0):
    ...
    pnts = np.array([pntBplus.coords[0], pnt.coords[0], pntAplus.coords[0]])
...
def parse_xl_address(s):
    ...
    return (alpha_to_zerobased(m.group(1)), int(m.group(2)) - 1)


def parse_xl_range(s):
    # note that this does *NOT* change to be exclusive indexing
    return [parse_xl_address(part) for part in s.split(":")]
...
def array_append(A, b=sentinel):
    ...
    if (
        (A.base is None)
        or type(A.base) in (str, bytes)
        or A.base.size == A.size
        or A.base.strides != A.strides
...
def array_concatenate(AB):
    ...
    if (
        ...
        # make it twice as long as A, and in case the old shape was 0, add 10
        # in for good measure.
        new_shape[0] = max(new_shape[0] * 2 + 10, new_shape[0] + len(B))
...
def recarray_del_fields(A, old_fields):
    new_dtype = [fld for fld in A.dtype.descr if fld[0] not in old_fields]
...
def point_in_polygon(pgeom, randomize=False):
    ...
    if randomize:
        ...
        horz_line = geometry.LineString([[min_x - 10.0, c.y], [max_x + 10.0, c.y]])
    ...
    for row in range(Nstations):
        for col in range(row + 1):
            ...
            if n_valid <= 1:
                ...
                c[row, col] = (
                    1.0 / (valid.sum() - 1.0) * (v1_anom * v2_anom)[valid].sum()
                )
...
def download_url(
    url, local_file, log=None, on_abort="pass", on_exists="pass", **extra_args
):
    """
    ...
    def cb(chunk):
        ...
        if bucket[0] > thresh:
            if log is not None:
                log.info("%6.3f Mbytes" % (byte_sum[0] / 1.0e6))
            bucket[0] = 0
...
def is_stale(target, srcs, ignore_missing=False, tol_s=0):
    ...
    for src in srcs:
        ...
        src_mtime = max(os.stat(src).st_mtime, os.lstat(src).st_mtime)
...
def set_keywords(obj, kw):
    ...
    for k in kw:
        try:
            ...
            raise Exception(
                "Setting attribute %s failed because it doesn't exist on %s" % (k, obj)
            )
        setattr(obj, k, kw[k])
...
def dominant_period(h, t, uniform=True, guess=None):
    ...
    if guess is None:
        ...
        freqs, psd = welch(h, fs=1.0 / dt_s, nperseg=min(len(h), 1024))
...
def ideal_solar_rad(t, Imax=1000, lat=37.775, lon=-122.419):
    ...
    lst = t + np.timedelta64(int(86400 * lon / 360.0), "s")  # local solar time.
    ...
    hour_angle = (hour - 12) * np.pi / 12.0  # 0 at solar noon, +-180 at solar midnight
    zenith = np.arccos(
