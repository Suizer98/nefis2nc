...
class UgridTranscriber(object):
        ...
        for mesh in self.submeshes:
            ...
            if node_coordinates:
                if node_coordinates != mesh_var.node_coordinates:
                    raise Exception(
                        "Can only deal with a single global node_coordinates variable"
                    )
            else:
    ...
    def copy_nc_variable(self, name, new_name=None):
        ...
        var_new = self.nc_out.createVariable(
            new_name, var_orig.dtype, var_orig.dimensions, **self.var_kwargs
        )
    ...
    def is_topology_variable(self, name):
        return (name in self.topology_variables) or (
            ("Mesh_" + name) in self.topology_variables
        )
...
class MergeUgridSubgrids(UgridTranscriber):
    ...
    def copy_edges(self):
        self.select_edges_from_submesh = select_edges_from_submesh = [None] * len(
            self.submeshes
        )
        self.submesh_edge_ranges = submesh_edge_ranges = np.zeros(
            (len(self.submeshes), 2), np.int32
        )
        ...
        for meshi, mesh in enumerate(self.submeshes):
            ...
            fn_con.sort(axis=1)
            select_edges_from_submesh[meshi] = selected = np.zeros(
                len(fn_con), np.bool8
            )
        ...
        edge_nodes = self.nc_out.createVariable(
            varname,
            int32,
        ...
        for meshi, mesh in enumerate(self.submeshes):
            ...
            edge_nodes[
                submesh_edge_ranges[meshi, 0] : submesh_edge_ranges[meshi, 1], :
            ] = fn_con[select_edges_from_submesh[meshi], :]
    ...
    def copy_faces(self):
        ...
        self.submesh_face_ranges = submesh_face_ranges = np.zeros(
            (len(self.submeshes), 2), np.int32
        )
        ...
        self.nc_out.createDimension(
            self.prefix_topo + "nmax_face_nodes", fn_con.shape[1]
        )
        self.nc_out.createDimension(self.prefix_topo + "nface", new_face_idx)

        # And repeat, this time copying topology
        varname = self.prefix_topo + "face_nodes"
        face_nodes = self.nc_out.createVariable(
            varname,
            np.int32,
            [self.prefix_topo + "nface", self.prefix_topo + "nmax_face_nodes"],
            **self.var_options(varname)
        )
        ...
        for meshi, mesh in enumerate(self.submeshes):
            ...
            # more efficient to bitmap the input, but write to a slice
            face_nodes[
                submesh_face_ranges[meshi, 0] : submesh_face_ranges[meshi, 1], :
            ] = fn_con[select_from_submesh[meshi], :]
    ...
    def copy_nc_submesh_variable(self, name):
        ...
        var_new = self.nc_out.createVariable(
            varname, var_orig_sub.dtype, new_dims, **my_var_kwargs
        )
        ...
        for ncattr in var_orig_sub.ncattrs():
            if ncattr == "_FillValue":
                ...
                new_vars = [
                    self.prefix_topo + old_var.replace(submesh + "_", "")
        ...
        for submeshi, submesh in enumerate(self.submeshes):
            ...
            for dimi, dim in enumerate(new_dims):
                if dim == self.prefix_topo + "nface":
                    ...
                    slices.append(
                        slice(
                            self.submesh_face_ranges[submeshi, 0],
                    ...
                    slices.append(
                        slice(
                            self.submesh_edge_ranges[submeshi, 0],
...
class VerticalAverage(UgridTranscriber):
            ...
            if face_dim in v.dimensions:
                ...
                if vert_dim in v.dimensions:
                    if self.time_var in v.dimensions:
                        self.copy_averaged_face_variable(
                            varname, self.avg_prefix + varname
                        )
                    else:
                        print(
                            "WARNING: %s has vertical, but no time - don't know how to average that"
                            % varname
                        )
                else:
                    if not self.avg_only:
    ...
    def copy_averaged_face_variable(self, varname, new_name=None):
        ...
        var_new = self.nc_out.createVariable(
            new_name, var_orig.dtype, avg_dims, **self.var_options(new_name)
        )
        ...
        for step in range(len(time_var)):
            sys.stdout.write(".")
            sys.stdout.flush()
            weights = self.ug.vertical_averaging_weights(
                step, ztop=self.ztop, zbottom=self.zbottom, dz=self.dz, mesh_name=mesh
            )
