...
class StreamlineQuiver(object):
    ...
    def process_one_streamline(self):
        ...
        if self.pack:
            ...
            trace = stream_tracer.steady_streamline_oneway(
                self.g, self.U, xy, max_t=self.max_t, max_dist=self.max_dist
            )
        else:
            # max_t=20.0 was decent.
            trace = stream_tracer.steady_streamline_twoways(
                self.g, self.U, xy, max_t=self.max_t, max_dist=self.max_dist
            )
    ...
    def pack_starting_point(self, xy):
        ...
        """
        trace = stream_tracer.steady_streamline_oneway(
            self.g, -self.U, xy, max_t=100 * self.max_t, max_dist=100 * self.max_dist
        )
    ...
    def pick_starting_point(self):
        ...
        while 1:
            ...
            radii = utils.dist(
                centers - self.tri.nodes["x"][self.tri.cells["nodes"][:, 0]]
            )
            radii[self.tri.cells["outside"] | self.tri.cells["deleted"]] = 0.0
            if self.clip is not None:
                clipped = (
                    (centers[:, 0] < self.clip[0])
            ...
            if not self.boundary.intersects(geometry.Point(xy)):
                ...
                if not cpoly.intersects(geometry.Point(xy)):
                    print(
                        "Constrained center %s fell outside cell %d.  Lie and mark cell 'outside'"
                        % (str(xy), best)
                    )
    ...
    def segments_and_speeds(self, include_truncated=True):
        ...
        """
        strings = self.tri.extract_linear_strings(
            edge_select=self.tri.edges["constrained"]
        )
        ...
        for string in strings:
            ...
            if np.all(~node_tips):
                continue
            if not include_truncated and np.any(
                self.tri.nodes["stream_code"][string] == self.TRUNC
            ):
                continue
    ...
    sym = 2.0 * np.array([[1.5, 0], [-0.5, 1], [0, 0], [-0.5, -1]])
    diam = np.array([[0.5, 0], [0, 0.5], [-0.5, 0], [0, -0.5]])
    ...
    def quiverkey(self, X, Y, U, label, **kw):
        ...
        ax.text(segs[0][-1][0] + pad_x, segs[0][-1][1], label, va="center")
