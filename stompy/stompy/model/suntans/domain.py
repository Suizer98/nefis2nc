...
class Domain(domain_plotting.DomainPlotting):
    ...
    def __init__(self, datadir="rundata"):
        ...
        self.sedi_conf = (
            self.sedi_config()
        )  # old sediment code - not really supported at this point
    ...
    def salinity_file(self):
        ...
        if readSalinity == 1:
            ...
            return os.path.join(
                self.partitioned_grid_dir(), self.conf.conf_str("InitSalinityFile")
            )
        elif readSalinity == 2:
            return os.path.join(
                self.partitioned_grid_dir(),
                self.conf.conf_str("InitSalinityFile") + ".0",
            )
        else:
            return None
    ...
    def temperature_file(self):
        ...
        if readTemperature == 1:
            # just reads vertical profile
            return os.path.join(
                self.partitioned_grid_dir(), self.conf.conf_str("InitTemperatureFile")
            )
        elif readTemperature == 2:
            return os.path.join(
                self.partitioned_grid_dir(),
                self.conf.conf_str("InitTemperatureFile") + ".0",
            )
        else:
            return None
    ...
    def global_markers_create(self, target, deps):
        ...
        try:
            ...
            self.log.error(
                "Exception while marking grid - removing potentially unmarked grid files"
            )
    ...
    def set_transforms(self, force=False):
        if force or (self.geo_to_local is None or self.local_to_geo is None):
            ...
            local.SetFromUserInput(self.spatial_ref)
            geo = osr.SpatialReference()
            geo.SetFromUserInput(self.lonlat_ref)
    ...
    def add_flows_from_shp(self, gforce, flows_shp, bc_type="bc_type"):
        ...
        for feat_id in range(len(flows)):
            ...
            fields = {fld: flows[fld][feat_id] for fld in flows.dtype.names}

            self.log.debug(
                "Processing flow data from shapefile: %s" % fields.get("name", "n/a")
            )
    ...
    def partitioned_create(self, target, deps):
        ...
        if os.path.abspath(self.global_bc_grid_dir()) != os.path.abspath(
            self.partitioned_grid_dir()
        ):
            for f in [
                "cells.dat",
                ...
                shutil.copy(
                    os.path.join(self.global_bc_grid_dir(), f),
            ...
            if self.sedi_conf:
                self.sedi_conf.write_config(
                    os.path.join(self.global_bc_grid_dir(), self.sedi_dat)
                )
            if self.sedi_ls_conf:
                self.sedi_ls_conf.write_config(
                    os.path.join(self.global_bc_grid_dir(), self.sedi_ls_dat)
                )
            ...
            if self.sedi_conf:
                self.sedi_conf.write_config(
                    os.path.join(self.partitioned_grid_dir(), self.sedi_dat)
                )
            if self.sedi_ls_conf:
                self.sedi_ls_conf.write_config(
                    os.path.join(self.partitioned_grid_dir(), self.sedi_ls_dat)
                )
        ...
        if rstretch == 0.0:
            if not os.path.exists(
                ...
                self.log.debug(
                    "Copying vertspace.dat.in from %s to %s"
                    % (self.vertspace_dat_in_file(), self.partitioned_grid_dir())
                )
                shutil.copy(
                    self.vertspace_dat_in_file(),
                    os.path.join(self.partitioned_grid_dir(), "vertspace.dat.in"),
                )
        else:
            self.log.debug(
                "No need for vertspace.dat.in because rstretch=%s" % rstretch
            )
    ...
    def run_create(self):
        ...
        rule(
            self.suntans_dat_template_file(),
            [],
            self.create_suntans_dat_template,
            always_run=True,
        )
        ...
        rule(
            self.global_bc_grid_dir(),
            [],
            lambda target, deps: os.mkdir(self.global_bc_grid_dir()),
        )
        rule(
            suntans_dat,
            [self.suntans_dat_template_file(), self.global_bc_grid_dir()],
        ...
        rule(
            global_markers_file,
            [original_grid_file, suntans_dat],
        ...
        if self.enable_edge_based_bathymetry:
            ...
            rule(
                self.edgedepth_untrimmed_file(),
                [original_grid_file] + self.extra_depth_depends(),
            ...
            if self.thalweg_max_cells > 0:
                ...
                rule(
                    self.edgedepth_thalwegged_file(),
        ...
        rule(
            self.vertspace_dat_in_file(),
            [self.suntans_dat_template_file()],
            self.create_vertspace_dat_in,
        )

        rule(partitioned_file, partition_deps, self.partitioned_create)

        boundaries_dat_file = os.path.join(
            self.partitioned_grid_dir(), "boundaries.dat.0"
        )
        ...
        if self.enable_edge_based_bathymetry:
            ...
            rule(
                partitioned_edgedepths_file,
                [self.edgedepth_file(), partitioned_file],
        ...
        if 0:  # do this on a per-run basis - see cmd_replace_initcond()
            if self.salinity_file():
                rule(
                    self.salinity_file(),
                    [partitioned_file, suntans_dat] + edge_deps,
        ...
        if self.section_endpoints is not None:
            rule(
                self.sections_input_file(), [global_markers_file], self.sections_create
            )
            rule(
                self.partitioned_sections_input_file(),
                [self.sections_input_file()],
                self.file_copy,
            )
    ...
    def partitioned_edgedepths_create(self, target, deps):
        ...
        if self.enable_edge_based_bathymetry:
            ...
            link_to_global_edgedepths = os.path.join(
                self.partitioned_grid_dir(), os.path.basename(edgedepth_file)
            )
            ...
            ew = edge_depths.EdgeDepthWriter(
                delete_friction_strips=self.edgedepths_delete_friction_strips
            )
            ew.run(sun=sun)
            ...
            for proc in range(sun.num_processors()):
                ...
                for j in range(localg.Nedges()):
                    ...
                    if nc1 < 0:
                        ...
                        nc2 = nc1
                    de = min(abs(global_depth[nc1, 2]), abs(global_depth[nc2, 2]))
    ...
    def depth_untrimmed_create(self, target, deps):
        ...
        if bad_count:
            raise Exception(
                "%d of %d cell-center depths were nan - see %s"
                % (bad_count, len(depths), target)
            )
    ...
    def edgedepth_untrimmed_create(self, target, deps):
        ...
        edge_field = field.XYZField(X=g.edge_centers(), F=all_edge_depths)
        edge_field.write_text(target)

        bad_count = np.sum(np.isnan(all_edge_depths))
        if bad_count:
            raise Exception(
                "%d of %d edge depths were nan - see %s"
                % (bad_count, len(all_edge_depths), target)
            )
    ...
    def edgedepth_thalwegged_create(self, thalwegged, deps):
        ...
        for j in to_thalweg:
            ...
            g.carve_thalweg(
                xyz[:, 2], threshold, j, mode="edges", max_count=self.thalweg_max_cells
            )
    ...
    def depth_thalwegged_create(self, thalwegged, deps):
        ...
        for j in to_thalweg:
            self.log.debug("  Carving from edge %d" % j)
            g.carve_thalweg(
                xyz[:, 2], threshold, j, mode="cells", max_count=self.thalweg_max_cells
            )
    ...
    def sections_input_file(self):
        return os.path.join(
            self.global_bc_grid_dir(), self.conf.conf_str("SectionsInputFile")
        )

    def partitioned_sections_input_file(self):
        return os.path.join(
            self.partitioned_grid_dir(), self.conf.conf_str("SectionsInputFile")
        )
    ...
    def sections_create(self, sections_input_file, deps):
        ...
        for section_path in self.section_endpoints:
            ...
            uniq = concatenate(([True], diff(this_path) != 0))
    ...
    def get_instrument(self, name_or_inst, sun, chain_restarts=1):
        ...
        for sun in suns:  # sun.chain_restarts():
            ...
            self.log.info(
                "Processing sun run JD %g-%g"
                % (prof_absdays[0] - jd0, prof_absdays[-1] - jd0)
            )
            ...
            adcpU = U[
                :n_steps, 0, :
            ].copy()  # copy so nans don't pollute summations below
            ...
            for i in range(n_steps):
                ...
                records_wc["z_bot"][i, :] = (
                    z_tops_i[1 : nk + 1] + sun.bathymetry_offset()
                )
            ...
            if len(all_records) > 0:
                ...
                first_new_output = searchsorted(
                    records["jd"], last_output_jd, side="right"
                )
                if first_new_output < len(records):
                    self.log.debug(
                        "Trimming a bit off the beginning of profile data from a restart"
                    )
                    all_records.append(records[first_new_output:])
                    all_records_wc.append(records_wc[first_new_output:])
                else:
                    self.log.info(
                        "A restart didn't have any new profile data, so we skipped it"
                    )
            else:
        ...
        virt_ctd = instrument.Instrument(
            records=records,
            comment=label,
            records_wc=records_wc,
            xy=pnt,
        ...
        all_adcpU = array(
            transpose(concatenate([adcpU for adcpU, adcpV, adcpt in all_adcps])), copy=1
        )
        all_adcpV = array(
            transpose(concatenate([adcpV for adcpU, adcpV, adcpt in all_adcps])), copy=1
        )
        all_adcpt = concatenate([adcpt for adcpU, adcpV, adcpt in all_adcps])

        virt_ctd.adcp = adcp.AdcpMoored.from_values(
            all_adcpU, all_adcpV, z_mids, all_adcpt, label
        )
    ...
    def copy_to_datadir(self, dest_dir, src_dir=None, symlink=True):
        ...
        read_salt = self.conf.conf_int("readSalinity")
        read_temp = self.conf.conf_int("readTemperature")
        ...
        for f in per_processor:
            ...
            for p in range(self.np):
                ...
                if os.path.exists(src_file):
                    maybe_link(src_file, os.path.join(dest_dir, datname + ".%i" % p))
        ...
        for f in global_files:
            ...
            if os.path.exists(src_file):
                maybe_link(src_file, os.path.join(dest_dir, datname))

        # Section analysis
        section_input_file = self.conf.conf_str("SectionsInputFile")

        if section_input_file is not None and os.path.exists(
            os.path.join(src_dir, section_input_file)
        ):
        ...
        for dat_file in [self.sedi_dat, self.sedi_ls_dat, self.wave_dat]:
            if os.path.exists(os.path.join(src_dir, dat_file)):
                ...
                maybe_link(
                    os.path.join(src_dir, dat_file), os.path.join(dest_dir, dat_file)
                )
    ...
    def setup_startfiles(self, src_dir, dest_dir, symlink=True):
        ...
        for p in range(self.np):
            copier(
                os.path.join(src_dir, store + ".%i" % p),
    ...
    def main(self, args=None):
        ...
        def usage():
            ...
            print(
                "    -m     parallelize using mpi - indicates that the script is being invoked by mpirun or friends "
            )
            print(
                "    -M     parallelize using mpi, and take care of queuing the process with the given number of processes from -n"
            )
            print(" Command is one of:")
            print("  setup [<datadir>] - prepare the given directory for a run")
            print(
                "       If no directory is given, the files are left in rundata/npNNN"
            )
            print(
                "  continue <old> <new> [+<duration>] - setup for continuing an ended or failed run in a new directory"
            )
            print(
                "  match_spinup <spin_dir> <new_dir> [date] - create a short run that ends at the same time"
            )
            print(
                "       as the spin_dir, or coincides with a full output step near/before the given date"
            )
            print(
                "  continue_with_spin <old> <spin> <new> [+<duration>] - like continue, but some scalars (currently only salt)"
            )
            print(
                "  write_instruments <datadir> - in a 'instruments' subdirectory, write virtual instrument data"
            )
            ...
            print(
                "  plot_forcing <datadir>- dump plots of the various forcing datasources"
            )
    ...
    def run_crashed(self, datadir):
        ...
        return (
            os.path.exists(datadir + "/crash.dat.0")
            and os.stat(crash).st_mtime > os.stat(store).st_mtime
        )
    ...
    def cmd_write_instruments(self, datadir):
        ...
        for inst in self.virtual_instruments():
            ...
            fname = os.path.join(dest_dir, v_inst.safe_name() + ".inst")
            v_inst.save(fname)
                ...
                if r["geom"].geom_type == "Point":
                    ...
                    try:
                        ...
                        coords = linestring_utils.upsample_linearring(
                            coords, resolution, closed_ring=0
                        )
                    except AttributeError:
                        pass
    ...
    def cmd_write_matlab(self, datadir, chain_restarts=1, insts=None):
        ...
        if chain_restarts:
            ...
            for inst in insts:
                mat_fn = os.path.join(dest_dir, instrument.sanitize(inst.name)) + ".mat"
                ...
                fname = os.path.join(dest_dir, v_inst.safe_name() + ".mat")
    ...
    def cmd_initialize_run(self, datadir, interval):
        ...
        start_date, end_date = parse_interval.parse_interval(
            interval, default_start=def_start, default_end=def_end
        )
        if 0:  #
            ...
            if not self.conf.is_grid_compatible(sun.conf):
                raise Exception(
                    "Looks like the differences between these suntans.dat files would make incompatible grids"
                )
    ...
    def cmd_run(self, datadir, wait=1, max_retries="0"):
        ...
        while 1:
            ...
            if not self.run_can_be_continued(datadir):
                print(
                    "The run didn't finish, but didn't get far enough along to consider continuing"
                )
                break
    ...
    def cmd_continue(
        self, old_dir, new_dir, interval=None, symlink=True, base_config=None
    ):
        """
        ...
        if start_date != def_start:
            print(interval)
            raise Exception(
                "For restarts you can only specify :end_date, :+duration or +duration"
            )
    ...
    def cmd_replace_forcing(self, data_dir):
        ...
        """
        for f in glob.glob(
            os.path.join(data_dir, self.conf.conf_str("BoundaryInput") + "*")
        ):
            os.unlink(f)
    ...
    def cmd_match_spinup(self, spin_dir, new_dir, end_date=None):
        ...
        spin_outputs = date2num(spin_sun.time_zero()) + spin_sun.timeline(
            units="days", output="grid"
        )
        ...
        print(
            "This run will end on %s to match a spinup output time"
        ...
        print(
            "Hydro spinup time is %g days. This run will be %g days"
            % (self.hydro_spinup_days, end_absdays - start_absdays)
        )
    ...
    def cmd_copy_spin_scalars(self, spin_dir, new_dir, unsymlink=True):
        ...
        if os.path.exists(start_file_fn):
            ...
            print(
                "Copy_spin_scalars: no start file - will overwrite salinity initial conditions"
            )
            read_salinity = sun.conf_int("readSalinity")
            if read_salinity != 2:
                print(
                    "Modifying suntans.dat: readSalinity  %d => %d" % (read_salinity, 2)
                )
    ...
    def cmd_make_shps(self, depth_fn=None):
        ...
        if self.private_rundata:
            raise Exception(
                "Sorry - make_shps is not currently compatible with private_rundata"
            )
    ...
    def cmd_make_subdomain_shps(self, datadir):
        ...
        for proc in range(Nprocs):
            ...
            g = sun.grid(proc)
            g.write_shp(
                os.path.join(datadir, "grid-boundaries%03i.shp" % proc), overwrite=1
            )
    ...
    def process_scalar_release(
        self, sun, mapper, unsymlink=True, species="temperature"
    ):
        ...
        if not os.path.exists(start_file_fn):
            raise Exception(
                "scalar_release:: failed to find StartFile %s." % start_file_fn
            )
    ...
    def process_ssc_release(self, sun, mapper, unsymlink, species):
        """see process_scalar_release"""
        print(
            "Will write over suspended sediment concentration for species %d" % species
        )
    ...
    def cmd_scalar_release_polygon(
        self, datadir, n=0, unsymlink=True, species="temperature"
    ):
        ...
        return self.process_scalar_release(
            sun, mapper=mapper, unsymlink=unsymlink, species=species
        )

    def cmd_scalar_release_gaussian_2D(
        self, datadir, xy, sd, unsymlink=True, species="temperature"
    ):
        ...
        def mapper(proc, cell, k):
            ...
            pnt = g.vcenters()[cell]
            return peak * exp(
                -((pnt[0] - xy[0]) ** 2) / (2 * sd[0] ** 2)
                - (pnt[1] - xy[1]) ** 2 / (2 * sd[1] ** 2)
            )
