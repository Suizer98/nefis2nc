...
def station_output(run_dir, param=None):
    ...
    start_dt = datetime.datetime(
        year=int(param["OPT", "start_year"]),
        month=int(param["OPT", "start_month"]),
        day=int(param["OPT", "start_day"]),
    )

    t_start = np.datetime64(start_dt) + float(
        param["OPT", "start_hour"]
    ) * 3600 * np.timedelta64(1, "s")
...
class SchismModel(hm.HydroModel, hm.MpiModel):
    ...
    def load(cls, run_dir):
        ...
        model.grid = unstructured_grid.UnstructuredGrid.read_gr3(
            os.path.join(model.run_dir, "hgrid.gr3")
        )
        return model
    ...
    def load_defaults(self):
        self.param = Namelist(
            os.path.join(os.path.dirname(__file__), "data", "param.nml")
        )
        self.sediment = Namelist(
            os.path.join(os.path.dirname(__file__), "data", "sediment.nml")
        )
    ...
    def update_config(self):
        ...
        self.param["CORE", "rnday"] = (self.run_stop - self.run_start) / np.timedelta64(
            86400, "s"
        )
        ...
        self.param["OPT", "start_hour"] = (
            dt_start.hour + dt_start.minute / 60.0 + dt_start.second / 3600.0
        )
            ...
            for bc in self.bcs:
                if isinstance(bc, hm.StageBC) or isinstance(bc, hm.FlowBC):
                    ...
                    if bc.dredge_depth is not None:
                        ...
                        self.dredge_boundary(
                            bc.geom,
                            dredge_depth=bc.dredge_depth,
            ...
            for bc_id, bc, bc_edges in open_bcs:
                ...
                for child_bc in self.bcs:
                    if isinstance(child_bc, hm.ScalarBC) and child_bc.parent == bc:
                        ...
                        assert (
                            child_bc.scalar in scalar_names
                        ), "Expecting scalar (%s), got %s" % (
                            ", ".join(scalar_names),
                            child_bc.scalar,
                        )
                ...
                fp.write(
                    " ".join(
                        ["%d" % n for n in [1 + len(bc_edges)] + flags]
                    )  # number of nodes
                    + "\n"
                )
    ...
    def write_th(self, fn, data):
        ...
        data_dt = min(
            [
                np.diff(datum.time.values).min() / np.timedelta64(1, "s")
                for datum in data
            ]
        )
        ...
        for di, datum in enumerate(data):
            ...
            result[:, 1 + di] = np.interp(t_s, datum_t_s, datum.values)
    ...
    def write_grid(self):
        ...
        self.grid.write_gr3(
            os.path.join(self.run_dir, "hgrid.gr3"),
            bc_marks=True,
            z=-self.grid.nodes["node_z_bed"],
        )
        self.grid.write_gr3(
            os.path.join(self.run_dir, "drag.gr3"),
            z=self.grid.nodes["drag"],
            bc_marks=False,
        )
        self.grid.write_gr3(
            os.path.join(self.run_dir, "elev.ic"),
            z=np.maximum(self.grid.nodes["node_z_bed"], self.initial_water_level()),
            bc_marks=False,
        )
    ...
    def mark_open_edges(self, linestring):
        ...
        g.edges["mark"][
            j_open
        ] = g.OPEN  # no difference here in flow, velocity, or stage BC.
    ...
    def mark_land_edges(self):
        ...
        for si, string in enumerate(strings):
            ...
            if (
                len(land_strings) > 1
                and land_strings[0][0][0] == land_strings[-1][-1][2]
            ):
            ...
            if (len(land_strings) == 1) and (
                land_strings[0][0][0] == land_strings[0][-1][-1]
            ):
                ls = land_strings[0]
                L = len(ls)
    ...
    def write_monitor_points(self):
        ...
        with open(os.path.join(self.run_dir, "station.in"), "wt") as fp:
            fp.write(
                "1 0 0 0 0 0 1 1 1 !on (1)|off(0) flags for elev, air pressure, windx, windy, T, S, u, v, w\n"
            )
    ...
    def read_vgrid_in(self):
        ...
        with open(vgrid_fn, "rt") as fp:
            ...
            if ivcor == 1:
                ...
                lsc = np.zeros(
                    self.grid.Nnodes(),
                    dtype=[("k0", np.int32), ("sigma", np.float64, nvrt)],
                )
                lsc["sigma"] = np.nan
...
class MultiSchism(multi_ugrid.MultiUgrid):
    def __init__(self, grid_fn, paths, xr_kwargs={}):
        ...
        self.rev_meta = {
            "nSCHISM_hgrid_node": "node_dimension",
