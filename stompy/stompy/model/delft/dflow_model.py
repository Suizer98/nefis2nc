...
class DFlowModel(hm.HydroModel, hm.MpiModel):
    ...
    def write(self):
        if not bool(self.restart):
            ...
            if self.restart_deep:
                ...
                self.log.warning(
                    "Shallow restart logic in DFlowModel.write() is sketchy"
                )
    ...
    def copy_files_for_restart(self):
        ...
        with open(
            self.restart_from.mdu.filepath(("external forcing", "ExtForceFile"))
        ) as fp:
            flowfm_ext = fp.read()
        ...
        for fn in os.listdir(self.restart_from.run_dir):
            ...
            do_copy = (
                (suffix in [".tim", ".pli", ".pliz", ".ext", ".xyz", ".ini", ".xyn"])
                or (fn in flowfm_ext)
                or (fn in flowfm_mdu)
    ...
    def write_forcing(self, overwrite=True):
        ...
        assert (
            bc_fn
        ), "DFM script requires old-style BC file.  Set [external forcing] ExtForceFile"
    ...
    def set_grid(self, grid):
        ...
        if "node_z_bed" not in node_fields:
            if "z_bed" in node_fields:
                ...
                self.log.info(
                    "Duplicating z_bed to node_z_bed for less ambiguous naming"
                )
            elif "depth" in node_fields:
                self.grid.add_node_field("node_z_bed", self.grid.nodes["depth"])
                self.log.info(
                    "Duplicating depth to node_z_bed for less ambiguous naming, and assuming it was already positive-up"
                )
    ...
    def dredge_boundary(self, linestring, dredge_depth):
        super(DFlowModel, self).dredge_boundary(
            linestring,
            dredge_depth,
    ...
    def dredge_discharge(self, point, dredge_depth):
        super(DFlowModel, self).dredge_discharge(
            point,
            dredge_depth,
            node_field="node_z_bed",
            edge_field=None,
    ...
    def load_from_mdu(self, fn):
        ...
        try:
            self.grid = ugrid.UnstructuredGrid.read_dfm(
                self.mdu.filepath(("geometry", "NetFile"))
            )
    ...
    def load_gazetteer_from_run(self):
        ...
        if fn and os.path.exists(fn):
            stations = pd.read_csv(
                self.mdu.filepath(["output", "ObsFile"]),
                sep=r"\s+",
                names=["x", "y", "name"],
                quotechar="'",
            )
    ...
    def load_bcs(self, load_data=True, ext_fn=None, keep_anonymous=True):
        ...
        for rec in recs:
            ...
            if ext == ".pli":
                pli_fn = os.path.join(os.path.dirname(ext_fn), rec["FILENAME"])
                pli = dio.read_pli(pli_fn)
                rec["pli"] = pli
                ...
                if load_data:
                    ...
                    if (
                        ...
                        for node_i, node_xy in enumerate(rec["coordinates"]):
                            ...
                            if os.path.exists(tim_fn):
                                t_ref, t_start, t_stop = self.mdu.time_range()
                                tim_ds = dio.read_dfm_tim(
                                    tim_fn, t_ref, columns=["stage"]
                                )
                                tim_ds["x"] = (), node_xy[0]
                                tim_ds["y"] = (), node_xy[1]
                ...
                if load_data:
                    xyz_fn = os.path.join(os.path.dirname(ext_fn), rec["FILENAME"])
            ...
            if rec["QUANTITY"].upper() == "WATERLEVELBND":
                bc = hm.StageBC(
                    name=rec["name"], geom=rec["geom"], water_level=rec["data"]
                )
                rec["bc"] = bc
                ...
                if rec.get("data", None) is not None:
                    ...
                    if load_data:
                        print(
                            "Reading discharge boundary, did not find data (%s)"
                            % tim_fn
                        )
                ...
                if warn_tracer_no_parent:
                    self.log.warning(
                        "Parsing external forcing: tracers not yet connected to parents"
                    )
                    warn_tracer_no_parent = False
                tracer = rec["QUANTITY"].replace("bnd", "").lower()
                rec["bc"] = hm.ScalarBC(
                    name=rec["name"], geom=rec["geom"], scalar=tracer, data=rec["data"]
                )
                ...
                rec["bc"] = hm.SourceSinkBC(
                    name=rec["name"], geom=rec["geom"], flow=flow
                )
                ...
                def add_scalar_bc(tracer, da):
                    bc = hm.ScalarBC(parent=rec["bc"], scalar=tracer, value=da)
                    extra_recs.append(
                        dict(bc=bc, name=rec["name"], geom=rec["geom"], child=True)
                    )
                ...
                if keep_anonymous:
                    while True:
                        ...
                        if warn_anonymous:
                            self.log.warning(
                                "Tracers for source/sink BC beyond temp and salinity will be anonymous"
                            )
                ...
                if warn_mass_balance_not_parsed:
                    self.log.warning(
                        "Parsing external forcing. WAQ mass balance areas are not parsed"
                    )
        ...
        for k in unhandled:
            print(
                "Encountered %d BCs with quantity=%s that weren't fully parsed"
                % (unhandled[k], k)
            )
    ...
    def partition(self, partition_grid=None):
        ...
        if partition_grid:
            ...
            cmd = [
                "--partition:ndomains=%d:icgsolver=6" % self.num_procs,
            ...
            if self.restart_deep:
                for proc in range(self.num_procs):
                    ...
                    self.log.info(
                        "Copying pre-partitioned grid files: %s => %s"
                        % (old_grid_fn, new_grid_fn)
                    )
    ...
    def chain_restarts(self):
        ...
        while 1:
            ...
            restart_mdu = os.path.normpath(
                os.path.join(os.path.dirname(mdu.filename), restart_mdu)
            )
            if not os.path.exists(restart_mdu):
                self.log.warning(
                    "Expected preceding restart at %s, but not there" % restart_mdu
                )
                break
    ...
    def dia_fn(self):
        ...
        dia_fn_bases = [dia_fn_base + "_0000.dia", dia_fn_base + ".dia"]
        ...
        for dia_fn_base in dia_fn_bases:
            for dia_path in dia_paths:
                ...
                assert dia_fn != self.mdu.filename, (
                    "Probably case issues with %s" % dia_fn
                )
        ...
        with open(dia_fn, "rb") as fp:
            ...
            def parse_time(days, hours):
                ...
                seconds = sum(
                    [
                        mult * int(val)
                        for mult, val in zip([3600, 60, 1], hours.split(":"))
                    ]
                )
    ...
    def update_config(self):
        ...
        self.mdu.set_time_range(
            start=self.run_start, stop=self.run_stop, ref_date=self.ref_date
        )
        ...
        if self.fixed_weirs is not None:
            ...
            dio.write_pli(
                self.mdu.filepath(("geometry", "FixedWeirFile")), self.fixed_weirs
            )
    ...
    def read_tim(self, fn, time_unit=None, columns=["val1", "val2", "val3"]):
        ...
        return dio.read_dfm_tim(
            fn, time_unit=time_unit, ref_time=ref_time, columns=columns
        )
    ...
    def write_tim(self, da, file_path, trim_time=None):
        ...
        if "time" not in da.dims:
            ...
            if trim_time:
                ...
                if times[-1] < start:
                    log.warning(
                        f"{file_path}: data ends ({times.max()}) before simulation period: {start} - {stop}."
                    )
                    times = np.array([start, stop])
                    values = np.r_[values[-1], values[-1]]
                elif times[0] > stop:
                    log.warning(
                        f"{file_path}: data starts ({times.min()}) after simulation period: {start} - {stop}."
                    )
                    times = np.array([start, stop])
    ...
    def write_source_bc(self, bc, **write_opts):
        ...
        scalar_names, scalar_das = self.default_source_sink_forcing(
        ...
        for scalar_bc in self.bcs:
            if isinstance(scalar_bc, hm.ScalarBC) and scalar_bc.parent == bc:
                ...
                try:
                    ...
                    raise Exception(
                        "Scalar %s not in known list %s" % (scalar, scalar_names)
                    )
    ...
    def write_gen_bc(
        self, bc, quantity, da=None, write_ext=True, write_data=True, write_geom=True
    ):
        """
        ...
        if write_geom:
            ...
            if (bc.geom is None) and (None not in bc.geom_type):
                raise Exception(
                    "BC %s, name=%s has no geometry. Maybe missing from shapefiles?"
                    % (bc, bc.name)
                )
            ...
            if quantity == "source":
                if ndim == 2 and bc.z is not None:
                    ...
                    for z_val, idx in [(bc.z, -1), (bc.z_src, 0)]:
                        if z_val is None:
                            continue
                        if z_val == "bed":
                            z_val = -10000
    ...
    def write_scalar_bc(self, bc):
        ...
        assert isinstance(
            parent_bc, (hm.StageBC, hm.FlowBC)
        ), "Haven't implemented point-source scalar yet"
    ...
    def write_roughness_bc(self, bc):
        ...
        # write_data()
        da = bc.data()
        xyz = np.c_[da.x.values, da.y.values, da.values]
    ...
    def map_dataset(
        self, force_multi=False, grid=None, chain=False, xr_kwargs={}, clone_from=None
    ):
        """
        ...
        if not chain:
            if self.num_procs <= 1 and not force_multi:
                ...
                if self._mu is None:
                    self._mu = multi_ugrid.MultiUgrid(
                        self.map_outputs(),
                        grid=grid,
                        xr_kwargs=xr_kwargs,
            ...
            if self._mu is None:
                ...
                for proc in range(self.num_procs):
                    one_proc_dss = [
                        xr.open_dataset(fn, **xr_kwargs) for fn in map_fns[:, proc]
                    ]
                    proc_dasks = []
                ...
                if len(all_proc_dss) == 1 and not force_multi:
                    ...
                    self._mu = multi_ugrid.MultiUgrid(
                        all_proc_dss, grid=grid, xr_kwargs=xr_kwargs
                    )
            return self._mu
    ...
    @classmethod
    def clean_his_dataset(
        cls, fn, decode_geometry=True, set_coordinates=True, refresh=False, **xr_kwargs
    ):
        """
        ...
        if refresh:
            ...
            his_ds = xr.open_dataset(
                fn, **xr_kwargs
            )  # is there a way to know if it's cached?
            his_ds.close()
        ...
        if set_coordinates:
            ...
            for coord, names in [
                ...
                if len(coord_vals) > len(np.unique(coord_vals)):
                    ...
                    mask = [
                        val not in coord_vals[:i] for i, val in enumerate(coord_vals)
                    ]
    ...
    def his_dataset(
        ...
        **xr_kwargs,
        ...
        if chain:
            ...
            his_dss = [self.clean_his_dataset(fn, **clean_kwargs) for fn in his_fns]
            his_dasks = []
            for ds in his_dss[::-1]:
                if len(his_dasks) > 0:
    ...
    def hyd_output(self):
        ...
        return os.path.join(
            self.run_dir, "DFM_DELWAQ_%s" % self.mdu.name, "%s.hyd" % self.mdu.name
        )
    ...
    def create_restart(self, deep=True, mdu_suffix="", **kwargs):
        ...
        new_model = self.__class__(
            configure=False, **kwargs
        )  # in case of subclassing, rather than DFlowModel()
    ...
    def set_restart_from(self, model, deep=True, mdu_suffix=""):
        ...
        self.mdu_basename = os.path.basename(
            model.mdu_basename.replace(".mdu", mdu_suffix + ".mdu")
        )
        self.mdu.set_filename(os.path.join(self.run_dir, self.mdu_basename))
        ...
        self.restart = True
        self.restart_from = (
            model  # used to be restart_model, but I don't think makes sense
        )
        self.ref_date = model.ref_date
        self.run_start = model.restartable_time()
        assert (
            self.run_start is not None
        ), "Trying to restart run that has no restart data"
    ...
    def set_restart_file(self):
        ...
        """
        self.log.info(
            "set_restart_file: Setting RestartFile based on self.restart_from"
        )
        rst_base = os.path.join(
            self.restart_from.mdu.output_dir(),
            (
                self.restart_from.mdu.name
                + "_"
                + utils.to_datetime(self.run_start).strftime("%Y%m%d_%H%M%S")
                + "_rst.nc"
            ),
        )
        ...
        self.mdu["restart", "RestartFile"] = os.path.relpath(
            rst_base, start=self.run_dir
        )
    ...
    def restart_inputs(self):
        ...
        if self.num_procs > 1:
            rsts = [
                (rst_base[:-23] + "_%04d" % p + rst_base[-23:])
    ...
    def modify_restart_data(self, modify_ic):
        ...
        for proc, rst in enumerate(self.restart_inputs()):
            ...
            new_rst = os.path.join(
                os.path.basename(rst)
            )  # 'run_dye_test-p06b/DFM_OUTPUT_flowfm/flowfm_0000_20161210_060000_rst.nc'
    ...
    def extract_section(
        self, name=None, chain_count=1, refresh=False, xy=None, ll=None, data_vars=None
    ):
        """
    ...
    def translate_vars(self, ds, requested_vars=None):
        ...
        """
        lookup = {"flow": "cross_section_discharge", "water_level": "waterlevel"}
    ...
    def extract_station(
        self, xy=None, ll=None, name=None, refresh=False, data_vars=None
    ):
        his = xr.open_dataset(self.his_output())
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Command line manipulation of DFM runs"
    )

    parser.add_argument("--restart", action="store_true", help="restart a run")
    parser.add_argument(
        "--mdu", metavar="file.mdu", default=None, help="existing MDU file"
    )
    ...
    if args.restart:
        ...
        restart.mdu.write()
        print(
            "Shallow restart: %s to %s, mdu=%s"
            % (restart.run_start, restart.run_stop, restart.mdu.filename)
        )
...
def extract_transect_his(his_ds, pattern):
    ...
    for i, name in enumerate(his_ds.station_name.values):
        if name in names:
            continue  # on the off chance that names are repeated.
    ...
    dsxr = ds.rename(
        stations="sample",
        station_x_coordinate="x_sample",
        station_y_coordinate="y_sample",
    )
    z_renames = dict(
        laydim="layer",
        laydimw="interface",
    ...
    # add distance?
    xy = np.c_[dsxr.x_sample.values, dsxr.y_sample.values]
...
def dem_to_cell_bathy(dem, g, fill_iters=20):
    ...
    for c in utils.progress(range(g.Ncells()), msg="dem_to_cell_bathy: %s"):
        ...
        cell_means[c] = np.nanmean(
            dem.polygon_mask(g.cell_polygon(c), return_values=True)
        )
...
def dem_to_cell_node_bathy(dem, g, cell_z=None):
    ...
    print(
        "Adjustments to node elevations are %.2f to %.2f"
        % (node_delta.min(), node_delta.max())
    )
...
def rst_mappers(rst, g, signed=True):
    ...
    cc = g.cells_center()
    elem_xy = np.c_[rst.FlowElem_xzw.values, rst.FlowElem_yzw.values]
