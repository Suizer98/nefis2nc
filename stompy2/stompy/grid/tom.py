...
class Tom(object):
    ...
    smooth = 1
    simplify_tolerance = (
        0.0  # length scale for geometry simplification before smoothing
    )
    ...
    def usage(self):
        ...
        print(
            "         --slide-interior     # Allow nodes on interior lines to slide [default]"
        )
        print(
            "         --rigid-interior     # Force nodes on interior lines to stay put"
        )
        print("         -s scale.shp         # scale shapefile             ")
        if field.has_apollonius:
            print(
                "         -a telescoping_scale.shp # auto telescoping scale shapefile"
            )
        ...
        print(" interior.shp:")
        print(
            "   A shapefile containg line segments which will be used to guide the orientation of cells"
        )
        ...
        print(
            " raster of scale field: the given region will be rasterized and output to scale-raster.tif"
        )
    ...
    def run(self, argv):
        try:
            opts, rest = getopt.getopt(
                argv[1:],
                "hb:s:a:t:i:c:r:R:dv:np:om:i:f:g:C:O:D:I:",
        ...
        if not self.resume_checkpoint_fn:
            ...
            if self.smooth:
                ...
                wkb2shp.wkb2shp(
                    os.path.join(self.output_path, self.smoothed_poly_shp),
                    [self.p.poly],
                    overwrite=True,
                )
    ...
    def check_parameters(self):
        ...
        if self.resume_checkpoint_fn is None:
            if len(self.scale_shps) + len(self.tele_scale_shps) == 0:
                print(
                    "ERROR: Must specify scale shapefile or telescoping scale shapefile"
                )
                show_usage = 1
            ...
            if len(self.tele_scale_shps) > 0 and not field.has_apollonius:
                print(
                    "ERROR: Telescoping scales supplied, but Apollonius is not available"
                )
                show_usage = 1
            if not self.smooth:
                print("WARNING: You are brave to disable smoothing.")
                print(
                    "         without smoothing the scale may be larger than the shoreline permits"
                )
        else:
            if (
                len(self.scale_shps) + len(self.tele_scale_shps)
            ) > 0 or self.boundary_shp is not None:
    ...
    def prepare_boundary(self):
        ...
        if gtype == ogr.wkbLineString:
            if self.boundary_poly_shp == self.boundary_shp:
                ...
                exit(1)
            join_features.process_layer(
                layer,
                self.boundary_poly_shp,
            ...
            if n_features > 1:
                print(
                    "Sorry - no support yet for multiple polygons - must be a single polygon with holes"
                )
                exit(1)
        ...
        if feat_is_multi_polygon:
            ...
            while 1:
                ...
                original_geometries.append(
                    shapely.wkb.loads(feat.GetGeometryRef().ExportToWkb())
                )
    ...
    def prepare_density(self):
        ...
        if len(self.scale_shps) > 0:
            if constrained_delaunay:
                density = constrained_delaunay.ConstrainedXYZField.read_shps(
                    self.scale_shps, value_field=self.scale_shp_field_name
                )
            else:
                if len(self.scale_shps) == 1:
                    density = field.XYZField.read_shp(
                        self.scale_shps[0], value_field=self.scale_shp_field_name
                    )
                else:
                    # really this could be implemented very easily - just been too lazy to do it.
                    raise Exception(
                        "constrained_delaunay unavailable, but you specified multiple scale files"
                    )
        else:
            density = None
        ...
        if len(self.tele_scale_shps) > 0:
            ...
            tele_density = field.ApolloniusField.read_shps(
                self.tele_scale_shps,
