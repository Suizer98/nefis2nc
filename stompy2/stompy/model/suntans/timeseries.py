...
class Timeseries(object):
    ...
    def splice(ts1, ts2, verbose=0):
        ...
        dirty_splice = Timeseries(t=splice_t, x=splice_x)
    ...
    def load_csv(fn, date_fmt=None, skiprows=0, delimiter=",", force_year_before="now"):
        ...
        d = np.loadtxt(
            fn, skiprows=skiprows, converters=converters, delimiter=delimiter
        )
    ...
    def clip(self, t_start, t_stop):
        ...
        return Timeseries(t=self.t[i_start:i_stop], x=self.x[i_start:i_stop])
    ...
    def sanitize(self, max_missing_samples=None, verbose=0):
        ...
        if len(self.t) == 0:
            ...
            raise Exception(
                "Timeseries has one point - this is probably not what you want!"
            )
        ...
        if (
            max_missing_samples is not None
            and all_dt.max() > max_missing_samples * basic_dt
        ):
        ...
        if len(missing) > 0 or len(too_short) > 0:
            if verbose:
                if len(missing) > 0:
                    ...
                    print(
                        "Median dt: %f  Min dt: %f  count(dt<%f)=%d"
                        % (basic_dt, all_dt.min(), 0.99 * basic_dt, len(too_short))
                    )
...
class IEPFile(Timeseries):
    def __init__(self, filename):
        ...
        for i in range(self.n_records):
            ...
            h = float(timepart[:2])
            m = float(timepart[2:])
            d = d + datetime.timedelta(((m / 60) + h) / 24.0)
