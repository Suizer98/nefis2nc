            ...
            while 1:
                ...
                for sub_idx, one_geom in enumerate(geolist):
                    if one_geom.type != "LineString":
                        raise Exception(
                            "All (sub)features must be linestrings (fid=%s, %s)"
                            % (fid, one_geom.type)
                        )
    ...
    for i in range(len(features)):
        ...
        while match:
            ...
            for j in range(i + 1, len(features)):
                ...
                match = check_match(features[i], features[j])
...
def clean_degenerate_rings(point_lists, degen_shpname="degenerate_rings.shp"):
    ...
    if degen_shpname is not None and len(degen_lines) > 0:
        wkb2shp.wkb2shp(
            degen_shpname, degen_lines, srs_text="EPSG:26910", overwrite=True
        )
...
def find_exterior_ring(point_lists):
    ...
    if len(open_strings) == 1:
        ...
        log.error(
            "Endpoints: %s"
            % (point_lists[open_strings[0]][0], point_lists[open_strings[0]][-1])
        )
...
def lines_to_polygons_slow(
    new_features, close_arc=False, single_feature=True, force_orientation=True
):
    """
    ...
    if close_arc and not closed_p:
        ...
        new_features[exterior_id] = np.concatenate(
            (new_features[exterior_id], closing_arc)
        )
            ...
            if prep_ext_poly and trust_prepared:
                ...
                if ext_poly.contains(int_poly):
                    ...
                    if prep_ext_poly is not None:
                        log.warning(
                            "A feature got through the prepared query, but the real query says it's outside the exterior"
                        )
                    else:
                        log.debug(
                            "Removing a feature that was outside the exterior ring"
                        )
                    extras.append(interior)
        else:
            log.debug(
                "Removing a feature that the fast query said was outside the exterior ring"
            )
            extras.append(interior)
...
def lines_to_polygons(
    ...
    log.info(
        "%d open strings, %d simple polygons" % (len(open_strings), len(simple_polys))
    )

    if len(open_strings):
        if not return_open:
            log.error(
                "New version of lines_to_polygons is faster but intolerant.  Cannot handle "
            )
            log.error("%d open strings" % len(open_strings))
            log.error(
                "First open string starts at %s" % (new_features[open_strings[0]][0])
            )
    ...
    while len(unassigned_idxs):
        ...
        ### Find exterior ring
        log.info(
            "Examining largest poly left with area=%f, %d potential interiors"
        ...
        for i in utils.progress(hit_indexes):
            ...
            if prep_ext_poly.contains(int_poly):
                ...
                ext_poly = shapely.geometry.Polygon(
                    ext_poly.exterior, list(ext_poly.interiors) + [int_poly.exterior]
                )
                prep_ext_poly = prepare_geometry(ext_poly)
                assigned_p[
                    i
                ] = True  # lazily remove from unassigned_idxs at top of loop
...
def process_layer(
    ...
    if create_polygons:
        if single_feature:
            geoms, extras = lines_to_polygons(
                new_features, close_arc=close_arc, single_feature=True
            )
        else:
            # geoms=[]
            ...
            geoms, extras = lines_to_polygons(
                new_features, close_arc=close_arc, single_feature=False
            )
    else:
        # Line output
    ...
    wkb2shp.wkb2shp(output_name, geoms, overwrite=True, **kws)
