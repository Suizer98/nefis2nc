...
class OptimizeGridMixin(object):
    ...
    def setup_optimizer(self):
        ...
        self.scale_reductions = np.array(
            [
                [xy_min[0], xy_min[1], 1e6],
    ...
    def update_apollonius_field(self):
        ...
        self.apollo = field.ApolloniusField(
            self.scale_reductions[:, :2],
            self.scale_reductions[:, 2],
            r=self.apollo_rate,
        )
        self.density = field.BinopField(self.original_density, minimum, self.apollo)
    ...
    def cell_scores(self, cell_ids=None, use_original_density=True):
        ...
        ec1 = 0.5 * (
            self.points[self.cells[cell_ids, 0]] + self.points[self.cells[cell_ids, 1]]
        )
        ec2 = 0.5 * (
            self.points[self.cells[cell_ids, 1]] + self.points[self.cells[cell_ids, 2]]
        )
        ec3 = 0.5 * (
            self.points[self.cells[cell_ids, 2]] + self.points[self.cells[cell_ids, 0]]
        )
    ...
    def relax_neighborhoods(
        self, score_threshold=0.1, count_threshold=5000, neighborhood_size=2
    ):
    ...
    def repave_neighborhoods(
        ...
        while 1:
            ...
            self.repave_neighborhood(
                c, neighborhood_size=neighborhood_size, scale_factor=scale_factor
            )
            print("Done")
    ...
    def repave_neighborhood(
        self, c, neighborhood_size=3, scale_factor=None, nbr_nodes=None
    ):
        """
        ...
        if nbr_nodes is None:
            if scale_factor is not None and self.use_apollo and scale_factor < 1.0:
                print(
                    "dynamically defining neighborhood based on radius of Apollonius Graph influence"
                )
        ...
        edges_to_kill = np.unique(
            np.concatenate([self.pnt2edges(n) for n in nbr_nodes])
        )
        # but don't remove boundary edges:
        # check both that it has cells on both sides, but also that it's not an
        # internal guide edge
        to_remove = (self.edges[edges_to_kill, 4] >= 0) & (
            self.edge_data[edges_to_kill, 1] < 0
        )
