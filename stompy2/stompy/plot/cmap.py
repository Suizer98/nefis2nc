...
def list_gradients():
    ...
    filenames = [
        os.path.basename(f)
        for f in glob.glob(os.path.join(os.path.dirname(__file__), "cmaps", "*"))
...
def write_sample():
    ...
    plt.savefig(
        os.path.join(os.path.dirname(__file__), "cmaps", "colormaps.png"),
        dpi=100,
        facecolor="gray",
    )
...
def ggr_cm(filename, reverse=False):
    ...
    return colors.LinearSegmentedColormap.from_list(
        os.path.basename(filename), rgb_tuples
    )
