...
def create_dataset(args):
    ...
    comp_field = field.CompositeField(
        shp_fn=args.shapefile,
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Directory for writing output, default 'output'",
        default="output",
    )
    parser.add_argument(
        "-p",
        "--path",
        help="Add to search path for source datasets",
        action="append",
        default=["."],
    )
    parser.add_argument(
        "-m",
        "--merge",
        help="Merge the tiles once all have been rendered",
        action="store_true",
    )
    parser.add_argument(
        "-r",
        "--resolution",
        help="Output resolution, default 10",
        default=10.0,
        type=float,
    )
    parser.add_argument(
        "-b",
        "--bounds",
        help="Bounds for output xmin xmax ymin ymax",
        nargs=4,
        type=float,
    )
    parser.add_argument(
        "-t", "--tilesize", help="Make tiles NxN, default 1000", default=None, type=int
    )
    parser.add_argument(
        "-v", "--verbose", help="Increase verbosity", default=1, action="count"
    )
    parser.add_argument(
        "-f", "--force", help="Overwrite existing tiles", action="store_true"
    )
    parser.add_argument(
        "-q", "--query", help="Query to select subset of features", default=None
    )
    parser.add_argument(
        "-d", "--date", help="Target date", default=None, type=np.datetime64
    )
    parser.add_argument(
        "-g", "--grid", help="Mask region by grid outline", default=None
    )
...
def main(argv=None):
    ...
    if tile_x is not None:
        total_tile_bounds = [
            np.floor(total_bounds[0] / tile_x) * tile_x,
    ...
    if args.merge:
        if tile_x is None:
            ...
            try:
                ...
                log.info(
                    "Failed to import gdal_merge, will try subprocess",
