    ...
    for i in range(len(cset.levels) - 1):
        ...
        for col in range(Z.shape[0] - 1):
            ...
            for a in range(d2, d1):
                ...
                locs = np.array(
                    [[X[c1, a], Y[c1, a]], [X[c1, b], Y[c1, b]], [X[c2, c], Y[c2, c]]]
                )
        ...
        pcoll = PolyCollection(
            my_paths, facecolors=coll.get_facecolor(), edgecolors=coll.get_facecolor()
        )
...
def contour_t(X, Y, Z, V=None, ax=None, **kwargs):
    ...
    for i in range(len(cset.levels)):
        ...
        lcoll = LineCollection(
            my_paths, colors=coll.get_edgecolor(), linestyles=coll.get_linestyle()
        )
