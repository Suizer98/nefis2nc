...
from ..spatial.linestring_utils import (
    upsample_linearring,
    downsample_linearring,
    resample_linearring,
)
...
class CList(object):
    ...
    def to_array(self, return_weights=False):
        ...
        if trav != self.head:
            raise Exception(
                "Thought we were making the full loop, but ended up elsewhere"
            )
    ...
    def __getstate__(self):
        ...
        return {"data": d_array, "weights": w_array}
    ...
    def unremove_iter(self, elt, metric=0.0):
        ...
        if elt.prv.nxt != elt.nxt:
            raise Exception(
                "Tried to unremove_iter, but it's state is inconsistent with the current list"
            )
        if elt.clist != self:
            raise Exception(
                "Tried to unremove_iter, but it belongs to a different clist"
            )
...
def point_in_triangle(pnt, tri):
    ...
    for t in tris:
        points = all_points[t]
        areas.append(
            0.5 * (points[i, 0] * points[ip1, 1] - points[ip1, 0] * points[i, 1]).sum()
        )
...
def one_point_cost(pnt, edges, target_length=5.0):
    ...
    all_angles = (np.pi - (abs_angles[:, i] - abs_angles[:, im1]) % (2 * np.pi)) % (
        2 * np.pi
    )
    ...
    if 1:
        ...
        worst_angle = abs(all_angles - 60 * np.pi / 180.0).max()
        ...
        if 1:
            ...
            scale_rad = 3.0 * np.pi / 180.0  # radians - e-folding scale of the cost
            # max_angle - 2.0*scale_rad works..
            thresh = (
                max_angle - 1.0 * scale_rad
            )  # angle at which the exponential 'cuts in'
            big_angle_penalty = np.exp((all_angles.max() - thresh) / scale_rad)
    else:
        alphas = (all_angles - 60 * np.pi / 180.0) / (max_angle - 60 * np.pi / 180.0)
        alphas = 10 * alphas**4
    ...
    if 1:  # the usual..
        ...
        thresh = 9.0  # 2.5*2.5
        length_penalty = (
            np.exp(rel_len_ab - thresh).sum() + np.exp(1.0 / rel_len_ab - thresh).sum()
        )
        length_penalty += (
            np.exp(rel_len_ca - thresh).sum() + np.exp(1.0 / rel_len_ca - thresh).sum()
        )
...
class Paving(paving_base, OptimizeGridMixin):
    ...
    def initialize_from_existing_grid(self):
        ...
        # boundary nodes -
        rings_and_children = self.edges_to_rings_and_holes(
            edgemask=(self.edges[:, 4] == -1)
        )
        if len(rings_and_children) != 1:
            print(
                "Loading existing grid - expected edges_to_rings_and_holes to return exactly 1, got %d"
                % len(rings_and_children)
            )
        ...
        for i in range(len(node_rings)):
            for j in range(len(node_rings[i])):
                ...
                self.node_data[
                    n, self.ORIG_RING
                ] = i  # even though they are really from many rings
        ...
        if len(rings) > 0:  # in weird special cases may not start with any rings
            ...
            for r in rings:
                if len(r) > 2:
                    ...
                    print(
                        "WARNING: dropping a ring because it didn't have enough unique points"
                    )
            rings = good_rings
    ...
    def clip_and_add_degenerate_ring(self, degen):
        ...
        while i < len(degen):
            ...
            conflicts = self.check_line_is_clear_new(
                n1=check_n1, n2=check_n2, p1=check_p1, p2=check_p2
            )
            ...
            if len(conflicts) == 0:
                ...
                if conflict[0] == "v":
                    ...
                    p_intersect = intersect_lines(
                        (self.points[conf_n1], self.points[conf_n2]), (p1, p2)
                    )
                    ...
                    if orig_ring1 >= 0 and (orig_ring1 == orig_ring2):
                        print(
                            "Figuring alpha from intersection with edge %d-%d"
                            % (conf_n1, conf_n2)
                        )
                        ...
                        span12 = (
                            ...
                            - self.node_data[conf_n1, self.ALPHA]
                        ) % len(self.original_rings[orig_ring])
                        span21 = (
                            self.node_data[conf_n1, self.ALPHA]
                            - self.node_data[conf_n2, self.ALPHA]
                        ) % len(self.original_rings[orig_ring])
                        ...
                        alpha = self.node_data[conf_n1, self.ALPHA] + span12 * (
                            norm(p_intersect - self.points[conf_n1])
                            / norm(self.points[conf_n2] - self.points[conf_n1])
                        )
                        print(
                            "n1 alpha=%g  new alpha=%g  n2 alpha=%g"
                            % (
                                self.node_data[conf_n1, self.ALPHA],
                                alpha,
                                self.node_data[conf_n2, self.ALPHA],
                    ...
                    n_intersect = self.add_node(
                        p_intersect, stat=self.RIGID, orig_ring=orig_ring, alpha=alpha
                    )
    ...
    def add_degenerate_ring(self, degen):
        ...
        if self.slide_internal_guides:
            ...
            if (~valid).sum() > 0:
                print(
                    "Removing %d points from internal guide because they're too close to the end"
                    % ((~valid).sum())
                )
            degen = degen[valid]
            if len(degen) < 2:
        ...
        if not self.slide_internal_guides:
            degen_sampled, degen_alpha = upsample_linearring(
                degen, self.density, closed_ring=0, return_sources=True
            )
        else:
            degen_sampled = degen
        ...
        for i in i_to_add:
            node_ids[i] = self.add_node(
                degen_sampled[i],
                stat=internal_node_stat,
                orig_ring=oring_id,
                alpha=degen_alpha[i],
            )
        ...
        if len(insert_iters) != 1:
            ...
            for ii in insert_iters:
                ...
                asort = self.angle_sort_nodes(
                    node_ids[0], np.array([ii.nxt.data, ii.prv.data, node_ids[1]])
                )
                asort = asort.tolist()
            ...
            if insert_iter is None:
                raise Exception(
                    "Failed to find a good iter in which to insert the degenerate edge"
                )
    ...
    def geom_to_rings(self, geom):
        ...
        if geom.type == "MultiPolygon":
            print(
                "Geometry is a multipolygon - will use the polygon with greatest area"
            )
    ...
    def smooth(self):
        ...
        if len(self.degenerate_rings):
            ...
            print(
                "WARNING: dangerous territory.  Smoothing does not know about degenerate rings :WARNING"
            )
        ...
        rings = [
            downsample_linearring(r, factor * self.min_density, closed_ring=1)
            for r in rings
        ]
    ...
    def adjust_density(self):
        ...
        self.density = adjust_scale(
            self.poly, self.requested_density, r=self.telescope_rate
        )
    ...
    def adjust_density_by_apollonius(self):
        ...
        self.density = field.BinopField(
            self.ag_density, np.minimum, self.requested_density
        )
        if self.verbose > 0:
            print("done with adjust_scale")
    ...
    def initialize_edges(self):
        ...
        for ri in range(len(self.unpaved)):
            ...
            for n1, n2 in uring.fwd_edge_iter():
                ...
                self.add_edge(
                    n1, n2, marker, c1, c2, oring=self.node_data[n1, self.ORIG_RING]
                )
    ...
    def boundary_slider(self, ri, alpha, beta=0.0):
        ...
        normal = (1 - frac) * self.oring_normals[ri][i] + frac * self.oring_normals[ri][
            ip1
        ]
        offset = beta * normal
        ...
        return (
            offset
            + (1 - frac) * self.original_rings[ri][i]
            + frac * self.original_rings[ri][ip1]
        )
    ...
    def plot(
        ...
        if self.n_history == 0 or plot_step is None:
            ...
            if len(segments) == 0:
                print(
                    "WARNING: no segments to display - maybe clip=%s is too strict"
                    % str(clip)
                )
                return
        ...
        if not just_save:
            ...
            if plot_step is None and dyn_zoom and len(self.cells) > 0:
                ...
                if (
                    ...
                    and p[1] > cur[2]
                    and p[1] < cur[3]
                    and (cur[1] - cur[0]) < 3.5 * fact * scale
                ):
                    pass
                else:
                    ax.axis(
    ...
    def split_edge(self, nodeA, nodeB, nodeC):
        ...
        self.edge_data = array_append(
            self.edge_data, [self.step, self.edge_data[e1, 1]]
        )
        ...
        while i < len(self.unpaved):
            if len(self.unpaved[i]) <= 3:
                if len(self.unpaved[i]) == 2:
                    e = self.find_edge(
                        (self.unpaved[i].head.data, self.unpaved[i].head.nxt.data)
                    )
                ...
                if self.verbose > 1:
                    print(
                        "Removing unpaved ring that has only %d nodes"
                        % len(self.unpaved[i])
                    )
                del self.unpaved[i]
            else:
        ...
        while 1:  # loop, because we may make structural changes and have to re-choose
            ...
            for use_beta in use_betas:
                ...
                if not fill_status and len(self.unpaved[ri]) == 4:
                    ...
                    fill_status = self.fill(
                        center_elt=center_elt.nxt, use_beta=use_beta
                    )
    ...
    def resample_neighbors(self, center_elt, new_node_stat=SLIDE):
        ...
        for direc in ["forward", "backward"]:
            ...
            #   path distance.
            free_boundary_length, end_elt, max_dist = self.free_distance_along_ring(
                center_elt, direc, "unpaved", upper_bound=10 * local_scale
            )
            if self.verbose > 1:
                if end_elt is not None:
                    print(
                        "resample neighbors: free distance=%g from node %d -> %d %s"
                        % (free_boundary_length, center_elt.data, end_elt.data, direc)
                    )
                else:
            ...
            if end_elt and (end_elt == prev_elt or end_elt == center_elt):
                if self.verbose > 0 and free_boundary_length > 0:
                    ...
                    print(
                        "Next node forward is same as one node back.  proceed with caution"
                    )
                closed_loop = 1
            else:
                closed_loop = 0
            ...
            if self.verbose > 2:
                self.plot(
                    plot_title="resample: %s, clsd=%d ctr=%s"
                    % (direc, closed_loop, center_elt)
                )
            ...
            if free_boundary_length > 0:
                if end_elt is not None:
                    ...
                    if quant_scale >= max_dist:
                        if self.verbose > 0 and quant_scale >= 1.05 * max_dist:
                            log.info(
                                "Too tight.  We wanted to resample at a scale of %f, but the"
                                % quant_scale
                            )
                    ...
                    if self.verbose > 1:
                        log.info(
                            "resample_neighbors: will try to make that into %d segments"
                            % n_segments
                        )
                else:
                    if self.verbose > 1:
                ...
                if n_segments > 1:
                    ...
                    new_elt = self.resample_boundary(
                        center_elt,
                        direc,
                        local_scale=quant_scale,
                        new_node_stat=new_node_stat,
                    )
                    if not new_elt:
                        log.warning(
                            "resample didn't return an element.  could be a problem"
                        )
                    elif n_segments == 2:
                        if self.verbose > 1:
                            log.info(
                                "resample_neighbors: n_segments is 2, so remove all nodes between new and end_elt"
                            )
                            log.info(
                                "  resample_boundary, from %s in direction %s, returned %s"
                                % (center_elt, direc, new_elt)
                            )
                        to_delete = []
                        trav = new_elt
                        while 1:
                        ...
                        for elt in to_delete:
                            if self.verbose > 1:
                                print(
                                    "  resample_neighbors: deleting node %d on ring %s"
                                    % (elt.data, elt.clist)
                                )
                        ...
                        if self.verbose > 2:
                            ...
                            self.plot(
                                plot_title="step %d: just cleared free boundary"
                                % self.step
                            )
                else:
                    if n_segments < 1:
                        print(
                            "Dynamic boundary sampling - free, but short! still trying..."
                        )
                        n_segments = 1
            ...
            if self.verbose > 2:
                self.plot(
                    plot_title="step %d after resampling %d in dir %s"
                    % (self.step, center_elt.data, direc)
                )
        ...
        for unpaved in self.unpaved:
            ...
            for pi in possible:
                ...
                if d1 * d2 < -self.TEST_SMALL:
                    print(
                        "two-way intersection w/products %g and %g" % (prod1, d1 * d2)
                    )
                    ...
                    if a in [n1, n2] or b in [n1, n2]:
                        print(
                            "they share a node - consider increasing TEST_SMALL or learning to write robust code"
                        )
                        continue
                    ...
                    plt.plot(
                        [pa[0] + ref[0], pb[0] + ref[0]],
                        [pa[1] + ref[1], pb[1] + ref[1]],
                        "m",
                        lw=3,
                    )
                    plt.plot(
                        [p1[0] + ref[0], p2[0] + ref[0]],
                        [p1[1] + ref[1], p2[1] + ref[1]],
                        "c",
                        lw=3,
                    )
    ...
    def free_distance_along_ring(
        self, from_elt, direc, path="original", upper_bound=None
    ):
        """
        ...
        while 1:
            ...
            if self.edge_data[e, 1] < 0:
                print(
                    "Stopping free_distance because hit an edge that is not on the original ring"
                )
                to_elt = A
                break
            ...
            if (
                self.node_data[B.data, self.STAT] in [self.RIGID, self.SLIDE]
                or B == from_elt
            ):
        ...
        if path == "unpaved":
            ...
            dist_sqr = ((self.points[from_elt.data] - self.points[nodes]) ** 2).sum(
                axis=1
            )
            ...
            metrics = [
                self.node_data[from_elt.data, self.ALPHA],
            ...
            return (
                self.coarse_boundary_length(orig_ring_i, metrics[0], metrics[1]),
                to_elt,
                np.nan,
            )
    ...
    def close_free_boundary(self, start_elt, end_elt, direction):
        ...
        if end_elt == opp_nbr:
            ...
            if unpaved_nbr != -2:
                raise Exception(
                    "Expected unpaved, but interior nbr cell was %s" % unpaved_nbr
                )
        ...
        if status == -1:
            ...
            if old_cell_nbr >= 0:
                if self.verbose > 0:
                    print(
                        "Would be adding a cell back in, using the same 3rd node as before"
                    )
                ...
                if self.edges[new_e, 3] == trigrid.BOUNDARY:
                    ...
                    raise Exception(
                        "Neither side of the new edge is a boundary. wrong!"
                    )
    ...
    def resample_boundary(self, start_elt, direction, local_scale, new_node_stat=SLIDE):
        """
        ...
        if self.slide_internal_guides and self.verbose > 1:
            print(
                "Resampling on edge e=%d,  start node=%d,  node to move=%d\n"
                % (e, start_elt.data, to_move)
            )
            print("Resampling on edge e=%d,  %d->%d" % (e, start_elt.data, to_move))
            print(
                "is %d in degenerate_rings: " % (self.edge_data[e, 1]),
                self.degenerate_rings,
            )
        ...
        if self.edge_data[e, 1] in self.degenerate_rings:
            ...
            if (direction == "forward") != (
                self.node_data[start_elt.data, self.ALPHA]
                < self.node_data[to_move, self.ALPHA]
            ):
                if self.verbose > 1:
        ...
        node_bounds = start_nbrs[
            [(to_move_i - 1) % len(start_nbrs), (to_move_i + 1) % len(start_nbrs)]
        ]
        ...
        if self.verbose > 1:
            print(
                "resample_boundary: start_elt=%s dir=%s scale=%f"
                % (start_elt, direction, local_scale)
            )
        ...
        if full_loop:
            ...
            new_alpha = alpha + direction * (
                local_scale
                / norm(
                    (oring[i] + sliding_beta * nring[i])
                    - (oring[im1] + sliding_beta * nring[im1])
                )
            )
            ...
            if np.floor(direction * new_alpha) != np.floor(direction * alpha):
                ...
                raise Exception(
                    "Thought that we were on the same segment, but alphas are different"
                )
        else:
            ...
            new_alpha = im1 + direction * (local_scale - near_dist) / (
                far_dist - near_dist
            )
        ...
        if full_loop and verbose > 1:
            print(
                "Boundary sliding: wanted length = %g, got length = %g"
                % (local_scale, norm(new_point - start_point))
            )
        ...
        ## ANGLE CHECK
        if len(start_nbrs) > 2:
            new_point_angle = np.arctan2(
                new_point[1] - start_point[1], new_point[0] - start_point[0]
            )
            past_cw = (new_point_angle - cw_angle_bound) % (2 * np.pi)
            if past_cw > angle_range:
                print(
                    "Nope - the angle of the new point was bad relative to neighbors."
                )
        ...
        closest_existing_node = self.find_closest_node_to_alpha(
            start_elt, direction, new_alpha
        )
        if abs(self.points[closest_existing_node] - new_point).sum() < 1e-5:
            # print "will use a pre-existing node rather than a probe point"
            constr_edges = self.check_line_is_clear_new(
                n1=original_start_elt.data, n2=closest_existing_node
            )
        elif (
            abs(
                live_dt.distance_left_of_line(
                    new_point,
                    self.points[original_start_elt.data],
                    self.points[closest_existing_node],
                )
            )
            < 1e-7
        ):
            ...
            constr_edges = self.check_line_is_clear_new(
                n1=original_start_elt.data, p2=new_point
            )
        ...
        if direction > 0:
            ...
            alpha_span = (
                self.node_data[start_node, self.ALPHA] - int_min_alpha
            ) % len_b
        ...
        if not new_node_passes_checks:
            print(
                "resample_neighbors: node failed some geometry checks - will stick with the current neighbor"
            )
            ...
            if self.node_data[n, self.STAT] < new_node_stat:
                print(
                    "Changing its status from %s to %s"
                    % (self.node_data[n, self.STAT], new_node_stat)
                )
        ...
        try:
            ...
            n = self.clear_boundary_by_alpha_range(
                start_elt, direction, new_alpha, keep_last=1
            )

            if self.verbose > 1:
                print(
                    "resample_boundary: clear_boundary_by_alpha range gave back node n=%s"
                    % n
                )

            if self.verbose > 2:
                self.plot(
                    plot_title="step %d: after clear_boundary_by_alpha" % self.step
                )

            if n is not None:
                if self.verbose > 1:
                    print(
                        "resample_boundary: Sliding node %d instead of deleting and re-inserting"
                        % n
                    )
                ...
                if self.check_new_alpha(n, new_alpha):
                    ...
                    raise Exception(
                        "Alpha from node from clear_boundary_by_alpha() is invalid"
                    )
    ...
    def clear_boundary_by_alpha_range(
        self, start_elt, direction, end_alpha, keep_last=0
    ):
        """this makes sure that there aren't any nodes that are too close (any that are
        will be deleted)
        ...
        while 1:
            ...
            if n_stat != self.HINT:
                ...
                log.debug(
                    "   encountered non-HINT node %d before alpha was reached" % n
                )
                if (not keep_last) and (n_stat != self.SLIDE):
                    log.warning(
                        "Actually it's worse - this node has stat=%s and can't be moved at all"
                        % n_stat
                    )
                    log.warning(
                        "   and keep_last is not set, so we're going to try to move it"
                    )
                break
            ...
            if dist > 1.001 * span:
                ...
                log.debug(
                    "clear_boundary_by_alpha: fudged the span to include an extra node"
                )
    ...
    def delete_edge_unpaved_handler(self, e, rollback, handle_unpaved):
        ...
        self.push_op(
            self.undelete_edge_unpaved_handler,
            added_clists,
            removed_clists,
            added_iters,
        )
    ...
    def coarse_boundary_length(self, ring_i, alpha1, alpha2):
        ...
        coarse_points = np.concatenate(
            (
                [self.boundary_slider(ring_i, alpha1)],
                coarse_points,
    ...
    def elt_smallest_internal_angle(self, ring_i):
        while 1:
            ...
            if self.verbose > 2:
                print(
                    "HEAP: pulled %s with metric %f=%.2fdeg"
                    % (
                        e,
            ...
            if self.unpaved[ring_i].metric(e) == 0.0:  # need to recompute the angle
                ...
                if (
                    self.prefer_original_ring_nodes
                    and self.node_data[e.data, self.ORIG_RING] < 0
                ):
                    new_metric += 10
    ...
    def safe_relax_one(self, n, use_beta=0, max_cost=np.inf):
        ...
        if stat == self.RIGID:
            ...
            raise Exception(
                "I don't think we should be relaxing HINT nodes.  Maybe forgot to update stat?"
            )
        else:
    ...
    def nonlocal_wall(self, left_elt, right_elt, local_scale, shoot_ray=1):
        ...
        scale_avg = np.sqrt(
            np.sum((self.points[left_elt.data] - self.points[right_elt.data]) ** 2)
        )
        ...
        for nbr in dt_ordered:
            ...
            dist = max(norm(pR - self.points[nbr]), norm(pL - self.points[nbr]))
            ...
            if nbr in local_nodes:
                ...
                if d_boundary / dist < 1.25:
                    ...
                    log.debug(
                        "d_boundary %d => %d = %g vs line %g"
                        % (n, nbr, d_boundary, dist)
                    )
                    log.debug(
        ...
        if best_nbr is None and shoot_ray:
            ...
            if e is not None:
                ...
                if (self.edges[e, 3] == trigrid.UNMESHED) and (
                    self.edges[e, 4] == trigrid.BOUNDARY
                ):
                    ...
                    if (da / local_scale < close_fac) and (
                        self.edges[e, 0] not in nbrs
                    ):
                        print("ray found an existing node %d" % self.edges[e, 0])
                        return self.edges[e, 0]
                    if (db / local_scale < close_fac) and (
                        self.edges[e, 1] not in nbrs
                    ):
                    ...
                    if self.freeze or self.holding:
                        print(
                            "WARNING: DT is frozen or holding - cannot double check validity of nonlocal bisector"
                        )
                        return new_elt.data
    ...
    def nonlocal_bisector(self, center_elt, local_scale, shoot_ray=1):
        ...
        scale_left = np.sqrt(
            np.sum(
                (self.points[center_elt.prv.data] - self.points[center_elt.data]) ** 2
            )
        )
        scale_right = np.sqrt(
            np.sum(
                (self.points[center_elt.nxt.data] - self.points[center_elt.data]) ** 2
            )
        )
        ...
        if len(dt_ordered) > 0:
            ...
            for nbr in dt_ordered:
                ...
                if dist > best_dist or dist > 1.25 * local_scale:
                    if self.verbose > 1:
                        log.debug(
                            "Discarding nonlocal because relative distance is %.3f, best is %.3f"
                            % (dist / local_scale, best_dist / local_scale)
                        )
                    continue
        ...
        if best_nbr is None and shoot_ray:
            ...
            if e is not None:
                ...
                valid_external = (self.edges[e, 3] == trigrid.UNMESHED) and (
                    self.edges[e, 4] == trigrid.BOUNDARY
                )
                valid_internal = (
                    (self.edges[e, 3] == trigrid.UNMESHED)
                    and (self.edges[e, 4] == trigrid.UNMESHED)
    ...
    def fill(self, center_elt, use_beta=0):
        ...
        join_okay = self.FREE in [
            self.node_data[ordered[0], self.STAT],
            self.node_data[ordered[2], self.STAT],
        ]
        ...
        elif theta > np.pi / 180.0 * (160.0 - (scale_factor - 1.0) * 50):
            strategies = ["wall"]
        ...
        elif theta > 85 * np.pi / 180.0 * (1.0 / scale_factor) ** 1.5:
            strategies = ["bisect", "cutoff", "wall"]
        elif theta > 30 * np.pi / 180.0:
            ...
            if on_boundary:
                ...
                if current_degree == 3:
                    ...
                    if current_degree < min_degree:
                        if self.verbose > 2:
                            print(
                                "HEY! giving bisect preference because of a boundary angle"
                            )
        ...
        boundary_diffs = (
            None  # a record of the changes that will need to be made to unpaved
        )
        ...
        while len(strategies):  #  for strategy in strategies:
            ...
            try:
                if self.verbose > 1:
                    print(
                        "strategy: %s, nodes=%d %d %d"
                        % (strategy, ordered[0], ordered[1], ordered[2])
                    )
                ...
                if strategy == "cutoff":
                    ...
                    if len(self.check_line_is_clear(ordered[0], ordered[2])) > 0:
                        print(
                            "EASY There.  Cutoff wasn't clear - will try something else"
                        )
                    ...
                    if len(unpaved) == 4:
                        ...
                        new_unpaved = [
                            (unpaved_mod.remove_iters, center_elt_mod),
                    ...
                    if self.verbose > 2:
                        print(
                            "Did a cutoff, and the update cells are: ",
                            self.updated_cells,
                        )
                elif strategy == "close":
                    ...
                    if n is None:
                        if self.verbose > 1:
                            print(
                                "No good bisector from %s, length=%g"
                                % (str(center_elt), local_length)
                            )
                    ...
                    self.updated_cells += self.cells_from_last_new_edge
                    log.debug(
                        "bisect_nonlocal got cells %s from new edge"
                    ...
                    for n_iter in self.all_iters_for_node(n):
                        new_unpaved.append(
                            (self.resample_neighbors, n_iter, self.SLIDE)
                        )
                    ...
                    if len(ordered) != 3:
                        raise Exception(
                            "How can ordered not be three nodes for a bisect?"
                        )
                    else:
                        new_unpaved = [
                    ...
                    if self.node_data[a, self.STAT] == self.FREE:
                        ...
                        raise StrategyFailed(
                            "Joining cannot work - neither node is free"
                        )
                    ...
                    if self.node_on_boundary(to_keep):
                        at_risk_length = norm(
                            self.points[at_risk] - self.points[to_keep]
                        )
                        ...
                        if keep_dir == "forward":
                            ...
                            while 1:
                                ...
                                if point_in_triangle(self.points[trav.data], annex):
                                    elts_to_delete.append(
                                        trav
                                    )  # NB: may not be able to delete this...
                        ...
                        for etd in elts_to_delete:
                            ...
                            # and that means
                            if (
                                not self.node_on_boundary(etd.data)
                                or len(self.pnt2edges(etd.data)) != 2
                            ):
                                undeletable.append(etd.data)

                        if len(undeletable) > 0:
                            print(
                                "Was trying a join, found nodes that needed to be cleared out"
                            )
                            ...
                            last_alpha = self.node_data[
                                elts_to_delete[-1].data, self.ALPHA
                            ]

                            print(
                                "Okay - found some conflicting nodes ahead of a join.  Will clear them out"
                            )
                            for e in elts_to_delete:
                                print(str(e))

                            if keep_dir == "forward":
                                self.close_free_boundary(
                                    center_elt.nxt, elts_to_delete[-1].nxt, "forward"
                                )
                            else:
                                self.close_free_boundary(
                                    center_elt.prv, elts_to_delete[-1].prv, "backward"
                                )
                    ...
                    tk_nbrs = np.setdiff1d(
                        tk_nbrs, [to_keep, b]
                    )  # remove common nbr, too
                    ...
                    if at_risk == intervener:
                        ...
                        all_nbrs = np.concatenate(
                            (np.setdiff1d(td_nbrs, [at_risk]), tk_nbrs)
                        )
                    else:
                        double_join = False
                        all_nbrs = np.concatenate((td_nbrs, tk_nbrs))

                    all_nbrs, all_angles = self.angle_sort_nodes(
                        to_keep, all_nbrs, return_angles=True
                    )
                    ...
                    def check_relaxation():
                        ...
                        new_all_nbrs = np.concatenate(
                            (sorted_nbrs[bi:], sorted_nbrs[:bi])
                        )
                        if self.verbose > 1:
                    ...
                    if 1:
                        all_nbrs_from_tk = self.angle_sort_nodes(
                            to_delete, all_nbrs, return_angles=False
                        )
                        bi = np.nonzero(all_nbrs_from_tk == b)[0][0]
                        all_nbrs_from_tk = np.concatenate(
                            (all_nbrs_from_tk[bi:], all_nbrs_from_tk[:bi])
                        )
                        if np.any(all_nbrs_from_tk != all_nbrs):
                            if self.verbose > 1:
                                ...
                                self.plot()
                                self.plot_nodes(
                                    [to_keep, to_delete] + all_nbrs.tolist()
                                )
                    ...
                    for nbr in td_nbrs:
                        ...
                        if self.verbose > 1:
                            print(
                                "for join, delete_edge(%d) nodes %d-%d"
                    ...
                    if self.verbose > 2:
                        self.plot(
                            "step %d: cleared edges, about to add new" % self.step
                        )
                    ...
                    for it in self.all_iters_for_node(to_delete):
                        ...
                        if it.nxt == center_elt or it.prv == center_elt:
                            ...
                            continue
                        print(
                            "Join: updating iter %s to replace %d with %d"
                            % (it, to_delete, to_keep)
                        )
                ...
                try:
                    for i in range(self.relaxation_iterations):
                        cells_to_check = np.unique(
                            np.array(self.updated_cells, np.int32)
                        )
                        ...
                        if self.verbose > 1:  ## pre-check
                            ...
                            print(
                                " Checking cells *before* more optimization: ",
                                cells_to_verify,
                            )

                            angles = self.tri_angles(cells_to_verify)

                            failed = cells_to_verify[
                                np.any(angles > self.max_angle, axis=1)
                            ]
                        ...
                        for n in nodes_to_check:
                            ...
                            cost = self.cost_for_point(
                                n
                            )  # accounts for all adjacent cells.
                            if cost < self.cost_threshold:
                                ...
                                # the cost?
                                self.safe_relax_one(
                                    n, use_beta=use_beta, max_cost=cost
                                )  # min(cost,200.0) )

                                if (
                                    post_relax_check is not None
                                    and not post_relax_check()
                                ):
                                    print(
                                        "Post-relax check failed.  Reverting the relaxation"
                                    )
                        ...
                        if np.any(angles > self.max_angle):
                            ...
                            if self.verbose > 2:
                                self.plot(
                                    plot_title="after optimization, still have some bad cells"
                                )
        ...
        if not all_okay:
            ...
            self.post_fill(ordered=ordered, modified_nodes=nodes_to_check)
    ...
    def check_nonlocal_connection(self, n):
        ...
        for i in order:
            ...
            if self.verbose > 2:
                print(
                    "Checking to make sure that node %d is part of the unpaved boundary"
                )
            n_iters = self.all_iters_for_node(best_nbr)
            if len(n_iters) == 0:
                if self.verbose > 1:
                    print(
                        "Almost tried to make a non-local connection to a node that isn't even in unpaved"
                    )
                continue
            ...
            if self.verbose > 2:
                print(
                    "Checking if non-local edge [%d,%d], with dist=%g is within the domain"
                    % (n, best_nbr, min_dist)
                )

            pnts = np.array(
                [0.999 * xy[0] + 0.001 * xy[1], 0.001 * xy[0] + 0.999 * xy[1]]
            )
                ...
                while 1:
                    ...
                    if straightline < local_radius or s <= steps:
                        if trav.data not in local_nodes:
                            ...
                            local_nodes[trav.data] = min(local_nodes[trav.data], dsum)
                    else:
                        break
    ...
    def iters_for_edge(self, edge):
        ...
        if self.edges[edge, 3] == trigrid.UNMESHED:
            left_iter = [
                li
                for li in self.all_iters_for_node(self.edges[edge, 0])
                if li.nxt.data == self.edges[edge, 1]
            ][0]
        if self.edges[edge, 4] == trigrid.UNMESHED:
            right_iter = [
                ri
                for ri in self.all_iters_for_node(self.edges[edge, 1])
                if ri.nxt.data == self.edges[edge, 0]
            ][0]
    ...
    def update_unpaved_boundaries(self, e):
        ...
        def choose_iter(m, n):
            ...
            raise Exception(
                "Bad mojo trying to find iters in update_unpaved_boundaries"
            )
    ...
    def flip_edge(self, e):
        ...
        all_points = np.unique(np.concatenate([self.cells[nc1], self.cells[nc2]]))

        new_edge_points = [v for v in all_points if v not in self.edges[e, :2]]

        if len(new_edge_points) == 2:
            print(
                "Will swap edge %d-%d for edge %d-%d"
                % (
                    self.edges[e, 0],
                    self.edges[e, 1],
                    new_edge_points[0],
    ...
    def relax_one(self, i, boundary=False, use_beta=0, max_cost=np.inf):
        ...
        if not boundary:
            ...
            if not use_beta:
                ...
                new_nd_alpha = my_fmin(bdry_point_cost_a, 1, disp=0, xtol=xtol)[0]
                ...
                if self.verbose > 1:
                    print(
                        "relax_one(%d) max_cost=%f new_cost=%f  alpha %f -> %f "
                        % (i, max_cost, new_cost, starting_alpha, new_alpha)
                    )
            else:
                ...
                starting_nd_alpha_beta = self.node_data[
                    i, self.ALPHA : (self.BETA + 1)
                ].copy()
                starting_nd_alpha_beta[0] -= starting_alpha - 1
                starting_nd_alpha_beta[1] = 1 + starting_beta / local_length
                new_nd_alpha_beta = my_fmin(
                    bdry_point_cost_ab, starting_nd_alpha_beta, disp=0, xtol=xtol
                )
            ...
            if new_cost > max_cost:
                ...
                if (
                    abs(new_beta - starting_beta) + abs(new_alpha - starting_alpha)
                    > 1e-4
                ):
    ...
    def check_new_alpha(self, i, new_alpha):
        ...
        if (new_alpha - lower_bound) % len_b > (upper_bound - lower_bound) % len_b:
            if 0:  # enable if alpha checks are failing a lot
                print(
                    "Alpha check failed:  %g  ?%g?  %g"
                    % (lower_bound, new_alpha, upper_bound)
                )
    ...
    def node_alpha_bounds(self, i):
        ...
        # bottleneck.
        same_ring_nodes = np.where(
            (self.node_data[:, self.ORIG_RING] == ring_i)
            & (self.node_data[:, self.STAT] != self.HINT)
            & (self.node_data[:, self.STAT] != self.DELETED)
        )[0]
        ...
        if diff_alphas[0] != 0:
            raise Exception(
                "The first diff should be us, unless we are somehow a HINT node"
            )
    ...
    def slide_node(self, n, new_alpha, new_beta=None):
        ...
        if len(all_ui) > 1 and self.verbose > 1:
            print(
                "WARNING: sliding a node (%d) that belongs to multiple unpaved rings. "
                % n
            )
        ...
        if ri in self.degenerate_rings:
            ...
            if new_alpha == 0 or new_alpha == len_b - 1:
                ...
                if n == replace:
                    ...
                    print(
                        "%d is trying to slide on top of the end node %d, with nbrs %s"
                        % (n, replace, nbrs)
                    )
                    if len(nbrs) == 1 and nbrs[0] == n:
                        ...
                        print(
                            "Whoa - this is bad - trying to slide a node on top of another node, but we can't"
                        )
                        print(
                            "get rid of the other node because it has other neighbors"
                        )
    ...
    def splice_in_grid(self, gridB, join_tolerance=0.25):
        ...
        for nB in range(gridB.Npoints()):
            ...
            if dist < join_tolerance:
                ...
                nodeBtoA[nB] = self.add_node(gridB.points[nB], stat=self.RIGID)
    ...
    def save_stride_filename(self):
        d = {"label": self.label, "step": self.step}
        if d["label"] is None:
            d["label"] = "checkpoint"
    ...
    def finish_stats(self):
        ...
        print(
            "Relative scale error:      % .3f ---  % .3f+-% .3f  --- % .3f"
            % (
                self.min_scale_rel_error,
        ...
        print(
            "Relative clearance error:  % .3f ---  % .3f+-% .3f  --- % .3f"
            % (
                self.min_clearance_rel_error,
...
def plot_cost_field(p, node, scale=None, samples=300):
    ...
    return plot_cost_field_at_point(
        p.points[node], edge_points, local_length, scale, samples
    )
...
def plot_cost_field_at_point(pnt, edge_points, local_length, scale, samples):
    ...
    for r in range(len(y)):
        for c in range(len(x)):
            costs[r, c] = one_point_cost(
                np.array([x[c], y[r]]), edge_points, local_length
            )
...
def plot_density(density, center=None, scale=None, samples=200):
    if center is None:
        z = plt.axis()
        center = [0.5 * (z[1] + z[0]), 0.5 * (z[3] + z[2])]
        scale = 0.5 * (z[1] - z[0])
    ...
    D = density(XY)
    plt.imshow(
        D, extent=[x[0], x[-1], y[0], y[-1]], origin="bottom", interpolation="nearest"
    )
