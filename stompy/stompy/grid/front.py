...
from . import unstructured_grid, exact_delaunay, shadow_cdt
...
def circumcenter_py(p1, p2, p3):
    ...
    dd = max(dd, 1e-40)
    return [
        (b1 * (p1y - p3y) - b2 * (p1y - p2y)) / dd + ref[0],
...
def one_point_cost(pnt, edges, target_length=5.0):
    ...
    if 1:
        ...
        worst_angle = np.abs(all_angles - 60 * np.pi / 180.0).max()
...
class Curve(object):
    ...
    def __init__(self, points, closed=True, ccw=None):
        ...
        if self.closed:
            if np.all(self.points[0] == self.points[-1]):
                ...
                self.points = np.concatenate((self.points, self.points[:1, :]))
        else:
    ...
    def __call__(self, f, metric="distance"):
        if metric == "distance":
            ...
            assert not np.any(
                f > self.distances[-1]
            ), "Curve: Range or round off problem"
            idxs = idxs.clip(
                0, len(self.distances) - 2
            )  # to be sure equality doesn't push us off the end

            alphas = (f - self.distances[idxs]) / (
                self.distances[idxs + 1] - self.distances[idxs]
            )
    ...
    def point_to_f(self, x, f_start=0, direction=1, rel_tol=1e-4):
        ...
        if rel_tol == "best":
            ...
            segs = np.stack([self.points[:-1, :], self.points[1:, :]], axis=1)
            ...
            if direction == 1:
                ...
                try:
                    return self.point_to_f(
                        x, f_start=f_start, direction=1, rel_tol=rel_tol
                    )
    ...
    def reverse(self):
        return Curve(points=self.points[::-1, :], closed=self.closed)
...
class WallStrategy(Strategy):
    ...
    def execute(self, site):
        ...
        new_c = grid.add_cell(nodes=[nb, nc, nd], edges=[j0, j1, j2])
        return {"nodes": [nd], "cells": [new_c]}
...
class BisectStrategy(Strategy):
    ...
    def execute(self, site):
        ...
        new_c1 = grid.add_cell(nodes=[nb, nc, nd], edges=[j_bc, j_cd, j_bd])
        new_c2 = grid.add_cell(nodes=[na, nb, nd], edges=[j_ab, j_bd, j_ad])

        return {"nodes": [nd], "cells": [new_c1, new_c2], "edges": [j_cd, j_bd, j_ad]}
...
class ResampleStrategy(Strategy):
    ...
    def distances(self, site):
        ...
        p_pa, p_a, p_c, p_pc = site.grid.nodes["x"][
            [pre_a, site.abc[0], site.abc[2], post_c]
        ]
        dists = [utils.dist(p_pa - p_a), utils.dist(p_c - p_pc)]
        return dists
...
class CutoffStrategy(Strategy):
    ...
    def metric(self, site):
        ...
        if theta > 179 * np.pi / 180:
            ...
            return np.abs(theta - ideal * np.pi / 180.0)
    ...
    def execute(self, site):
        ...
        if he_da.node_rev() == he_cd.node_fwd():
            ...
            internal_angles = (np.pi - ((np.roll(seg_theta, -1) - seg_theta))) % (
                2 * np.pi
            )
            ...
            if cutoff_bd > cutoff_ac:
                ...
                j_bd = grid.add_edge(
                    nodes=[nb, nd], cells=[grid.UNMESHED, grid.UNMESHED]
                )
                c_abd = site.grid.add_cell(
                    nodes=[na, nb, nd], edges=[j_ab, j_bd, he_da.j]
                )
                c_bcd = site.grid.add_cell(
                    nodes=[nb, nc, nd], edges=[j_bc, he_cd.j, j_bd]
                )
                ret["cells"].extend([c_abd, c_bcd])
            else:
                j_ca = grid.add_edge(
                    nodes=[nc, na], cells=[grid.UNMESHED, grid.UNMESHED]
                )

                c_abc = site.grid.add_cell(nodes=site.abc, edges=[j_ab, j_bc, j_ca])
                c_cda = site.grid.add_cell(
                    nodes=[nc, nd, na], edges=[he_cd.j, he_da.j, j_ca]
                )
            ...
            c = site.grid.add_cell(nodes=site.abc, edges=[j_ab, j_bc, j_ca])
            ret["cells"].append(c)
        return ret
...
class JoinStrategy(Strategy):
    ...
    def execute(self, site):
        ...
        if j_ac is not None:
            ...
            # not creation)
            if (grid.edges["cells"][j_ac, 0] >= 0) or (
                grid.edges["cells"][j_ac, 1] >= 0
            ):
        ...
        if grid.nodes["fixed"][na] == site.af.FREE:
            ...
            if (
                ring != grid.nodes["oring"][nc]
            ):  # this can maybe get relaxed to join onto a fixed node on multiple rings
...
class NonLocalStrategy(Strategy):
    ...
    def nonlocal_pair(self, site):
        ...
        if 1:
            ...
            ba_angle = np.arctan2(apnt[1] - bpnt[1], apnt[0] - bpnt[0])
            bc_angle = np.arctan2(cpnt[1] - bpnt[1], cpnt[0] - bpnt[0])
            ...
            for nbrs in old_each_dt_nbrs:
                ...
                # are consecutive
                angle_sum = (angles - bc_angle) % (2 * np.pi) + (ba_angle - angles) % (
                    2 * np.pi
                )
        ...
        if 0:
            ...
            for incr, node, ref_pnt in [
                (lambda x: x.rev(), lambda x: x.node_rev(), apnt),  # walk along b->a
                (lambda x: x.fwd(), lambda x: x.node_fwd(), cpnt),
            ]:  # walk along b->c
                trav = incr(he0)
        ...
        for n, dt_nbrs in zip(site.abc, each_dt_nbrs):
            ...
            good_nbrs = [nbr for nbr in dt_nbrs if nbr not in all_nbrs]
            if not good_nbrs:
                continue

            dists = [
                utils.dist(af.grid.nodes["x"][n] - af.grid.nodes["x"][nbr])
    ...
    def execute(self, site):
        ...
        j = grid.add_edge(
            nodes=[site_node, nonlocal_node], cells=[grid.UNMESHED, grid.UNMESHED]
        )

        return {"nodes": [], "cells": [], "edges": [j]}
...
class QuadSite(FrontSite):
    ...
    def resample_neighbors(self):
        ...
        self.resample_status = True
        for n, anchor, direction in [(a, b, -1), (d, c, 1)]:
...
class AdvancingFront(object):
    ...
    def add_curve(self, curve=None, interior=None, nodes=None, closed=True):
        ...
        if nodes is not None:
            ...
            self.grid.nodes["ring_f"][nodes[~on_a_ring]] = curve.distances[
                : len(nodes)
            ][~on_a_ring]
    ...
    def initialize_boundaries(self, upsample=True):
        ...
        for curve_i, curve in enumerate(self.curves):
            ...
            pairs = zip(np.arange(Ne), (np.arange(Ne) + 1) % Ne)
            for na, nb in pairs:
                self.grid.add_edge(
                    nodes=[nodes[na], nodes[nb]],
                    cells=[self.grid.UNMESHED, self.grid.UNDEFINED],
                    oring=curve_i + 1,
                    ring_sign=1,
                )
    ...
    def choose_site(self):
        ...
        if len(sites):
            scores = [site.metric() for site in sites]
    ...
    def free_span(self, he, max_span, direction):
        ...
        while pred(trav) and (trav != anchor) and (span < max_span):
            span += utils.dist(self.grid.nodes["x"][last] - self.grid.nodes["x"][trav])
            nodes.append(trav)
            if direction == 1:
                he = he.fwd()
    ...
    def resample(self, n, anchor, scale, direction):
        ...
        span_length, span_nodes = self.free_span(
            he, self.max_span_factor * scale, direction
        )
        ...
        if anchor_oring != oring:
            self.log.warning(
                "resample: anchor on different rings.  Cautiously resample"
            )
            if n_oring == oring:
            ...
            anchor_f = self.curves[oring].point_to_f(
                self.grid.nodes["x"][anchor], n_f, direction=0
            )

        if n_oring != oring:
            # anchor_f is valid regardless of its original oring
            n_f = self.curves[oring].point_to_f(
                self.grid.nodes["x"][n], anchor_f, direction=0
            )
        ...
        def node_f(m):
            ...
            if m == n:
                ...
                return curve.point_to_f(self.grid.nodes["x"][m], n_f, direction=0)
        ...
        if 1:  # "new" way
            ...
            assert edge_ring_sign != 0, "Edge %d has sign %d, should be set" % (
                he.j,
                edge_ring_sign,
            )
        ...
        if rel_curve_fwd(anchor_f, end_span_f, new_f) and end_span_f != anchor_f:
            self.log.warning(
                "n_segments=%s, but distance_away blew past it" % n_segments
            )
            return handle_one_segment()
        ...
        try:
            if method == "slide":
                ...
                jnew, nnew, j_next = self.grid.split_edge(
                    j, x=new_x, ring_f=new_f, oring=oring + 1, fixed=self.HINT
                )
        except self.cdt.IntersectingConstraints as exc:
    ...
    def resample_cycles(self):
        ...
        for cyc in cycs:
            ...
            while 1:
                ...
                res = self.resample(
                    b, a, scale=self.scale(self.grid.nodes["x"][a]), direction=1
                )
    ...
    def relax_free_node(self, n):
        ...
        init_cost = cost(x0)
        new_x = opt.fmin(cost, x0, xtol=local_length * 1e-4, disp=0)
        opt_cost = cost(new_x)
    ...
    def relax_slide_node(self, n):
        ...
        new_f = opt.fmin(cost_slide, [f0], xtol=local_length * 1e-4, disp=0)

        if not self.curves[ring].is_forward(slide_limits[0], new_f, slide_limits[1]):
    ...
    def find_slide_limits(self, n, cutoff=None):
        ...
        if curve.is_forward(
            self.node_ring_f(nbrs[0], n_ring), n_f, self.node_ring_f(nbrs[1], n_ring)
        ):
        ...
        for sgn, nbr in zip([-1, 1], nbrs):
            ...
            while 1:
                ...
                if (cutoff is not None) and (
                    sgn * (self.node_ring_f(trav[1], n_ring) - n_f)
                ) % L > cutoff:
                    break
        ...
        limits = [self.node_ring_f(m, n_ring) for m in stops]
    ...
    def plot_summary(self, ax=None, label_nodes=True, clip=None):
        ax = ax or plt.gca()
        ax.cla()
...
class AdvancingTriangles(AdvancingFront):
    ...
    def cost_function(self, n):
        ...
        cell_nodes = [self.grid.cell_to_nodes(c) for c in my_cells]
        ...
        Alist = [[e[0], e[1]] for e in edge_points[:, 0, :]]
        Blist = [[e[0], e[1]] for e in edge_points[:, 1, :]]
        ...
        def cost_cc_and_scale_py(x0):
            ...
            for A, B in zip(Alist, Blist):
                ...
                if 0:
                    ...
                    this_cc_cost = (
                        math.exp(min(100, cc_fac * leftAB / local_length))
                    ...
                    # maybe?
                    this_cc_cost = (
                        math.exp(min(100, cc_fac * leftAB / magABs))
                ...
                alpha = 1.0
                avg_length = (
                    alpha * local_length + (1 - alpha) * (magABs + magBCs + magCAs) / 3
                )
                this_scale_cost = (
                    (magABs - avg_length) ** 2
                    + (magBCs - avg_length) ** 2
...
class AdvancingQuads(AdvancingFront):
    ...
    def cost_function(self, n):
        ...
        quad_nodes = [np.roll(quad, -list(quad).index(n)) for quad in cell_nodes[:, :4]]
        ...
        edge_scales = np.zeros(
            [0, 3], "f8"
        )  # np.array( [ [x,y,target_distance], ... ] )
...
class DTNode(object):
    ...
    def __init__(self, af, parent=None):
        ...
        self.active_child = (
            None  # we don't manipulate this, but just signal that it's fresh
        )
...
class DTChooseSite(DTNode):
    def __init__(self, af, parent=None):
        ...
        priors = [site.metric() for site in sites]
...
class DTChooseStrategy(DTNode):
    def __init__(self, af, parent, site):
        ...
        actions = site.actions()
        priors = [a.metric(site) for a in actions]
    ...
    def try_child(self, i):
        ...
        cost = np.max([(self.af.eval_cost(n) or 0.0) for n in nodes])
