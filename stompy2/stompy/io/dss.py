...
"""
    ...
    def read_DSS_record(
        self, record_name, start_time=None, end_time=None, csv_file=None
    ):
        """
    ...
    def worker(self, record_name, tstart, tend, csv_file=None):
        if (csv_file is None) or utils.is_stale(csv_file, [self.dss_file_name]):
            ...
            args = dict(
                dss_file_name=self.dss_file_name, dss_path=record_name, csv_out=csv_file
            )
            ...
            self.stdout = subprocess.check_output(
                [self.get_hec_dssvue(), self.script_fn], stderr=subprocess.STDOUT
            )
...
def read_records(
    ...
    if cache_dir is not None:
        ...
        if cache_by_full_path:
            ...
            key = memoize.memoize_key(
                os.path.basename(filename), dss_path, start_time, end_time
            )
        else:
