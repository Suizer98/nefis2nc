...
def read_wind_rec(tok):
    ...
    name = tok("line")
    return dict(
        x=x, y=y, press=press, temp=temp, direction=direction, speed=speed, name=name
    )
...
def read_one_time(tok):
    ...
    for i in range(n_prof):
        prof.append(
            dict(elev_ft=float(tok()), direc=float(tok()), speed_kts=float(tok()))
        )
    ...
    full_timestamp = timestamp0 + (hour * 3600 + minute * 60 + second) * np.timedelta64(
        1, "s"
    )
...
pnts = [geometry.Point(rec.x * 1000, rec.y * 1000) for idx, rec in summary.iterrows()]
