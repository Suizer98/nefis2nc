...
def get_nef():
    return nefis.Nefis("data/trim-tut_fti_waq.dat", "data/trim-tut_fti_waq.def")


def test_open():
    nef = nefis.Nefis("data/trim-tut_fti_waq.dat", "data/trim-tut_fti_waq.def")
    assert nef
    nef.close()
...
def test_getelt_with_shape():
    ...
    data = nef["map-series"].getelt(
        "U1", [slice(1, 3), slice(None), slice(None), slice(None)]
    )
...
def test_character_element():
    nef = nefis.Nefis("data/tut_fti_waq.hda", "data/tut_fti_waq.hdf")
