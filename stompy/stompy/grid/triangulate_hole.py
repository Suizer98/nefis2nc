...
def triangulate_hole(
    ...
    if method == "rebay" and hole_rigidity != "all":
        raise Exception(
            "Currently rebay can only handle fully rigid hole (hole_rigidity='all')"
        )
    ...
    if method == "front":
        ...
        if AT.loop():
            ...
            return (
                AT  # for debugging -- need to keep a handle on this to see what's up.
            )
    elif method == "rebay":
        ...
        rad = rebay.RebayAdvancingDelaunay(
            grid=grid_to_pave, scale=scale, heap_sign=1, **method_kwargs
        )
        if dry_run:
            if return_value == "grid":
                return grid_to_pave
    ...
    if not splice:
        ...
        edges_to_replace = (
            []
        )  # node in AT.grid and node in grid that should get an edge after merge
        for n in src_hints:
            n_nbrs = grid.node_to_nodes(n)
            if len(n_nbrs) <= 2:
                continue  # can't have any extra edges
            ...
            for nbr in n_nbrs:
                ...
                if nbr not in nodes:
                    ...
                    if nbr_idx in [(n_idx + 1) % N, (n_idx - 1) % N]:
                        ...
                        logger.warning(
                            "Untested territory -- a non-hint edge joining two hint nodes"
                        )
                        n_new = AT.grid.select_nodes_nearest(grid.nodes["x"][n])
