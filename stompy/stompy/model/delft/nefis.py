...
def load_nef_lib():
    ...
    if os.path.exists(nefis_dll_path):
        try:
            return ctypes.CDLL(nefis_dll_path, ctypes.RTLD_GLOBAL)
        except OSError:
            try:
    ...
    if sys.platform.startswith("linux"):
        ...
        log.warning(
            "Need to add support in nefis.nef_lib() for platform=%s" % sys.platform
        )
        return None
...
class NefisElement(object):
    def __init__(self, nefis, name, typ, nbytsg, quantity, unit, desc, shape):
        ...
        if self.np_type_code is None:
            raise NefisException(
                0, "Unknown type: {}, {} bytes".format(self.typ, self.nbytsg)
            )

    def __str__(self):
        return "<NefisElement {} {} [{}]>".format(
            self.name, self.typ, fmt_shape(self.shape)
        )
...
class NefisGroup(object):
    ...
    def __str__(self):
        return "<NefisGroup {}:{}, [{}]>".format(
            self.name, self.cellname, fmt_shape(self.shape)
        )
    ...
    def getelt(self, element, read_shape=None, shape_only=False):
        ...
        if data_nbytes > u32_max:
            msg = "Buffer size for getelt (%d) would exceed uint32.max (%d)" % (
                data_nbytes,
                u32_max,
            )
    ...
    def attrs_int(self):
        nef = nef_lib()
        return self.attrs_gen(lambda: c_int(-1), nef.Inqfia, nef.Inqnia)

    def attrs_real(self):
        nef = nef_lib()
        return self.attrs_gen(lambda: c_float(-1), nef.Inqfra, nef.Inqnra)

    def attrs_str(self):
        nef = nef_lib()
        return self.attrs_gen(lambda: create_string_buffer(17), nef.Inqfsa, nef.Inqnsa)
...
class Nefis(object):
    ...
    def get_header(self):
        ...
        self.with_err(nef_lib().Gethdf(byref(self.fd), byref(header_buff)))
    ...
    def group_summary(self):
        ...
        for grp in self.groups():
            lines.append(
                "  {}:{} [{}]".format(grp.name, grp.cellname, fmt_shape(grp.shape))
            )
        return "\n".join(lines)
    ...
    def cell_summary(self):
        ...
        for cel in self.cells():
            ...
            for elt_name in cel.element_names:
                ...
                lines.append(
                    "     {}:{} [{}]".format(elt.name, elt.typ, fmt_shape(elt.shape))
                )
