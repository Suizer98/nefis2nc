...
def names_to_omegas(names):
    ...
    omega_per_sec = omega_deg_per_hour * (1.0 / 3600) * (1 / 360.0)
...
def omegas_to_names(omegas):
    ...
    """
    speed_rad_per_sec = tide_consts.speeds * (1.0 / 3600) * (np.pi / 180)
    idxs = [np.argmin(np.abs(speed_rad_per_sec - omega)) for omega in omegas]
    names = [
        tide_consts.const_names[idx] if omega > 0 else "DC"
