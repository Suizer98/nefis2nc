...
def interp_to_time_per_ll(
    df, tstamp, lat_col="latitude", lon_col="longitude", value_col="value"
):
    """
    ...
    def interp_col(grp):
        ...
        return pd.Series([value, dist], [value_col, "time_offset"])

    # for some reason, using apply() ignores as_index
    result = (
        df.groupby([lat_col, lon_col], as_index=False).apply(interp_col).reset_index()
    )
    assert np.all(np.isfinite(result[value_col].values))
    return result


def weighted_grid_extrapolation(
    g,
    samples,
    alpha=1e-5,
    x_col="x",
