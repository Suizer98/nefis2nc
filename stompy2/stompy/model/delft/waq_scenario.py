            ...
            while not self.signal_stop:
                ...
                if next_line == "":
                    ...
                    if "Completed" in last_line:
                        ...
                        try:
                            pct_complete = float(
                                re.match(r"([0-9\.]+).*", last_line.strip()).group(1)
                            )
                            ...
                            if len(sample_pcts) > 1:
                                ...
                                if mb[0] != 0:
                                    ...
                                    etf = datetime.datetime.now() + datetime.timedelta(
...
class Hydro(object):
    ...
    def t_dn(self):
        ...
        return utils.to_dnum(self.time0) + self.t_secs / 86400.0
    ...
    def scen_t_secs(self):
        ...
        start_i, stop_i = np.searchsorted(
            hydro_datetimes, [self.scenario.start_time, self.scenario.stop_time]
        )
        if start_i > 0:
            start_i -= 1
    ...
    def write_are(self):
        ...
        with open(self.are_filename, "wb") as fp:
            for t_sec in utils.progress(
                self.scen_t_secs.astype("i4"), msg="writing area: %s"
            ):
    ...
    def write_flo(self):
        ...
        with open(self.flo_filename, "wb") as fp:
            for t_sec in utils.progress(
                self.scen_t_secs.astype("i4"), msg="writing flo: %s"
            ):
                fp.write(t_sec.tobytes())  # write timestamp
    ...
    def text_atr(self):
        ...
        if np.any(attrs1 != 1):  # departs from default
            count += 1
            lines += [
                "1 1 1 ; num items, feature, input here",
        ...
        if np.any(attrs2 != 0):  # departs from default
            count += 1
            lines += [
                "1 2 1 ; num items, feature, input here",
                "    1 ; all segs, without defaults",
    ...
    def write_atr(self):
        ...
        with open(
            os.path.join(self.scenario.base_path, self.fn_base + ".atr"), "wt"
        ) as fp:
    ...
    def write_len(self):
        ...
        """
        with open(
            os.path.join(self.scenario.base_path, self.fn_base + ".len"), "wb"
        ) as fp:
    ...
    def write_poi(self):
        ...
        """
        with open(
            os.path.join(self.scenario.base_path, self.fn_base + ".poi"), "wb"
        ) as fp:
    ...
    def write_vol(self):
        ...
        with open(self.vol_filename, "wb") as fp:
            for t_sec in utils.progress(
                self.scen_t_secs.astype("i4"), msg="writing vol: %s"
            ):
                fp.write(t_sec.tobytes())  # write timestamp
    ...
    def write_geom(self):
        ...
        dest = os.path.join(self.scenario.base_path, self.flowgeom_filename)
        ds.to_netcdf(dest)
    ...
    def add_parameters(self, hyd):
        ...
        if has_vert_diffs:
            ...
            hyd["VertDisper"] = ParameterSpatioTemporal(
                func_t=self.vert_diffs, times=self.t_secs, hydro=self
            )
    ...
    def infer_2d_elements(self):
        ...
        if self.seg_to_2d_element is None:
            n_2d_elements = 0
            seg_to_2d = (
                np.zeros(self.n_seg, "i4") - 1
            )  # 0-based segment => 0-based 2d element.
            ...
            for seg in range(self.n_seg):  # 0-based segment
                if seg % 50000 == 0:
                    self.log.info(
                        "Inferring 2D elements, %d / %d 3-D segments"
                        % (seg, self.n_seg)
                    )
                ...
                def trav(seg, elt, k):
                    ...
                    for nbr in v_nbrs2:  # a segment 'above' us
                        if nbr > 0 and seg_to_2d[nbr] < 0:
                            ...
                            self.log.warning(
                                "infer_2d_elements: spurious segments on top.  segment ordering may be off"
                            )
    ...
    def mats_QtodV(self):
        ...
        if self._QtodV is None:
            ...
            QtodV = sparse.coo_matrix((vals, (rows, cols)), (self.n_seg, self.n_exch))
            QtodVabs = sparse.coo_matrix(
                (np.abs(vals), (rows, cols)), (self.n_seg, self.n_exch)
            )
            self._QtodV = QtodV
            self._QtodVabs = QtodVabs
        return self._QtodV, self._QtodVabs

    def check_volume_conservation_incr(
        self,
        seg_select=slice(None),
        ...
        """
        assert (tidx_select.step is None) or (
            tidx_select.step == 1
        ...
        summary = np.zeros(
            len(segs),
            [("seg", "i4"), ("rel_err", "f8"), ("vol_err", "f8"), ("Q_err", "f8")],
        )
        summary["seg"] = segs
        ...
        for i, ti in enumerate(t_idxs):
            ...
            if i > 0:
                ...
                if verbose and (rel_err[seg_select].max() > 1e-4):
                    self.log.warning(
                        "****************BAD Volume Conservation*************"
                    )
                    self.log.warning(
                        "  t=%10d   RMSE: %e    Max rel. err: %e"
                        % (t_sec, rmse, rel_err[seg_select].max())
                    )
                    self.log.warning(
                        "  %d segments above rel err tolerance"
                        % np.sum(rel_err[seg_select] > 1e-4)
                    )
                    self.log.info(
                        "Bad segments: %s" % (np.nonzero(rel_err[seg_select] > 1e-4)[0])
                    )

                    bad_seg = np.arange(len(rel_err))[seg_select][
                        np.argmax(rel_err[seg_select])
                    ]
                    self.log.warning("  Worst segment is index %d" % bad_seg)
                    self.log.warning(
                        "  Vlast=%f  Vpred=%f  Vnow=%f"
                        % (Vlast[bad_seg], Vpred[bad_seg], Vnow[bad_seg])
                    )
                    if seg_plan_areas is not None:
                        self.log.warning(
                            "  z error=%f m" % (err[bad_seg] / seg_plan_areas[bad_seg])
                        )
                    ...
                    self.log.warning(
                        "  Condition of dV: Qmag=%f Qnet=%f mag/net=%f"
                        % (Qmag, bad_Q.sum(), Qmag / bad_Q.sum())
                    )
    ...
    boundary_dtype = [("id", "S20"), ("name", "S20"), ("type", "S20")]
    ...
    def boundary_defs(self):
        ...
        if self._boundary_defs is None:
            ...
            if self.boundary_scheme == "id":
                ...
                self.log.warning(
                    "This is outdated, and may be incorrect for DFM output with out-of-order exchanges"
                )
                bc_segs = self.bc_segs()
                self.infer_2d_elements()
                if self.boundary_scheme == "element":
                    bdefs["type"] = [
                        "element %d" % (self.seg_to_2d_element[seg]) for seg in bc_segs
                    ]
                elif self.boundary_scheme == "grouped":
                    bc_groups = self.group_boundary_elements()
                    bdefs["type"] = [
                        bc_groups["name"][self.seg_to_2d_element[seg]]
                        for seg in bc_segs
                    ]
                ...
                if 0:
                    ...
                    self.infer_2d_links()
                    bdefs["type"] = [
                        bc_lgroups["name"][self.exch_to_2d_link["link"][exch]]
    ...
    def write_hyd(self, fn=None):
        ...
        # already arbitrary
        fn = fn or os.path.join(self.scenario.base_path, self.fn_base + ".hyd")
    ...
    def infer_2d_links(self):
        ...
        if self.exch_to_2d_link is None:
            ...
            exch_to_2d_link = np.zeros(
                self.n_exch_x + self.n_exch_y, [("link", "i4"), ("sgn", "i4")]
            )
            ...
            for exch_i, (a, b, _, _) in enumerate(
                poi0[: self.n_exch_x + self.n_exch_y]
            ):
                if a < 0 and b < 0:
            ...
            if 1:
                ...
                for bc_link in bc_links:
                    ...
                    if np.any(~sel0):
                        ...
                        self.log.warning(
                            "Some exchanges had to be flipped when flattening to 2D links"
                        )
                    ...
                    if len(np.unique(segs)) < len(segs):
                        self.log.warning(
                            "In flattening exchanges to links, link %d has ambiguous multiple exchanges for the same segment"
                            % bc_link
                        )
    ...
    def write_2d_links(self):
        ...
        np.savetxt(os.path.join(path, "links.csv"), self.links, fmt="%d")
        np.savetxt(
            os.path.join(path, "exch_to_2d_link.csv"), self.exch_to_2d_link, fmt="%d"
        )
    ...
    def flowlink_to_edge(self, g):
        ...
        cc = g.cells_center()
        elem_xy = np.c_[geom.FlowElem_xcc.values, geom.FlowElem_ycc.values]
        ...
        for link, (eltA, eltB) in utils.progress(enumerate(self.links)):
            ...
            if eltA < 0:  # eltA<0 means it's a boundary.
                ...
                for j in g.cell_to_edges(cB):
                    ...
                    print(
                        "Link %d -- %d does not map to a grid boundary, likely a discharge, and will be ignored."
                        % (eltA, eltB)
                    )
                ...
                if j is None:
                    raise Exception(
                        "%d to %d was not an edge in the grid" % (eltA, eltB)
                    )
    ...
    def path_to_transect_exchanges(
        self, xy, on_boundary="warn_and_skip", on_edge=False
    ):
        """
        xy: [N,2] points.
        ...
        g = self.grid()
        input_nodes = [g.select_nodes_nearest(p) for p in xy]
        if on_edge:
        ...
        for ncheck in legs:
            ...
            if count > 1:
                raise Exception(
                    "Encountered duplicate nodes. May need to clean MPI output grid, or revert to ZZ edge search"
                )
            elif count == 0:
                if on_edge:
                    raise Exception(
                        "Node search failed and on_edge is set, but supplied points do not line up."
                    )
                else:
                    raise Exception(
                        "Node search failed but on_edge is not set.  Something very wrong"
                    )
        ...
        for a, b in zip(legs[:-1], legs[1:]):
            ...
            if j is None:
                ...
                if on_edge:
                    ...
                    raise Exception(
                        "edge couldn't be found, but it came from the grid."
                    )
            ...
            if nhits == 0:
                if np.any(c1_c2 < 0):
                    self.log.warning(
                        "Discarding boundary edge in path_to_transect_exchanges"
                    )
                    continue
                else:
    ...
    link_group_dtype = [("id", "i4"), ("name", "O"), ("attrs", "O")]
    ...
    def group_boundary_links(self):
        ...
        bc_lgroups[
            "id"
        ] = -1  # most links are internal and not part of a boundary group
    ...
    group_dtype = [
        ("id", "i4"),  # 0-based id of this elements group, -1 for unset.
        ("name", "O"),  # had been S40, but that get confusing with bytes vs. str
        ("attrs", "O"),
    ]  # a place for add'l key-value pairs
    ...
    def flo_dtype(self):
        return np.dtype([("tstamp", "<i4"), ("flow", "<f4", self.n_exch)])

    def are_dtype(self):
        return np.dtype([("tstamp", "<i4"), ("area", "<f4", self.n_exch)])

    def vol_dtype(self):
        return np.dtype([("tstamp", "<i4"), ("volume", "<f4", self.n_seg)])
...
class HydroFiles(Hydro):
    ...
    def t_secs(self):
        if self._t_secs is None:
            ...
            conv_step = waq_timestep_to_timedelta(
                self.hyd_toks["conversion-timestep"].strip("'")
            )
            ...
            if abs(step_secs - round(step_secs)) > 1e-5:
                print(
                    "WARNING: total seconds in step was not an integer: %s" % step_secs
                )
    ...
    def __getitem__(self, k):
        ...
        if k in [
            ...
            return int(val)
        elif k in ["water-quality-layers", "hydrodynamic-layers"]:
            return val
        else:
            return val.strip("'")
    ...
    def n_seg(self):
        if self._n_seg is None:
            ...
            n_seg_dense = (
                self["number-water-quality-layers"]
                * self["number-water-quality-segments-per-layer"]
            )
            ...
            if os.path.exists(self.get_path("grid-coordinates-file")):
                ...
                g = unstructured_grid.UnstructuredGrid.read_dfm(
                    self.get_path("grid-coordinates-file")
                )
                n_elts_nc = g.Ncells()
            ...
            if 1:  # allow for sparse exchanges and dense segments
                ...
                pred_n_steps = are_size / 4.0 / (self.n_exch + 1)

                if pred_n_steps == nsteps:
                    pass  # great
                elif abs(pred_n_steps - nsteps) <= self.nstep_mismatch_threshold:
                    self.log.info(
                        "Area file has %s steps vs. %s expected - proceed with caution"
                        % (pred_n_steps, nsteps)
                    )
                else:
                ...
                for step_error in range(1 + self.nstep_mismatch_threshold):
                    for vol_n_steps in (
                        pred_n_steps - step_error,
                        pred_n_steps + step_error,
                    ):
                        n_seg = (vol_size / float(vol_n_steps) - 4) / 4.0
                        if n_seg % 1.0 != 0.0:
                            n_seg = -1  # continue
                        else:
                            n_seg = int(n_seg)
    ...
    def write_are(self):
        if not self.enable_write_symlink:
            ...
            rel_symlink(
                self.get_path("areas-file"), self.are_filename, overwrite=self.overwrite
            )
    ...
    def areas(self, t, memmap=False):
        ...
        if memmap:
            if self._areas_mmap is None:
                ...
                if (ti >= self._areas_mmap.shape[0]) or (
                    self._areas_mmap["tstamp"][ti] != t
                ):
                    self.log.warning(
                        "area memmap failed -- too short or mismatched timestamp"
                    )
        ...
        if ti < ti_req:
            self.log.warning(
                "Area data ends early by %d steps. Use previous" % (ti_req - ti)
            )

        tstamp = np.frombuffer(tstamp_data, np.int32)[0]
        if (ti == ti_req) and (tstamp != t):
            self.log.warning(
                "WARNING: time stamp mismatch: %d [file] != %d [requested]"
                % (tstamp, t)
            )
    ...
    def seg_func(self, t_sec=None, fn=None, label=None, memmap=False):
        ...
        def f(t_sec, closest=False, memmap=memmap):
            ...
            if memmap:
                if self._seg_mmap is None:
                    self._seg_mmap = {}  # jit init
                if self._seg_mmap.get(filename, None) is False:
                    ...
                    if filename not in self._seg_mmap:
                        ...
                        self._seg_mmap[filename] = np.memmap(
                            filename, self.vol_dtype(), mode="r"
                        )
                ...
                if len(tstamp) == 0 or tstamp[0] != t_sec:
                    if 0:  # old behavior, no scanning:
                        if len(tstamp) == 0:
                            ...
                            print(
                                "WARNING: time stamp mismatch: %s != %d should scan but won't"
                                % (tstamp[0], t_sec)
                            )
                        ...
                        if err != 0:
                            print(
                                "WARNING: time stamp mismatch after inferring nonstandard time step"
                            )
                        ...
                        if ti < 0:
                            if t_sec >= 0:
                                warning = (
                                    "WARNING: inferred time index %d is negative!" % ti
                                )
                            else:
                                ...
                                warning = (
                                    "INFO: inferred time index %d is negative, ignoring as t=%d"
                                    % (ti, t_sec)
                                )
                            ti = 0
                        ...
                        if ti >= max_ti:
                            warning = (
                                "WARNING: inferred time index %d is beyond the end of the file!"
                                % ti
                            )
                            ti = max_ti - 1
                        # try that again:
    ...
    def write_flo(self):
        if not self.enable_write_symlink:
            ...
            rel_symlink(
                self.get_path("flows-file"), self.flo_filename, overwrite=self.overwrite
            )
    ...
    def flows(self, t, memmap=False):
        ...
        if memmap and self._flows_mmap is not False:
            if self._flows_mmap is None:
                self._flows_mmap = np.memmap(flo_fn, self.flo_dtype(), mode="r")
            if ti >= len(self._flows_mmap):
                self.log.warning(
                    "Flow data ends early by %d steps" % (len(self.t_secs) - 1 - ti)
                )
        ...
        with open(flo_fn, "rb") as fp:
            ...
            if len(tstamp_data) < 4:
                if ti == len(self.t_secs) - 1:
                    self.log.info(
                        "Short read on last frame of flow data - fabricate zero flows"
                    )
                else:
                    self.log.warning(
                        "Flow data ends early by %d steps" % (len(self.t_secs) - 1 - ti)
                ...
                if tstamp != t:
                    self.log.warning(
                        "flows: time stamp mismatch: %d != %d" % (tstamp, t)
                    )
                data = np.fromfile(fp, np.float32, self.n_exch)
                if len(data) != self.n_exch:
                    self.log.warning(
                        "flow: incomplete frame, %d items < %d exchanges"
                        % (len(data), self.n_exch)
                    )
                    return np.zeros(self.n_exch, "f4")
    ...
    def update_flows(self, t, new_flows):
        ...
        with open(flo_fn, "rb+") as fp:
            ...
            if len(tstamp_data) < 4:
                ...
                if tstamp != t:
                    self.log.warning(
                        "update_flows: time stamp mismatch: %d != %d" % (tstamp, t)
                    )
    ...
    def adjust_boundaries_for_conservation(self, tidx_select=slice(None)):
        ...
        self.check_volume_conservation_incr(
            seg_select=bc_adj_segs,
        ...
        if self._read_seg_attrs is None and "attributes-file" in self.hyd_toks:
            ...
            with open(self.get_path("attributes-file"), "rt") as fp:
                ...
                for const_contrib in range(n_const_blocks):
                    ...
                    assert (
                        toker() == 1
                    )  # input is in this file, nothing else implemented.
    ...
    def write_geom(self):
        ...
        if os.path.exists(dest) or os.path.lexists(dest):
            if self.overwrite:
                ...
                raise Exception(
                    "Overwrite is not true, and geometry file %s already exists" % dest
                )
    ...
    def grid(self, force=False):
        if force or self._grid is None:
            ...
            try:
                ...
                dg = unstructured_grid.UnstructuredGrid.read_dfm(
                    orig, cleanup=self.clean_mpi_dfm_grid
                )
                self._grid = dg
        return self._grid
    ...
    def add_parameters(self, hyd):
        ...
        for var, key in [
            ("vertdisper", "vert-diffusion-file"),
            ...
            if os.path.exists(fn):
                ...
                hyd[var] = ParameterSpatioTemporal(seg_func_file=fn, hydro=self)
        return hyd
    ...
    def read_2d_links(self):
        ...
        if os.path.exists(links_fn):
            ...
            exch_to_2d_link = np.loadtxt(
                os.path.join(path, "exch_to_2d_link.csv"),
    ...
    def group_boundary_links_bnd(self, bnds):
        ...
        for rec_i, bnd in enumerate(bnds):
            ...
            for bc_elt in bnd[1]["link"]:
                ...
                if gbl["id"][link0] >= 0:
                    print(
                        "WARNING: multiple id values map to the same link (%s)" % bnd[0]
                    )
                gbl["id"][link0] = rec_i
        ...
        if n_bc_links != n_grouped_links:
            self.log.warning(
                "labeling boundary links: %d expected bc links != %d that got names from bnd"
...
class DwaqAggregator(Hydro):
    ...
    def dfm_map_file(self, p):
        ...
        map_fn = self.hydro_in.hyd_path.replace("DFM_DELWAQ_", "DFM_OUTPUT_").replace(
            ".hyd", "_map.nc"
        )
        if os.path.exists(map_fn):
            ...
            self.log.warning(
                "Tried to find DFM map output at %s, but it wasn't there" % map_fn
            )
            return None
        ...
        if p not in self._flowgeoms_xr:
            ...
            if self.flowgeom_use_dfm_map:
                if p == 0:
                    ...
                    msg = self.log.debug
                msg(
                    "[proc=%d] Checking to see if a DFM map file can be used instead of flowgeom"
                    % p
                )
            ...
            if ("NetNode_x" in ds) and ("NetElemNode" in ds["NetNode_x"].dims):
                self.log.info(
                    "Trying to triage bad dimensions in NetCDF (probably ddcouplefm output)"
                )
            ...
            if self.nprocs == 1:
                ...
                if "FlowElemGlobalNr" not in ds:
                    self.log.info(
                        "Synthesizing multi domain data for single domain run"
                    )
                    ds["FlowElemGlobalNr"] = (elem_dim,), 1 + np.arange(
                        n_elem, dtype=np.int32
                    )
            ...
            if "FlowLink" not in ds:
                ...
                # edges.
                self.flowlink_to_edge = np.nonzero((edge_type == 1) | (edge_type == 2))[
                    0
                ]
                n_flowlinks = len(self.flowlink_to_edge)

                # Fill in FlowLink-related fields --
                ds["FlowLink_xu"] = (link_dim,), ds.mesh2d_edge_x.values[
                    self.flowlink_to_edge
                ]
                ds["FlowLink_yu"] = (link_dim,), ds.mesh2d_edge_y.values[
                    self.flowlink_to_edge
                ]
                ...
                # minor sanity check
                assert np.all(
                    FlowLink[:, 1] >= start_index
                ), "Bad assumption on boundary links"
                ...
                FlowLink[bc_links, 0] = (
                    ds.dims[face_dim] + 1 + np.arange(bc_links.sum())
                )

                assert np.all(
                    FlowLink[:, 0] >= start_index
                ), "Missed some boundary links??"
                ...
                print(
                    "FlowLink near end of open_flowgeom_ds(%d), id(ds)=%s" % (p, id(ds))
                )
                print(ds.FlowLink)
                ...
                if "FlowLinkDomain" not in ds:
                    ...
                    ds["FlowLinkDomain"] = (link_dim,), ds["FlowElemDomain"].values[
                        FlowLink[:, 1] - start_index
                    ]
    ...
    seg_local_dtype = [
        ("agg", "i4"),  # aggregated segment it maps to, even if ghost
        ("ghost", "i1"),  # whether this is a ghost segment
    ]
    exch_local_dtype = [
        ("agg", "i4"),  # the aggregated exchange it maps to
    ...
    def find_maxima(self):
        ...
        for p in range(self.nprocs):
            ...
            if "nFlowLink" in nc.dims:
                ...
                # an upper bound
                self.log.warning(
                    "No Link information in waqgeom, so will estimate max by number of edges"
                )
        ...
        n_global_elements = int(
            max_gid
        )  # ids should be 1-based, so this is also the count
        ...
        self.log.debug(
            "Max 3D exchanges per processor: %d (h: %d,v: %d)"
            % (max_exch_per_proc, max_hor_exch_per_proc, max_ver_exch_per_proc)
        )
        self.log.debug(
            "Max 3D boundary exchanges per processor: %d" % (max_bc_exch_per_proc)
        )
    ...
    def init_elt_mapping(self):
        ...
        if not self.merge_only:
            agg_centers = np.array(
                [p.centroid.coords[0] for p in self.elements["poly"]]
            )
        ...
        for p in range(self.nprocs):
            ...
            if "FlowElem_zcc" in nc:
                ...
                raise Exception(
                    "flowgeom does not have cell depths.  Maybe there is DFM map output that we didn't find?"
                )
            ...
            global_ids = (nc.FlowElemGlobalNr.values - 1).astype(
                np.int32
            )  # make 0-based
            ...
            for local_i in range(n_local_elt):
                ...
                if self.merge_only:
                    ...
                    poly_i = match_center_to_agg_poly(ccx[local_i], ccy[local_i])
                    if poly_i is None:
                        continue
                ...
                self.elements[poly_i]["plan_area"] = (
                    self.elements[poly_i]["plan_area"] + areas[local_i]
                )
                # weighted by area, to be normalized below.
                self.elements[poly_i]["zcc"] = (
                    self.elements[poly_i]["zcc"] + vols[local_i]
                )

            msg = "Processor %4d: %6d 2D elements within an aggregation poly" % (
                p,
                hits,
            )
            if hits:
        ...
        if (not self.merge_only) and self.nudge_elements:
            ...
            for nudge_iter in range(5):
                ...
                for proc in range(self.nprocs):
                    ...
                    ncg = unstructured_grid.UnstructuredGrid.read_dfm(
                        nc, cleanup=True
                    )  # dfm_grid.DFMGrid(nc)
                    ...
                    for j in np.nonzero(e2c.min(axis=1) >= 0)[0]:  # only internal edges
                        ...
                        if agg_j is None:
                            ...
                            if glb_c1 in global_remapped:
                                assert (
                                    glb_c2 not in global_remapped
                                ), "Both sides of this edge have been remapped!"
                            ...
                            assert len(
                                potential_agg_c1s
                            ), "May have been too strict with intersection"
                            ...
                            if new_agg_1 is None:
                                self.log.warning(
                                    "No matches with geometry, will use topology for proc=%d loc_c1=%d"
                                    % (proc, loc_c1)
                                )
                                new_agg_1 = potential_agg_c1s[
                                    0
                                ]  # punt. Could try computing distances?
                            # assert new_agg_1 is not None

                            if len(geometric_matches) > 1:
                                self.log.warning(
                                    " Multiple choices of where to map this problem cell"
                                )
                ...
                if nudge_count == 0:
                    ...
                    self.log.info(
                        "Had to nudge %d elements - will loop again" % nudge_count
                    )
        ...
        for elt2d in self.elements[:n_elt_to_print]:
            self.log.info(
                "{:<20}   area: {:6.2f} km2  mean"
                " depth to 0 datum: {:5.2f} m".format(
                    elt2d["name"], elt2d["plan_area"] / 1e6, elt2d["zcc"]
                )
            )
    ...
    def init_seg_mapping(self):
        ...
        self.seg_local = np.zeros(
            (self.nprocs, self.max_segs_per_proc), self.seg_local_dtype
        )
    ...
    link_ownership = (
        "FlowLinkDomain"  # trust subdomain FlowLinkDomain to resolve ownership
    )
    ...
    def init_exch_mapping(self):
        ...
        self.exch_local = np.zeros(
            (self.nprocs, self.max_exch_per_proc), self.exch_local_dtype
        )
        self.exch_local["agg"][...] = -1
        self.exch_local["sgn"][
            ...
        ] = 0  # sign is separate - should be 1 or -1 (or 0 if no mapping)
        ...
        for p in range(self.nprocs):
            ...
            if self.link_ownership == "FlowLinkDomain":
                if p == 0 and "FlowLinkDomain" not in nc:
                    self.log.warning(
                        "FlowLinkDomain not found, so link ownership will use min element"
                    )
            ...
            for local_i in range(len(pointers)):
                ...
                if local_from == self.CLOSED or local_to == self.CLOSED:
                    ...
                    raise Exception(
                        "Failed assumption that boundary exchange is always first"
                    )
                elif local_from < 0:
                    ...
                    if direc == "x":  # horizontal - could be a ghost
                        ...
                        if (
                            elem_dom_id[from_2d - 1] == p
                            and elem_dom_id[to_2d - 1] == p
                        ):
                            # it's local
                            pass
                        else:
                            ...
                            if self.link_ownership == "FlowLinkDomain":
                                ...
                                if link_dom_id[link_idx] != p:
                                    ...
                                    if (
                                        ...
                                        print(
                                            "              dom=%d    dom=%d"
                                            % (elem_dom_id[from_2d], elem_dom_id[to_2d])
                                        )
                                        print(
                                            "Consider setting link_ownership='owner_of_min_elem'"
                                        )
                                        assert False
                                ...
                                # ownership.
                                if p == min(
                                    elem_dom_id[from_2d - 1], elem_dom_id[to_2d - 1]
                                ):
                ...
                if local_from < 0:
                    ...
                    agg_from, agg_to = self.seg_local["agg"][
                        p, pointers[local_i, :2] - 1
                    ]
                    ...
                    if agg_from == -1 and agg_to == -1:
                        ...
                        if 1:  # new style
                            ...
                            sel_fwd = (self.agg_exch["from"] == agg_from) & (
                                self.agg_exch["to"] == agg_to
                            )
                            sel_rev = (self.agg_exch["from"] == agg_to) & (
                                self.agg_exch["to"] == agg_from
                            )
    ...
    def reg_agg_exch(
        self, direc, agg_from, agg_to, proc, local_exch, local_from, local_to
    ):
    ...
    def reindex(self):
        ...
        exch_order = np.lexsort(
            (agg_exch["from"], agg_exch["to"], agg_exch["k"], agg_exch["direc"])
        )
        ...
        for exch in self.agg_exch:
            ...
            assert (
                exch["from"] == REINDEX
            )  # sanity, make sure nothing is getting mixed up.
            n_bdry_exch += 1
        ...
        self.log.info(
            " %5d exchanges (%d,%d,%d)"
            % (len(self.agg_exch), self.n_exch_x, self.n_exch_y, self.n_exch_z)
        )
    ...
    def elt_to_elt_length(self, elt_a, elt_b):
        ...
        if key not in self.lookup:
            if elt_a < 0:
                a_len = b_len = np.sqrt(self.elements["poly"][elt_b].area) / 2.0
            elif elt_b < 0:
                a_len = b_len = np.sqrt(self.elements["poly"][elt_a].area) / 2.0
            else:
                ...
                a_len = apoly.area / iface_length / 2.0
                b_len = bpoly.area / iface_length / 2.0
    ...
    def update_agg_to_local(self):
        ...
        for p in range(self.nprocs):
            for j_local in np.nonzero(self.exch_local["agg"][p, :] >= 0)[0]:
                ...
                self.exch_agg_to_local[j_agg].append(
                    (p, j_local, self.exch_local["sgn"][p, j_local])
                )

            seg_sel = (self.seg_local["agg"][p, :] >= 0) & (
                self.seg_local["ghost"][p, :] == 0
            )
    ...
    def add_exchange_data(self):
        ...
        for p in range(self.nprocs):
            ...
            for j in np.nonzero(self.exch_local["agg"][p, :] >= 0)[0]:
                ...
                if n_local_from > 1:
                    ...
                    count[
                        agg_j
                    ] += 999  # signal that we can't use unaggregated data here.
                    continue
                if n_local_to > 1:
                    # print("Really?")
                    count[
        ...
        if nduped > 0:
            ...
            self.log.info(
                "%d aggregated exchanges come from multiple unaggregated exchanges"
                % nduped
            )
    ...
    def get_agg_exchange(self, agg_from, agg_to, direc=None):
        ...
        if agg_from < 0:
            ...
            self.log.warning(
                "get_exchange with a negative/boundary for the *to* segment - likely incomplete coverage"
            )
            assert (
                self.agg_boundaries
            )  # if this is a problem, port the above stanza to here.
            agg_to = REINDEX
    ...
    def init_seg_matrices(self):
        ...
        for p in range(self.nprocs):
            ...
            S = sparse.coo_matrix(
                (vals, (rows, cols)), (self.n_seg, hyd.n_seg), dtype="f4"
            )
    ...
    def init_exch_matrices(self):
        ...
        for p in range(self.nprocs):
            hyd = self.open_hyd(p)
            n_exch_local = (
                hyd["number-horizontal-exchanges"] + hyd["number-vertical-exchanges"]
            )
            ...
            Eflow = sparse.coo_matrix(
                (values, (rows, cols)), (self.n_exch, n_exch_local), dtype="f4"
            )
            self.flow_matrix[p] = Eflow.tocsr()

            # areas always sum
            Earea = sparse.coo_matrix(
                (np.abs(values), (rows, cols)), (self.n_exch, n_exch_local), dtype="f4"
            )
    ...
    def init_boundary_matrices(self):
        ...
        for p in range(self.nprocs):  # following exch and area matrix code
                ...
                for local_exch_idx in local_exch_match_idxs:
                    ...
                    if seg1 < 0:
                        if not warned_internal_boundary_seg:
                            self.log.info(
                                "had to choose internal segment for agg boundary"
                            )
                        ...
                        self.log.error(
                            "Boundary exchange had local exch where neither local segment is internal"
                        )
                        assert False
            ...
            if len(rows):
                Eboundary = sparse.coo_matrix(
                    (vals, (rows, cols)), (self.n_boundaries, n_seg_local), dtype="f4"
                )
                ...
                if n_local_bc_exch:
                    print(
                        "WARNING: a priori test showed that there should be local bc exchanges!"
                    )
                    print("  Processor: %d" % p)
    ...
    def volumes(self, t_sec, explicit=False):
        if not explicit:  # much faster on dense outputs
            return self.segment_aggregator(
                t_sec=t_sec, seg_fn=lambda _: 1.0, normalize=False
            )
        else:
    ...
    # worked.
    exch_area_min = 1.0
    exch_z_area_constant = (
        True  # if true, force all segment in a column to have same plan area.
    )
    ...
    def segment_aggregator(
        self, t_sec, seg_fn, normalize=True, min_volume=0.00001, nan_method="pass"
    ):
        """
    ...
    def seg_func(self, t_sec=None, label=None, param_name=None):
        ...
        def f_param(t, param_name=param_name):
            per_proc = (
                lambda proc: self.open_hyd(proc)
                .parameters(force=False)[param_name]
                .evaluate(t=t)
                .data
            )
    ...
    def vert_diffs(self, t_sec):
        ...
        diffs = self.segment_aggregator(
            t_sec, lambda proc: self.open_hyd(proc).vert_diffs(t_sec), normalize=True
        )
    ...
    def planform_areas(self):
        ...
        if mode == "constant":
            ...
            if np.any(missing):
                self.log.warning(
                    "Some segments have no vertical exchanges - will revert to element area"
                )
            ...
            if mode == "lazy":
                ...
                return ParameterSpatioTemporal(
                    func_t=planform_area_func, times=self.t_secs, hydro=self
                )
    ...
    def depths(self):
        ...
        def clean_depth(data):
            ...
            if np.any(sel):
                self.log.warning(
                    "Depths: %d steps*segments with invalid depth" % (np.sum(sel))
                )
            data[sel] = 0
        ...
        if mode == "lazy":
            ...
            return ParameterSpatioTemporal(
                times=self.t_secs, func_t=depth_func, hydro=self
            )
    ...
    def add_parameters(self, hparams):
        ...
        for pname in hyd0_params:
            if pname.lower() in ["surf", "bottomdept"]:
                self.log.info(
                    "Original hydro has parameter %s, but it will not be copied to aggregated"
                    % pname
                )
                continue
            ...
            hparams[pname] = ParameterSpatioTemporal(
                func_t=self.seg_func(param_name=pname),
    ...
    def get_geom(self):
        ...
        ds["FlowElem_zcc"] = xr.DataArray(
            self.elements["zcc"],
            dims=["nFlowElem"],
            attrs=dict(
                long_name=(
                    "Flow element average" " bottom level (average of all corners)"
                ),
                positive="down",
                mesh="FlowMesh",
                location="face",
        ...
        ds["FlowElem_bac"] = xr.DataArray(
            self.elements["plan_area"],
            dims=["nFlowElem"],
            attrs=dict(
        ...
        poly_points = [np.array(p.exterior.coords)[:-1] for p in self.elements["poly"]]
        ...
        ds["FlowElemContour_x"] = xr.DataArray(
            packed[..., 0],
        ...
        ds["FlowElemContour_y"] = xr.DataArray(
            packed[..., 1],
            dims=["nFlowElem", "nFlowElemContourPts"],
            attrs=dict(
                units="m",
        ...
        links = np.array([self.agg_exch["from"][sel], self.agg_exch["to"][sel]]).T
        bc = links < 0
        self.infer_2d_elements()
        ...
        # 1-based
        ds["FlowLink"] = xr.DataArray(
            links + 1,
            dims=["nFlowLink", "nFlowLinkPts"],
            attrs=dict(long_name="link/interface between two flow elements"),
        )

        ds["FlowLinkType"] = xr.DataArray(
            2 * np.ones(len(links)),
            dims=["nFlowLink"],
            attrs=dict(
        ...
        ds["FlowElemDomain"] = xr.DataArray(
            np.zeros(len(self.elements), "i2"),
            dims=["nFlowElem"],
            attrs=dict(long_name="Domain number of flow element"),
        )

        ds["FlowLinkDomain"] = xr.DataArray(
            np.zeros(len(links), "i2"),
            dims=["nFlowLink"],
            attrs=dict(long_name="Domain number of flow link"),
        )
        ds["FlowElemGlobalNr"] = xr.DataArray(
            1 + np.arange(len(self.elements)),
            dims=["nFlowElem"],
        ...
        ds["FlowElemContour_node"] = xr.DataArray(
            nodes,
            dims=["nFlowElem", "nFlowElemContourPts"],
            attrs=dict(
        ...
        # Edges
        points = np.array([ds.Node_x.values, ds.Node_y.values]).T
        cells = np.array(ds.FlowElemContour_node.values)
        ug = unstructured_grid.UnstructuredGrid(points=points, cells=cells)
        ug.make_edges_from_cells()

        # Note that this isn't going to follow any particular ordering
        ds["FlowEdge_node"] = xr.DataArray(
            ug.edges["nodes"],
            dims=["nFlowEdge", "nEdgePts"],
            attrs=dict(
        ...
        if layers in sub:
            ...
            attrs = dict(
                standard_name="ocean_zlevel_coordinate",
            ...
            ds[layers] = xr.DataArray(sub[layers].values, dims=[layers], attrs=attrs)
            # this syntax works better:
            if layer_bnds in sub:
                ds[layer_bnds] = ([layers, "d2"], sub[layer_bnds].values.copy())

        ds["FlowMesh"] = xr.DataArray(
            1,
            attrs=dict(
        ...
        for proc in range(self.nprocs):
            ...
            for sub_bnd in sub_bnds:
                ...
                for seg in segs:
                    ...
                    if np.all(x[0] == x[1]):
                        self.log.warning(
                            "Vertical/source entry - this is probably outdated code"
                        )
                        ...
                        link1, fromto = np.nonzero(
                            sub_geom.FlowLink.values == bc_elt_pos + 1
                        )
                        if len(link1) != 1:
                            print(
                                "Trouble finding the FlowLink which goes with this [src] bc element"
                            )
                            print("  link1: %s" % link1)
                        link1 = link1[0]
                        fromto = fromto[0]
                        ...
                        assert (
                            sub_geom.FlowLink.values[link1, 1 - fromto] - 1
                            == elt_inside
                        ), "Sanity comparison on source element failed"
                        ...
                        if len(link1) != 1:
                            print(
                                "Trouble finding the FlowLink which goes with this bc element"
                            )
                            print("  link1: %s" % link1)
                        link1 = link1[0]
                        fromto = fromto[0]
                    ...
                    # additional matches.
                    bc_flow_links = np.nonzero(
                        (flow_link_inside == elt_agg + 1) & (flow_link_outside < 0)
                    )[0]
                    ...
                    if 1:  # trying
                        ...
                        assert (
                            sub_hyd.links[link1, 1 - fromto] == elt_inside
                        ), "links in flowgeom may not be same as sub_hyd.links"
                        assert (
                            not self.agg_boundaries
                        ), "This code only makes sense with unaggregated boundaries"
                        # which exchanges are involved in the unaggregated grid?
                        sub_link_exchs = np.nonzero(
                            sub_hyd.exch_to_2d_link["link"] == link1
                        )[0]
                        ...
                        assert (
                            agg_link["sgn"] == 1
                        ), "Expecting no sign flips for bc links"
                        bc_flow_link = agg_link["link"]
                        assert (
                            bc_flow_link in bc_flow_links
                        ), "Validation against old approach failed"
                        bc_flow_links = np.array([bc_flow_link])

                    # Used to assert this was a unique match
                    if (
                        1
                    ):  # this is probably better -- at least when not aggregating boundaries
                        ...
                        if len(bc_flow_links) != 1:
                            ...
                            # pdb.set_trace()
                            print(
                                "Matching bc links expected 1 match, but elt_agg=%d, name=%s matched %s"
                        ...
                        for bc_flow_link in bc_flow_links:
                            ...
                            if bc_id in bc_names:
                                ...
                                print(
                                    "bc_id %d already mapped to %s, would have mapped to %s"
                            ...
                            bc_x[bc_id].append(
                                x
                            )  # Will have to deal with this x not being unique!
...
class HydroAggregator(DwaqAggregator):
    ...
    def __init__(self, hydro_in, **kwargs):
        ...
        super(HydroAggregator, self).__init__(nprocs=1, **kwargs)
        ...
        for group_id, bci in enumerate(sel_bc):
            ...
            for extra in m_groups[1:]:
                ...
                if extra["name"] != m_groups[0]["name"]:
                    self.log.warning(
                        "Not ready for aggregating boundary link groups - skipping %s"
                        % extra["name"]
                    )
            ...
            if len(m_groups):
                ...
                self.log.warning(
                    "Nobody matched to this aggregated boundary link group bci=%d" % bci
                )
    ...
    def plot_aggregation(self, ax=None):
        ...
        centers = np.array(
            [np.array(gagg.cell_polygon(c).centroid) for c in range(gagg.Ncells())]
        )
...
class HydroMultiAggregator(DwaqAggregator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.path = path
        super(HydroMultiAggregator, self).__init__(
            agg_shp=agg_shp, nprocs=nprocs, skip_load_basic=skip_load_basic, **kwargs
        )
    ...
    def open_hyd(self, p, force=False):
        ...
        if force or (p not in self._hyds):
            self._hyds[p] = HydroFiles(
                os.path.join(self.sub_dir(p), "%s_%04d.hyd" % (self.run_prefix, p))
            )
        return self._hyds[p]
    ...
    def dfm_map_file(self, p):
        ...
        map_fn = os.path.join(
            self.path,
            "DFM_OUTPUT_%s" % self.run_prefix,
            "%s_%04d_map.nc" % (self.run_prefix, p),
        )
        if os.path.exists(map_fn):
            return map_fn
        else:
    ...
    def infer_nprocs(self):
        ...
        for p in range(1 + max_nprocs):
            if not os.path.exists(self.sub_dir(p)):
                if p == 0:
                    raise WaqException(
                        "Failed to find any subdomains -- may be a serial run"
                    )
                return p
        else:
...
class FilterHydroBC(Hydro):
    ...
    def lowpass(self, data):
        if self.filter_type == "butter":
            ...
            flow_padded = np.concatenate((pad, data, pad))
            lp_flows = filters.lowpass(flow_padded, cutoff=self.lp_secs, dt=self.dt)
    ...
    def apply_filter(self):
        ...
        if self.use_memmap:
            ...
            assert isinstance(
                self.orig, HydroFiles
            ), "Memmap only works with HydroFiles"
            ...
            self.filt_volumes = np.array(
                [self.orig.volumes(t) for t in self.orig.t_secs]
            )
        ...
        for j in utils.progress(self.exchanges_to_filter()):
            ...
            tidal_volumes = np.concatenate(([0], tidal_volumes))
        ...
        if np.any(self.filt_volumes < self.min_volume):
            self.log.warning(
                "All volumes non-negative, but some below threshold of %f"
                % self.min_volume
            )
    ...
    def adjust_negative_volumes(self):
        has_negative = np.nonzero(np.any(self.filt_volumes < self.min_volume, axis=0))[
            0
        ]
        dt = np.median(np.diff(self.t_secs))
        ...
        for seg in has_negative:
            ...
            # Find a BC segment
            bc_exchs = np.nonzero(
                (self.pointers[:, 0] < 0) & (self.pointers[:, 1] == seg + 1)
            )[0]
            if len(bc_exchs) == 0:
                # will lead to a failure
                self.log.warning(
                    "Segment with negative volume has no boundary exchanges"
                )
                if self.force_min_volume:
                    V_add = self.min_volume - self.filt_volumes[:, seg].min()
                    self.log.warning(
                        "Forcing minimum volume. Adding %.3e  Original volumes max: %.3e, mean %.3e"
                        % (
                            V_add,
                            self.orig_volumes[:, seg].max(),
                            self.orig_volumes[:, seg].mean(),
                        )
                    )
                ...
                self.log.info(
                    "Segment with negative volume has multiple BC exchanges.  Choosing the first"
                )
            bc_exch = bc_exchs[0]
    ...
    def adjust_plan_areas(self):
        ...
        Vratio_filt_to_orig_2d = [
            np.bincount(groups[valid], volumes[ti, valid])
    ...
    def copy_mmap_or_delegate(self, fn_attr, mmap_attr, deleg, new_fn):
        ...
        if self.use_memmap and tmp_fn is not None:
            assert os.path.exists(tmp_fn), (
                "Hmm - should have memory mapped but %s is not there" % tmp_fn
            )
...
class FilterAll(FilterHydroBC):
    ...
    def adjust_plan_areas(self):
        ...
        for j in range(self.n_exch_x + self.n_exch_y):
            padded = np.concatenate((pad, self.filt_areas[:, j], pad))
        ...
        for key, param in iteritems(self.orig.parameters()):
            ...
            if key in hyd and key not in ["vertdisper"]:
                ...
                if self.filter_parameters:
                    self.log.info(
                        "  original parameter is spatiotemporal - let's FILTER"
                    )
                    ...
                    self.log.info(
                        "  original parameter is spatiotemporal - but filter_parameters=%s.  no filter"
...
def rediscretize(src_dx, src_y, n_sigma, frac_samples=None, intensive=True):
    ...
    cumul_mass = np.concatenate(([0], np.cumsum(src_y_ext)))
    frac_sum = np.concatenate(([0], src_xsum))
    if frac_samples is None:
        frac_samples = np.linspace(0, 1, n_sigma + 1)

    dest_y = np.diff(np.interp(frac_samples, frac_sum, cumul_mass))
    if intensive:
...
class Sigmified(Hydro):
    ...
    def init_exchanges(self):
        ...
        assert (
            self.hydro_z.pointers[
                (self.hydro_z.n_exch_x + self.hydro_z.n_exch_y) :, :2
            ].min()
            > 0
        )
    ...
    def init_lengths(self):
        ...
        lengths[: self.n_exch_x + self.n_exch_y, :] = np.tile(
            self.hydro_z.exchange_lengths[: self.n_2d_links, :], (self.n_sigma, 1)
        )
        lengths[self.n_exch_x + self.n_exch_y :, :] = 1.0 / self.n_sigma
    ...
    def areas(self, t):
        ...
        # horizontal first:
        # sum per 2d link:
        Af_per_link_z = np.bincount(
            self.hydro_z.exch_to_2d_link["link"], weights=Af_x_z
        )
        # then evenly divide by n_sigma
        Af_sig[: self.n_exch_x + self.n_exch_y] = (
            Af_per_link_z[self.exch_to_2d_link["link"]] / self.n_sigma
        )
    ...
    def volumes(self, t_sec):
        ...
        seg_to_elt = (
            self.hydro_z.seg_to_2d_element.copy()
        )  # negative for below-bed segments
    ...
    def flows(self, t):
        ...
        for link_i, link in enumerate(self.links):
            exch_sel_z = self.link_to_exch_z[
                link_i
            ]  # np.nonzero( self.hydro_z.exch_to_2d_link['link']==link_i )[0]
            exch_sel_sig = self.link_to_exch_sig[
                link_i
            ]  # np.nonzero( self.exch_to_2d_link['link']==link_i )[0]
            ...
            Q_sig[exch_sel_sig] = rediscretize(
                areas_z, q_z, self.n_sigma, frac_samples=frac_samples, intensive=False
            )
        ...
        if 0:  # reference implementation
            ...
            Vpred -= dt * np.bincount(
                seg_from[seg_from_valid],
                weights=Q_sig[: self.n_exch_x][seg_from_valid],
                minlength=len(Vpred),
            )

            Vpred += dt * np.bincount(
                seg_to, weights=Q_sig[: self.n_exch_x], minlength=len(Vpred)
            )
        ...
        if rel_err.max() >= rel_err_thresh:
            self.log.warning(
                "Vertical fluxes still had relative errors up to %.2f%%"
                % (100 * rel_err.max())
            )
            ...
            for bad_elt in bad_elts:
                ...
                exchs = (
                    ...
                    + self.n_exch_y
                    + bad_elt
                    + np.arange(self.n_sigma - 1) * self.n_2d_elements
                )
                assert np.all(
                    self.seg_to_2d_element[poi0[exchs, :2]] == bad_elt
                )  # sanity check
        ...
        if Vpred_min < 0.0:
            ...
            self.log.warning(
                "Some predicted volumes are negative, for min(dz)=%f at seg %d"
                % (dz_pred.min(), np.argmin(dz_pred))
            )
            ...
            self.log.warning(
                " Apparently the erros in the z-layer model are at least as bad"
            )
    ...
    def segment_interpolator(self, t_sec, scalar_z):
        ...
        if abs(orig_t_sec - t_sec) > 1.5 * dt:
            self.log.warning(
                "segment_interpolator: requested time and my time off by %.2f steps"
                % ((orig_t_sec - t_sec) / dt)
            )
        ...
        for elt in range(self.n_2d_elements):
            ...
            seg_sel_z = elt_to_seg_z[
                elt
            ]  # np.nonzero( self.hydro_z.seg_to_2d_element==elt )[0]
            seg_v_z = vol_z[seg_sel_z]
            seg_scal_z = scalar_z[seg_sel_z]

            seg_scal_sig = rediscretize(
                seg_v_z,
                seg_scal_z,
                self.n_sigma,
                intensive=True,
    ...
    def param_z_interpolate(self, param_z):
        if isinstance(param_z, ParameterSpatioTemporal):
            ...
            def interped(t_sec, self=self, param_z=param_z):
                return self.segment_interpolator(
                    t_sec=t_sec, scalar_z=param_z.evaluate(t=t_sec).data
                )

            # this had been using self.t_secs, but for temperature, and probably in general,
            # we should respect the original times
            return ParameterSpatioTemporal(
                func_t=interped, times=param_z.times, hydro=self
            )
        elif isinstance(param_z, ParameterSpatial):
            # interpolate based on the initial volumes
            seg_sig = self.segment_interpolator(
                t_sec=self.t_secs[0], scalar_z=param_z.data
            )
    ...
    def infer_2d_elements(self):
        ...
        if self.seg_to_2d_element is None:
            ...
            self.seg_to_2d_element = np.tile(
                np.arange(self.n_2d_elements), self.n_sigma
            )
    ...
    def get_geom(self):
        ...
        ds["nFlowMesh_layers"] = xr.DataArray(
            centers,
        ...
        # order correct?
        bounds_d2 = np.array([bounds[:-1], bounds[1:]]).T
...
class ModelForcing(object):
            ...
            if write_supporting:
                if self.data_file_subdirectory is not None:
                    dir_name = os.path.join(
                        self.scenario.base_path, self.data_file_subdirectory
                    )
                    rel_path = os.path.join(
                        self.data_file_subdirectory, self.supporting_file
                    )
                else:
                ...
                fn = os.path.join(dir_name, self.supporting_file)
    ...
    def render_data_to_text(self):
        ...
        if isinstance(data, pd.Series):
            ...
            data = (utils.to_dnum(data.index.values), data.values)
...
class BoundaryCondition(ModelForcing):
    ...
    def __init__(self, boundaries, substances, data):
        ...
        super(BoundaryCondition, self).__init__(
            items=boundaries, substances=substances, data=data
        )
...
class Discharge(object):
    ...
    def update_fields(self):
        ...
        if self.seg_id is None:
            ...
            self.seg_id = self.scenario.hydro.segment_select(
                element=self.element, k=self.k
            )[0]
    ...
    def text(self):
        ...
        return fmt.format(seg=self.seg_id + 1, self=self)  # to 1-based
...
class Load(ModelForcing):
    ...
    def __init__(self, discharges, substances, data):
        """
        see ModelForcing or BoundaryCondition docstring
        """
        super(Load, self).__init__(items=discharges, substances=substances, data=data)
...
class ParameterConstant(Parameter):
    ...
    def __init__(self, value, scenario=None, name=None, hydro=None):
        super(ParameterConstant, self).__init__(
            name=name, scenario=scenario, hydro=hydro
        )
...
class ParameterSpatial(Parameter):
    ...
    def __init__(
        ...
        super(ParameterSpatial, self).__init__(
            name=name, scenario=scenario, hydro=hydro
        )
    ...
    def text(self, write_supporting=True):
        ...
        return ("PARAMETERS '{self.name}' " + grid_text + " " + data_text).format(
            self=self
        )

    def write_supporting(self):
        with open(
            os.path.join(self.scenario.base_path, self.supporting_file), "wb"
        ) as fp:
...
class ParameterTemporal(Parameter):
    ...
    def __init__(self, times, values, scenario=None, name=None, hydro=None):
        ...
        """
        super(ParameterTemporal, self).__init__(
            name=name, scenario=scenario, hydro=hydro
        )
        self.times = times
        self.values = values
    ...
    def text(self, write_supporting=False):
        ...
        if self.separate_data_file:
            if write_supporting:
                with open(
                    os.path.join(self.scenario.base_path, self.supporting_file), "wt"
                ) as fp:
                    fp.write(data)
...
class ParameterSpatioTemporal(Parameter):
    ...
    def __init__(
        ...
        super(ParameterSpatioTemporal, self).__init__(
            name=name, scenario=scenario, hydro=hydro
        )
        self.func_t = func_t
        self._times = times
    ...
    def text(self, write_supporting=True):
        ...
        if self.grid_name is None:
            return (
                "SEG_FUNCTIONS '{self.name}' {self.interpolation}"
                " ALL BINARY_FILE '{self.supporting_file}'"
            ).format(self=self)
        else:
            return (
                "SEG_FUNCTIONS '{self.name}' {self.interpolation}"
                " INPUTGRID '{self.grid_name}' BINARY_FILE '{self.supporting_file}'"
            ).format(self=self)
    ...
    def write_supporting(self):
        ...
        start_i, stop_i = np.searchsorted(
            datetimes, [self.scenario.start_time, self.scenario.stop_time]
        )
        start_i = max(0, start_i - 1)
        stop_i = min(stop_i + 1, len(tidxs))
        tidxs = tidxs[start_i:stop_i]
        msg = "write_supporting: writing %d of %d timesteps." % (
            len(tidxs),
            len(self.times),
        )
    ...
    def write_supporting_loop(self, tidxs, fp, name="n/a"):
        ...
        for tidx in utils.progress(tidxs, msg=name + ": %s"):
            ...
            if self.values is not None:
                ...
                if (len(values) == self.hydro.n_2d_elements) and (
                    len(values) != self.hydro.n_seg
                ):
                    if not self.warned_2d_to_3d:
                        self.scenario.log.warning(
                            "Padding parameter %s from 2D to 3D" % self.safe_name
                        )
    ...
    def load_from_segment_file(self, convert_time=False):
        ...
        self._mmap_data = np.memmap(
            self.seg_func_file,
            dtype=[("t", np.int32), ("value", np.float32, self.n_seg)],
            mode="r",
        )
        self._times = self._mmap_data["t"]
        if convert_time:
            self._times = self._times * np.timedelta64(1, "s") + utils.to_dt64(
                self.hydro.time0
            )
    ...
    def evaluate(self, **kws):
        ...
        if "t" in kws:
            ...
            param = ParameterTemporal(
                times=self.times.copy(), values=self.values[:, seg].copy()
            )
    ...
    def lowpass(self, lp_secs, volume_threshold=1.0, pad_mode="constant"):
        ...
        for seg in range(self.n_seg):
            ...
            padded = np.concatenate((prepad, values[:, seg], postpad))
            if volume_threshold > 0:
            ...
            if np.isnan(padded[0]):
                ...
                lp_values = filters.lowpass(padded, cutoff=lp_secs, dt=dt)
...
class NamedObjects(OrderedDict):
    ...
    def __init__(self, sort_key=None, cast_value=lambda x: x, **kw):
        """
...
class DispArray(object):
    ...
    def text(self, write_supporting=True):
        if write_supporting:
            self.write_supporting()
        return "XXX PARAMETERS '{self.name}' ALL BINARY_FILE '{self.supporting_file}'".format(
            self=self
        )
...
class Scenario(scriptable.Scriptable):
    ...
    n_bottom_layers = (
        0  # redundant with bottom_layers, but we have vestiges of old-style and delwaqg
    )
    water_grid = (
        None  # if using old style layered bed then this should probably be 'water-grid'
    )
    ...
    def multigrid_block(self):
        ...
        if self.hydro.n_seg != self.hydro.n_2d_elements * (kmax + 1):
            raise Exception(
                "You probably mean to be running with segment-dense hydrodynamics"
            )
...
0 ; no lags
    ...
    def set_deep_sediment_bc_from_ic(self, subs, subset=[]):
        ...
        """
        assert (
            self.bottom_grid is not None
        ), "Trying to set deep BCs, but bottom_grid is not configured"

        self.hydro.infer_2d_elements()
        n_2d = self.hydro.n_2d_elements

        for scalar in subset:
            if scalar not in subs:
                continue
            sub = subs[scalar]
            ...
            if np.isscalar(values):
                ...
                self.src_tags.append(
                    dict(tracer=scalar, items=[self.bottom_bc_prefix], value=values)
                )
            else:
                ...
                for item, value in zip(deep_bcs, values):
                    self.src_tags.append(
                        dict(tracer=scalar, items=[item["id"].decode()], value=value)
                    )
        ...
        for other in self.discharges:
            if other.load_id == disch.load_id:
                if on_exists == "exception":
                    raise Exception(
                        "Discharge with id='%s' already exists" % other.load_id
                    )
                elif on_exists == "ignore":
                    self.log.info(
                        "Discharge id='%s' exists - skipping duplicate" % other.load_id
                    )
                    return other
                else:
                    assert False
    ...
    def add_monitor_from_shp(self, shp_fn, naming="elt_layer", point_layers=True):
        ...
        for i, rec in enumerate(locations):
            ...
            if point_layers and (geom.type == "Point"):
                ...
                if elt is None:
                    self.log.warning(
                        "Monitor point %s was not found inside a cell" % xy
                    )
    ...
    def add_monitor_for_geometry(self, name, geom):
        ...
        # extend to segments:
        seg_sel = elt_sel[self.hydro.seg_to_2d_element] & (
            self.hydro.seg_to_2d_element >= 0
        )
        ...
        for i, rec in enumerate(locations):
            ...
            if geom.type == "LineString":
                ...
                if naming == "count":
                    ...
                    name = rec[naming]
                exchs = self.hydro.path_to_transect_exchanges(
                    xy, on_boundary=on_boundary, on_edge=on_edge
                )
                new_transects.append((name, exchs))
            else:
                self.log.warning("Not ready to handle geometry type %s" % geom.type)
        self.log.info(
            "Added %d monitored transects from %s" % (len(new_transects), shp_fn)
        )
    ...
    def add_area_boundary_transects(self, exclude="dummy"):
        ...
        exch_areas_tupes = set(
            [tuple(x) for x in exch_areas if x[0] != x[1] and x[0] >= 0]
        )
    ...
    def nef_history(self):
        ...
        if os.path.exists(hda):
            if nefis.nef_lib() is None:
                self.log.warning(
                    "Nefis library not configured -- will not read nef history"
                )
                return None
    ...
    def nc_map(self, nc_kwargs={}):
        ...
        try:
            ...
            if "time" in nc.variables:
                nc.time.units = "seconds since %s" % self.time0.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        finally:
            nef.close()
    ...
    def ugrid_nef(self, mode="map", nef=None, nc_kwargs={}, output_settings=None):
        ...
        try:
            if mode == "map":
                ...
                shape, dtype = nef["DELWAQ_PARAMS"].getelt(
                    "LOCATION_NAMES", shape_only=True
                )
                n_locations = shape[1]
                if n_locations > len(seg_elt):
                    self.log.info(
                        "Looks like there were %d transects, too?"
                        % (n_locations - len(seg_elt))
                    )
                elif n_locations < len(seg_elt):
                    self.log.warning(
                        "Weird - fewer output locations than anticipated! %d vs %d"
                        % (n_locations, len(seg_elt))
                    )
            else:
                assert False
            ...
            for group in nef.groups():
                for elt_name in group.cell.element_names:
                    ...
                    self.log.info(
                        "  %s.%s: %s (%s)" % (group.name, elt_name, vshape, nef_type)
                    )
            ...
            for group in nef.groups():
                ...
                for elt_name in group.cell.element_names:
                    ...
                    if subst is None:
                        self.log.info(
                            "No metadata from process library on %s" % repr(vname)
                        )
                    ...
                    for idx, name in enumerate(
                        grp_dim_names
                    ):  # okay since squeeze only does elt dims
                        if name:
                            dim_names[idx] = name
                    ...
                    if (
                        ...
                        group.name == "DELWAQ_RESULTS" and n_locations in squeeze_shape
                    ):
                    ...
                    if vmap is not None:
                        ...
                        if "quickplot_compat" in output_settings:
                            ...
                            self.log.info(
                                "Dropping location for quickplot compatibility"
                            )
                        else:
                    ...
                    if "time" in dim_names:
                        ...
                        for ti in range(proj_shape[0]):
                            ...
                            if (time.time() - t_last > 2) or (ti + 1 == proj_shape[0]):
                                t_last = time.time()
                                self.log.info(
                                    "  time step %d / %d" % (ti, proj_shape[0])
                                )
                                self.log.info(
                                    "  time for group so far: %fs"
                                    % (t_last - total_tic)
                                )
        ...
        if 1:
            if mode == "map":
                etavar = nc.createVariable(
                    "eta", np.float32, ["time", "nFlowElem"], zlib=True
                )
                ...
                if "totaldepth" in nc.variables and "nSegment" in nc.dimensions:
                    etavar = nc.createVariable(
                        "eta", np.float32, ["time", "nSegment"], zlib=True
                    )
                    etavar.standard_name = "sea_surface_height_above_geoid"
        ...
        if 1:  # extra mapping info for history files
            ...
            if mode == "history":
                ...
                if flowgeom:
                    ...
                    nc["element_x"]["nSegment"] = np.concatenate(
                        (xcc[seg_elt], np.nan * pad)
                    )
                    nc["element_y"]["nSegment"] = np.concatenate(
                        (ycc[seg_elt], np.nan * pad)
                    )
        ...
        if (mode == "map") and ("quickplot_compat" in output_settings):
            ...
            if "FlowElem_zcc" in nc and "Node_z" not in nc:
                ...
                nc.Node_z.long_name = (
                    "Bottom level at net nodes (flow element's corners)"
                )
            ...
            for k, v in six.iteritems(nc.variables):
                if "location" in v.ncattrs():
                    self.log.info(
                        "Stripping location attribute from %s for quickplot compatibility"
                        % k
                    )
                ...
                if (
                    "standard_name" in v.ncattrs()
                    and v.standard_name == "ocean_sigma_coordinate"
                ):
                    v.formula_terms = (
                        "sigma: nFlowMesh_layers eta: eta depth: FlowElem_bl"
                    )
    ...
    def cmd_write_bloominp(self):
        ...
        if not os.path.exists(self.original_bloominp_path):
            # pdb.set_trace()
            self.log.warning(
                "BLOOM not found (%s)! Tread carefully" % self.original_bloominp_path
            )
            return
    ...
    def write_binary_map_nc(self):
        ...
        dio.map_add_z_coordinate(
            map_ds, total_depth="TotalDepth", coord_type="sigma", layer_dim="layer"
        )
    ...
    def cmd_delwaq1(self):
        ...
        cmd = [self.delwaq1_path, "-waq", bloom_part, "-p", self.proc_path]
        ...
        try:
            ret = subprocess.check_output(
                cmd, shell=False, cwd=self.base_path, stderr=subprocess.STDOUT
            )
        ...
        if nerrors > 0 or nwarnings > 0:
            print(ret)
            raise WaqException(
                "delwaq1 found %d errors and %d warnings" % (nerrors, nwarnings)
            )
    ...
    def cmd_delwaq2(self, output_filename=None, delwaq2name=None):
        ...
        with open(output_filename, "wt") as fp_out:
            ...
            tail = MonTail(
                os.path.join(self.base_path, self.name + ".mon"),
                log=self.log,
            ...
            try:
                try:
                    ...
                    raise WaqException(
                        "delwaq2 exited with an error code - check %s" % output_filename
                    )
            finally:
                tail.stop()
        ...
        with open(output_filename, "rt") as fp:
            for line in fp:
                if "Stopping the program" in line:
                    raise WaqException(
                        "Delwaq2 stopped early - check %s" % output_filename
                    )
        self.log.info("Done")
...
class InpFile(object):
    ...
    def default_filename(self):
        return os.path.join(self.scenario.base_path, self.scenario.name + ".inp")
...
"""
    ...
    def fmt_time0(self):
        return self.scenario.fmt_datetime(
            self.scenario.time0
        )  # e.g. "1990.08.05 00:00:00"
    ...
    def fmt_scu(self):
        ...
        return "{:8}s".format(
            self.scenario.scu.seconds + 86400 * self.scenario.scu.days
        )

    def fmt_substance_names(self):
        return "\n".join(
            [
                "    {:4}  '{}'".format(1 + idx, s.name)
...
"""
        ...
        return block01.format(
            self=self,
            time0=self.fmt_time0(),
            scu=self.fmt_scu(),
    ...
    def text_map_start_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.map_start_time or self.scenario.start_time
        )

    @property
    def text_map_stop_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.map_stop_time or self.scenario.stop_time
        )

    @property
    def text_hist_start_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.hist_start_time or self.scenario.start_time
        )

    @property
    def text_hist_stop_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.hist_stop_time or self.scenario.stop_time
        )

    @property
    def text_mon_start_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.mon_start_time or self.scenario.start_time
        )

    @property
    def text_mon_stop_time(self):
        return self.scenario.fmt_datetime(
            self.scenario.mon_stop_time or self.scenario.stop_time
        )
...
""".format(
                n_points=len(self.scenario.monitor_areas)
            )
        ]
        ...
        for name, segs in self.scenario.monitor_areas:
            ...
            lines.append(
                "'{}' {} {}".format(
                    name,
                    len(segs),
                    textwrap.fill(" ".join(["%d" % (i + 1) for i in segs]), width=132),
                )
            )
    ...
    def text_monitor_transects(self):
        ...
        if len(self.scenario.monitor_areas) == 0:
            ...
            raise Exception(
                "DWAQ may not output transects when there are no monitor areas"
            )

        lines = [
            " 1   ; monitoring transects used",
...
"""
    ...
    def dispersions_declaration(self):
        ...
        if len(self.scenario.dispersions):
            subs = list(self.scenario.substances.keys())[
                : self.scenario.n_active_substances
            ]
            ...
            lines.append(
                " ".join(["%d" % assign for assign in assignments])
                + " ; assign to substances"
            )
        else:
    ...
    def dispersions_definition(self):
        if len(self.scenario.dispersions) == 0:
            ...
            if unsteady.min() != unsteady.max():
                raise Exception(
                    "Multiple dispersion arrays specified, with some "
            ...
            if unsteady:
                ...
                if len(disps) > 1:
                    self.log.warning(
                        "Brave soul! Multiple dispersion arrays have not been tested"
                    )
                ...
                for disp in disps:
                    ...
                    lines.append(
                        "2 ; this block -- information at breakpoints with linear time interpolation"
                    )
                    lines.append(
                        "%d ; number of items, equals number of exchanges"
                        % hydro.n_exch
                    )
...
""".format(
    ...
    def text_block08_new(self):
        ...
        defaults = "\n".join(
            [
                " {:e} ; {}".format(s.initial.default, s.name)
        ...
        lines = [
            ...
            " INITIALS ",
            # list of substances
            " ".join([" {} ".format(s) for s in self.scenario.substances.values()]),
        ]
        # pick up here.
    ...
    def text_block08_old(self):
        ...
        if self.scenario.restart_file is not None:
            ...
            if res_file not in fns:
                if len(fns) == 1:
                    self.scenario.log.warning(
                        "Will copy corrupt restart file to specified restart file"
                    )
                    shutil.copyfile(fns[0], res_file)
                elif len(fns) == 0:
                    raise Exception(
                        "Restart file '%s' not found (including corrupted variants)"
                        % res_file
                    )
                else:
                    raise Exception(
                        "Restart file '%s' not found, but multiple corrupt alternatives"
                        % res_file
                    )

            lines += [" 0 ; Restart file", "%s ; Binary restart file" % res_file]
        else:
        ...
        lines += [";", " #8 ; delimiter for the eighth block"]
    ...
    def text_ic_old_spatially_varying(self):
        ...
        if self.scenario.bottom_grid is not None:
            n_seg_sediment = (
                self.scenario.n_bottom_layers * self.scenario.hydro.n_2d_elements
            )
        else:
            n_seg_sediment = (
                0  # might have bed state in DelwaqG, but that is handled elsewhere.
            )
        ...
        for s in subs:
            if s.initial.seg_values is not None:
                ...
                with open(
                    os.path.join(self.scenario.base_path, supporting_fn), "wt"
                ) as fp:
                    ...
                    if len(seg_values) < n_seg_total:
                        ...
                        seg_values = np.concatenate(
                            [seg_values, np.zeros(n_seg_total - len(seg_values))]
                        )
                    supp_lines = [" %f" % val for val in seg_values]
                    fp.write("\n".join(supp_lines))

                lines.append(
                    "INCLUDE '%s' ; spatially varying for %s" % (supporting_fn, s.name)
                )
            else:
                lines.append(
                    " %d*%f ; default for %s" % (n_seg_total, s.initial.default, s.name)
                )
        return lines
    ...
    def text_block09(self):
        ...
        outputs = [
            self.scenario.mon_output,
            self.scenario.grid_output,
            self.scenario.hist_output,
            self.scenario.map_output,
        ]
        for spec, output_type in zip(outputs, [MONITOR, GRID, HIS, MAP]):
            ...
            if output_type in [MONITOR, HIS]:
                ...
                weighing = ""
            vnames = [
                "  '{name}' {weighing}".format(name=name, weighing=weighing)
                for name in spec
                if name != DEFAULT
            ]
            if len(spec) == 0:
                ...
                if vnames:
                    lines.append(
                        " 2 ; all substances and extra output, {}".format(output_type)
                    )
        ...
        if 1:  # allow formats to be configurable, too
            lines.append(
                "%d ; binary history file"
                % int("binary" in self.scenario.history_formats)
            )
            lines.append(
                "%d ; binary map file    " % int("binary" in self.scenario.map_formats)
            )
            lines.append(
                "%d ; nefis history file "
                % int("nefis" in self.scenario.history_formats)
            )
            lines.append(
                "%d ; nefis map file     " % int("nefis" in self.scenario.map_formats)
            )
        ...
        lines += ["; ", " #9 ; delimiter for the ninth block"]

        return "\n".join(lines)

    def text_block10(self):
        lines = [";", "; Statistical output"]

        for sub in self.scenario.stat_output:
            lines += [
                "output-operation 'STADAY'" "  substance '%s'" % sub,
                "  suffix    ' '",
...
class WaqModelBase(scriptable.Scriptable):
    ...
    def init_hydro_parameters(self):
        ...
        if self.hydro:
            ...
            self.log.warning("Why requesting hydro parameters with no hydro?")
            raise Exception(
                "Tried to initialize hydro parameters without a hydro instance"
            )
    ...
    def init_bcs(self):
        ...
        if 0:
            ...
            for src_tag in self.src_tags:
                # conc. defaults to 1.0
                self.add_bc(
                    src_tag["items"], src_tag["tracer"], src_tag.get("value", 1.0)
                )
    ...
    def add_monitor_from_shp(self, shp_fn, naming="elt_layer", point_layers=True):
        raise Exception(
            "add_monitor_from_shp is sub-class dependent, and apparently not implemented"
        )

    def add_transect(self, name, exchanges):
        raise Exception(
            "add_transect is sub-class dependent, and apparently not implemented"
        )

    def add_area_boundary_transects(self, exclude="dummy"):
        raise Exception(
            "add_are_boundary_transects is sub-class dependent, and apparently not implemented"
        )
    ...
    def add_transects_from_shp(
        ...
        raise Exception(
            "add_transects_from_shp is sub-class dependent, and apparently not implemented"
        )

    def add_monitor_for_geometry(self, name, geom):
        raise Exception(
            "add_monitor_for_geometry is sub-class dependent, and apparently not implemented"
        )
...
0 ; no lags
    ...
    def add_monitor_for_geometry(self, name, geom):
        ...
        #      the polygon.  Don't use full polygon, because that will pick up adjacent
        #      cells that share an edge.
        elt_sel = g.select_cells_intersecting(
            geom, by_center="representative"
        )  # few seconds
    ...
    def add_transects_from_shp(
        ...
        if on_edge:
            ...
            #  arises.
            self.log.warning(
                "add_transects_from_shp: on_edge was set. In WaqModel this does nothing"
            )
    ...
    def write_binary_map_nc(self):
        ...
        if os.path.exists(out_fn):
            if self.overwrite:
                ...
                raise Exception(
                    "While writing binary map nc to %s: file exists, overwrite is False"
                    % out_fn
                )
    ...
    def cmd_delwaq1(self):
        ...
        if nerrors > 0 or nwarnings > 0:
            ret = (
                ret.decode()
            )  # sometimes this comes back as bytes, and prints better if decoded
            print(ret)
            raise WaqException(
                "delwaq1 found %d errors and %d warnings" % (nerrors, nwarnings)
            )
    ...
    def bc_name_to_segment(self, name, layer="bed"):
        ...
        group_boundary_links = (
            self.hydro.group_boundary_links()
        )  # read boundary location and name information from DFM .bnd file

        g = self.hydro.grid()
        self.hydro.infer_2d_elements()

        bdn = np.nonzero(
            group_boundary_links["name"] == k
        )  # getting index for boundary
        assert len(bdn) == 1
...
class WaqOnlineModel(WaqModelBase):
    ...
    def write_waq(self):
        ...
        self.model.mdu[
            "processes", "DtProcesses"
        ] = 300  # TODO hard-coded to match DtUser in template .mdu
        self.model.mdu[
            "processes", "ProcessFluxIntegration"
        ] = 1  # 1 = Delwaq, 2 = Dflow
        ...
        for attr in [
            ...
            if getattr(self, attr) != getattr(WaqModelBase, attr):
                self.log.info(
                    "WaqOnlineModel: '%s' has been modified, but is not [yet] handled for online coupling"
                    % attr
                )
    ...
    def fix_substance_order(self):
        ...
        waq_bcs = [
            ...
            if getattr(bc, "scalar", "_dummy_").lower() in subs
        ]
        nonwaq_bcs = [
            bc
            for bc in self.model.bcs
            if getattr(bc, "scalar", "_dummy_").lower() not in subs
        ]
        ...
        sub_ordered += [
            s for s in subs if (s not in bc_subs) and self.substances[s].active
        ]
        sub_ordered += [
            s for s in subs if (s not in bc_subs) and not self.substances[s].active
        ]
        ...
        waq_bc_order = np.argsort(
            [sub_ordered.index(bc.scalar.lower()) for bc in waq_bcs]
        )
    ...
    def write_substance(self, f, substance):
        ...
        s = (
            f"substance '{substance.name}' {'in' * (not substance.active)}active\n"
    ...
    def write_processes(self, f):
        ...
        for proc_name in self.processes:
            if proc_name.lower() in skip_for_online:
                self.log.info(
                    "Ignoring process %s for online WAQ configuration (right?)"
                    % proc_name
                )
                continue
    ...
    def add_bcs(self, bc):
        ...
        if isinstance(bc, list):
            ...
            assert isinstance(
                bc, DelwaqScalarBC
            ), f"BC type {type(bc)} cannot be handled by Delwaq model."
            self.bcs.append(bc)
    ...
    def add_monitor_from_shp(self, shp_fn, naming=None, point_layers=None):
        ...
        if naming == "elt_layer":
            self.log.warning(
                "monitoring for online waq doesn't support naming='elt_layer'"
            )
    ...
    def add_transects_from_shp(
        self, shp_fn, naming="count", clip_to_poly=None, on_boundary=None, on_edge=None
    ):
        """
        Add monitor transects from a shapefile.
        By default transects are named in sequence.
        ...
        if clip_to_poly is not None:
            self.log.warning(
                "Online-coupled WAQ does not concern itself with clip_to_poly"
            )
        if on_boundary is not None:
            self.log.warning(
                "Online-coupled WAQ does not concern itself with on_boundary"
            )
