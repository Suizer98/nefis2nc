...
def coops_json_to_ds(json, params):
    ...
    for row in data:
        ...
        if params["product"] == "wind":
            ...
            # g: gust?
            values.append(
                [float_or_nan(row["s"]), float_or_nan(row["d"]), float_or_nan(row["g"])]
            )
        else:
    ...
    if bad_count:
        log.warning(
            "%d of %d data values were missing for %s"
            % (bad_count, values.size, params["product"])
        )
    ...
    for interval_start, interval_end in periods(start_date, end_date, days_per_request):
        if cache_dir is not None:
            ...
            cache_fn = os.path.join(
                cache_dir, "%s_%s_%s_%s.nc" % (station, product, begin_str, end_str)
            )
        else:
            cache_fn = None
        ...
        if (cache_fn is not None) and os.path.exists(cache_fn):
            ...
            if refetch_incomplete:
                ...
                if ds.time.values[-1] < min(utils.to_dt64(interval_end), end_date):
        ...
        if ds is None:
            ...
            params = dict(
                begin_date=fmt_date(interval_start),
            ...
            if product in [
                ...
                while 1:
                    ...
                    try:
                        ...
                        log.warning(
                            "Unable to connect to tidesandcurrents.noaa.gov -- possibly on HPC node"
                        )
                    ...
                    try:
                        ...
                        log.warning(
                            "Likely server error retrieving JSON data from tidesandcurrents.noaa.gov"
                        )
                        data = dict(error=dict(message="Likely server error"))
                        break
                    if ("error" in data) and (
                        ("datum" in data["error"]["message"].lower())
