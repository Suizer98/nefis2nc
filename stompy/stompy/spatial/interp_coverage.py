...
class InterpCoverage(object):
        ...
        for i in range(len(geoms)):
            ...
            def add_new_geoms(new_geom, new_src):
                ...
                if new_geom.type == "Polygon":
                    ...
                    or new_geom.type == "GeometryCollection"
                ):
    ...
    def prepare_point_to_intersection(self, tol=1e-3):
        ...
        cdf = constrained_delaunay.ConstrainedXYZField(
            X=vertices, F=np.zeros(len(vertices), np.int32), edges=edges
        )
        ...
        tri_to_geom_src = [
            None
        ] * DT.number_of_faces()  # maps a DT triangle to its (intersect_geom,source)
        ...
        for i in range(len(tri_centers)):
            ...
            if i > 0:  # try the last hit...
                ...
                if tri_to_geom_src[i - 1][0] and tri_to_geom_src[i - 1][0].contains(
                    pnt
                ):
    ...
    def point_to_intersection(self, pnt):
        ...
        if self.tri_to_geom_src is not None:
            ...
            try:
                ...
                print(
                    "ERROR: Failed to find a triangle from the vertices.  This *could* be because there are some colinear edges"
                )
                ...
                for i in k:
                    if i is not None:
                        print(
                            "vertex: %d (%f,%f)"
                            % (i, self.cdf.X[i, 0], self.cdf.X[i, 1])
                        )
                raise
...
if __name__ == "__main__":
    ...
    ic = InterpCoverage(
        "/home/rusty/classes/research/suntans/bathy_interp/test_input1.shp"
    )
