...
def rdradcp(
    ...
    if (cfg.prog_ver < 16.05 and cfg.prog_ver > 5.999) or cfg.prog_ver < 5.55:
        msg(
            "***** Assuming that the century begins year %d (info not in this firmware version)\n"
            % century
        )
    ...
    def ensemble_dates(ensx):
        ...
        dats = [
            ...
            + sum(
                ensx.rtc[i, 3:7]
                * [1.0 / 24, 1.0 / (24 * 60), 1.0 / 86400, 1.0 / 8640000]
            )
    ...
    if isinstance(nens, int) or isinstance(nens, integer):
        ...
        msg(
            "   Reading ensembles %d-%d, reducing by a factor of %d\n"
    ...
    for k in range(n):  # [python] k switched to zero-based
        ...
        if ens is None:  # ~isstruct(ens), # If aborting...
            msg(
                "Only %d records found..suggest re-running RDRADCP using this parameter\n"
                % ((k - 1) * num_av)
            )
            msg(
                "(If this message preceded by a POSSIBLE PROGRAM PROBLEM message, re-run using %d)\n"
                % ((k - 1) * num_av - 1)
            )
            n = k
            break
        ...
        adcp.ensemble_data["heading"][k] = (
            ssm.circmean(ens.heading * pi / 180.0) * 180 / pi
        )
        ...
        if type(vels) == str:
            adcp.bin_data["east_vel"][k, :] = nmean(
                ens.east_vel, 0
            )  # [python] axis changed to 0-based, and switched!
            adcp.bin_data["north_vel"][k, :] = nmean(
                ens.north_vel, 0
            )  # assume ens.east_vel[sample_index,bin_index]
        ...
        adcp.bin_data["corr"][k, :, :] = nmean(
            ens.corr, 0
        )  # added correlation RKD 9/00
        adcp.bin_data["status"][k, :, :] = nmean(ens.status, 0)

        adcp.bin_data["intens"][k, :, :] = nmean(ens.intens, 0)
        adcp.bin_data["perc_good"][k, :, :] = nmean(
            ens.percent, 0
        )  # felipe pimenta aug. 2006
        ...
        adcp.ensemble_data["bt_corr"][k, :] = nmean(
            ens.bt_corr, 0
        )  # felipe pimenta aug. 2006
    ...
    if fd.tell() + hdr.nbyte < naminfo.st_size:
        msg(
            "-->There may be another %d ensembles unread\n"
...
def rd_hdr(fd, msg=msg_print):
    ...
    # truncated at EOF
    while (
        len(cfgid) < 2 or (cfgid[0] != 0x7F or cfgid[1] != 0x7F) or not checkheader(fd)
    ):
...
def rd_fix(fd, msg=msg_print):
    ...
    if len(cfgid) == 0:
        ...
        msg(
            "WARNING: Fixed header ID %x incorrect - data corrupted or not a BB/WH raw file?\n"
            % cfgid[0]
        )
    # end
...
def rd_fixseg(fd):
    ...
    cfg.beam_pattern = getopt(
        config[0] & 8 == 8, "concave", "convex"
    )  # 1=convex,0=concave
    ...
    cfg.simflag = getopt(
        fromfile(fd, uint8, 1)[0], "real", "simulated"
    ...
    cfg.cell_size = fromfile(fd, uint16, 1)[0] * 0.01  # meters
    cfg.blank = fromfile(fd, uint16, 1)[0] * 0.01  # meters
    ...
    cfg.time_between_ping_groups = sum(
        fromfile(fd, uint8, 3) * array([60, 1, 0.01])
    )  # seconds
    ...
    cfg.xducer_misalign = fromfile(fd, int16, 1)[0] * 0.01  # degrees
    cfg.magnetic_var = fromfile(fd, int16, 1)[0] * 0.01  # degrees
    cfg.sensors_src = "%2o" % (fromfile(fd, uint8, 1)[0])
    cfg.sensors_avail = "%2o" % (fromfile(fd, uint8, 1)[0])
    cfg.bin1_dist = fromfile(fd, uint16, 1)[0] * 0.01  # meters
    cfg.xmit_pulse = fromfile(fd, uint16, 1)[0] * 0.01  # meters
    cfg.water_ref_cells = fromfile(fd, uint8, 2)
    cfg.fls_target_threshold = fromfile(fd, uint8, 1)[0]
    fd.seek(1, os.SEEK_CUR)  # fseek(fd,1,'cof')
    cfg.xmit_lag = fromfile(fd, uint16, 1)[0] * 0.01  # meters
    nbyte = 40
...
def rd_buffer(fd, num_av, msg=msg_print):
    ...
    while k + 1 < num_av:
        ...
        while search_cnt < num_search and (
            ...
            if len(nextbyte) == 0:  # End of file
                msg(
                    "EOF reached after %d bytes searched for next valid ensemble start\n"
                    % search_cnt
                )
                ens = None
                return ens, hdr, cfg, pos
        ...
        if search_cnt == num_search:
            ...
            print(
                "Not a workhorse/broadband file or bad data encountered: -> %x%x"
                % (id1[0], id1[1])
            )
            ens = None
            return ens, hdr, cfg, pos
        ...
        for n in range(len(hdr.dat_offsets)):  # n: 1 => 0 based
            ...
            if id_ == "0000":
                ...
                ens.depth[k] = fromfile(fd, uint16, 1)[0] * 0.1  # meters
                ens.heading[k] = fromfile(fd, uint16, 1)[0] * 0.01  # degrees
                ens.pitch[k] = fromfile(fd, int16, 1)[0] * 0.01  # degrees
                ens.roll[k] = fromfile(fd, int16, 1)[0] * 0.01  # degrees
                ens.salinity[k] = fromfile(fd, int16, 1)[0]  # PSU
                ens.temperature[k] = fromfile(fd, int16, 1)[0] * 0.01  # Deg C
                ens.mpt[k] = sum(
                    fromfile(fd, uint8, 3) * array([60, 1, 0.01])
                )  # seconds
                ens.heading_std[k] = fromfile(fd, uint8, 1)[0]  # degrees
                ens.pitch_std[k] = fromfile(fd, uint8, 1)[0] * 0.1  # degrees
                ens.roll_std[k] = fromfile(fd, uint8, 1)[0] * 0.1  # degrees
                ...
                if cfg.name == "bb-adcp":
                    if cfg.prog_ver >= 5.55:
                        ...
                        cent = fromfile(fd, uint8, 1)[
                            0
                        ]  # possibly also for 5.55-5.58 but
                        ens.rtc[k, :] = fromfile(
                            fd, uint8, 7
                        )  # I have no data to test.
                    ...
                    if int(cfg.prog_ver) in (8, 10, 16, 50, 51, 52):
                        ...
                        if (
                            cfg.prog_ver >= 10.01 and cfg.prog_ver <= 10.99
                        ) or cfg.prog_ver >= 16.05:  # Added more fields with century in clock 16.05
                ...
                # switched!
                vels = (
                    fromfile(fd, int16, 4 * cfg.n_cells).reshape([cfg.n_cells, 4])
                    * 0.001
                )  # m/s
                ...
                ens.corr[k, :, :] = fromfile(fd, uint8, 4 * cfg.n_cells).reshape(
                    [cfg.n_cells, 4]
                )
                nbyte = 2 + 4 * cfg.n_cells
            elif id_ == "0300":  # Echo Intensities
                # RCH check array ordering:
                ens.intens[k, :, :] = fromfile(fd, uint8, 4 * cfg.n_cells).reshape(
                    [cfg.n_cells, 4]
                )
                nbyte = 2 + 4 * cfg.n_cells
            elif id_ == "0400":  # Percent good
                ens.percent[k, :, :] = fromfile(fd, uint8, 4 * cfg.n_cells).reshape(
                    [cfg.n_cells, 4]
                )
                nbyte = 2 + 4 * cfg.n_cells
                ...
                if cfg.name == "os-adcp":
                    ...
                    ens.status[k, :, :] = fromfile(fd, uint8, 4 * cfg.n_cells).reshape(
                        [cfg.n_cells, 4]
                    )
                    nbyte = 2 + 4 * cfg.n_cells
                ...
                if SOURCE == 2:
                    ...
                    ens.bt_mode[k] = float64(
                        fromfile(fd, uint8, 1)[0]
                    )  # fromfile(fd,uint8,1)[0]
                ...
                # end
                ens.bt_range[k, :] = fromfile(fd, uint16, 4) * 0.01  #
                ...
                if (
                    cfg.prog_ver >= 5.3
                ):  # Version 4.05 firmware seems to be missing these last 11 bytes.
                    fd.seek(78 - 71, os.SEEK_CUR)
                    ens.bt_range[k, :] = (
                        ens.bt_range[k, :] + fromfile(fd, uint8, 4) * 655.36
                    )
                    nbyte += 11
                    ...
                    if cfg.name == "wh-adcp":
                        if (
                            cfg.prog_ver >= 16.20
                        ):  # RDI documentation claims these extra bytes were added in v 8.17
                            fd.seek(
                                4, os.SEEK_CUR
                            )  # but they don't appear in my 8.33 data - conversation with
            ...
            # end # id_==0600 # bottom track
                ...
                ens.smtime[k] = mtime + fromfile(fd, uint32, 1)[0] / 8640000.0
                fd.seek(4, os.SEEK_CUR)  # PC clock offset from UTC
                ens.slatitude[k] = fromfile(fd, int32, 1)[0] * cfac
                ens.slongitude[k] = fromfile(fd, int32, 1)[0] * cfac
                ens.emtime[k] = mtime + fromfile(fd, uint32, 1)[0] / 8640000.0
                ...
                ens.nmtime[k] = mtime + fromfile(fd, uint32, 1)[0] / 8640000.0
            ...
            # do it here, without an error message to emphasize that I am kludging the WINRIVER blocks only!
                ...
                if id_ == "2100":  # $xxDBT  (Winriver addition) 38
                    ...
                    id_ == "2101"
                ):  # $xxGGA  (Winriver addition) 94 in manual but 97 seems to work
                    ...
                    if l >= 0:
                        ...
                        try:
                            ...
                            hh, mm, ss = (
                                int(str_[l + 7 : l + 9]),
                                int(str_[l + 9 : l + 11]),
                                float(str_[l + 11 : l + 13]),
                            )
                            ens.smtime[k] = (hh + (mm + ss / 60.0) / 60.0) / 24.0
                        except ValueError:
                            msg("Corrupt GPS string - skipping")
                        # disp(['->' setstr_(str_(1:50)) '<-'])
                elif (
                    id_ == "2102"
                ):  # $xxVTG  (Winriver addition) 45 (but sometimes 46 and 48)
                    cfg.sourceprog = "WINRIVER"
                    if SOURCE != 2:
                        msg(
                            "\n***** Apparently a WINRIVER file - Raw NMEA data handler not yet implemented\n"
...
def nmedian(x, window=inf, dim=None):
    ...
    x.sort(
        axis=0
    )  # in place sort, and puts nan at the end, while -inf,inf are in order.
    [n1, n2] = x.shape
    ...
    if n1 == 1:  # each column has only one row - no stats to be taken, just copy.
        ...
        if n2 == 1:
            ...
            deviations = abs(x - (x1 + x2)[None, ...] / 2.0)
            x[deviations > window] = nan
            x.sort(axis=0)
            kk = sum(isfinite(x), axis=0)
            x[isnan(x)] = 0
    ...
    if len(fixes):
        ...
        if adjust_to_utc:
            ...
            if np.isnan(hour_correction):
                print(
                    "%s: WARNING: cannot determine time shift - no valid GPS data"
                    % (gps_fn)
                )
            else:
                if 1:  # hour_correction!=0.0:
                    print(
                        "%s: shifting by %d hours, based on GPS time"
                        % (gps_fn, hour_correction)
                    )
                shift_min = np.round(np.nanmin(deltas))
                shift_max = np.round(np.nanmax(deltas))
                if shift_min != shift_max:
                    print(
                        "%s: WARNING: time zone shift is ambiguous (%.0f-%.0fh)"
                        % (gps_fn, shift_min, shift_max)
                    )
                r.mtime += hour_correction / 24.0
    else:
...
def ship_to_earth(r):
    ...
    if r.config.coord_sys != "ship":
        raise Exception(
            "applying ship_to_earth but coord sys is %s" % r.config.coord_sys
        )
