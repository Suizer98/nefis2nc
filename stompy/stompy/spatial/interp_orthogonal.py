...
def simple_quad_gen_to_grid(sqg, aniso=None):
    ...
    for qg in sqg.qgs:
        ...
        if joined:
            node_map, edge_map, cell_map = joined.add_grid(
                grid, merge_nodes="auto", tol=0.01
            )
            joined.edges["K"][edge_map] = 0.5 * (
                joined.edges["K"][edge_map] + grid.edges["K"]
            )
        else:
            joined = grid
    return joined
...
class OrthoInterpolator(object):
    ...
    def __init__(self, region, samples=None, **kw):
        ...
        if samples is not None:
            ...
            if self.clip_samples:
                ...
                sel = [
                    boundary.contains(geometry.Point(xy))
        ...
        for geom, Kxy in self.overrides:
            ...
            to_drop = np.array(
                [
                    geom.contains(geometry.Point(xy))
    ...
    def solve(self):
        ...
        dirich_idxs = [
            grid.select_nodes_nearest(xy) for xy in samples.loc[:, ["x", "y"]].values
        ]
        ...
        for row in range(nrows):
            for col in range(ncols):
                ...
                if n in dirich:
                    ...
                    if node_north < 0:
                        node_north = node_south
                    if node_south < 0:
                        node_south = node_north
                    if node_west < 0:
                        node_west = node_east
                    if node_east < 0:
                        node_east = node_west
                    ...
                    if Kedge is not None:
                        Ks = [Kedge[grid.nodes_to_edge(n, nbr)] for nbr in nbrs]
                    else:
    ...
    def plot_result(self, num=1, **kw):
        ...
        ccoll = self.grid.contourf_node_values(self.result, 32, cmap="jet", **kw)
