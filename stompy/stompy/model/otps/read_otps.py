...
def tide_pred(modfile, lon, lat, time, z=None, conlist=None):
    ...
    for nn, om in enumerate(omega):
        for ii in range(0, nx):
            h[:, ii] += pf[nn] * h_re[nn, ii] * np.cos(
                om * tsec + v0u[nn] + pu[nn]
            ) - pf[nn] * h_im[nn, ii] * np.sin(om * tsec + v0u[nn] + pu[nn])
            u[:, ii] += pf[nn] * u_re[nn, ii] * np.cos(
                om * tsec + v0u[nn] + pu[nn]
            ) - pf[nn] * u_im[nn, ii] * np.sin(om * tsec + v0u[nn] + pu[nn])
            v[:, ii] += pf[nn] * v_re[nn, ii] * np.cos(
                om * tsec + v0u[nn] + pu[nn]
    ...
    for nn, om in enumerate(omega):
        for ii in range(0, nx):
            h[:, ii] += (
                damp[nn]
                * h_amp[nn, ii]
                * np.cos(om * tsec - (h_phs[nn, ii] + dphs[nn]))
            )
            u[:, ii] += (
                damp[nn]
                * u_amp[nn, ii]
                * np.cos(om * tsec - (u_phs[nn, ii] + dphs[nn]))
            )
            v[:, ii] += (
                damp[nn]
                * v_amp[nn, ii]
                * np.cos(om * tsec - (v_phs[nn, ii] + dphs[nn]))
            )
...
def tide_pred_old(modfile, lon, lat, time, z=None, conlist=None):
    ...
    for nn, om in enumerate(omega):
        for ii in range(0, nx):
            h[:, ii] += h_re[nn, ii] * np.cos(om * tsec) + h_im[nn, ii] * np.sin(
                om * tsec
            )
            u[:, ii] += u_re[nn, ii] * np.cos(om * tsec) + u_im[nn, ii] * np.sin(
                om * tsec
            )
            v[:, ii] += v_re[nn, ii] * np.cos(om * tsec) + v_im[nn, ii] * np.sin(
                om * tsec
            )
...
def extract_HC(modfile, lon, lat, z=None, conlist=None):
    ...
    from ...spatial.interpXYZ import (
        interpXYZ,
    )  # implemented in this file, just for IDW method.
...
def read_OTPS_UV(uvfile, ic):
    ...
    X, Y = np.meshgrid(
        np.linspace(th_lim[0], th_lim[1], n), np.linspace(ph_lim[0], ph_lim[1], m)
    )
...
def read_OTPS_grd(grdfile):
    ...
    X, Y = np.meshgrid(
        np.linspace(lons[0], lons[1], n[0]), np.linspace(lats[0], lats[1], m[0])
    )
    return X, Y, hz, mask
