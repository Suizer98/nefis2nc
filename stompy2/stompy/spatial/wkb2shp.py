...
def wkb2shp(
    ...
    if (
        fields is not None and type(fields) == list
    ...
    for n in field_names:
        if len(n) > 10:
            raise Exception("Cannot have field names longer than 10 characters: %s" % n)
    ...
    new_layer = new_ds.CreateLayer(layer_name, srs=srs, geom_type=geom_type)
...
def shp2geom(
    ...
    for i in range(fld_count):
        ...
        if ogr_type == "String":
            ...
            getter = lambda f, i=i: np.datetime64(
                f.GetFieldAsString(i).replace("/", "-")
            )
        else:
            np_type = np.float64
