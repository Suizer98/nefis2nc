...
class Triangulation(unstructured_grid.UnstructuredGrid):
    ...
    def angle_sort_adjacent_nodes(self, n, ref_nbr=None, topo=True):
        if topo:
            ...
            return super(Triangulation, self).angle_sort_adjacent_ndoes(
                n, ref_nbr=ref_nbr
            )
    ...
    def topo_sort_adjacent_nodes(self, n, ref_nbr=None):
        ...
        if len(nbrs) < 3:
            ...
            he_nbrs = [self.nodes_to_halfedge(n, nbr) for nbr in nbrs]
    ...
    def tri_insert_in_edge(self, n, loc):
        ...
        for cell_data in cells_to_split:
            common = [n for n in cell_data["nodes"] if n != a and n != c][0]
    ...
    def flip_edge(self, j):
        ...
        self.log.debug(
            "Flipping edge %d, with cells %d, %d   nodes %d,%d"
            % (j, c_left, c_right, self.edges["nodes"][j, 0], self.edges["nodes"][j, 1])
        )
        ...
        # DBG
        if 0:
            for n, label in zip([na, nb, nc, nd], "abcd"):
                plt.text(self.nodes["x"][n, 0], self.nodes["x"][n, 1], label)
    ...
    def test_delete_node_dim_down(self, n):
        ...
        for c in self.valid_cell_iter():
            if n not in self.cell_to_nodes(c):
                ...
                return False
        pnts = [self.nodes["x"][i] for i in self.valid_node_iter() if i != n]
        a, b = pnts[:2]
        for c in pnts[2:]:
    ...
    def delete_node_2d(self, n):
        ...
        for nbrA, nbrB in circular_pairs(nbrs):
            ...
            if (
                (he is None)
                or (he.cell() < 0)
                or (n not in self.cell_to_nodes(he.cell()))
            ):
        ...
        while len(holes_nodes):
            ...
            for c in c_cand1:
                if c == "inf":
                    ...
                    robust_predicates.orientation(
                        self.nodes["x"][a], self.nodes["x"][b], self.nodes["x"][c]
                    )
                    > 0
                ):
                    c_cand2.append(c)
            ...
            while len(c_cand2) > 1:
                c = c_cand2[0]
                for d in c_cand2[1:]:
                    tst = robust_predicates.incircle(
                        self.nodes["x"][a],
                        self.nodes["x"][b],
                        self.nodes["x"][c],
                        self.nodes["x"][d],
        ...
        for c in self.valid_cell_iter():
            ...
            for n in self.valid_node_iter():
                ...
                if check > 0:
                    ...
                    msg = "Node %d is inside the circumcircle of cell %d (%d,%d,%d)" % (
                        n,
                        c,
                        nodes[0],
                        nodes[1],
    ...
    def propagating_flip(self, c, i):
        ...
        while edges:  # (! edges.empty()){
            ...
            if nbr >= 0:
                ...
                p_in_nbr = robust_predicates.incircle(
                    nbr_points[0], nbr_points[1], nbr_points[2], p
                )
        ...
        if self.dim() == 1:
            ...
            for n_nbr in n_nbrs:
                if n_nbr == nB:
                    ...
                    return history
                if ordered(A, self.nodes["x"][n_nbr], B):
                    trav = ("node", n_nbr)
                    history.append(trav)
    ...
    def locate_for_traversal_outside(self, p, p_other, loc_face, loc_type, loc_index):
        ...
        if dim < 0:
            ...
            if loc_type == self.OUTSIDE_AFFINE_HULL:
                ...
                oj = robust_predicates.orientation(
                    p, self.nodes["x"][he.node_rev()], self.nodes["x"][he.node_fwd()]
                )
                ...
                ojo = robust_predicates.orientation(
                    p_other,
                ...
                if ojo > 0:
                    ...
                    if o_loc_type == self.OUTSIDE_CONVEX_HULL:
                        ...
                        if o_loc_type == self.IN_VERTEX:
                            ...
                            oj2 = robust_predicates.orientation(
                                p,
                                self.nodes["x"][he.node_rev()],
                                self.nodes["x"][he.node_fwd()],
                            )
                    ...
                    o_rev = robust_predicates.orientation(
                        p, self.nodes["x"][he.node_rev()], p_other
                    )
                    if o_rev == 0.0:
                    ...
                    while 1:
                        n_fwd = he.node_fwd()
                        o_fwd = robust_predicates.orientation(
                            A, self.nodes["x"][n_fwd], B
                        )
                        if o_fwd == 0.0:
                            return ("node", n_fwd)
                        if o_fwd < 0:
                            return (
                                "edge",
                                he,
                            )  # had been he.j, but we should return half-edge
                        # must go further!
                        he_opp = he.opposite()
                        he = he.fwd()
                ...
                orient = robust_predicates.orientation(p, self.nodes["x"], p_other)
                if orient != 0.0:
                    return (None, None)
            ...
            while 1:
                ...
                o_j = robust_predicates.orientation(
                    p, self.nodes["x"][n_rev], self.nodes["x"][n_fwd]
                )
                if o_j < 0:
                    ...
                    o_rev = robust_predicates.orientation(
                        p, self.nodes["x"][n_rev], p_other
                    )
                    o_fwd = robust_predicates.orientation(
                        p, self.nodes["x"][n_fwd], p_other
                    )
                    if o_rev == 0.0:
                        if o_fwd == 0.0:
                            assert o_j == 0.0
    ...
    def locate_for_traversal(self, p, p_other):
        ...
        if loc_type in [self.OUTSIDE_AFFINE_HULL, self.OUTSIDE_CONVEX_HULL]:
            return self.locate_for_traversal_outside(
                p, p_other, loc_face, loc_type, loc_index
            )
        elif loc_type == self.IN_VERTEX:
    ...
    def gen_intersected_elements(self, nA=None, nB=None, pA=None, pB=None):
        ...
        if dim == 0:
            ...
            if (
                trav[0] == "node"
            ):  # get a first half-edge going in the correct direction
                n_nbrs = self.node_to_nodes(trav[1])
                for n_nbr in n_nbrs:
                    if ordered(A, self.nodes["x"][n_nbr], B) or np.all(
                        B == self.nodes["x"][n_nbr]
                    ):
                        he = self.nodes_to_halfedge(nA, n_nbr)
                        break
                else:
    ...
    def add_constraint(self, nA, nB):
        ...
        for elt in int_elts[1:-1]:
            if elt[0] == "node":
                raise self.ConstraintCollinearNode(
                    "Constraint intersects a node", node=elt[1]
                )
            if elt[0] == "cell":
                dead_cells.append(elt[1])
            if elt[0] == "edge":
                if self.edges["constrained"][elt[1].j]:
                    raise IntersectingConstraints(
                        "Constraint intersects a constraint", edge=elt[1].j
                    )
    ...
    def node_to_constraints(self, n):
        return [j for j in self.node_to_edges(n) if self.edges["constrained"][j]]
    ...
    def init_from_grid(
        ...
        if set_valid:
            self.add_cell_field(
                "valid", np.zeros(self.Ncells(), np.bool8), on_exists="pass"
            )
        ...
        all_segs = [g.edges["nodes"][j] for j in g.valid_edge_iter()]
        while all_segs:
    ...
    def split_constraint(self, x, j):
        ...
        for f in j_data:
            if f in ["nodes", "cells", "deleted"]:
                continue
            self.edges[f][js] = j_data[f]

        return n_new

    def add_constrained_linestring(
        self, coords, on_intersection="exception", on_exists="exception", closed=False
    ):
        """
        ...
        for a, b in ab_list:
            if on_intersection == "insert":
                sub_nodes, sub_edges = self.add_constraint_and_intersections(
                    a, b, on_exists=on_exists
                )
    ...
    def bulk_init(self, points):  # ExactDelaunay
        ...
        for c in range(self.Ncells()):
            for i, (a, b) in enumerate(circular_pairs(self.cells["nodes"][c])):
                ...
                j = self.add_edge(nodes=[a, b], cells=[c, c_nbr])
    ...
    def point_clearance(self, x, hint=None):
        ...
        if loc_type == self.OUTSIDE_CONVEX_HULL:
            ...
            min_clearance = min(
                dist(self.nodes["x"][loc_index.node_fwd()], x),
                dist(self.nodes["x"][loc_index.node_rev()], x),
            )
            j = loc_index.j
            if self.edges["constrained"][j]:
                j_clearance = point_segment_distance(
                    x, self.nodes["x"][self.edges["nodes"][j]]
                )
            ...
            raise Exception(
                "Loc type OUTSIDE_AFFINE_HULL is not implemented for point_clearance"
            )
