...
class QuickVar(object):  # wraps netcdf variable
    ...
    def __getitem__(self, k):
        ...
        if isinstance(k, dict):
            dims = self.dims
            slices = [k.get(d, slice(None)) for d in self.dims]
            k = tuple(slices)
        ...
        if isinstance(k[0], str):
            ...
            untranspose = [self._transpose.index(i) for i in range(myv.ndim)]
            ...
            retranspose = [
                i
                for i in self._transpose
                if (
                    isinstance(k_untransposed[i], slice)
                    or isinstance(k_untransposed[i], Iterable)
                )
            ]
    ...
    def _rich_take(self, myv, idxs):
        ...
        for idx in idxs:
            ...
            if isinstance(idx, Iterable):
                idx = np.asarray(idx)
                if idx.ndim != 1 or (len(idx) > 1 and np.any(np.diff(idx) <= 0)):
...
class QDataset(netCDF4.Dataset):
    ...
    class VarProxy(object):
        ...
        def create(self, dims, data, **kwargs):
            ...
            if data.dtype == object:
                if np.all(
                    [isinstance(v, str) or isinstance(v, unicode) for v in data.ravel()]
                ):
                    data = data.astype("S")
            ...
            variable = self.dataset.createVariable(
                self.varname, var_dtype, dims, **kwargs
            )
            variable[:] = data
    ...
    def interpolate_dimension(
        ...
        if len(int_ncvar.dimensions) > 1:
            ...
            if max_gap:
                raise QncException(
                    "max_gap not implemented for multi-dimensional variable"
                )
        else:
            if max_gap:
                ...
                for idx in gap_idx:
                    ...
                    to_mask = slice(
                        *np.searchsorted(new_coordinate, [gap_left, gap_right])
                    )
                    gapped[to_mask] = True
            ...
            if varname == int_var:
                ...
                if len(int_all_dims) == 1:
                    if ("int" in src_val.dtype.name) and (int_mode == "nearest"):
                        ...
                        interper = interpolate.interp1d(
                            self.variables[int_var][:],
                            src_val,
                            axis=merge_idx,
                            bounds_error=False,
                            assume_sorted=False,
            ...
            if len(dim_names) == 0:
                ...
                for slc_dim, slc_sel in iteritems(kwargs):
                    if slc_dim in dim_names:
                        ...
                        print(
                            "slice dimension %s not in dim_names %s"
                            % (slc_dim, dim_names)
                        )
    ...
    def within(self, **kwargs):
        ...
        for slc_varname, slc_range in iteritems(kwargs):
            ...
            selects[slc_var.dimensions[0]] = utils.within(
                slc_var[:], slc_range, as_slice=True
            )
        ...
        for k, v in iteritems(self.variables):
            ...
            for attr_name in v.ncattrs()[:max_n_attrs]:
                ...
                if isinstance(a_val, str) and len(a_val) > max_attr_len:
                    a_val = a_val[:max_attr_len] + "... [%d more bytes]" % (
                        len(a_val) - max_attr_len
                    )
                    a_val = '"' + a_val + '"'
    ...
    for varname in ncs[0].variables.keys():
        ...
        if cat_idx is not None:
            ...
            if not constant:
                if new_dim is None:
                    raise QncException(
                        "Non-concatenated variable %s "
                        "does not match %s != %s" % (varname, parts[0], parts[n])
                    )
                else:
                    print(
                        "Variable values of %s will go into new dimension %s"
                        % (varname, new_dim)
                    )
        ...
        if dim in ncvar.dimensions:
            ...
            if True:  # lowpass!=1: # older, slower way:
                if lowpass:
                    ...
                    val_lp = lp_filter.lowpass_fir(
                        val, winsize, axis=dim_idx, nan_weight_threshold=0.5
                    )
                else:
                    val_lp = val
...
def mat_to_nc(mat, dim_map={}, autosqueeze=True):
    ...
    for k, v in iteritems(dim_map):
        ...
        if k in mat:
            ...
            if x.ndim != len(v):
                raise QncException(
                    "dim_map: field %s - %s doesn't match with %s" % (k, v, x.shape)
                )
...
def ortho_to_transect_nc(src_nc, src_x_var, src_y_var, transect_xy, dst_nc=None):
    ...
    src_xy = np.array(
        [src_nc.variables[src_x_var][:], src_nc.variables[src_y_var][:]]
    ).T
    ...
    dst_nc["distance"][station_dim] = utils.dist_along(
        dst_nc[src_x_var][:], dst_nc[src_y_var][:]
    )
