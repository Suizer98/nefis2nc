...
class DFMGrid(unstructured_grid.UnstructuredGrid):
    def __init__(
        self, nc=None, fn=None, cells_from_edges="auto", max_sides=6, cleanup=False
    ):
        """
        ...
        if (
            0
        ):  # This is for hints to possibly handling non-contiguous indices in the future. caveat emptor.
...
def dredge_boundary(g, linestring, dredge_depth):
    ...
    g.nodes["depth"][nodes_to_dredge] = np.minimum(
        g.nodes["depth"][nodes_to_dredge], dredge_depth
    )
