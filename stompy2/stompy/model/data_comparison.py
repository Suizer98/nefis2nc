...
def combine_sources(all_sources, dt=np.timedelta64(900, "s"), min_period=True):
    ...
    resample_bins = np.arange(
        utils.floor_dt64(t_min, dt), utils.ceil_dt64(t_max, dt) + dt, dt
    ).astype("M8[ns]")
...
def assemble_comparison_data(
    models, observations, model_labels=None, period="model", extract_options={}
):
    """
    ...
    base_obs = observations[
        0
    ]  # defines the variable and location for extracting model data
    ...
    if base_var == "water_level":
        ...
        loc_extract_opts["data_vars"] = [
            "ZNit",
            "NO3",
        ]  # want to extract both to calculate age and compare with nitrogen
    else:
        ...
        for tgt_var in tgt_vars:
            try:
                ...
                for dv in ds.data_vars:
                    ...
                    raise Exception(
                        "Could not find %s by name or standard_name" % (tgt_var)
                    )
...
def calc_metrics(x, ref, combine=False):
    ...
    metrics["spearman_rho"], metrics["spearman_p"] = spearmanr(
        x.values[valid], ref.values[valid]
    )
...
def fix_date_labels(ax, nticks=3):
    ...
    xax.set_major_locator(
        dates.AutoDateLocator(
            minticks=nticks, maxticks=nticks + 1, interval_multiples=False
        )
    )


def calibration_figure_3panel(
    all_sources,
    combined=None,
    metric_x=1,
    metric_ref=0,
    ...
    if combined is None:
        ...
        if combined is None:
            log.warning(
                "Combined sources was None -- likely no overlap between data sets"
            )
            return None
        ...
        for i in range(len(combined.source)):
            ...
            for k in ["color", "zorder"]:
                if k in style:
                    kw[k] = style[k]
            ax.plot(
                combined.isel(source=scatter_x_source) - offsets[scatter_x_source],
    ...
    if metric_x is not None:
        ...
        for mx in metric_x:
            rec = calc_metrics(
                x=combined.isel(source=mx) - offsets[mx],
        ...
        with pd.option_context("expand_frame_repr", False, "display.precision", 3):
            tbl = str(df)
    ...
    ts_ax.axis(xmin=combined.time.values[0], xmax=combined.time.values[-1])
    ...
    if has_lp_data:
        ...
        if lowpass:
            lp_ax.text(
                0.5,
                0.5,
                "Insufficient data for low-pass",
                transform=lp_ax.transAxes,
                ha="center",
                va="center",
            )
