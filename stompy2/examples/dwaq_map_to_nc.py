...
parser = argparse.ArgumentParser(
    description="Convert D-WAQ binary map output to NetCDF."
)

parser.add_argument(
    "map_fn", metavar="somefile.map", type=str, help="path to map file output"
)
parser.add_argument("hyd_fn", metavar="other.hyd", type=str, help="path to hyd file")
parser.add_argument(
    "--totaldepth",
    default="TotalDepth",
...
if args.totaldepth != "none":
    ...
    if total_depth not in map_ds:
        ...
        map_ds[total_depth] = ("time", "layer", "face"), np.ones(
            (len(map_ds.time), len(map_ds.layer), len(map_ds.face)), "<i1"
        )

dio.map_add_z_coordinate(
    map_ds, total_depth=total_depth, coord_type="sigma", layer_dim="layer"
)
