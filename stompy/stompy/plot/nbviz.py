...
class UGDataset(Dataset):
    ...
    def __init__(self, ds, grid=None, **kw):
        ...
        if ("eta" in cell_vars) and ("bed_elev" in cell_vars):
            ...
            self.expressions.append(
                BoundExpression(name="depth", dims=("time", "face"), func=depth_func)
            )
    ...
    def select_data(self, varname, dims):
        v = self.ds[varname]
        isels = {k: dims[k] for k in dims if k in v.dims}
        if len(isels):
            v = v.isel(**isels)
        return v
    ...
    def update_free_dims(self):
        ...
        for dim in self.local_dims:
            if dim not in self.free_dims:
                self.free_dims[dim] = self.ds.ds[
                    dim
                ]  # can we just publish these as xr data arrays?
                changed = True
...
class UGTextLayer(UGLayer):
    ...
    def init_plot(self, Fig):
        ...
        self.Fig = Fig
        data = self.get_data()
        self.text = self.Fig.ax.text(
            0.05, 0.99, str(data), transform=self.Fig.ax.transAxes, va="top"
        )
...
class UGCellLayer(UGLayer):
    ...
    def init_plot(self, Fig):
        ...
        self.Fig = Fig
        data = self.get_data()
        self.ccoll = self.ds.grid.plot_cells(
    ...
    def layer_edit_pane(self):
        ...
        var_selector = widgets.Dropdown(
            options=self.available_vars(), value=self.variable, description="Var:"
        )
        ...
        update_clims = widgets.Checkbox(
            value=True,
            description="Update clim",
...
class UGEdgeLayer(UGLayer):
    ...
    def init_plot(self, Fig):
        ...
        self.Fig = Fig
        self.ecoll = self.ds.grid.plot_edges(
            values=self.get_data(), cmap="turbo", ax=self.Fig.ax
        )
        self.cax = self.Fig.get_cax()
...
class NBViz(widgets.AppLayout):
    ...
    def init_layout(self):
        ...
        self.layer_selector = widgets.Select(
            options=[self.new_layer], description="Layer:"
        )
        ...
        for dim in self.global_dims:
            ...
            if dim == "time":
                ...
                self.time_slider = widgets.IntSlider(
                    min=vmin, max=vmax, value=value, continuous_update=False
                )
                ...
                time_control = widgets.HBox(
                    [widgets.Label("Time:"), self.play, self.time_slider]
                )
    ...
    def global_controls_widget(self):
        ...
        show_axes = widgets.Checkbox(
            value=True, description="Show axes", style=style, layout=Layout(width="40%")
        )
        ...
        self.save_enabled = widgets.Checkbox(
            value=False, description="Save images", style=style
        )
        ...
        vbox = widgets.VBox(
            [
                widgets.Box([show_axes, tight_layout], layout=row_layout),
                widgets.Box([self.save_enabled, self.save_path], layout=row_layout),
            ]
        )
        return vbox
