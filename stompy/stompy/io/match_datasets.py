...
class MatchVarsCruise(object):
    def __init__(self, varA, varB, B_type):
        ...
        mapBtime_to_A = np.searchsorted(varB.time.values, varA.time.values)
        if 1:
            mapBtime_to_A = np.ma.array(
                mapBtime_to_A, mask=utils.isnat(varA.time.values)
            )

        new_coords["time"] = xr.DataArray(
            varB.time.values[mapBtime_to_A], dims=varA.time.dims
        )
        ...
        new_coords["x"] = xr.DataArray(
            B_xy[mapBstn_to_A, 0], dims=["Distance_from_station_36"]
        )
        new_coords["y"] = xr.DataArray(
            B_xy[mapBstn_to_A, 1], dims=["Distance_from_station_36"]
        )
        ...
        new_depths = B_depth.values[
            mapBtime_to_A, mapBstn_to_A[None, :, None], mapBdepth_to_A
        ]

        new_coords["depth"] = xr.DataArray(
            np.ma.array(new_depths, mask=mask), dims=varA.dims
        )
    ...
    def __call__(self, varB):
        ...
        newBvalues = varB.values[
            self.map_time, self.map_station[None, :, None], self.map_depth
        ]
        newBvalues = np.ma.array(newBvalues, mask=self.mask)

        # the primary dimensions are copied from A
        Bcoords = [(d, self.varA[d]) for d in self.varA.dims]
        newB = xr.DataArray(newBvalues, dims=self.varA.dims, coords=Bcoords)
