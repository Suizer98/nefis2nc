...
class ZLayerAggregator(waq_scenario.DwaqAggregator):
    ...
    def init_seg_mapping_sort(self):
        ...
        for elt_i in range(len(self.elements)):
            ...
            for p in range(self.nprocs):
                ...
                if not np.any(sel_elt):
                    self.log.info(
                        "This proc has no local elements in this aggregated element"
                    )
                    continue
                ...
                for local_elt in np.nonzero(sel_elt)[
                    0
                ]:  # Loop over local water columns
                    segs = np.nonzero((seg_to_2d == local_elt))[0]
            ...
            for k_agg in range(n_layers_for_elt):
                ...
                segs_this_layer = elt_segs[
                    order[break_idxs[k_agg] : break_idxs[k_agg + 1]]
                ]
    ...
    def init_seg_mapping_combine_bed(self):
        ...
        for elt_i in range(len(self.elements)):
            ...
            layer_vols = defaultdict(
                lambda: 0.0
            )  # accumulate volume per original layer
            ...
            all_dz = [layer_vols[k] / agg_elt_area for k in range(Nk)]
