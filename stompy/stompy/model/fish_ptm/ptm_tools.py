...
class PtmBin(object):
    ...
    def read_timestep(self, ts=0):
        ...
        part_dtype = [("id", "i4"), ("x", "3f8"), ("active", "i4")]
        part_size = 2 * 4 + 3 * 8
    ...
    def plot(
        ...
        **kwargs,
        ...
        self.title = ax.set_title(
            "Particle positions at %s" % (datetime.strftime(t, "%Y-%m-%d %H:%M:%S"))
        )
        if zoom:
...
class PtmState(object):
    ...
    def read_header(self):
        ...
        attr_dtype = [("attr_id", np.int32), ("name", "S80")]
        attrs = np.fromfile(self.fp, attr_dtype, Nfields)

        part_dtype = [("id", np.int32), ("x", np.float64, 3), ("status", np.int32)]
    ...
    def scan_to_step(self, step):
        ...
        while start_step < step:
            ...
            start_step += 1
            framesize = (
                self.step_header_dtype.itemsize
                + hdr["Npart"] * self.part_dtype.itemsize
            )
            self.step_offsets[start_step] = (
                self.step_offsets[start_step - 1] + framesize
            )
    ...
    def read_timestep(self, step):
        ...
        return (
            datetime(hdr["year"], hdr["month"], hdr["day"], hdr["hour"], hdr["minute"]),
            particles,
        )
...
class PtmConcentration(object):
    ...
    def seek_step(self, step):
        if self.stride == 0:
            assert step == 0
        assert step < self.nsteps, "Cannot seek to %d, only %d available" % (
            step,
            self.nsteps,
        )
...
def calc_agebin(binfile, ncfile, polyfile, ntout):
    ...
    xypoly = [np.array(p.exterior.coords) for p in recs["geom"]]
