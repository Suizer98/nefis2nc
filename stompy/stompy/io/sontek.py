...
def _surveyor_csv_to_xr(base):
    ...
    ds["frequency"] = ("sample",), [
        float(s.replace("MHz", "")) for s in df_snr["Frequency (MHz)"]
    ]
    ds["profile_type"] = ("sample",), df_snr["Profile Type"]
    ds["depth_m"] = ("sample",), df_snr["Depth (m)"]

    snr_data = np.zeros((len(ds.sample), len(ds.layer), len(ds.beam)), "f4")
    ...
    x = np.zeros((len(ds.sample), len(ds.layer)), "f4")
    per_cell = {
        "Ve": x.copy(),
        "Vn": x.copy(),
        "Vu": x.copy(),
...
def _surveyor_mat_to_xr(base, target_ref="bt"):
    ...
    water_velocity = mat["WaterTrack"][0, 0][
        "Velocity"
    ]  # 85,4,167   bins,beams,samples
    ...
    ds["time_raw"] = ("sample",), mat["System"]["Time"][0, 0][
        :, 0
    ]  # seconds since 2000-01-01
    ds["time"] = ds["time_raw"] * np.timedelta64(1, "s") + np.datetime64(
        "2000-01-01 00:00"
    )

    # reported in kHz.
    ds["frequency"] = ("sample",), mat["WaterTrack"]["WT_Frequency"][0, 0][
        :, 0
    ] / 1000.0
    ...
    ds["snr"] = ("sample", "layer", "beam"), mat["System"]["SNR"][0, 0].transpose(
        2, 0, 1
    )
    ds["snr"].attrs["units"] = "dB"
    ...
    velo_label = ["sys", "bt", "gps", "gps"][
        velo_reference
    ]  # last two differ by GGA vs. VTG
    ...
    ds["z_ctr"] = ("sample", "layer"), (
        ds.cell_start.values[:, None]
        + ds.cell_size.values[:, None] * (0.5 + np.arange(len(ds.layer))[None, :])
    )
