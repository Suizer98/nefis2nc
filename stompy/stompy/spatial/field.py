...
if gdal:
    numpy_type_to_gdal = {
        np.int8: gdal.GDT_Byte,  # meh.  not quite real, most likely
...
class Field(object):
    ...
    def to_grid(
        ...
        return SimpleGrid(
            extents=[xmin, xmax, ymin, ymax], F=newF, projection=self.projection()
        )
...
class XYZField(Field):
    ...
    def plot(self, **kwargs):
        ...
        def_args = {"c": self.F, "antialiased": False, "marker": "s", "lw": 0}
        def_args.update(kwargs)
        plt.scatter(self.X[:, 0].ravel(), self.X[:, 1].ravel(), **def_args)
    ...
    def tri(self, aspect=1.0):
        if aspect != 1.0:
            return delaunay.Triangulation(self.X[:, 0], aspect * self.X[:, 1])
        if self._tri is None:
            self._tri = delaunay.Triangulation(self.X[:, 0], self.X[:, 1])
        return self._tri
    ...
    def nn_interper(self, aspect=1.0):
        if aspect != 1.0:
            try:
                ...
                raise Exception(
                    "Request for nearest-neighbors, which was discontinued by mpl"
                )
    ...
    def to_grid(
        ...
        try:
            griddedF = interper[
                aspect * ymin : aspect * ymax : ny * 1j, xmin : xmax : nx * 1j
            ]
    ...
    def write_text(self, fname, sep=" "):
        with open(fname, "wt") as fp:
            for i in range(len(self.F)):
                fp.write(
                    "%f%s%f%s%f\n" % (self.X[i, 0], sep, self.X[i, 1], sep, self.F[i])
                )
    ...
    def decimate(self, factor):
        ...
        return XYZField(
            self.X[chooser, :], self.F[chooser], projection=self.projection()
        )
    ...
    def cs2cs(
        ...
        if pnts.shape != self.X.shape:
            raise Exception(
                "Size of converted points is %s, not %s" % (pnts.shape, self.X.shape)
            )
        self.X = pnts
    ...
    def write_shp(self, shp_name, value_field="value"):
        ...
        ### Create the layer
        olayer = ods.CreateLayer(layer_name, srs=srs, geom_type=ogr.wkbPoint)
...
class PyApolloniusField(XYZField):
    ...
    def __init__(self, X=None, F=None, r=1.1, redundant_factor=None):
        ...
        if (X is None) or (redundant_factor is not None):
            super(PyApolloniusField, self).__init__(
                X=np.zeros((0, 2), np.float64), F=np.zeros(0, np.float64)
            )
        else:
        ...
        for shp_name in shp_names:
            ...
            for i in range(len(layer)):
                ...
                if np.isfinite(scale) and scale > 0.0:
                    ...
                    values.append(scale)
        return PyApolloniusField.from_polylines(
            lines, values, r=r, redundant_factor=redundant_factor
        )
    ...
    class ApolloniusField(XYZField):
        ...
        def insert(self, xy, f):
            ...
            if self.locality_on_insert and self.last_inserted is not None:
                ...
                # up.
                try:
                    self.last_inserted = self.ag.insert(
                        Site_2(pnt, w), self.last_inserted
                    )
                except Exception:  # no direct access to the real type, ArgumentError
                    print(
                        "CGAL doesn't have locality aware bindings.  This might be slower"
                    )
                ...
                s = v.site()
                print(
                    "            %f,%f,%f" % (s.point().x(), s.point().y(), s.weight())
                )
        ...
        def value_at_point(self, pnt):
            ...
            if self.locality_on_query and self.last_query_vertex is not None:
                ...
                try:
                    ...
                    print(
                        "CGAL doesn't have locality aware query bindings.  May be slower."
                    )
            ...
            site = v.site()
            dist = np.sqrt(
                (pnt.x() - site.point().x()) ** 2 + (pnt.y() - site.point().y()) ** 2
            )
        ...
        def read_shps(shp_names, value_field="value", r=1.1, redundant_factor=None):
            ...
            return ApolloniusField.from_polylines(
                lines, values, r=r, redundant_factor=redundant_factor
            )
...
class ConstrainedScaleField(XYZField):
    ...
    def check_all(self):
        ...
        if any(slopes > self.r - 1.0):
            ...
            for e in bad_edges:
                ...
                print(
                    "%d:%f --[L=%g]-- %d:%f > %f"
                    % (a, self.F[a], L, b, self.F[b], allowed)
                )
                print("  " + str(edges[e]))
            return False
        return True
    ...
    def add_point(self, pnt, value, allow_larger=False):
        ...
        if not allow_larger and (value > old_value):
            print(
                "Not adding this point, because it is actually larger than existing ones"
            )
            return None
    ...
    def check_scale(self, i, old_value=None):
        ...
        while len(to_visit) > 0:
            ...
            for e in edges:
                ...
                allowed = min(ORIG + L * (self.r - 1.0), A + La * (self.r - 1.0))
                ...
                # elif (B < 0.8*allowed) and (old_value is not None) and (A<0.8*old_value) and (L > 5*A):
                    ...
                    if alpha < 0.65:
                        ...
                        newL = np.sqrt(
                            (t.x[orig_i] - new_point[0]) ** 2
                            + (t.y[orig_i] - new_point[1]) ** 2
                        )

                        # constrained by valid value based on distance from starting point as well as
                        # the old value
                        new_value = min(
                            ORIG + 0.95 * newL * (self.r - 1.0),  # allowed value
                            0.99 * (alpha * B + (1 - alpha) * old_value),
                        )  # value along the old line
...
class SimpleGrid(QuadrilateralGrid):
    ...
    def copy(self):
        return SimpleGrid(
            extents=list(self.extents), F=self.F.copy(), projection=self.projection()
        )
    ...
    def trace_contour(
        self, vmin, vmax, union=True, method="mpl", gdal_contour="gdal_contour"
    ):
        """
        ...
        if method == "mpl":
            ...
            res = subprocess.run(
                [gdal_contour, "-fl", str(vmin), str(vmax), fname_tif, fname_shp],
                capture_output=True,
            )
            print(res.stdout)
            print(res.stderr)
    ...
    def plot(self, **kwargs):
        ...
        return ims(
            ...
            origin="lower",
            extent=[
                self.extents[0] - 0.5 * dx + offset[0],
                self.extents[1] + 0.5 * dx + offset[0],
                self.extents[2] - 0.5 * dy + offset[1],
                self.extents[3] + 0.5 * dy + offset[1],
            ],
            **kwargs
        )
    ...
    def to_unstructured(self, node_var="z", trim=np.isnan):
        ...
        g = unstructured_grid.UnstructuredGrid(
            max_sides=4, extra_node_fields=[(node_var, self.F.dtype)]
        )
    ...
    def crop(self, rect=None, indexes=None):
        ...
        new_extents = [
            self.extents[0] + min_col * self.dx,
    ...
    def interpolate(self, X, interpolation=None, fallback=True):
        ...
        if interpolation == "nearest":
            ...
            bad = (
                (rows < 0)
                | (rows >= self.F.shape[0] - 1)
                | (cols < 0)
                | (cols >= self.F.shape[1] - 1)
            )
        else:
        ...
        if interpolation == "nearest":
            ...
            result = (
                self.F[rows, cols] * (1.0 - row_alpha) * (1.0 - col_alpha)
        ...
        if interpolation == "linear" and fallback and np.any(bad):
            result[bad] = self.interpolate(
                X[bad], interpolation="nearest", fallback=False
            )
    ...
    def upsample(self, factor=2):
        x = np.linspace(
            self.extents[0], self.extents[1], 1 + factor * (self.F.shape[1] - 1)
        )
        y = np.linspace(
            self.extents[2], self.extents[3], 1 + factor * (self.F.shape[0] - 1)
        )
    ...
    def downsample(self, factor, method="decimate"):
        ...
        if method == "decimate":
            ...
            F2 = F2.transpose([0, 2, 1, 3]).reshape(
                [nr // factor, nc // factor, factor * factor]
            )
    ...
    def polygon_mask(self, poly, crop=True, return_values=False):
        ...
        target_field = SimpleGrid(
            F=np.zeros(self.F.shape, np.int32), extents=self.extents
        )
            ...
            for row in range(len(Y)):
                if np.isfinite(self.F[row, col]):
                    if straddle is None:
                        ...
                        if (
                            not poly.contains(p)
                        ) ^ invert:  # i hope that's the right logic
    ...
    def write_gdal_rgb(self, output_file, **kw):
        ...
        dst_ds.SetGeoTransform(
            [self.extents[0] - 0.5 * dx, dx, 0, self.extents[3] - 0.5 * dy, 0, dy]
        )
    ...
    def warp_to_match(self, target):
        ...
        te = "-te %f %f %f %f " % (
            target.extents[0] - halfdx,
            target.extents[2] - halfdy,
            target.extents[1] + halfdx,
            target.extents[3] + halfdy,
        )
        ts = "-ts %d %d" % (target.F.T.shape)
        return self.warp(target.projection(), extra=te + ts)
    ...
    def warp(self, t_srs, s_srs=None, fn=None, extra=[]):
        ...
        cmd = (
            [self.gdalwarp, "-s_srs", s_srs, "-t_srs", t_srs, "-dstnodata", "nan"]
            + extra
    ...
    def write_gdal(self, output_file, nodata=None, overwrite=False, options=None):
        ...
        dst_ds = driver.Create(
            output_file, self.F.shape[1], self.F.shape[0], 1, gtype, options
        )
        raster = self.F
        ...
        if self.projection() not in ("", None):
            ...
            if srs.SetFromUserInput(self.projection()) != 0:
                log.warning(
                    "Failed to set projection (%s) on GDAL output" % (self.projection())
                )
            dst_ds.SetProjection(srs.ExportToWkt())
    ...
    def extract_tile(
        self, xxyy=None, res=None, match=None, interpolation="linear", missing=np.nan
    ):
        ...
        return SimpleGrid(extents=xxyy, F=Ftmp)
    ...
    def overlay_rgba(self, other):
        ...
        if np.issubdtype(self.F.dtype, np.floating):
            ...
            if self.F[:, :, 3].min() == 255:
                ...
                my_alpha = (self.F[:, :, 3] / 255.0).clip(0, 1)
                new_alpha = alpha + my_alpha * (1 - alpha)
                inv_alpha = 1.0 / new_alpha
                inv_alpha[new_alpha == 0] = 0
        ...
        for chan in range(3):
            self.F[:, :, chan] = (
                self.F[:, :, chan] * (1 - alpha) + other.F[:, :, chan] * alpha
            ) * inv_alpha
...
class GtxGrid(SimpleGrid):
    def __init__(self, filename, is_vertcon=False, missing=9999, projection="WGS84"):
        ...
        ll_lon = (ll_lon + 180) % 360.0 - 180

        nrows, ncols = np.fromstring(fp.read(2 * 4), ">i4")

        heights = np.fromstring(fp.read(nrows * ncols * 8), ">f4").reshape(
            (nrows, ncols)
        )
        heights = (
            heights.byteswap().newbyteorder().astype(np.float64).copy()
        )  # does this fix byte order?
        ...
        SimpleGrid.__init__(
            ...
            extents=[
                ll_lon,
                ll_lon + (ncols - 1) * delta_lon,
                ll_lat,
                ll_lat + (nrows - 1) * delta_lat,
            ],
            F=heights,
    ...
    def __init__(
        ...
        if target_projection is not None:
            if (source_projection is None) or (source_projection == ""):
                raise Exception(
                    "Target projection was given, but there is no source projection for %s"
                    % filename
                )
            ...
            if src_srs.IsSame(tgt_srs):
                ...
                if geo_bounds is not None:
                    ...
                    geo_bounds = proj_utils.reproject_bounds(
                        geo_bounds, target_projection, source_projection, mode="outside"
                    )
        ...
        if geo_bounds is not None:
            ...
            bounds = [ix_start, ix_end, iy_start, iy_end]
...
def rasterize_grid_cells(
    ...
    if stretch:
        vmin = values.min()
        vmax = values.max()
        fac = 1.0 / (vmax - vmin) * (np.iinfo(dtype).max - 1)
        values = 1 + ((values - vmin) * fac).astype(dtype)
    else:
        values = values.astype(np.uint16)
    polys = [g.cell_polygon(c) for c in np.arange(g.Ncells())[cell_mask]]
    poly_ds = wkb2shp.wkb2shp(
        "Memory", polys, fields=dict(VAL=values.astype(np.uint32))
    )
    ...
    # write 1000 into the array where the polygon falls.
    gdal.RasterizeLayer(
        target_ds, [1], poly_ds.GetLayer(0), options=["ATTRIBUTE=VAL"] + extra_options
    )
    # None,None,[1000],[])
    new_raster = GdalGrid(target_ds)
...
if ogr:
    ...
    class BlenderField(Field):
        ...
        def __init__(
            self, shp_fn=None, delegates=None, factory=None, subset=None, shp_data=None
        ):
            ...
            if shp_fn is not None:  # read from shapefile
                ...
                self.shp_fn = None
                self.ic = interp_coverage.InterpCoverage(
                    regions_data=shp_data, subset=subset
                )
        ...
        def bounds(self):
            raise Exception(
                "For now, you have to specify the bounds when gridding a BlenderField"
            )
    ...
    class MultiBlender(Field):
        ...
        def __init__(
            self, shp_fn, factory=None, priority_field="priority", buffer_field=None
        ):
            ...
            for pri in self.priorities:
                ...
                self.bfs.append(
                    BlenderField(shp_data=self.sources, factory=factory, subset=subset)
                )
        ...
        def flatten_with_buffer(self, buffer_field="buffer"):
            ...
            for pri in priorities[::-1]:
                ...
                for sel_idx in sel_idxs:
                    ...
                    if union_geom is not None:
                        ...
                        vis_sel_geom = sources["geom"][sel_idx] = sel_geom.difference(
                            union_geom
                        )
                    else:
            ...
            valid = np.array([(source["geom"].area > 0.0) for source in sources])
            invalid_count = np.sum(~valid)
            if invalid_count:
                print(
                    "MultiBlenderField: %d source polygons were totally obscured"
                    % invalid_count
                )
...
class CompositeField(Field):
    ...
    def __init__(
        ...
        self.shp_fn = shp_fn
        if shp_fn is not None:  # read from shapefile
            self.sources, self.projection = wkb2shp.shp2geom(
                shp_fn, return_srs=True, query=shp_query
            )
        else:
        ...
        if target_date is not None:
            selA = np.array(
                [isnat(d) or d <= target_date for d in self.sources["start_date"]]
            )
            selB = np.array(
                [isnat(d) or d > target_date for d in self.sources["end_date"]]
            )
    ...
    def to_grid(
        ...
        relevant_srcs = np.nonzero(
            [box.intersects(geom) for geom in self.sources["geom"]]
        )[0]
        ...
        for src_i in ordered_srcs:
            ...
            log.info(
                "   data mode: %s  alpha mode: %s"
                % (self.data_mode[src_i], self.alpha_mode[src_i])
            )

            source = self.load_source(
                src_i
            )  # HERE - need to be smarter about overlapping bounds, and also reproject on the fly

            if (
                isinstance(source, SimpleGrid) and source.F.size == 0
            ):  # could be other type of field.
                # So the geom overlapped the current tile, but the raster itself came up empty.
                log.info(
                    "Source %s came up empty after cropping. Check projection, and whether polygon intersects data"
            ...
            src_alpha = SimpleGrid(
                extents=src_data.extents, F=np.ones(src_data.F.shape, "f8")
            )
            ...
            for mode in [self.data_mode[src_i], self.alpha_mode[src_i]]:
                if mode is None or mode.strip() in ["", b""]:
                    continue
            ...
            result_data.F = (
                result_data.F * result_alpha.F * (1 - src_alpha.F)
                + cleaned * src_alpha.F
            )
    ...
    def ortho_diffuser(self, res, aniso, source, src_data, src_geom, result_data):
        ...
        oink = interp_orthogonal.OrthoInterpolator(
            region=src_geom, background_field=result_data, anisotropy=aniso, nom_res=res
        )
        fld = oink.field()

        rast = fld.to_grid(
            bounds=result_data.bounds(), dx=result_data.dx, dy=result_data.dy
        )
        src_data.F[:, :] = rast.F
    ...
    def diffuser(self, src, src_data, src_geom, result_data):
        ...
        curve = linestring_utils.resample_linearring(
            np.array(src_geom.exterior), dx, closed_ring=1
        )
        g = unstructured_grid.UnstructuredGrid()
        nodes, edges = g.add_linestring(curve, closed=True)
        g = triangulate_hole.triangulate_hole(
            g, nodes=nodes, hole_rigidity="all", method="rebay"
        )
        bnodes = g.boundary_cycle()
        bvals = result_data(g.nodes["x"][bnodes])

        nd = quad_laplacian.NodeDiscretization(g)
        dirich = {n: val for n, val in zip(bnodes, bvals)}
...
class MultiRasterField(Field):
    ...
    def __init__(self, raster_file_patterns, **kwargs):
        ...
        for patt in raster_file_patterns:
            ...
            if isinstance(patt, str):
                ...
                raise Exception(
                    "Expected a string regexp or a Field instance. Got %s" % patt
                )
    ...
    def polygon_mask(self, poly, crop=True, return_values=False):
        ...
        assert (
            return_values == True
        ), "MultiRasterField only makes sense for return_values=True"
    ...
    def build_index(self):
        ...
        # these are x,x,y,y
        tuples = [(i, extent, None) for i, extent in enumerate(self.sources["extent"])]
    ...
    def report(self):
        ...
        for fi, rec in enumerate(self.sources):
            print(
                "%4d %4.1f %6.1f: %s"
                % (fi, rec["order"], rec["resolution"], rec["filename"])
            )
    ...
    def source(self, i):
        ...
        if self.sources["field"][i] is None:
            if self.open_count >= self.max_count:
                ...
                current = np.nonzero(
                    (self.sources["last_used"] >= 0)
                    & (self.sources["filename"] != None)
                )[0]
    ...
    def ordered_hits(self, xxyy):
        ...
        hits = hits[
            np.argsort(self.sources[hits], order=("order", "resolution", "filename"))
        ]
        return hits
    ...
    def to_grid(
        ...
        if not len(hits):
            ...
            if dx is None or dy is None:
                raise Exception(
                    "No hits, and dx/dy not specified so resolution is unknown"
                )
...
class TileMaker(object):
    ...
    def tile(self, xmin=None, ymin=None, xmax=None, ymax=None):
        ...
        for xi in range(nx):
            for yi in range(ny):
                ll = [xmin + xi * self.tx, ymin + yi * self.ty]
                ur = [ll[0] + self.tx, ll[1] + self.ty]
                ...
                if self.force or not os.path.exists(output_fn):
                    ...
                    pad_bounds = np.array(
                        [left - pad_x, right + pad_x, bottom - pad_y, top + pad_y]
                    )
                    ...
                    if self.fill_iterations + self.smoothing_iterations > 0:
                        ...
                        blend.fill_by_convolution(
                            self.fill_iterations, self.smoothing_iterations
                        )
                    print("Saving")
                    if self.post_render:
                        blend = self.post_render(
                            blend,
                            output_fn=output_fn,
                            bounds=bounds,
...
if __name__ == "__main__":
    ...
    corr_cv = CurvilinearGrid(
        tile_utm.X, corr_on_tile, projection=tile_utm.projection()
    )
