...
def sun_to_flowgeom(sun, proc, filename, overwrite=True):
    ...
    bdry_edges = (edge_marks > 0) & (
        edge_marks < 4
    )  # non ghost edges which have only 1 cell nbr
    ...
    nc = qnc.empty(
        fn=filename,  # os.path.join(dwaq_dir,'%04d_flowgeom.nc'%proc),
        overwrite=overwrite,
    ...
    nc["FlowElemContour_x"]["nFlowElem", "nFlowElemContourPts"] = g.points[
        g.cells[:, :], 0
    ]
    ...
    nc["FlowElemContour_y"]["nFlowElem", "nFlowElemContourPts"] = g.points[
        g.cells[:, :], 1
    ]
    ...
    if 0:
        ...
        nc["FlowElemDomain"]["nFlowElem"] = (proc * np.ones(g.Ncells())).astype(
            np.int16
        )
        nc["FlowLinkDomain"]["nFlowLink"] = (proc * np.ones(np.sum(edge_sel))).astype(
            np.int16
        )
    else:
    ...
    nc["FlowElemContour_node"]["nFlowElem", "nFlowElemContourPts"] = g.cells.astype(
        "i4"
    )
    ...
    mesh.edge_node_connectivity = (
        "FlowEdge_node"  # attribute required if variables will be defined on edges
    )
    ...
    bounds = np.concatenate((z_bookended[:-1, None], z_bookended[1:, None]), axis=1)
...
def postprocess(sun=None, sun_dir=None, force=False):
    ...
    for proc in range(nprocs):
        nc_fn = os.path.join(
            dfm_dwaq_path, "DFM_DELWAQ_sun_%04d" % proc, "sun_%04d_flowgeom.nc" % proc
        )
...
class SunHydro(waq_scenario.HydroFiles):
                ...
                for feat_id in range(len(flows)):
                    ...
                    if grp.cell_based():
                        ...
                        for cell in sun_cells:
                            g_nodes = [node_sun_to_g(n) for n in sun_g.cells[cell]]
                    ...
                    details = dict(flow_shp=flow_shp, feat_id=feat_id)
                    ...
                    # in bc_elts
                    cells = [
                        c for c in cells if (groups["id"][c] < 0) and (c in bc_elts)
                    ]
                    if len(cells):
                        groups["id"][cells] = ngroups
                        groups["name"][cells] = details.get(
                            "name", "group %d" % ngroups
                        )
                        ...
                        self.log.warning(
                            "Feature %d from %s (name=%s) overlaps another flow or wasn't"
                            " found as a boundary, "
                            " and will be skipped"
                            % (feat_id, flow_shp, details.get("name", "n/a"))
                        )
        ...
        if force or self._bc_lgroups is None:
            ...
            for flow_shp in self.flow_shps:
                ...
                for feat_id in range(len(flows)):
                    ...
                    links = [l for l in bc_links if self.links[l, 1] in cells]
                    ...
                    for l in links:
                        if lgroups["id"][l] < 0:
                            ...
                            self.log.warning(
                                "Feature %d from %s (name=%s) maps to a boundary link"
                    ...
                    if len(clean_links):
                        ...
                        lgroups["name"][clean_links] = details.get(
                            "name", "group %d" % ngroups
                        )
                        ...
                        self.log.warning(
                            "Feature %d from %s (name=%s) wasn't"
                            " found as a boundary, "
                            " and will be skipped"
                            % (feat_id, flow_shp, details.get("name", "n/a"))
                        )
    ...
    def add_parameters(self, hyd):
        ...
        for label, pname in [
            ("vert-diffusion-file", "VertDisper"),
            ...
            if not os.path.exists(fn):
                ...
                continue
            self.log.info(
                "SunDWAQ: seg_fn %s will use %s, symlink=%s"
                % (pname, fn, self.enable_write_symlink)
            )
            hyd[pname] = waq_scenario.ParameterSpatioTemporal(
                seg_func_file=fn, enable_write_symlink=self.enable_write_symlink
            )
...
class SunSplicer(object):
    ...
    def cmd_merge(self, sun_dir, dense=False):
        ...
        agg = waq_scenario.HydroMultiAggregator(
            agg_shp=agg_grid,
