...
def free_node_bounds(points, max_angle=85 * pi / 180.0):
...
def free_node_bounds_conservative(points, max_angle=85 * pi / 180.0):
    ...
    l_leg = l_ab * 0.5 / (tan(max_angle / 2.0) * sin(max_angle))
...
def free_node_bounds_fine(points, max_angle=85 * pi / 180.0, region_steps=3):
    """assumed that the first point can be moved and the
    other two are fixed
...
class OrthoMaker(trigrid.TriGrid):
    ...
    def stats(self):
        ...
        print(
            "angle > 90 degrees: %d  %g%%"
            % (n_bad_angles, 100.0 * n_bad_angles / self.Ncells())
        )
        print(
            "angle > %g degrees: %d  %g%%"
            % (
                self.max_angle * 180 / pi,
                n_med_angles,
    ...
    def pass_three(self, depths=None):
        ...
        for depth in depths:
            ...
            bc = self.bad_cells()
            print(
                "Start of cell-nudge with search depth %d, bad_cell count %d "
                % (depth, len(bc))
            )
    ...
    def try_to_fix_boundary_cell(self, bad_cell, bdry_points, verbose=False):
        ...
        for bdry_point in bdry_points:
            ...
            if (2 * pi - angle) / n_cells > self.max_angle:
                if verbose:
                    print(
                        "Yep, there are %d cells, interior angle %g, violates max angle"
                        % (n_cells, 360 - angle * 180 / pi)
                    )

                # common case - the node is along a straight or nearly straight boundary
                if abs(pi - angle) < 10 * pi / 180.0 and n_cells == 2:
                    ...
                    if angle_at_interior / (n_cells_at_interior - 1) > self.max_angle:
                        if verbose:
                            print(
                                "Hmm - can't remove %d because it would leave too few cells for %d"
                                % (bdry_point, int_point)
                            )
                    else:
                        if verbose:
                            print(
                                "Great - we should merge %d and %d, and remove node %d"
                                % (cellA, cellB, bdry_point)
                            )
    ...
    def merge_cells(self, cellA, cellB, dead_node):
        ...
        points_on_merge_edge = setdiff1d(
            ravel(self.edges[edges_to_merge, :2]), [dead_node]
        )
    ...
    def plot_cells(
        self, c_list, nbr_count=0, label_nodes=True, label_cells=True, label_edges=False
    ):
        ...
        for c in c_list:
            if 0:  # enable if there are possible edge/cell discrepancies
                ...
                plot(self.points[nodes, 0], self.points[nodes, 1], "b-")
        ...
        for c in c_list:
            edge_list = concatenate(
                (edge_list, find(any(self.edges[:, 3:] == c, axis=1)))
            )
    ...
    def try_to_fix_quad(self, node, verbose=False):
        ...
        for i in range(4):
            for j in range(4):
                ...
                if all(
                    sort(self.cells[cells[j]])
                    == sort([node, quad_verts[i], quad_verts[(i + 1) % 4]])
                ):
        ...
        north_points = setdiff1d(
            concatenate((self.cells[cell_nw], self.cells[cell_ne])), [node]
        )
        south_points = setdiff1d(
            concatenate((self.cells[cell_sw], self.cells[cell_se])), [node]
        )
    ...
    def fast_vor_clearances(self):
        ...
        for edge_i in range(3):
            ...
            # third node:
            dist = where(rowdot(AV_perp, AC_perp) < 0, -dist, dist)
    ...
    def can_node_be_repositioned(self, free_node, region_steps=3):
        ...
        for nbr_cell in self.pnt2cells(free_node):
            ...
            free_bounds = free_node_bounds_fine(
                points, max_angle=self.max_angle, region_steps=region_steps
            )
    ...
    def nudge_cell_search(self, bad_cell, depth=1, verbose=False):
        ...
        for node in self.cells[bad_cell]:
            ...
            if self.nudge_node_search(
                node, frozen=frozen, depth=depth, verbose=verbose
            ):
                print("SUCCESS")
    ...
    def nudge_node_search(self, free_node, frozen, depth, verbose=False, history=[]):
        ...
        for i in range(len(nbr_cells)):
            ...
            free_bounds = free_node_bounds_fine(
                points, max_angle=self.max_angle, region_steps=100
            )
            geom = geometry.Polygon(free_bounds)
        ...
        if all_intersection.area > 0.0:
            ...
            print(
                indent,
                "Found a good intersection, nudging node %i without branching"
                % free_node,
            )
            return True
        ...
        if depth > 0:
            ...
            for omit in range(len(nbr_cells)):
                ...
                partial_intersection = intersect_geoms(
                    all_geoms[:omit] + all_geoms[omit + 1 :]
                )
                ...
                if partial_intersection.area > 0.0:
                    ...
                    for nbr_node in self.cells[omitted_cell]:
                        ...
                        if nbr_node not in sub_frozen:
                            res = self.nudge_node_search(
                                nbr_node, sub_frozen, depth - 1, history=history
                            )
                            if res:
    ...
    def plot_spacing(self, max_clearance=20, include_boundary=False):
        ...
        self.plot(
            all_cells=False,
            line_collection_args={"cmap": cm.gray},
            vmax=1000,
            vmin=-1000,
        )
        ...
        self.plot(
            ...
            line_collection_args={"linewidth": 3},
        )
