...
class QuadGrower:
    ...
    def __init__(
        self, grid, nodes=None, seed_point=None, max_nodes=-1, remove_fillets=True
    ):
        self.grid = grid
    ...
    def remove_fillets(self):
        ...
        """
        node_halo = np.unique(
            np.concatenate([self.grid.node_to_nodes(n) for n in self.nodes])
        )
        ...
        for a, b in utils.circular_pairs(self.nodes):
            ...
            if len(opp_nodes) == 3:
                ...
                if (self.grid.cell_Nsides(c1) == 4) and (
                    self.grid.cell_Nsides(c2) == 4
                ):
                    print(" fillet!")
    ...
    def setup_ij(self):
        ...
        (self.node_idxs, self.ijs) = self.grid.select_quad_subset(
            ctr=None, node_set=node_halo
        )
    ...
    def setup_norm_dir(self):
        ...
        # versus the halo
        boundary_ij = np.array(
            [self.n_to_ij[n] for n in self.nodes if n in self.n_to_ij]
        )
    ...
    def ij_to_xy(self, ij):
        ...
        edges = np.concatenate([grid.node_to_edges(n) for n in nbrs]).astype(np.int32)
        ...
        edges = [
            ...
            for j in edges
            if (
                (grid.edges["nodes"][j, 0] in self.n_to_ij)
    ...
    def ij_to_node_or_add(self, ij):
        ...
        if n < 0:
            ...
            self.n_to_ij[n] = np.array(
                ij
            )  # array to be consistent with existing entries
