...
def base_model(force=True, num_procs=1, run_dir="run-dfm-test"):
    ...
    periodic_Q = 50 * np.sin(
        (t - t[0]) / np.timedelta64(1, "s") * 2 * np.pi / (4 * 3600.0)
    )
    Q = xr.DataArray(periodic_Q, dims=["time"], coords={"time": t})
    inflow = dflow_model.FlowBC(name="inflow", geom=np.array([[0, 0], [0, 500]]), Q=Q)
...
def test_agg():
    ...
    hyd_path = os.path.join(
        model.run_dir, "DFM_DELWAQ_%s" % model.mdu.name, "%s.hyd" % model.mdu.name
    )
    assert os.path.exists(hyd_path)

    waq_hydro_editor.main(
        args=["-i", hyd_path, "-a", agg_shp_fn, "-o", "output_agg/output"]
    )

    # aggregated accumulates some additional error - 1e-6 is not alarming.
    waq_hydro_editor.main(
        args=["-i", "output_agg/com-output.hyd", "-c"]
    )  # aggregated run
    ...
    waq_hydro_editor.main(
        args=["-i", "output_agg/com-output.hyd", "-l", "-o", "output_agg_lp/output"]
    )
...
def test_mpi():
    ...
    waq_hydro_editor.main(
        args=["-m", "run-dfm-test-mpi/flowfm.mdu", "-s", "-o", "output_splice/output"]
    )
