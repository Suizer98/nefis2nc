...
def greedy_edge_mean_to_node(g, orig_node_depth=None, edge_depth=None, n_iter=100):
    ...
    for loop in range(n_iter):
        ...
        # nodes=
        jj_nbrs = np.concatenate([g.node_to_edges(n) for n in g.edges["nodes"][j_bad]])
        ...
        def cost(ds):
            ...
            conn_depth[n_bad] = ds
            new_errors = (
                conn_depth[g.edges["nodes"][jj_nbrs]].mean(axis=1) - edge_depth[jj_nbrs]
            )
            # weight high edges 10x more than low edges:
            cost = (
                new_errors.clip(0, np.inf).sum()
                - 0.5 * new_errors.clip(-np.inf, 0).sum()
            )
            return cost
        ...
        if verbose:
            print(
                "Loop %d: %d/%d edges  starting error: j=%d => %.4f"
                % (loop, potential.sum(), len(potential), j_bad, errors[j_bad])
            )
...
def min_graph_elevation_for_edge(g, dem, j, starts="lowest"):
    ...
    if (
        (tile.extents[0] > xxyy[0])
        or (tile.extents[1] < xxyy[1])
        or (tile.extents[2] > xxyy[2])
        or (tile.extents[3] < xxyy[3])
    ):
    ...
    if starts in ["circumcenter", "centroid"]:
        ...
        ijs = np.array(
            [[i0[linear0_min], j0[linear0_min]], [i1[linear1_min], j1[linear1_min]]]
        )
    else:
