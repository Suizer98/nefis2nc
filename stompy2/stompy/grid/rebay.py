...
class RebayAdvancingDelaunay(front.AdvancingFront):
    ...
    def init_rebay(self):
        ...
        good_ext = (
            (e2c < 0) | (g.cells["stat"][e2c] == DONE) | (g.cells["stat"][e2c] == EXT)
        )
    ...
    def add_next_node(self, c_target, j):
        ...
        g = self.grid
        xm = 0.5 * (
            g.nodes["x"][g.edges["nodes"][j, 0]] + g.nodes["x"][g.edges["nodes"][j, 1]]
        )
        self.xm = xm
    ...
    def update_state(self, new_n):
        ...
        for c in live_cells[~done]:
            # ACTIVE or WAITING?
            nbrs = g.cell_to_cells(
                c, ordered=True
            )  # ordered doesn't really matter I guess
            for nbr in nbrs:
