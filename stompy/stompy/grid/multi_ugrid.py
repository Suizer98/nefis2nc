...
class MultiVar(object):
    ...
    def __init__(self, mu, sub_vars):
        ...
        for dim in self.dims:
            if self.mu.rev_meta.get(dim, None) in [
                "face_dimension",
                "node_dimension",
                "edge_dimension",
            ]:
    ...
    def isel(self, **kwargs):
        ...
        assert (
            len(part_kwargs) <= 1
        ), "Not ready for multiple partitioned dimensions on one var"
        ...
        for key in part_kwargs:
            ...
            if val.shape == ():  # scalar
                ...
                svs = [mv.sub_vars[proc].isel(**{key: loc}) for proc, loc in g2l[val]]
                return xr.concat(svs, dim=key)

    def sel(self, **kwargs):
        return MultiVar(self.mu, [da.sel(**kwargs) for da in self.sub_vars])
...
class MultiUgrid(object):
    ...
    def __init__(
        ...
        ** (grid_kwargs): keyword arguments passed to read_ugrid.
        ...
        self.grids = [
            unstructured_grid.UnstructuredGrid.read_ugrid(ds, **grid_kwargs)
            for ds in self.dss
        ]
        ...
        if cleanup_dfm == "auto":
            cleanup_dfm = "Deltares" in self.dss[0].attrs.get(
                "Conventions", ""
        ...
        if cleanup_dfm:
            for g in self.grids:
                ...
                for f in ["mesh2d_face_x_bnd", "mesh2d_face_y_bnd"]:
                    if f in g.cells.dtype.names:
                        log.warning(
                            "Dropping extra cell field %s to avoid max_sides issues" % f
                        )
            ...
            for ds in self.dss:
                if ("nFlowElem" not in ds.dims) or ("nNetElem" not in ds.dims):
                    break
                if ds.dims["nFlowElem"] != ds.dims["nNetElem"]:
                    log.warning(
                        "Expected dimensions nFlowElem and nNetElem to be duplicates, but %d!=%d"
    ...
    def load(self, **xr_kwargs):
        if isinstance(self.paths[0], str):
            ...
            raise Exception(
                "Unsure whether paths has Datasets or paths (%s)" % self.paths[0]
            )
    ...
    def create_global_grid_and_mapping(self, grid=None, match_grid_tol=1e-3):
        ...
        if grid is None:
            ...
            self.grid.add_cell_field(
                "ghostness",
                np.zeros(self.grid.Ncells(), np.int32),
                on_exists="overwrite",
            )
            # initialize to -1 for unset.
            self.grid.add_cell_field(
                "proc",
                np.zeros(self.grid.Ncells(), np.int32) - 1,
                on_exists="overwrite",
            )
            generate = False
        ...
        for gnum, g in enumerate(self.grids):
            ...
            if generate:
                if gnum == 0:
                    ...
                    n_map, j_map, c_map = self.grid.add_grid(
                        g, merge_nodes="auto", tol=self.merge_tol
                    )
        ...
        if k in varnames:
            ...
            if k in self.rev_meta:
                ...
                if meta == "face_dimension":
                    ...
                    raise KeyError(
                        "Not ready for on-the-fly coordinate for partitioned dimension..."
                    )
            else:
        ...
        if valid:
            ...
            for d in self.dss[0][k].dims:
                if d in self.rev_meta:
                    ...
                    break
            if partitioned:
                return MultiVar(self, [ds[k] for ds in self.dss])
            else:
    ...
    def isel(self, **kwargs):
        ...
        for dv in sub.data_vars:
            ...
            if len(part_dims) == 0:
                ...
                if free_part_dims:
                    log.info(
                        "Dropping %s because it has unselected partitioned dimensions"
                        % dv
                    )
                    continue
                ...
                v_part_kwargs = {
                    d: part_kwargs[d] for d in part_kwargs if d in sub[dv].dims
                }
