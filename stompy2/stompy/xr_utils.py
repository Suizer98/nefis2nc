...
def gradient(ds, varname, coord):
    ...
    for idx in np.ndindex(*newshape):
        ...
        if np.sum(valid) > 1:
            ...
            result[idx] = np.nan
    return xr.DataArray(
        result, coords=[ds[dim] for dim in newdims], name="d%s/d%s" % (varname, coord)
    )
...
def redimension(ds, new_dims, intragroup_dim=None, inplace=False, save_mapping=False):
    """
    ...
    orig_dims = [ds[vname].values.copy() for vname in new_dims]
    ...
    for und in uni_new_dims:
        try:
            if np.any(und < 0):
                log.warning(
                    "New dimensions have negative values -- will continue but you probably want to drop those first"
                )
        except TypeError:
    ...
    new_idxs = [np.searchsorted(und, od) for und, od in zip(uni_new_dims, orig_dims)]
...
def first_finite(da, dim):
    ...
    new_shape = [slice(length) for d, length in enumerate(da.shape) if d != dimi]
...
def z_from_sigma(dataset, variable, interfaces=False, dz=False):
    ...
    if dz:
        assert (
            interfaces is not False
        ), "Currently must enable interfaces to get thickness dz"
    ...
    for v in sigma_vars:
        if set(dataset[v].dims) <= set(da_dims):
            assert (
                sigma_ctr_v is None
            ), "Multiple matches for layer center sigma coordinate"
    ...
    if interfaces is False:
        ...
        for v in maybe_int_vars:
            ...
            if len(dataset[ctr_only[0]]) + 1 == len(dataset[int_only[0]]):
                assert (
                    sigma_int_v is None
                ), "Multiple matches for layer interface sigma coordinate"
