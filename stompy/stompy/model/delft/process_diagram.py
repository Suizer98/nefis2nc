...
class ProcDiagram(object):
    ...
    def load_tables(self):
        ...
        table_dir = os.path.join(
            self.delft_src, "src/engines_gpl/waq/packages/waq_kernel/src/waq_tables"
        )
                ...
                if m:
                    ...
                    while 1:
                        ...
                        if m:
                            ...
                            l = six.next(lines).strip()
                            input_src = (
                                re.match(r"\s*[Uu]sing (.*)$", l).group(1).strip()
                            )
                            ...
                            if input_src.startswith("substance"):
                                ...
                                m = re.match(r"output from proces \[(.*)\]", input_src)
                                src = ("process", m.group(1).strip())
                            elif input_src.startswith("flux from proces"):
                                m = re.match(r"flux from proces \[(.*)\]", input_src)
                                src = ("process", m.group(1).strip())
    ...
    def init_dot(self):
        ...
        dot = Digraph(
            name="schematic-%s" % self.run_name,
            comment="WAQ Processes",
            strict=True,
            engine="dot",
        )
