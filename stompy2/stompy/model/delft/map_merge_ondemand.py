...
from xr.core.pycompat import PY3, OrderedDict, basestring, iteritems, suppress
...
class DFMMergeMap(xr.backends.AbstractDataStore):
    ...
    def scan_datasets(self):
        ...
        max_elem_global = max(
            [ds.FlowElemGlobalNr.values.max() for ds in self.each_ds()]
        )
        min_elem_global = min(
            [ds.FlowElemGlobalNr.values.min() for ds in self.each_ds()]
        )
    ...
    def merge_var(self, name, var0):
        ...
        v_data = v.data  #
        v_attributes = OrderedDict((k, v.getncattr(k)) for k in v.ncattrs())
    ...
    def load(self):
        ...
        return (
            xr.core.utils.FrozenOrderedDict(ds0.variables),
...
class NetCDF4DataStore(WritableCFDataStore, DataStorePickleMixin):
    ...
    def open(
        ...
        if len(filename) == 88 and LooseVersion(nc4.__version__) < "1.3.1":
            warnings.warn(
        ...
        ds = opener()
        return cls(
            ds, mode=mode, writer=writer, opener=opener, autoclose=autoclose, lock=lock
        )
    ...
    def get_attrs(self):
        with self.ensure_open(autoclose=True):
            attrs = FrozenOrderedDict(
                (k, self.ds.getncattr(k)) for k in self.ds.ncattrs()
            )
        return attrs

    def get_dimensions(self):
        with self.ensure_open(autoclose=True):
            dims = FrozenOrderedDict(
