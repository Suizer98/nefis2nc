...
def fetch_range(lon_range, lat_range, time_range, cache_dir):
    ...
    times = pd.date_range(
        start=utils.floor_dt64(time_range[0], np.timedelta64(24, "h")),
    ...
    for idx, t in enumerate(times):
        ...
        cache_name = os.path.join(
            cache_dir,
            "%s-%.2f_%.2f_%.2f_%.2f.nc"
            % (time_str, lon_range[0], lon_range[1], lat_range[0], lat_range[1]),
        )
...
def fetch_one_day(t, output_fn, lon_range, lat_range):
    ...
    # that's unkind.
    if t >= datetime.datetime(2017, 2, 1, 0, 0) and t < datetime.datetime(
        2017, 6, 1, 12, 0
    ):
        ncss_base_url = "http://ncss.hycom.org/thredds/ncss/GLBv0.08/expt_92.8"
    elif t >= datetime.datetime(2017, 6, 1, 12, 0) and t <= datetime.datetime(
        2017, 10, 1, 9, 0
    ):
        ncss_base_url = "http://ncss.hycom.org/thredds/ncss/GLBv0.08/expt_57.7"
    elif t >= datetime.datetime(2017, 10, 1, 12, 0) and t <= datetime.datetime(
        2018, 3, 17, 9, 0
    ):
    ...
    time_args = [
        ("time_start", t.strftime(time_fmt)),
    ...
    if valid:
        with open(output_fn, "rb") as fp:
            head = fp.read(1000)
            if (b"500 Internal Server Error" in head) or (b"No such file" in head):
