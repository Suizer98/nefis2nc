...
class ParticlesKGS(basic.UgridParticles):
    ...
    def load_grid(self, grid=None):
        ...
        def alpha_beta(
            c, j
        ):  # coefficient on u[j] with respect to cell center velocity of c
            return (
                (1.0 / self.Ac[c])
                * self.edge_norm[j]
    ...
    def update_particle_velocity_for_new_step(self):
        ...
        if 1:  # And now we do some extra work
            ...
            if self.dt_s is None:
                # disgusting!
                self.dt_s = (
                    np.diff(self.current_nc.time[:2])[0]
            ...
            for c in self.g.valid_cell_iter():
                ...
                for ji, j in enumerate(c_edges):
                    ...
                    self.mAs[c, 2, ji] = (1e5 * self.bndry[j]) + (
                        1.0 / self.Ac[c]
                    ) * dz_edge[j] * self.edge_len[j] * self.signs[c, ji]
        else:
    ...
    def set_coeffs(self):
        ...
        if 0:  # old iterative way:
            ...
            for c in range(self.g.Ncells()):
                c_edge = self.cell_edges[c, :]
                u_norm = self.edge_flux[c_edge] / (
                    self.dz_cell[c] * self.edge_len[c_edge]
                )
        ...
        if 1:  # vectorized
            ...
            u_norms = self.edge_flux[self.cell_edges] / (
                self.dz_cell[:, None] * self.edge_len[self.cell_edges]
            )
    ...
    def move_particles_ode(self, stop_t):
        ...
        for i, p in enumerate(self.P):
            ...
            new_c = None
            for meth in [
                dict(name="vode", method="bdf", order=3, nsteps=3000),
    ...
    def move_particles(self, stop_t):
        ...
        for i, p in enumerate(self.P):
            ...
            for it in range(2000):
                if start_t >= stop_t:
                    break
                (part_xy, start_t, cell, last_edge) = self.move_particle_in_cell(
                    part_xy, start_t, stop_t, cell, last_edge
                )
                # traj.append(part_xy)
            else:
    ...
    def plot_cell_velocity(self, c, ax):
        ...
        UV = np.array([self.vel_interp(c, xy) for xy in XY])

        ax.quiver(XY[:, 0], XY[:, 1], UV[:, 0], UV[:, 1])
    ...
    def exact_integrator(self, cell):
        ...
        if det != 0:  # and np.isreal(eps):
            ...
            def xy(
                part_xy, t, eps=eps, lamb_avg=lamb_avg, a=a, b=b, c=c, d=d, bx=bx, by=by
            ):
                x0, y0 = part_xy - cc
