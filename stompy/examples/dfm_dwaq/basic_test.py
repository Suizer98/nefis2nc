...
def base_model(force=True, num_procs=1, run_dir="dfm_run"):
    ...
    inflow = hm.FlowBC(name="inflow", geom=np.array([[0, 0], [0, 100]]), flow=Q)
    ...
    point_src = hm.SourceSinkBC(name="pnt_source", geom=np.array([300, 300]), flow=10)
...
wm = dwaq.WaqModel(overwrite=True, base_path="dwaq_run", hydro=base_hydro)
...
for ti in range(len(ds.time) - 1):
    for ax, scal in zip(axs, tracers):
        ...
        ccoll = grid_ds.plot_cells(
            values=ds[scal].isel(time=ti, layer=0), ax=ax, cmap=cmap, clim=clim
        )
        ax.axis("equal")
