...
class ReadGrid(Op):
    ...
    def run(self, args):
        ...
        stack.append(g)
        log.info(
            "Read grid (%d cells, %d edges, %d nodes)"
            % (g.Ncells(), g.Nedges(), g.Nnodes())
        )
...
class Dualify(Op):
    def run(self, args):
        g = stack.pop()
        gd = g.create_dual(
            center="circumcenter", create_cells=True, remove_disconnected=True
        )
        stack.append(gd)
...
parser.add_argument(
    "-i",
    "--input",
    help="Read a grid, fmt one of %s" % ReadGrid.format_list(),
    metavar="fmt:path",
    nargs=1,
    action=ReadGrid,
)
parser.add_argument(
    "-o",
    "--output",
    help="Write a grid, fmt one of %s" % WriteGrid.format_list(),
    metavar="fmt:path",
    nargs=1,
    action=WriteGrid,
)
parser.add_argument(
    "-d", "--dual", help="Convert to dual of grid", nargs=0, action=Dualify
)
parser.add_argument(
    "-c",
    "--clobber",
    help="Allow overwriting of existing grids",
    nargs=0,
    action=SetClobber,
)
