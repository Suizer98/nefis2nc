...
class GlobalForcing(object):
    ...
    def add_groups_bulk(self, defs=None, shp=None, bc_type_field="bc_type"):
        ...
        for feat_id in range(len(defs)):
            fields = {fld: defs[fld][feat_id] for fld in defs.dtype.names}
            geo = fields["geom"]
            points = np.array(geo)
            ...
            if typ in ["SURFACE", "BED"]:
                ...
                if len(points) == 1:
                    ...
                    grp = self.new_group(points=[points[0], points[-1]])
            groups[feat_id] = grp
...
class ForcingGroup(object):
    ...
    def __init__(
    ...
    ):  # edges with centers within the given shapely.Polygon
        self.gforce = gforce
        self.edges = None
        self.cells = None
        ...
        if boundary_cells:
            if self.cells is not None:
                raise Exception(
                    "boundary_cells specified, but cells have already been chosen"
                )
            if self.edges is None:
                raise Exception(
                    "boundary_cells specified, but no edges have been chosen"
                )
        ...
        if (self.edges is None or len(self.edges) == 0) and (
            self.cells is None or len(self.cells) == 0
        ):
    ...
    def copy(self):
        fg = ForcingGroup(gforce=self.gforce, edges=self.edges, cells=self.cells)
    ...
    def hydro_datasource(self):
        ...
        for tup in self.datasources:
            if tup[0] in hydro_forcings:
                print(
                    "NB: just returning the first datasource.  there could be a second one"
                )
                return tup[1]
        return None
            ...
            if "cells" in self.spec and self.spec["cells"] is "all":
                ...
                for i in c.cells:
                    try:
                        ...
                        print(
                            "Mapped global cell %d to local cell %d on grids %s,%s"
                            % (i, new_c, oldg.processor, newg.processor)
                        )
...
class Constant2Vector(DataSource):
    ...
    def plot_overview(self, tmin, tmax):
        ...
        # for the given simulation period, as specified by absdays
        quiver(
            [0.5 * (date2num(tmin) + date2num(tmax))],
            [0],
            [self.value[0]],
            [self.value[1]],
        )
        ...
        annotate(
            "Constant 2-vector: (%f,%f)" % (self.value[0], self.value[1]),
...
class Harmonics(DataSource):
    ...
    def calc_values(self, t):
        ...
        if t.shape:
            v = (
                self.amplitudes * cos(self.omegas * t[..., newaxis] + self.phases)
            ).sum(axis=1)
        else:
    ...
    def write_config(self, fp, sun):
        if len(self.omegas) != len(self.phases) or len(self.phases) != len(
            self.amplitudes
        ):
...
class OtisHarmonics(Harmonics):
    ...
    def __init__(self, label, otis_output, h_offset=0.0):
        ...
        Harmonics.__init__(
            self, label=label, omegas=omegas, phases=phases, amplitudes=amplitudes
        )
    ...
    def parse_otis(self, fn):
        ...
        for constituent in constituents:
            ...
            period = 1.0 / (speed / 360.0)
            omega = speed / 3600.0 * pi / 180.0
...
class Timeseries(DataSource):
    def __init__(self, label, t0=None, dt=None, data=None, lag_s=None):
        ...
        if self.data is not None and self.t0 is not None and self.dt is not None:
            self.absdays = date2num(self.t0) + arange(len(self.data)) * self.dt / (
                24 * 3600.0
            )
    ...
    def calc_values(self, t):
        ...
        my_t = date2num(self.t0) + (self.dt / (24.0 * 3600.0)) * arange(len(self.data))
    ...
    def set_times(self, sun, Tabsdays):
        ...
        if len(Tabsdays) > 0:
            ...
            if len(Tabsdays) > 1:
                ...
                total_seconds = (
                    tdelta.days * 86400 + tdelta.seconds + tdelta.microseconds * 1e-6
                )
...
class Timeseries2Vector(TimeseriesVector):
    ...
    def plot_overview(self, tmin, tmax):
        ...
        # plot something representative into a single axes,
        # for the given simulation period, as specified by absdays
        quiver(self.absdays, 0 * self.absdays, self.data[:, 0], self.data[:, 1])
        axis(ymin=-1, ymax=1)
        DataSource.plot_overview(self, tmin, tmax)
...
class NoaaHarmonics(Harmonics):
    ...
    def get_data(self):
        ...
        [
            ...
            self.amplitudes,
            self.phases,
        ] = opendap.convert_noaa_to_otis_xtides(int(self.gage.external_id), start_year)
    ...
    def choose_constituents(self, n_consts):
        if type(n_consts) == int:
            ...
            noaa_names, consts = opendap.get_prediction_consts(
                int(self.gage.external_id)
            )
...
class NoaaGage(Timeseries):
    ...
    def __init__(
        ...
        Timeseries.__init__(
            self, label="%s observed tidal height" % station_name, lag_s=lag_s
        )
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        self.h = lp_filter.lowpass(
            self.raw_h, absdays * 24, cutoff=3.0  # Time in hours
        )  # cutoff of 3 hours
...
class CompositeNoaaGage(NoaaGage):
    ...
    def fill_in_missing_data(self, sim_start, sim_end, data):
        ...
        new_data[searchsorted(new_data[:, 0] + 0.5 * basic_dt, data[:, 0]), 1] = data[
            :, 1
        ]
        ...
        for i in range(len(gap_starts)):
            ...
            if (
                gap_starts[i] != 0
                and gap_ends[i] != data.shape[0]
                and (gap_ends[i] - gap_starts[i] <= self.max_missing_samples)
            ):
                data[gap_starts[i] : gap_ends[i], 1] = f(
                    data[gap_starts[i] : gap_ends[i], 0]
                )
            else:
        ...
        if self.backup_station_name is not None:
            ...
            f = interp1d(
                backup_observations[:, 0],
                backup_observations[:, 1],
                bounds_error=False,
                fill_value=nan,
            )
...
class MergeTidalTimeseriesFilter(Timeseries):
    ...
    def fill_in_missing_data(self, sim_start, sim_end, data):
        ...
        if isnan(basic_dt):
            try:
                basic_dt = self.pri.parms["dt_s"] / 86400.0
            except:
                pass
        if isnan(basic_dt):
            try:
                basic_dt = self.sec.parms["dt_s"] / 86400.0
            except:
                pass
        if isnan(basic_dt):
        ...
        for i in range(len(gap_starts)):
            ...
            if (
                f
                and gap_t_start > f_tmin
                and gap_t_end < f_tmax
                and (gap_ends[i] - gap_starts[i] <= self.max_missing_samples)
            ):
                new_data[gap_starts[i] : gap_ends[i], 1] = f(
                    new_data[gap_starts[i] : gap_ends[i], 0]
                )
            else:
                ...
                right_w_overlap = min(
                    gap_ends[i] + self.max_missing_samples, len(new_data)
                )
                ...
                if len(overlap_data) > 0:
                    if all(overlap_data[:, 0] <= new_data[gap_starts[i], 0]) or all(
                        overlap_data[:, 0] >= new_data[gap_ends[i], 0]
                    ):
                        degree = 0
                    else:
                        degree = 1
                    error = overlap_data[:, 1] - fbackup(overlap_data[:, 0])
                    fit = polyfit(overlap_data[:, 0], error, degree)
...
def read_cimis_csv(filename, col_names):
    ...
    if filename is None:
        filename = os.path.join(
            os.environ["HOME"],
            "classes/research/suntans/forcing/wind/cimis/union_city_171_2005_2010.csv",
        )
    ...
    for line in fp:
        ...
        record = [date2num(d) + (h + m / 60.0) / 24.0]
...
class CimisEvapPrecip(Timeseries):
    ...
    def __init__(self, name, csv_file=None, lag_s=0.0):
        ...
        Timeseries.__init__(self, label="%s CIMIS evap-precip" % name, lag_s=lag_s)
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        valid = (vals[:, 0] > date2num(sim_start) - 1) & (
            vals[:, 0] < date2num(sim_end) + 1
        )
        vals = vals[valid, :]
...
class JapaneseEvapPrecip(Timeseries):
    ...
    def __init__(self, name, tab_delim_file=None, lag_s=0.0):
        ...
        Timeseries.__init__(self, label="%s Japanese precip" % name, lag_s=lag_s)
...
class CimisWind(Timeseries2Vector):
    ...
    def __init__(self, name, csv_file=None, lag_s=0.0):
        ...
        Timeseries2Vector.__init__(self, label="%s CIMIS wind" % name, lag_s=lag_s)
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        or tidal filtering
        """
        data = read_cimis_csv(self.csv_file, ["Wind Speed (m/s)", "Wind Dir (0-360)"])
        ...
        #
        wind_dir = (
            (180 + 90 - data[:, 2]) * pi / 180.0
        )  # radians CCW from +x, velocity vector
...
class JapaneseWind(Timeseries2Vector):
    ...
    def __init__(self, name, tab_delim_file=None, lag_s=0.0):
        ...
        Timeseries2Vector.__init__(self, label="%s Japanese wind" % name, lag_s=lag_s)
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        or tidal filtering
        """
        data = read_Japanese_met(
            self.tab_delim_file, ["Wind Speed (m/s)", "Wind Dir (0-360)"]
        )
...
class NoaaWind(Timeseries2Vector):
    ...
    def __init__(self, station_name, lag_s=0.0, lowpass_hours=0):
        Timeseries2Vector.__init__(
            self, label="%s vector wind" % station_name, lag_s=lag_s
        )
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        if len(absdays) < 12:
            print(
                "NOAA Wind Data for station %s, period %s - %s is no good"
                % (self.station_name, sim_start, sim_end)
            )
            print(absdays)
            print(uv_values)
        ...
        if self.lowpass_hours > 0:
            uv_values[:, 0] = lp_filter.lowpass(
                uv_values[:, 0], absdays, self.lowpass_hours / 24.0, order=4
            )
            uv_values[:, 1] = lp_filter.lowpass(
                uv_values[:, 1], absdays, self.lowpass_hours / 24.0, order=4
            )
...
class SFBayMeanEvaporation(Timeseries):
    ...
    orig_data = array(
        [1.27, 1.81, 3.60, 5.28, 6.85, 7.82, 8.42, 7.39, 5.74, 3.78, 1.98, 1.28]
    )
    ...
    def __init__(self, lag_s=0.0):
        Timeseries.__init__(
            self, label="SF Bay evaporation, monthly climatology", lag_s=lag_s
        )
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        start_absday = date2num(
            datetime.datetime(all_months[0] // 12, (all_months[0] % 12) + 1, 15)
        )
        end_absday = date2num(
            datetime.datetime(all_months[-1] // 12, (all_months[-1] % 12) + 1, 15)
        )
...
class NDOI(Timeseries):
    ...
    def __init__(self, amplification=1.0, lag_s=0.0):
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        if 0:
            ...
            d = loadtxt(
                ...
                delimiter=",",
                converters={
                    0: lambda s: date2num(datetime.datetime.strptime(s, "%d-%b-%y"))
                },
            )
            # [cfs] => [m3/s]
            d[:, 1] *= 0.028316847
...
class FlowCsvMgd(Timeseries):
    ...
    def raw_data(self, sim_start, sim_end):
        ts = timeseries.Timeseries.load_csv(
            self.fn, skiprows=1, date_fmt="%Y-%m-%d %H:%M"
        )
...
class UsgsGage(Timeseries):
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        self.url = (
            ...
            + "site_no=%d&cb_00060=on&begin_date=%s&end_date=%s&format=rdb"
            % (self.station_code, begin_date, end_date)
        )
        print(self.url)
        fp = urlopen(self.url)
        ...
        if any(invalid):
            print(
                "USGS gage %s has some missing data.  will attempt to interpolate"
                % self.station_code
            )
            cumecs[invalid] = interp(
                absdays[invalid],
                absdays[~invalid],
                cumecs[~invalid],
                left=cumecs[~invalid][0],
                right=cumecs[~invalid][-1],
            )
...
class CompositeUsgsGage(Timeseries):
    ...
    def raw_data(self, sim_start, sim_end):
        ...
        for gage_id, weight in zip(self.gage_ids, self.weights):
            one_gage = UsgsGage(
                station_code=gage_id, label="temp", amplification=self.amplification
            )
...
class LowpassTimeseries(Timeseries):
    ...
    def raw_data(self, start_datetime, end_datetime):
        ...
        source_times, source_data = self.source.raw_data(
            start_datetime - pad, end_datetime + pad
        )

        lp_data = lp_filter.lowpass(
            source_data, source_times, self.cutoff_days, order=self.order
        )
...
class FillByLastValid(Timeseries):
    ...
    def raw_data(self, start_datetime, end_datetime):
        ...
        make sure it's evenly spaced, low-pass filter, truncate, and return
        """
        source_times, source_data = self.source.raw_data(start_datetime, end_datetime)
...
class ShiftTimeseries(Timeseries):
    ...
    def raw_data(self, start_datetime, end_datetime):
        ...
        make sure it's evenly spaced, low-pass filter, truncate, and return
        """
        delay_delta = datetime.timedelta(self.delay_s / 86400.0)

        print("ShiftTimeSeries: end_datetime: %s" % end_datetime)
        print("                 shifted end: %s" % (end_datetime - delay_delta))

        source_times, source_data = self.source.raw_data(
            start_datetime - delay_delta, end_datetime - delay_delta
        )

        source_times += self.delay_s / 86400.0

        print(
            "Resulting range of data: %s - %s"
            % (num2date(source_times[0]), num2date(source_times[-1]))
        )
    ...
    if version == 2:
        ...
        for data_i in range(data_count):
            ...
            if dtype in ("TIMESERIES", "TIMESERIES_2VEC"):
                ...
                t0 = base_date + datetime.timedelta(t0_sun_seconds / (24.0 * 3600.0))
                ...
                if dtype == "TIMESERIES":
                    ...
                    for i in range(sample_count):
                        values[i] = tok_float()
                    dsource = Timeseries(
                        "timeseries%i" % data_i, t0=t0, dt=dt, data=values, lag_s=0
                    )
                else:
                    ...
                    dsource = Timeseries2Vector(
                        "timeseries2vec%i" % data_i, t0=t0, dt=dt, data=values, lag_s=0
                    )
            elif dtype == "CONSTANT":
                value = tok_float("VALUE")
                dsource = Constant("const%i" % data_i, value=value)
                ...
                for sec, vals in zip(
                    ["OMEGAS", "PHASES", "AMPLITUDES"], [omegas, phases, amps]
                ):
                    tok_tag(sec)
...
def read_datasource(fn, sun):
    ...
    if ds_type == "TIMESERIES":
        ...
        tok_tag("END_DATA")
        dsource = Timeseries(comment, t0=t0, dt=dt, data=data, lag_s=0)
    else:
