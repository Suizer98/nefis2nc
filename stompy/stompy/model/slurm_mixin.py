...
class SlurmMixin:
    ...
    def slurm_pack_options(cls, n):
        ...
        if len(packs):
            ...
            print(
                f"Scanned PACK_GROUPS: {n} requested can be satisfied with {options[0]}",
                flush=True,
            )
            return options
        ...
        if n_het_tasks > 0:
            print(
                f"Scanned PACK_GROUPS and found {n_het_tasks} < {n}. No can do.",
                flush=True,
            )
            ...
            if n_tasks == n:
                ...
                options = ["-n", str(n)]
                print(
                    f"Homogeneous oversized job.  Add {' '.join(options)}", flush=True
                )
                return options
