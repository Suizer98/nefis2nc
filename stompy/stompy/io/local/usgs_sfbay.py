...
def cruise_dataset(start, stop):
    ...
    time_slc = slice(
        *np.searchsorted(full_remote_usgs_ds["s.time"].values, [start, stop])
    )
    remote_usgs_ds = full_remote_usgs_ds.isel(s=time_slc)

    # Drop the annoying s. prefix
    renames = dict([(v, v[2:]) for v in remote_usgs_ds.data_vars if v.startswith("s.")])
    ...
    if 1:
        ...
        spatial = ["StationNumber", "latitude", "longitude", "StationName"]
        for fld in spatial:
            ...
            try:
                ...
                try:
                    ds4[fld] = (
                        ds4[fld].fillna(b"").max(dim="date").max(dim="prof_sample")
                    )
                except TypeError:
                    ds4[fld] = (
                        ds4[fld].fillna("").max(dim="date").max(dim="prof_sample")
                    )
        ...
        for fld in nutrients:
            ...
            if (
                0
            ):  # show some add'l info on how often there is more than 1 sample per profile:
                print("%s: max per cast %d" % (fld, max_count))
                for mc in range(max_count):
                    print(
                        "    %d casts have %d samples"
                        % (np.sum((mc + 1) == np.sum(valid, axis=2)), mc + 1)
                    )
    ...
    utm_xy = proj_utils.mapper("WGS84", "EPSG:26910")(
        np.array([ds5.longitude, ds5.latitude]).T
    )
...
def station_number_to_lonlat(s):
    ...
    if station_locs is None:
        ...
        station_locs = pd.read_csv(
            os.path.join(this_dir, "usgs_sfbay_station_locations.csv"), skiprows=[1]
...
def query_usgs_sfbay(period_start, period_end, cache_dir=None, days_per_request="M"):
    ...
    if days_per_request is not None:
        ...
        dfs = []
        for interval_start, interval_end in periods(
            period_start, period_end, days_per_request
        ):
            df = query_usgs_sfbay(
                interval_start, interval_end, cache_dir=cache_dir, days_per_request=None
            )
            if df is not None:
                dfs.append(df)
    ...
    time_of_day = (hours * 3600 + minutes * 60).astype(np.int32) * np.timedelta64(
        1, "s"
    )
...
def usgs_sfbay_dataset(start_date, end_date, cache_dir=None, days_per_request="M"):
    ...
    if len(hier) != len(polaris):
        logging.warning(
            "After grouping by date, station and depth, there were some duplicates."
        )
