...
class Tweaker(object):
    ...
    def nudge_all_orthogonal(self, cell_thresh=0.01, max_iter=10, expand_after=4):
        g = self.g
        for it in range(max_iter):
            ...
            print(
                f"Iteration {it}: {bad_cells.sum()}/{len(cell_errors)} cells have error>{cell_thresh}"
            )
    ...
    def adjust_for_edge_quality(self, j, expand=True):
        ...
        nodes = np.unique(
            np.concatenate([g.cell_to_nodes(c) for c in g.edge_to_cells(j)])
        )
        if expand:
            n_orig = len(nodes)
        ...
        def cost(X, adj_cells=adj_cells, adj_edges=adj_edges, x0=x0):
            ...
            cell_errors = g.circumcenter_errors(
                cells=adj_cells, radius_normalized=True, cc=cc
            )
    ...
    def merge_all_by_edge_clearance(self):
        ...
        j_candidates = np.nonzero(
            np.isfinite(bad_edges2) & (bad_edges2 < clearance_thresh)
        )[0]
    ...
    def local_smooth(
        ...
        if ij is None:
            node_idxs, ij = g.select_quad_subset(
                ctr=None, max_cells=None, max_radius=None, node_set=node_idxs
            )
        ...
        XY = np.nan * np.zeros(
            (pad + 1 + ij[:, 0].max(), pad + 1 + ij[:, 1].max(), 2), np.float64
        )
        ...
        for count in range(n_iter):
            new_XY[...] = XY
            for ni, n in enumerate(node_idxs):
                if halos[ni] < min_halo:
                    continue
                if (free_nodes is not None) and (n not in free_nodes):
                    continue
                ...
                XY_sten = (
                    XY[stencil[:, 0] + ij[ni, 0], stencil[:, 1] + ij[ni, 1]] - XY[i, j]
                )
                valid = np.isfinite(XY_sten[:, 0])

                xcoefs, resid, rank, sing = np.linalg.lstsq(
                    M[valid], XY_sten[valid, 0], rcond=-1
                )
                ycoefs, resid, rank, sing = np.linalg.lstsq(
                    M[valid], XY_sten[valid, 1], rcond=-1
                )
        ...
        for smooth_it in range(smooth_iters):
            ...
            for ni, n in enumerate(n_free):
                ...
                for orient in [0, 90]:
                    ...
                    new_xy = np.array(
                        [interpolate.splev(ds, x_tck), interpolate.splev(ds, y_tck)]
                    )
...
def precalc_stencils(self, n_free):
    ...
    stencils = (
        np.zeros(
            (len(n_free), 1 + 2 * stencil_radius, 1 + 2 * stencil_radius), np.int32
        )
        - 1
    )
...
# so a node
# @utils.add_to(tweaker)
def local_smooth_flex(self, node_idxs, n_iter=3, free_nodes=None, min_halo=2):
    """
    Fit regular grid patches iteratively within the subset of nodes given
    by node_idxs.
    Currently requires that node_idxs has a sufficiently large footprint
