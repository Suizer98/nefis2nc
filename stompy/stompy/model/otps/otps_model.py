...
class OTPS(object):
    ...
    def extract_HC(
        ...
        inp_data = "\n".join(
            [
                "%s      ! 1. tidal model control file" % self.model_file,
        ...
        speeds = [
            tide_consts.speeds[tide_consts.const_names.index(c.upper())]
        ...
        remapping = [np.argmin(utils.haversine(ll, ll_output)) for ll in lonlats]
...
def reconstruct(harms, times):
    ...
    const_idxs = [tide_consts.const_names.index(c.upper()) for c in harms.const.values]
